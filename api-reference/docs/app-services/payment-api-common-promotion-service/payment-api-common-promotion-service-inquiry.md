...
| sceneInfos | 是 | List | 匹配券的场景条件列表，要求列表内的sceneId唯一。如果场景信息匹配上券，对应的sceneId会在响应参数matchedCoupon返回。 |
...
| coupons | 否 | List | 场景条件匹配上的平台券信息列表。 |
| matchedCoupon | 否 | List | 各场景条件匹配到的券号信息。 |
