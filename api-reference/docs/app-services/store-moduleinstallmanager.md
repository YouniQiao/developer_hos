...
content_hash: "a23fdffec0ee"
synced_at: "2026-07-28T16:52:33.595876"
---
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| MODULE_DEBUG_FILES_SIZE_OVER_LIMIT | -10 | 单次请求所有模块调试文件大小总和超过8G。 **起始版本：** 5.0.2(14) |
| MODULE_DEBUG_FILES_NOT_EXIST | -9 | 模块调试文件不存在。 **起始版本：** 5.0.2(14) |
...
| FAILURE | -1 | 失败。 |
| SUCCESS | 0 | 成功。 |
| DOWNLOAD_WAIT_WIFI | 1 | 当前使用的是流量，开发者需要调用[showCellularDataConfirmation](#moduleinstallmanagershowcellulardataconfirmation)接口，提醒用户确认是否使用流量下载。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回请求、监听/注销监听调用结果。 |
...
| callback | Callback | 是 | 回调函数，使用Callback的方式获取结果。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 固定值"moduleInstallStatus"。 |
| callback | Callback | 否 | 回调函数，使用Callback的方式获取结果。不传该参数则会取消当前应用的所有监听。 |
