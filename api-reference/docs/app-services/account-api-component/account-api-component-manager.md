...
@hms.core.account.LoginComponent模块提供华为账号登录UI组件，包含组件：[LoginPanel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-loginpanel#loginpanel)和[LoginWithHuaweiIDButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-huawei-id-button#loginwithhuaweiidbutton)，以及核心逻辑管理器loginComponentManager。该模块包含以下核心要素：

- LoginPanel：登录面板组件，集成了华为账号登录按钮、隐私协议、其他登录方式入口的标准登录面板。
- LoginWithHuaweiIDButton：登录按钮组件，支持多种样式和灵活布局，可在应用自定义登录页面中嵌入华为账号登录按钮。
- loginComponentManager：华为账号登录组件管理器，提供控制器和组件参数类型，用于自定义按钮组件样式、监听用户操作事件并处理登录结果。

开发者可通过loginComponentManager与两个UI组件组合使用，帮助开发者快速、规范地集成华为账号登录功能。

loginComponentManager为华为账号登录组件管理器。该命名空间定义了以下核心要素：

- 控制器类：LoginPanelController和LoginWithHuaweiIDButtonController，分别用于控制登录面板组件、登录按钮组件的登录行为。控制器类提供组件内部点击事件（点击登录、点击关闭、点击协议等）的回调函数注册方法，开发者可传入回调函数以响应用户点击事件。
- 组件参数结构体：LoginPanelParams和LoginWithHuaweiIDButtonParams，用于配置登录类型、按钮样式、隐私协议文本等组件参数。

LoginPanel和LoginWithHuaweiIDButton则是具体的UI组件，接收从loginComponentManager构造的参数和控制器，完成界面渲染并将用户操作结果通过控制器回调给开发者。
...
[LoginWithHuaweiIDButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-huawei-id-button#loginwithhuaweiidbutton)组件控制器，用来控制组件内部状态变化、注册回调函数以监听组件内部的点击事件。支持应用主动设置用户协议状态（[setAgreementStatus](#setagreementstatus)）、设置是否可点击（[setEnabled](#setenabled)）、继续登录（[continueLogin](#continuelogin)），并支持通过链式调用注册各类事件的回调函数：

- [onClickLoginWithHuaweiIDButton](#onclickloginwithhuaweiidbutton)：注册华为账号登录事件回调，登录成功时返回[HuaweiIDCredential](#huaweiidcredential)。
- [onClickEvent](#onclickevent)：注册华为账号登录按钮点击事件回调，可用于运营统计。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| agreementStatus | [AgreementStatus](#agreementstatus) | 是 | 当登录类型为[LoginType.QUICK_LOGIN](#logintype)时必须设置协议状态为ACCEPTED才可以完成华为账号登录。 当登录类型不是[LoginType.QUICK_LOGIN](#logintype)时，如果需要用户同意协议才能完成华为账号登录，请先设置协议状态为NOT_ACCEPTED，当用户同意协议后设置协议状态为ACCEPTED，才可以完成华为账号登录。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| [LoginWithHuaweiIDButtonController](#loginwithhuaweiidbuttoncontroller) | [LoginWithHuaweiIDButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-huawei-id-button#loginwithhuaweiidbutton)组件控制器。 |

示例：

参见[LoginWithHuaweiIDButtonController.setLocale](#setlocale)。

#### [h2]onClickEvent

onClickEvent(callback: AsyncCallback<ClickEvent>): LoginWithHuaweiIDButtonController

注册点击事件的回调函数。当前支持监听华为账号登录按钮的点击事件，可用于运营统计。使用callback异步回调。
...
注册华为账号登录事件的回调函数。当华为账号用户登录成功时触发回调，返回[HuaweiIDCredential](#huaweiidcredential)，可在此回调中处理响应结果、关闭面板完成登录。使用callback异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当华为账号登录成功返回结果，err为undefined，data为获取到的[HuaweiIDCredential](#huaweiidcredential)对象；否则为错误对象。 |
...
| [1001500002](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-account-kit#section1001500002-重复请求) | This error code is reported when a request is already being processed. |
...
| [1001502012](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-account-kit#section1001502012-用户取消授权) | The user canceled the authorization. |
...
| [1005300001](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-account-kit#section1005300001-用户未同意协议) | The user did not accept the agreement. 适用版本：5.0.0(12)+ |
...
继续登录方法。当用户未同意协议时点击登录按钮会弹出协议确认弹框，用户点击同意并登录按钮时可调用该接口继续触发登录。使用callback异步回调
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当用户点击协议弹框的同意并登录按钮操作成功，err为undefined，否则为错误对象。 |
...
| 错误码ID | 错误信息 |
| --- | --- |
| [1005300002](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-account-kit#section1005300002-用户未点击华为账号一键登录按钮) | The user did not click the HUAWEI ID login button. |
...
setLocale(locale: string): LoginWithHuaweiIDButtonController
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| locale | string | 是 | 地区语言代码。 当传入参数无效或系统不支持指定语言时，按钮文本将跟随系统语言进行显示。全量的地区语言代码可通过调用系统资源管理对象的[getLocales](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-resource-manager#getlocales11)方法获取。以下为常见的地区语言代码示例。 ar：阿拉伯语 bo-CN：藏语（中国） de：德语 en-US：英语（美国） en-GB：英语（英国） es：西班牙语 es-US：西班牙语（美国） fr：法语 id：印尼语 it：意大利语 ja：日语 ko：韩语 lo：老挝语 ms：马来语 my：缅甸语 my-MM：缅甸语（缅甸） pl：波兰语 pt：葡萄牙语 pt-PT：葡萄牙语（葡萄牙） ru：俄语 th：泰语 tr：土耳其语 ug：维吾尔语 vi：越南语 zh-CN：中文（简体） zh-HK：繁体中文（香港特别行政区） zh-TW：繁体中文（中国台湾） |
...
[LoginPanel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-loginpanel#loginpanel)组件控制器类，用来控制组件内部状态变化、注册回调函数以监听组件内部的点击事件。支持应用主动设置用户协议状态（[setAgreementStatus](#setagreementstatus-1)）、设置是否在用户点击其他登录方式时展示协议弹框（[setShowAgreementForOptionalLogin](#setshowagreementforoptionallogin)），并支持通过链式调用注册各类事件的回调函数：

- [onClickLoginWithHuaweiIDButton](#onclickloginwithhuaweiidbutton-1)：注册华为账号登录事件回调，登录成功时返回[HuaweiIDCredential](#huaweiidcredential)。
- [onClickOptionalLoginButton](#onclickoptionalloginbutton)/[onClickOptionalLoginIcon](#onclickoptionalloginicon)：注册可选登录按钮/图标点击事件回调，用于响应其他方式登录。
- [onClickPrivacyText](#onclickprivacytext)：注册富文本协议链接点击事件回调，可获取对应tag以跳转具体协议页面。
- [onClickCloseButton](#onclickclosebutton)：注册关闭面板或侧滑返回事件回调。
- [onChangeAgreementStatus](#onchangeagreementstatus)：注册协议勾选框状态变化回调。
- [onClickEvent](#onclickevent-1)：注册华为账号登录按钮点击事件回调，可用于运营统计。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当注册华为账号登录返回结果成功，err为undefined，data为获取到的[HuaweiIDCredential](#huaweiidcredential)对象；否则为错误对象。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| [LoginPanelController](#loginpanelcontroller) | [LoginPanel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-loginpanel#loginpanel)组件控制器。 |
...
注册可选登录按钮点击事件的回调函数。可选登录按钮文本可通过[LoginPanelParams.optionalLoginButtonAttr](#loginpanelparams)配置，可在此回调中引导用户跳转至其他登录方式页面。使用callback异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当用户点击可选登录按钮操作成功，err为undefined，否则为错误对象。 |
...
注册可选登录图标点击事件的回调函数。可选登录图标可通过[LoginPanelParams.optionalLoginAreaAttr](#loginpanelparams)配置，可在此回调中引导用户跳转至其他登录方式页面。使用callback异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当用户点击[LoginIcon](#loginicon)操作成功，err为undefined，data为获取到的[LoginIcon](#loginicon)的tag；否则为错误对象。 |
...
注册隐私文本点击事件的回调函数，仅类型为富文本的隐私文本被用户点击时会触发回调函数。隐私文本属性可通过[LoginPanelParams.privacyText](#loginpanelparams)配置，可在此回调中跳转到应用的隐私协议页面。使用callback异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当注册隐私内容为富文本的点击操作成功时，err为undefined，data为获取到的[PrivacyText](#privacytext)的tag；否则为错误对象。 |
...
注册用户协议状态变化事件的回调函数。当登录面板中的协议勾选框状态变化时触发此回调，返回协议同意状态[AgreementStatus](#agreementstatus)。使用callback异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当用户同意或者取消同意协议成功，err为undefined，data为获取到的[AgreementStatus](#agreementstatus)；否则为错误对象。 |
...
注册关闭按钮点击事件或用户侧滑返回事件的回调函数。使用callback异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当注册关闭按钮的点击操作或侧滑返回成功，err为undefined，否则为错误对象。 |
...
华为账号登录按钮样式类。通过new loginComponentManager.ButtonStyle()构造实例。
...
| 类型 | 说明 |
| --- | --- |
| [ButtonStyle](#buttonstyle) | 华为账号登录按钮样式。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceStr](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resourcestr) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮文字字体。 默认字体'HarmonyOS Sans'。 应用当前支持'HarmonyOS Sans'字体和[注册自定义字体](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-font)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceColor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resourcecolor) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮背景颜色。 当已经设置[CustomButtonParams](#custombuttonparams)的backgroundColor属性时，优先取此参数值。 不设置则为 Red 红色背景。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BackgroundEffectOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-background#backgroundeffectoptions11) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮背景属性，包括饱和度，亮度，颜色。 不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BackgroundBrightnessOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-background#backgroundbrightnessoptions12对象说明) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮背景提亮效果，包括：亮度变化速率，提亮程度。 不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [LinearGradient](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-border-image#lineargradient) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮背景颜色线性渐变效果，不设置则无效果。 **说明：** 使用此属性时，按钮不能有背景色，否则按钮边缘会出现锯齿毛边，需把按钮背景色设置为透明色，如backgroundColor(Color.Transparent)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [SweepGradient](#sweepgradient-1) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮背景颜色角度渐变效果。 不设置则无效果。 **说明：** 使用此属性时，按钮不能有背景色，否则按钮边缘会出现锯齿毛边，需把按钮背景色设置为透明色，如backgroundColor(Color.Transparent)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [RadialGradient](#radialgradient-1) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮背景颜色径向渐变效果。 不设置则无效果。 **说明：** 使用此属性时，按钮不能有背景色，否则按钮边缘会出现锯齿毛边，需把按钮背景色设置为透明色，如backgroundColor(Color.Transparent)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | [Resource](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resource) | 是 | 元素的不透明度，取值范围为0~1，1表示不透明，0表示完全透明，达到隐藏组件效果，但是在布局中占位。 默认值：1。 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮的不透明度。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BorderOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#borderoptions) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮的边框样式，不设置则无效果。 **说明：** 设置内边框需要预留充足的空间展示按钮，请确保按钮父组件宽高大于按钮本身。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BorderImageOption](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-border-image#borderimageoption对象说明) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮的图片边框样式，不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [OutlineOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#outlineoptions11对象说明) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮的外描边样式，不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ClickEffect](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-click-effect#clickeffect对象说明) | 是 | 当按钮类型为[BUTTON_CUSTOM](#style)时，此参数可设置按钮的点击回弹效果，不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [LoadingStyle](#loadingstyle-1) | 是 | 此参数可设置按钮在登录过程中是否显示加载态。 默认不显示加载态。 **说明：** 纯图标按钮不支持设置加载态，此参数不生效。 |
...
登录面板参数。可用于配置登录类型（loginType）、隐私文本内容（privacyText，可区分纯文本和可点击的富文本）、应用信息（appInfo）、匿名手机号（anonymousPhoneNumber）、其他登录方式属性（optionalLoginAreaAttr或optionalLoginButtonAttr）等。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| appInfo | [AppInfo](#appinfo) | 否 | 否 | 应用信息，包含图标、名称等。 |
| privacyText | [PrivacyText](#privacytext)[] | 否 | 是 | 隐私文本内容，支持纯文本和富文本类型，可用于跳转协议链接。 |
| optionalLoginButtonAttr | [OptionalLoginButtonAttr](#optionalloginbuttonattr) | 否 | 是 | 可选登录按钮属性。 |
| loginType | [LoginType](#logintype) | 否 | 是 | 华为账号登录类型。默认值：LoginType.ID。 一键登录请使用LoginType.QUICK_LOGIN。 |
| anonymousPhoneNumber | string | 否 | 是 | 华为账号绑定的匿名手机号。当登录类型为[LoginType.QUICK_LOGIN](#logintype)时需要设置该参数。可参考[华为账号一键登录客户端开发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-phone-unionid-login#客户端开发)获取。 **起始版本：** 5.0.0(12) |
...
| optionalLoginAreaAttr | [OptionalLoginAreaAttr](#optionalloginareaattr) | 否 | 是 | 可选登录区域属性，支持设置图标，可用于展示其他登录方式对应图标，以跳转不同的自定义登录页面。 如果optionalLoginButtonAttr和optionalLoginAreaAttr同时存在，优先展示optionalLoginAreaAttr。 **起始版本：** 5.0.0(12) |
| riskLevel | boolean | 否 | 是 | 是否需要获取华为账号用户风险等级。 仅登录类型为[LoginType.QUICK_LOGIN](#logintype)时需要设置该参数。 true：需要[获取用户风险等级](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-risklevel-byquicklogin)。 false：不获取用户风险等级。 默认值：false。 **起始版本：** 5.1.0(18) |
| securityVerification | boolean | 否 | 是 | 用户开启华为账号一键登录增强身份验证后，应用会在登录过程中通过华为账号使用生物识别或短信进行身份验证。如果需要获取用户一键登录增强身份验证的开关状态，需设置该字段为false。 仅登录类型为[LoginType.QUICK_LOGIN](#logintype)时需要设置该参数。 true：响应结果HuaweiIDCredential将不会返回[enableSecurityVerification](#huaweiidcredential)。 false：响应结果HuaweiIDCredential将返回[enableSecurityVerification](#huaweiidcredential)。 默认值：true。 **起始版本：** 6.0.0(20) |

#### LoginWithHuaweiIDButtonParams

华为账号登录按钮参数，支持配置登录类型（loginType）、按钮样式（style）等。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| style | [Style](#style) | 否 | 否 | [LoginWithHuaweiIDButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-huawei-id-button#loginwithhuaweiidbutton)组件的样式。 |
| borderRadius | number | 否 | 是 | 按钮边框圆角半径。 取值范围：[0,+∞)，值小于0时，按0处理。 默认值：height属性取值的一半。 单位：vp。 |
| iconRadius | number | 否 | 是 | Icon图标的半径。 取值范围：[0,+∞)，值小于0时，按0处理。 默认值：24。 单位：vp。 |
| supportDarkMode | boolean | 否 | 是 | 表示按钮的样式是否随系统深浅色模式变化。 true：按钮的样式会随着系统深浅色模式变化。 false：按钮的样式不会随着系统深浅色模式变化。 默认值：true。 |
| loginType | [LoginType](#logintype) | 否 | 是 | 华为账号登录类型。默认值：LoginType.ID。 一键登录请使用LoginType.QUICK_LOGIN。 |
| textAndIconStyle | boolean | 否 | 是 | 是否展示图文混合样式的华为账号登录按钮。 true：按钮支持Icon和文字混合样式。 false：按钮仅支持文本样式。 默认值：false。 当loginType不等于[LoginType.QUICK_LOGIN](#logintype)且style等于[BUTTON_RED](#style)、[BUTTON_WHITE](#style)、[BUTTON_WHITE_OUTLINE](#style)、[BUTTON_BLACK](#style)、[BUTTON_GRAY](#style)时该参数生效。 **起始版本：** 5.0.0(12) |
| customButtonParams | [CustomButtonParams](#custombuttonparams) | 否 | 是 | [BUTTON_CUSTOM](#style)按钮样式参数。 **起始版本：** 5.0.0(12) |
...
| extraStyle | [ExtraStyle](#extrastyle) | 否 | 是 | 如果应用想使用华为账号提供的固定样式之外的效果，可使用此接口自定义按钮样式。 **起始版本：** 5.0.0(12) |
| loginButtonTextType | [LoginButtonTextType](#loginbuttontexttype) | 否 | 是 | 当loginType为[LoginType.QUICK_LOGIN](#logintype)时，可传入此参数，控制按钮文本内容显示。 默认值：[LoginButtonTextType.QUICK_LOGIN](#loginbuttontexttype) - 当该参数为[LoginButtonTextType.QUICK_LOGIN](#loginbuttontexttype)时，按钮文本内容显示“华为账号一键登录”。 - 当该参数为[LoginButtonTextType.QUICK_REGISTRATION](#loginbuttontexttype)时，按钮文本内容显示“华为账号一键注册”。 **起始版本：** 5.0.0(12) |
| riskLevel | boolean | 否 | 是 | 是否需要获取华为账号用户风险等级。 仅登录类型为[LoginType.QUICK_LOGIN](#logintype)时需要设置该参数。 true：需要[获取用户风险等级](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-risklevel-byquicklogin)。 false：不获取用户风险等级。 默认值：false。 **起始版本：** 5.1.0(18) |
| securityVerification | boolean | 否 | 是 | 用户开启华为账号一键登录增强身份验证后，应用会在登录过程中通过华为账号使用生物识别或短信进行身份验证。如果需要获取用户一键登录增强身份验证的开关状态，需设置该字段为false。 仅登录类型为[LoginType.QUICK_LOGIN](#logintype)时需要设置该参数。 true：响应结果HuaweiIDCredential将不会返回[enableSecurityVerification](#huaweiidcredential)。 false：响应结果HuaweiIDCredential将返回 [enableSecurityVerification](#huaweiidcredential)。 默认值：true。 **起始版本：** 6.0.0(20) |

#### HuaweiIDCredential

华为账号登录成功响应，用于获取用户相关信息和关联华为账号。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| unionID | string | 是 | 否 | UnionID。华为账号用户在同一个开发者账号下的唯一标识，同一个用户，同一个开发者账号下的不同应用，UnionID值相同。具体格式要求请参考[OpenID和UnionID的格式说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-faq-9)。 |
| openID | string | 是 | 否 | OpenID。华为账号用户在应用/元服务的唯一标识，同一个用户，不同应用，OpenID值不同。具体格式要求请参考[OpenID和UnionID的格式说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-faq-9)。 |
| authorizationCode | string | 是 | 否 | Authorization Code。授权码，为临时凭据，应用服务端可使用Authorization Code调用[获取用户级凭证](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-obtain-user-token#接口原型)接口向华为账号服务器请求获取Access Token。Authorization Code有效时间5分钟，并且只能使用1次。长度限制1-1024。 |
...
| enableSecurityVerification | boolean | 是 | 是 | enableSecurityVerification。华为账号用户是否开启一键登录增强身份验证。如果开发者需要获取该字段，请将参数[securityVerification](#loginwithhuaweiidbuttonparams)设置为false。 仅登录类型为[LoginType.QUICK_LOGIN](#logintype)时会返回该字段。 true：表示用户已启用增强身份验证。 false：表示用户未启用增强身份验证。 **起始版本：** 6.0.0(20) |

#### AppInfo

应用信息。支持设置应用图标、应用名称、应用描述。
...
| appDescription | [ResourceStr](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resourcestr) | 否 | 是 | 应用的详细描述，长度限制1-44个字符。 **说明：** - 在4.1.0(11)版本，为必填参数。 - 从5.0.0(12)版本开始，为非必填参数。仅当登录类型为[LoginType.QUICK_LOGIN](#logintype)时不需要设置该值。 |

#### PrivacyText

隐私协议文本数据结构，用于定义隐私协议的文本类型（包含纯文本和富文本）、文本内容等。
...
自定义按钮参数。当配置[LoginWithHuaweiIDButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-huawei-id-button#loginwithhuaweiidbutton)按钮为[BUTTON_CUSTOM](#style)时使用。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| fontColor | [FontColor](#fontcolor) | 否 | 是 | [LoginWithHuaweiIDButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-huawei-id-button#loginwithhuaweiidbutton)按钮文字颜色。 默认值：[FontColor.WHITE](#fontcolor) |
| backgroundColor | [ResourceColor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resourcecolor) | 否 | 是 | [LoginWithHuaweiIDButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-huawei-id-button#loginwithhuaweiidbutton)按钮背景颜色。 默认值：Red |

#### ExtraStyle

[LoginWithHuaweiIDButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-huawei-id-button#loginwithhuaweiidbutton)按钮拓展参数。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| buttonStyle | [ButtonStyle](#buttonstyle) | 否 | 是 | 按钮样式，支持设置颜色、大小等属性。 |
| customButtonStateStyles | [StateStyles](#statestyles) | 否 | 是 | 按钮多态样式，支持设置按压、聚焦等场景下的按钮样式。仅对[BUTTON_CUSTOM](#style)类型按钮生效。 |
| textAndIconButtonParams | [TextAndIconButtonParams](#textandiconbuttonparams) | 否 | 是 | 图文混合样式按钮参数，用于控制图标与文字的间距。 |
| iconButtonParams | [IconButtonParams](#iconbuttonparams) | 否 | 是 | 纯图标按钮参数。用于设置纯图标按钮内图标的显示半径。 |
| canvasAnimationParams | [CanvasAnimationParams](#canvasanimationparams) | 否 | 是 | 自定义样式按钮下设置该参数，开发者可在按钮提供的画布上绘制自定义动效，推荐使用lottie动效。仅对[BUTTON_CUSTOM](#style)类型按钮生效。 **起始版本：** 26.0.0 |

#### CanvasAnimationParams

画布动效参数。用于在自定义样式按钮提供的画布上绘制自定义动效，推荐使用lottie动效。仅对[BUTTON_CUSTOM](#style)类型按钮生效。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| canvasRenderingContext | [CanvasRenderingContext2D](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-canvasrenderingcontext2d) | 否 | 否 | 绘制动效画布的上下文。 |
| onReady | Callback | 否 | 否 | 画布就绪的回调函数，开发者可在该回调中进行动效的初始化操作。 |
...
| colors | Array | 否 | 否 | 指定某百分比位置处的渐变色颜色，设置非法颜色直接跳过。 默认无效果。 |
...
| colors | Array | 否 | 否 | 指定某百分比位置处的渐变色颜色，设置非法颜色直接跳过。 不设置则无效果。 |
| repeating | boolean | 否 | 是 | 为渐变的颜色重复着色。 true：重复着色。 false：不重复着色。 默认值：false。 |
...
华为账号登录按钮加载态样式，用于控制是否在登录过程中展示加载态。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| show | boolean | 否 | 是 | 华为账号登录按钮在登录过程中是否展示加载态。 true：展示登录按钮加载态 false：不展示登录按钮加载态 默认值：false |

#### TextAndIconButtonParams

图文混合样式按钮参数，用于控制图标与文字的间距。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| textAndIconMargin | number | 否 | 是 | 图文混合按钮中图标与文字的间距。默认值：8vp，取值范围：4~16vp。当小于4vp时取值为4vp，当大于16vp时取值为16vp。 |

#### IconButtonParams

纯图标按钮参数。支持设置纯图标按钮内图标的显示半径。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| logoRadius | number | 否 | 是 | 纯图标按钮内华为图标的半径。 默认值：14vp，最小值：8vp。取值范围：按钮高度的20%~32%。当小于20%时取值为20%，当大于32%时取值为32%。 |

#### StateStyles

华为账号登录按钮的多态样式。支持设置按压、聚焦等场景下的按钮样式。仅对[BUTTON_CUSTOM](#style)类型按钮生效。
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| PLAIN_TEXT | 0 | 纯文本类型，不支持点击。 |
| RICH_TEXT | 1 | 富文本类型，展示为蓝色，支持点击。用于展示《华为账号用户认证协议》和应用相关隐私协议。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| BUTTON_RED | 0 | 红色背景白色文字按钮，字体大小会随着按钮的宽高自适应，最小为9号，最大为16号。 根据开发场景选择[【华为账号一键登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section41792374210)、[【华为账号登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section2624430102713)视觉规范。 |
| BUTTON_WHITE | 1 | 白色背景黑色文字按钮，字体大小会随着按钮的宽高自适应，最小为9号，最大为16号。 根据开发场景选择[【华为账号一键登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section41792374210)、[【华为账号登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section2624430102713)视觉规范。 |
| BUTTON_WHITE_OUTLINE | 2 | 白色背景黑色文字带边框按钮，字体大小会随着按钮的宽高自适应，最小为9号，最大为16号。 根据开发场景选择[【华为账号一键登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section41792374210)、[【华为账号登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section2624430102713)视觉规范。 |
| BUTTON_BLACK | 3 | 黑色背景白色文字按钮，字体大小会随着按钮的宽高自适应，最小为9号，最大为16号。 根据开发场景选择[【华为账号一键登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section41792374210)、[【华为账号登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section2624430102713)视觉规范。 |
...
| BUTTON_GRAY | 9 | 灰色背景黑色文字按钮，字体大小会随着按钮的宽高自适应，最小为9号，最大为16号。 根据开发场景选择[【华为账号一键登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section41792374210)、[【华为账号登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section2624430102713)视觉规范。 **起始版本：** 5.0.0(12) |
| BUTTON_CUSTOM | 10 | 支持自定义背景颜色、文字颜色及圆角等效果的按钮，字体大小会随着按钮的宽高自适应，最小为9号，最大为16号。开发者可以通过[CustomButtonParams](#custombuttonparams)设置按钮自定义的参数，且需要自行适配深浅色模式。 根据开发场景选择[【华为账号一键登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section41792374210)、[【华为账号登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section2624430102713)视觉规范。 **起始版本：** 5.0.0(12) **说明：** 此类型仅支持纯文本按钮。 |
...
华为账号登录按钮支持的文字颜色枚举。仅支持类型为[BUTTON_CUSTOM](#style)的按钮设置。
