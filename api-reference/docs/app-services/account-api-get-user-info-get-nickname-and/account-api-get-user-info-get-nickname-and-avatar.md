...
![](./img/note_3.0-zh-cn.png) 获取Authorization Code时不携带profile（昵称和头像）scope，调用成功后响应中将不包含昵称和头像。
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| access_token | 是 | String | 通过[获取用户级凭证](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-obtain-user-token)获取的Access Token。获取凭证时Authorization Code需包含profile scope的授权。 |
...
| displayName | 否 | String | 用户昵称，该字段返回场景详见[获取头像昵称](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-avatar-nickname)。 - 请求参数“getNickName”为0或不传时，返回匿名化账号。 - 请求参数“getNickName”为1时，返回昵称，没有昵称时返回匿名化账号。 **说明：** 调用成功后响应中若不包含用户昵称displayName，请确认获取的Authorization Code是否携带了profile（昵称和头像）scope。参考[客户端开发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-avatar-nickname#客户端开发) |
| displayNameFlag | 否 | int | 返回的昵称类型 。 0：昵称。 1：匿名账号。 **说明：** 调用成功后响应中若不包含昵称类型displayNameFlag，请确认获取的Authorization Code是否携带了profile（昵称和头像）scope。参考[客户端开发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-avatar-nickname#客户端开发) |
| headPictureURL | 否 | String | 用户头像，该字段返回场景详见[获取头像昵称](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-get-avatar-nickname)。用户未设置头像时不返回。 **说明：** 当用户更新头像后，原头像链接会立即失效。为确保头像正常显示，建议先将头像下载保存后再使用，避免因用户头像链接失效而影响业务流程。 |

调用失败时，响应消息返回如下：

| 参数 | 参数位置 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| NSP_STATUS | header | String | 错误码，详见本章节[错误码](#错误码)。 |
...
![](./img/note_3.0-zh-cn.png) Response Header中的NSP_STATUS字段，在处理成功时不会返回。

| NSP_STATUS | 描述 | 解决方法 |
| --- | --- | --- |
| 6 | 会话失效，session timeout。 可能原因: - access_token无效或已过期 - access_token格式不正确 - 其他内部原因 | - 请检查传参是否正确，如无问题请尝试重新获取。 - 未对access_token进行URLEncode处理，可参考[示例代码](#示例代码)组装参数。 - 根据返回的错误描述进行处理，若仍无法解决，请通过[在线提单](https://developer.huawei.com/consumer/cn/support/feedback/#/)提交问题。 |
