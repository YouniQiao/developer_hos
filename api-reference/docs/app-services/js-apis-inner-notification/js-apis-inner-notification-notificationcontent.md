...
content_hash: "2673d8ac253f"
synced_at: "2026-07-09T01:01:28.727673"
---
...
- 当该类型通知与其他通知形成组通知时，该通知显示默认与[普通文本](#notificationbasiccontent)相同。展开组通知后，标题显示为展开时的标题longTitle，多行文本内容lines多行显示。 当该类型通知单独呈现时，该通知标题显示为展开时的标题longTitle，多行文本内容lines多行显示。
...
| lines | Array | 否 | 否 | 通知的多行文本（最多支持三行，每行大小不超过1024字节，超出部分会被截断）。 |
...
| icon | [image.PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap) | 否 | 是 | 胶囊图片。图标像素的总字节数不超过192KB（图标像素的总字节数通过[getPixelBytesNumber](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap#getpixelbytesnumber7)获取），建议图标像素长宽为128*128。实际显示效果依赖于设备能力和通知中心UI样式。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| names | Array | 否 | 是 | 按钮名称（最多支持3个）。 |
| icons | Array | 否 | 是 | 按钮图片（最多支持3个）。图标像素的总字节数不超过192KB（图标像素的总字节数通过[getPixelBytesNumber](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap#getpixelbytesnumber7)获取），建议图标像素长宽为128*128。实际显示效果依赖于设备能力和通知中心UI样式。 |
| iconsResource12+ | Array | 否 | 是 | 按钮资源（最多支持3个）。 |
