...
```
 ![](./img/note_3.0-zh-cn.png) 回传结果验签调用方法verify(String content,String signature)中： content：签名内容拼接字段，拼接方式可参考[验签计算规则](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/store-attribution-receive#验签计算规则)。 signature：回传请求中的签名值。
