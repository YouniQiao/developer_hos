...
content_hash: "96a63f5eeb44"
synced_at: "2026-07-28T16:53:07.641552"
---
...
| linkDevicePass | 是 | linkDevicePass | 链接设备参数，用于保存出行凭证管理台服务器地址、公钥信息以及是否使能卡券的NFC能力。 |
...
| readerMatchValue | 是 | String | 出行凭证标识，建议字节不超过20字节（第一字节：三方标识，第二字节：品牌/系列标识，后续字节保证在三方内唯一）, 只能包含0-9，A-F。 |
| deviceType | 是 | String | 当前出行凭证开通的设备类型，如Phone：手机，Wear：穿戴。 |
...
参见[申请出行凭证](#申请出行凭证)的请求体，如果是全量更新，与申请出行凭证接口请求体相同。如果是局部更新，则只需传入需要变更的数据体。

#### [h2]请求示例

```
POST /hmspass/v2/key_travel/instance/100006 HTTP/1.1
