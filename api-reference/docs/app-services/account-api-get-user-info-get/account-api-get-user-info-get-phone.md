...
- 需确保调用端网络正常。
- 应用获取手机号之前，需要完成phone（获取您的手机号）的scope权限申请，在获取Authorization Code时携带phone（获取您的手机号）scope，详见[业务流程](https://developer.huawei.com/consumer/cn/doc/atomic-guides/account-guide-atomic-get-phonenumber#业务流程)。

![](./img/note_3.0-zh-cn.png) 应用未申请phone（获取您的手机号）的scope权限，或获取Authorization Code时未携带phone（获取您的手机号）scope，调用成功后响应中将不包含手机号。
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| access_token | 是 | String | 通过[获取用户级凭证](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-obtain-user-token)获取的Access Token。获取凭证时Authorization Code需包含phone scope的授权。 |
...
![](./img/note_3.0-zh-cn.png) 如字段无特殊说明，华为账号服务器返回的手机号码格式如下(返回数据实际为明文)：
