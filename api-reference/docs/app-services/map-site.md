...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回[SearchByTextResult](#searchbytextresult)。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回[NearbySearchResult](#nearbysearchresult)。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回[QueryAutoCompleteResult](#queryautocompleteresult)。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回[SearchByIdResult](#searchbyidresult)。 |
...
![](./img/note_3.0-zh-cn.png) 根据地址获取地点的空间坐标，如经纬度，最多返回10条记录。
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回[GeocodeResult](#geocoderesult)。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回[ReverseGeocodeResult](#reversegeocoderesult)。 |
...
| pageIndex | number | 否 | 是 | 当前页数。取值范围：[1, 500]，默认值为1。 **说明：** pageIndex * pageSize 模型约束： 此接口仅可在Stage模型下使用。
...
| poiTypes | Array | 否 | 是 | 返回指定的华为分类体系的地点，取值范围参见[HwLocationType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-poi#hwlocationtype)。 **元服务API：** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
...
| pageIndex | number | 否 | 是 | 当前页数。取值范围：[1, 500]，默认值为1。 **元服务API：** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 **说明：** pageIndex * pageSize 模型约束： 此接口仅可在Stage模型下使用。
...
| sortRule | [SortRule](#sortrule) | 否 | 是 | POI的排序规则。 默认值为[SortRule](#sortrule).DISTANCE，即按距离排序。 **起始版本：** 5.0.0(12) **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
...
| aois | Array | 否 | 是 | Aoi面信息。 |
| pois | Array | 否 | 是 | POI信息。 |
| roads | Array | 否 | 是 | 道路信息。 |
| intersections | Array | 否 | 是 | 交叉点信息。 |
...
| addressComponent | [AddressComponent](#addresscomponent) | 否 | 否 | 地址详细信息。 **元服务API：** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
...
| viewport | [mapCommon.LatLngBounds](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-common#latlngbounds) | 否 | 是 | 地点的视口范围。 **说明：** [queryAutoComplete](#queryautocomplete)不支持返回此字段。 **元服务API：** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| distance | number | 否 | 是 | 预测地点和传参location之间的直线距离，单位：m。 **说明：** 目前仅关键字搜索和周边搜索支持返回此字段。 **元服务API：** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| poi | [Poi](#poi) | 否 | 是 | 如果地点是POI类型，返回POI信息。 **元服务API：** 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
...
| **名称** | **类型** | 只读 | 可选 | **说明** |
| --- | --- | --- | --- | --- |
| poiTypes | Array | 否 | 是 | 华为POI分类体系，取值范围请参见[HwLocationType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-poi#hwlocationtype)。 |
| poiTypeIds | Array | 否 | 是 | 华为POI分类体系Id，预留字段，当前无使用场景。 |
...
| childNodes | Array | 否 | 是 | POI的子节点信息。 |
...
| **名称** | **类型** | 只读 | 可选 | **说明** |
| --- | --- | --- | --- | --- |
| texts | Array | 否 | 是 | 每个星期的开放时间段的描述。 |
| periods | Array | 否 | 是 | 开放时间段的详细说明。 |
...
| poiTypes | Array | 否 | 是 | 华为分类体系，取值范围参见[HwLocationType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-poi#hwlocationtype)。 |
