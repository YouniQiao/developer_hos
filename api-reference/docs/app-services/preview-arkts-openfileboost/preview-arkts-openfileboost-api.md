...
| state | [FilePreloadState](#filepreloadstatedeprecated) | 否 | 否 | 文件预加载状态 |
...
| callback | Callback | 是 | 回调函数，用于应用获取预加载文件状态变化信息。 |
...
1. 需要先调用[openFileBoost.on('filePreloadStateChanged')](#openfileboostonfilepreloadstatechangeddeprecated)接口后再调用该接口添加文件预加载状态监听。
2. 当前一个应用最多添加50个文件监听。
...
| 类型 | 说明 |
| --- | --- |
| [FilePreloadStatusInfo](#filepreloadstatusinfodeprecated) | 文件预加载状态信息 |
