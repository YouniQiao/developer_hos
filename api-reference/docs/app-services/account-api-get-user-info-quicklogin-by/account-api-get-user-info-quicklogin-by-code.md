...
upstream_id: "harmonyos-references/account-api-get-user-info-quicklogin-by-code"
catalog: "harmonyos-references"
content_hash: "1c304697baff"
synced_at: "2026-07-28T16:52:30.620673"
---

# 一键登录获取华为账号绑定号码和UnionID/OpenID

![](./img/caution_3.0-zh-cn.png) 为了更安全的网络访问，请务必使用TLS1.2协议及规定内的加密套件。若使用协议是TLS1.0、TLS1.1或规定外的加密套件，可能无法正常访问华为账号服务。
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| Content-Type | 是 | String | 取值为：application/json。 **说明：** Request Body传参，请务必遵循此请求头格式，否则可能导致请求失败，具体传参方式可参考[示例代码](#示例代码)。 |

#### [h2]Request Body

| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| code | 是 | String | 通过[华为账号一键登录（获取手机号和UnionID/OpenID）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-phone-unionid-login)场景获取的Authorization Code，详情可参考一键登录[客户端开发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-phone-unionid-login#客户端开发)。 |
| clientId | 是 | String | 在创建应用后，由AppGallery Connect（简称AGC）为应用分配的唯一标识。参数取值详见[查看应用基本信息](https://developer.huawei.com/consumer/cn/doc/app/agc-help-appinfo-0000001100014694)中的**OAuth 2.0客户端ID（凭据）-Client ID**参数。 **说明：** 该参数与获取code参数时的Client ID必须一致，否则会报错（响应消息中resultCode=60180003），如出现此报错，请参考[配置Client ID](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-client-id)排查处理。 |
...
| phoneNumber | 是 | String | 华为账号绑定号码（含国家码），使用该手机号完成一键登录的功能，详见[华为账号一键登录（获取手机号和UnionID/OpenID）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-phone-unionid-login)。 |
| phoneNumberValid | 是 | Integer | 通过一键登录功能获取的华为账号绑定号码的实时有效性。 若发起一键登录时[LoginPanelParams](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-component-manager#loginpanelparams)的verifyPhoneNumber参数值传递为true，表示华为代为验证手机号有效性，开发者无需关注此返回值； 若verifyPhoneNumber参数值传递为false，需要根据返回的状态值进行处理。 0：在过去90天内，无法证明当前手机号码可以触达用户， 需要进行验证。 1：在过去90天内，当前手机号码被证明可以触达用户，可以直接使用。 |
...
| 参数 | 参数类型 | 描述 |
| --- | --- | --- |
| resultCode | int | 错误码，详见本章节[错误码](#错误码)。 |
...
| 错误码 | 描述 | 解决方法 |
| --- | --- | --- |
| 60010002 | 参数不合法。 | 请按照错误描述及接口[Request Body](#request-body)参数说明检查入参。 |
...
| 60180003 | code解析得到的Client ID与入参clientId不一致。 | code参数获取时的clientId与当前接口参数clientId不一致导致，请检查入参clientId是否与[配置Client ID](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-client-id)中的值一致。 |
...
| 60180007 | code未授权华为账号一键登录权限。 | code未授权华为账号一键登录权限，可能原因如下： - 在进行华为账号一键登录客户端功能开发时，登录按钮的[LoginWithHuaweiIDButtonParams.loginType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-component-manager#loginwithhuaweiidbuttonparams)属性未设置为LoginType.QUICK_LOGIN。 - 客户端传给服务器的Authorization Code，是通过华为账号一键登录预取号阶段获取到的Authorization Code。 - 客户端传给服务器的Authorization Code，不是华为账号一键登录场景获取到的。 以上可能原因的解决方法，请参考[60180007 服务端通过Authorization Code无法获取到华为账号一键登录手机号如何解决](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-faq-21)。 |
