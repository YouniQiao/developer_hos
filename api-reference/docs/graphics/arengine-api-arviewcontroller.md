...
#### [h2]ARViewContext.removeGSModel
...
| **类型** | **说明** |
| --- | --- |
| Promise | Promise对象，返回模型移除结果，true代表移除成功，false代表移除失败。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [ARViewCallback](#arviewcallback) | 是 | ARViewCallback抽象类，声明ARView的回调接口。使用callback方式异步返回结果。 |
