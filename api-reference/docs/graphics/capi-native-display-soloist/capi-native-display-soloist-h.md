...
content_hash: "d50be707dbd2"
synced_at: "2026-07-28T16:52:10.833474"
---
...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| [DisplaySoloist_ExpectedRateRange](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-displaysoloist-expectedraterange) | DisplaySoloist_ExpectedRateRange | 期望帧率范围结构体，用于设置DisplaySoloist（可变帧率独立线程绘制）的期望帧率范围。设置的期望帧率范围将作为系统调度的参考，系统会尽量在此范围内调整绘制帧率。 |
| [OH_DisplaySoloist](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-oh-displaysoloist) | OH_DisplaySoloist | OH_DisplaySoloist结构体声明，此结构体用于需要在独立线程中实现帧率控制的Native侧业务。 |
...
| [OH_DisplaySoloist* OH_DisplaySoloist_Create(bool useExclusiveThread)](#oh_displaysoloist_create) | - | 创建一个[OH_DisplaySoloist](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-oh-displaysoloist)实例，每次调用都会产生一个新的实例。useExclusiveThread参数决定线程模式：独占线程模式下，该实例拥有独立的线程，性能更高但资源消耗更大；共享线程模式下，多个实例共享线程，资源消耗较小但可能存在调度延迟。 |
| [int32_t OH_DisplaySoloist_Destroy(OH_DisplaySoloist* displaySoloist)](#oh_displaysoloist_destroy) | - | 销毁[OH_DisplaySoloist](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-oh-displaysoloist)实例并回收对象占用的内存。销毁前应先调用[OH_DisplaySoloist_Stop](#oh_displaysoloist_stop)停止回调。销毁后不应再访问该实例或依赖其回调。 |
...
| 参数项 | 描述 |
| --- | --- |
| bool useExclusiveThread | 表示此[OH_DisplaySoloist](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-oh-displaysoloist)实例是否独占线程。true表示独占线程，false表示共享线程（与其他实例共享线程）。 |
...
| 参数项 | 描述 |
| --- | --- |
| [OH_DisplaySoloist](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-oh-displaysoloist)* displaySoloist | 一个指向[OH_DisplaySoloist](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-oh-displaysoloist)实例的指针，不能为空。 |
...
#### [h2]OH_DisplaySoloist_SetExpectedFrameRateRange()
...
| --- | --- |
| [OH_DisplaySoloist](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-oh-displaysoloist)* displaySoloist | 一个指向[OH_DisplaySoloist](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-oh-displaysoloist)实例的指针，不能为空。 |
| [DisplaySoloist_ExpectedRateRange](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-displaysoloist-expectedraterange)* range | 一个指向期望帧率范围[DisplaySoloist_ExpectedRateRange](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativedisplaysoloist-displaysoloist-expectedraterange)实例的指针，不能为空。包含expected、min、max三个字段，单位为帧/秒（fps），字段需为非负整数，取值范围为[0, 设备最大帧率]，且满足min 返回：
