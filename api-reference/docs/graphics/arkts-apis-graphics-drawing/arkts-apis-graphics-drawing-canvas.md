...
content_hash: "55d406290965"
synced_at: "2026-07-09T01:00:51.975362"
---
...
![](./img/note_3.0-zh-cn.png) 画布自带一个默认画刷，该画刷为黑色，开启反走样，不具备其他任何样式效果。当画布中没有主动设置画刷和画笔时，该默认画刷生效。
...
| colors | Array | null | 是 | 描述顶点对应颜色的数组，用于在三角形中进行插值。其可以为空，表明颜色效果为用户所设置的默认色；若不为空其长度必须等于vertexCount。 |
...
| vertices | Array | 是 | 顶点数组，指定网格的绘制位置，浮点数组，大小必须为((meshWidth+1) * (meshHeight+1) + vertOffset) * 2。 |
| vertOffset | number | 是 | 绘图前要跳过的vert元素数，大于等于0的整数。 |
| colors | Array | null | 是 | 颜色数组，在每个顶点指定一种颜色，整数数组，可为null，大小必须为(meshWidth+1) * (meshHeight+1) + colorOffset。 |
...
![](./img/zh-cn_image_0000002661613755.png)
...
![](./img/note_3.0-zh-cn.png) 执行该方法后，若pen的效果发生改变并且开发者希望该变化生效于接下来的绘制动作，需要再次执行该方法以确保变化生效。
...
![](./img/note_3.0-zh-cn.png) 执行该方法后，若brush的效果发生改变并且开发者希望该变化生效于接下来的绘制动作，需要再次执行该方法以确保变化生效。
