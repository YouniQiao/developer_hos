...
| callback | Callback | 是 | 回调函数。返回任务进度信息。 |
...
| callback | Callback | 否 | 回调函数。返回任务进度信息。需要取消监听的回调函数，需与订阅时传入的回调函数一致。若不填，则取消当前监听该事件的所有回调函数。 |
...
| callback | Callback | 是 | 回调函数。返回任务信息。 |
...
| callback | Callback | 是 | 回调函数。返回任务完成信息。 |
...
| callback | Callback | 是 | 回调函数。返回任务失败信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。 - 若资源托管至华为CDN，返回资源包下载清单manifestUrl。 - 若资源托管至三方CDN，则返回空字符串。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回下载任务taskID。 最大长度36个字节。 |
