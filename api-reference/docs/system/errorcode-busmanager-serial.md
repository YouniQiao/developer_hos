...
content_hash: "81a27e850ac7"
synced_at: "2026-08-14T15:54:46.383195"
---

# 串口管理错误码

![](./img/note_3.0-zh-cn.png) API参考中的参数说明与SDK中@ohos.busManager.serial.d.ts文件的注释保持一致。

本模块首批接口从API version 19开始支持。后续版本的新增接口，采用上标单独标记接口的起始版本。

本文档提供串口通信接口调用的错误码说明，帮助开发者快速定位和解决串口通信问题。适用于开发串口通信应用、调试串口通信异常等场景。以下仅介绍本模块特有错误码，通用错误码请参考[通用错误码说明文档](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal)。
...
2. 重新插拔USB设备后，重新调用[getSerialPortList](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial#serialgetserialportlist)获取设备列表，再调用[open](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial#open)接口打开设备，以恢复串口通信功能。
...
调用[open](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial#open)接口打开串口时，需确保串口端口未被其他应用或进程占用，否则会因端口已被占用而报错。
...
1. 关闭占用该串口的其他应用。
2. 调用[close](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial#close)关闭串口后重新打开。
...
调用[write](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial#write)接口写入数据时（需确保串口已打开），若数据传输超时则会报此错误。

前置条件：

- 必须先调用[open](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial#open)打开串口，才能调用write()接口。
- write()接口依赖串口处于打开状态。
...
4. 在确保串口正常打开的情况下，可降低写入频率或在每次写入之间调用[drain](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial#drain)接口等待前次数据发送完成后再发送，避免硬件缓冲区拥塞。
...
调用[open](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-busmanager-serial#open)接口打开串口时，系统会校验用户是否已允许应用访问目标串口，若用户拒绝授权，则抛出该错误码。
