...
content_hash: "5a5ef10bb88e"
synced_at: "2026-08-29T18:16:25.963502"
---
...
#### [h2]使用AES-CBC算法解密时调用doFinal失败

问题现象

解密时调用doFinal失败，返回错误码17630001。

![](./img/note_3.0-zh-cn.png) AES-CBC属于块加密算法，因此需要使用到填充算法PKCS7，将明文填充至块大小整数倍。
...
#### [h2]使用AES-ECB算法解密时调用doFinal失败

问题现象

AES-ECB解密时调用doFinal失败，返回错误码17630001。

![](./img/note_3.0-zh-cn.png) AES-ECB属于块加密算法，分组长度为128位（16字节），因此需要使用到填充算法PKCS7，将明文填充至分组大小的整数倍。
...
#### [h2]使用DES/3DES算法解密时调用doFinal失败

问题现象

DES或3DES解密时调用doFinal失败，返回错误码17630001。

![](./img/note_3.0-zh-cn.png) DES/3DES为分组加密算法，分组长度为64位（8字节）。ECB、CBC模式下，明文长度不是64位整数倍时，必须使用填充方法补足。
...
#### [h2]使用SM4-ECB/CBC算法解密时调用doFinal失败

问题现象

SM4-ECB或SM4-CBC解密时调用doFinal失败，返回错误码17630001。

![](./img/note_3.0-zh-cn.png) SM4为分组加密算法，分组长度为128位。ECB、CBC模式下，明文长度不是128位整数倍时，必须使用填充方法补足。
...
#### [h2]使用ChaCha20-Poly1305算法解密时调用doFinal失败
