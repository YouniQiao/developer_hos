...
| 类型 | 说明 |
| --- | --- |
| Promise | 无返回结果的Promise对象。当解除激活设备管理应用失败时，会抛出错误对象。 |
...
| policies | Array | 是 | [委托策略列表](#可委托策略列表)。 |
...
| parameters | Record | 是 | 自定义参数信息，其中Key值必须包含："activateId"，可以包含"customizedInfo"、"localDeactivationPolicy"。 - activateId：项目激活ID。 - customizedInfo：企业自定义信息。 - localDeactivationPolicy：从API version 22开始支持，本地延迟取消激活时间（单位：小时） |

![](./img/caution_3.0-zh-cn.png) 1、此接口只能在企业设备上生效。
...
| 类型 | 说明 |
| --- | --- |
| Promise | 无返回结果的Promise对象。当激活设备管理应用失败时，会抛出错误对象。 |
...
| notify_upgrade_packages | [systemManager.notifyUpdatePackages](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterprise-systemmanager#systemmanagernotifyupdatepackages) [systemManager.getUpdateResult](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterprise-systemmanager#systemmanagergetupdateresult) | 通知系统更新包信息。 获取系统更新结果。 |
