...
content_hash: "4406cee870fd"
synced_at: "2026-08-21T15:35:21.538519"
---
...
![](./img/note_3.0-zh-cn.png) 本模块首批接口从API version 11 开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。
...
设置[Tag.IS_PERSISTENT](#tag)属性时，需要申请ohos.permission.STORE_PERSISTENT_DATA权限，申请方式请参考[声明权限](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/declare-permissions)。
...
如果要设置[Tag.IS_PERSISTENT](#tag)属性，需要申请ohos.permission.STORE_PERSISTENT_DATA权限，申请方式请参考[声明权限](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/declare-permissions)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| attributesArray | Array | 是 | 待新增关键资产的属性集合数组，包括关键资产明文、访问控制属性、自定义数据等。数组长度最大值为100。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回批量操作结果，包含失败关键资产的错误信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| assetsToBeRemoved | Array | 是 | 待删除关键资产的搜索条件数组，如别名、访问控制属性、自定义数据等。数组长度最大值为100。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sourceAttributes | Array | 是 | 待更新关键资产的搜索条件数组。数组最大长度为100，数组中所有元素的[Tag.GROUP_ID](#tag)和[Tag.REQUIRE_ATTR_ENCRYPTED](#tag)属性值必须相同。 |
| destAttributes | Array | 是 | 待更新关键资产的属性集合数组。数组最大长度为100，且应与sourceAttributes长度保持一致，数组中所有元素的[Tag.GROUP_ID](#tag)和[Tag.REQUIRE_ATTR_ENCRYPTED](#tag)属性值必须相同。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回挑战值。 **说明：** 挑战值用于后续的用户认证。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| BOOL | 0x01 系统能力： SystemCapability.Security.Asset

![](./img/note_3.0-zh-cn.png) 以下为Tag类型的全量枚举值，每个接口可传的Tag枚举及对应的Value取值范围不同，详见[各个场景的开发指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/asset-store-kit-overview)。
...
| DATA_LABEL_CRITICAL_1 | TagType.BYTES | 0x20 | 关键资产附属信息，内容由业务自定义且**有完整性保护**。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_CRITICAL_2 | TagType.BYTES | 0x21 | 关键资产附属信息，内容由业务自定义且**有完整性保护**。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_CRITICAL_3 | TagType.BYTES | 0x22 | 关键资产附属信息，内容由业务自定义且**有完整性保护**。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_CRITICAL_4 | TagType.BYTES | 0x23 | 关键资产附属信息，内容由业务自定义且**有完整性保护**。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_NORMAL_1 | TagType.BYTES | 0x30 | 关键资产附属信息，内容由业务自定义且**无完整性保护**。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_NORMAL_2 | TagType.BYTES | 0x31 | 关键资产附属信息，内容由业务自定义且**无完整性保护**。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_NORMAL_3 | TagType.BYTES | 0x32 | 关键资产附属信息，内容由业务自定义且**无完整性保护**。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_NORMAL_4 | TagType.BYTES | 0x33 | 关键资产附属信息，内容由业务自定义且**无完整性保护**。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_NORMAL_LOCAL_112+ | TagType.BYTES | 0x34 | 关键资产附属的本地信息，内容由业务自定义且**无完整性保护**，该项信息不会进行同步。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_NORMAL_LOCAL_212+ | TagType.BYTES | 0x35 | 关键资产附属的本地信息，内容由业务自定义且**无完整性保护**，该项信息不会进行同步。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_NORMAL_LOCAL_312+ | TagType.BYTES | 0x36 | 关键资产附属的本地信息，内容由业务自定义且**无完整性保护**，该项信息不会进行同步。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| DATA_LABEL_NORMAL_LOCAL_412+ | TagType.BYTES | 0x37 | 关键资产附属的本地信息，内容由业务自定义且**无完整性保护**，该项信息不会进行同步。 **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
...
| 类型 | 说明 |
| --- | --- |
| Map | 表示值类型是Map，键值对取值范围分别参考[Tag](#tag)和[Value](#value)。 |
...
| --- | --- | --- |
| NEVER | 0 | 不允许同步关键资产。 |
| THIS_DEVICE | 1 元服务API： 从API version 14开始，该接口支持在元服务中使用。
...
| failedErrorInfos | Array | 否 | 否 | 批量操作中失败的关键资产的错误信息数组，全部成功时为空数组。 |
