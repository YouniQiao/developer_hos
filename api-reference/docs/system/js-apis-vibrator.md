...
| callback | Callback | 是 | 回调函数，回调参数数据为VibratorStatusEvent。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 监听类型，该值固定为vibratorStateChange。 |
| callback | Callback | 否 | 回调函数，回调参数数据为VibratorStatusEvent，不填此参数则为注销所有callback |
...
![](./img/note_3.0-zh-cn.png) 由于设备存在多样性，不同的设备可能预置不同的效果，建议使用预置效果前先使用[vibrator.isSupportEffect](#vibratorissupporteffect10-1)10+接口查询当前设备是否支持该预置效果。
...
| points | Array | 否 | 是 | 可选参数，表示振动调节曲线数组。 |
...
| --- | --- | --- | --- |
| duration | number | 是 | 马达振动时长, 单位ms。取值范围是（0,1800000]区间的所有整数。由于实际产品厂商驱动对器件保护设计规格不同，不同设备实际最大振动时长会有差异。单次触发长振动一般建议不超过10秒，以最大化用户体验。 |
| callback | AsyncCallback | 否 | 回调函数，当马达振动成功，err为undefined，否则为错误对象。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 8 开始支持，从API version 9 开始废弃，建议使用 [vibrator.stopVibration](#vibratorstopvibration9-1)9+代替。
...
![](./img/note_3.0-zh-cn.png) 从API version 8 开始支持，从API version 9 开始废弃，建议使用 [vibrator.stopVibration](#vibratorstopvibration9)9+代替。
...
| --- | --- | --- | --- |
| stopMode | [VibratorStopMode](#vibratorstopmode) | 是 | 马达停止指定的振动模式。 |
| callback | AsyncCallback | 否 | 回调函数，当马达停止振动成功，err为undefined，否则为错误对象。 |
