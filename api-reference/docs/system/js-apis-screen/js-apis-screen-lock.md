...
content_hash: "a2187035c2c9"
synced_at: "2026-09-24T18:53:41.009613"
---
...
![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，除Lite Wearable外，从API version 9开始废弃。替代接口仅面向系统应用开放。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。返回true表示屏幕已锁屏；返回false表示屏幕未锁屏。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，除Lite Wearable外，从API version 9开始废弃。
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回true表示屏幕已锁屏；返回false表示屏幕未锁屏。 |
...
```
 ![](./img/note_3.0-zh-cn.png) Lite Wearable 不支持 Promise/async/await 等 ES6 语法，请使用上述 callback 形式的接口（[isScreenLocked(callback)](#screenlockisscreenlockeddeprecated)）。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。返回true表示当前设备的屏幕锁定安全；返回false表示当前设备的屏幕锁定不安全。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回true表示当前设备的屏幕锁定安全；返回false表示当前设备的屏幕锁定不安全。 |
...
```
 ![](./img/note_3.0-zh-cn.png) Lite Wearable 不支持 Promise/async/await 等 ES6 语法，请使用上述 callback 形式的接口（[isSecureMode(callback)](#screenlockissecuremodedeprecated)）。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。解锁屏幕成功，err为undefined，否则为错误对象。 |
...
```
 ![](./img/note_3.0-zh-cn.png) Lite Wearable 不支持 Promise/async/await 等 ES6 语法，请使用上述 callback 形式的接口（[unlockScreen(callback)](#screenlockunlockscreendeprecated)）。
