...
| callback | Callback | 是 | 回调函数，返回星闪的开关状态。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'pairingStateChange'，表示配对请求事件。 当调用[remoteDevice.startPairing](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/nearlink-remote-device#startpairing)发起主动配对，或者本机设备收到其他设备的配对请求时，触发该事件。 |
