...
content_hash: "8ec0a750aeaa"
synced_at: "2026-07-09T00:59:26.353916"
---
...
| callback | Callback | 是 | 注册的回调函数。[Connection](#connection)返回的连接对象。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'connectionAccepted'，收到对端连接，触发该事件。 |
| callback | Callback | 否 | 注册的回调函数。[Connection](#connection)返回的连接对象。 |
...
| callback | Callback | 是 | 注册的回调函数，number为返回的错误码。 |
...
| callback | Callback | 否 | 注册的回调函数，number为返回的错误码。 |
