...
content_hash: "893d457c32dc"
synced_at: "2026-07-28T16:51:05.660271"
---
...
| 名称 | 描述 |
| --- | --- |
| [FFRT_C_API int ffrt_task_attr_init(ffrt_task_attr_t* attr)](#ffrt_task_attr_init) | 初始化任务属性。调用成功后，任务属性将被设置为默认值（如默认QoS为[ffrt_qos_default](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_qos_default_t)）。使用完毕后须调用[ffrt_task_attr_destroy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_task_attr_destroy)释放任务属性。 |
...
| [FFRT_C_API void ffrt_task_attr_destroy(ffrt_task_attr_t* attr)](#ffrt_task_attr_destroy) | 销毁任务属性。必须在已通过[ffrt_task_attr_init](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_task_attr_init)初始化的任务属性上调用本接口，用于释放属性持有的资源。销毁后该任务属性不可再使用。 |
...
| [FFRT_C_API ffrt_task_handle_t ffrt_submit_h_base(ffrt_function_header_t* f, const ffrt_deps_t* in_deps, const ffrt_deps_t* out_deps, const ffrt_task_attr_t* attr)](#ffrt_submit_h_base) | 提交任务调度执行并返回任务句柄。任务执行体、输入依赖、输出依赖和任务属性被一同提交到FFRT调度器，调度器根据依赖关系确定任务的执行时机。返回的任务句柄可用于通过[ffrt_wait_deps](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_wait_deps)等待任务完成，或作为其他任务的输入依赖以构建任务间的依赖关系。本接口为返回任务句柄的底层提交接口，若任务不需要销毁回调，可使用简化接口[ffrt_submit_h_f](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_h_f)。返回的任务句柄需通过[ffrt_task_handle_destroy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_task_handle_destroy)销毁，引用计数可通过[ffrt_task_handle_inc_ref](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_task_handle_inc_ref)和[ffrt_task_handle_dec_ref](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_task_handle_dec_ref)管理。 |
| [FFRT_C_API void ffrt_submit_f(ffrt_function_t func, void* arg, const ffrt_deps_t* in_deps, const ffrt_deps_t* out_deps, const ffrt_task_attr_t* attr)](#ffrt_submit_f) | 提交任务调度执行，是[ffrt_submit_base](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_base)接口的简化形式。该接口将给定的任务函数及其参数包装为通用任务结构体（ffrt_function_kind_general），其中用于处理执行后清理的任务销毁回调（after_func）会被设为NULL，因而省略任何额外清理动作。封装后的任务结构体随后通过[ffrt_submit_base](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_base)接口提交。若任务属性中已通过[ffrt_task_attr_set_delay](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_task_attr_set_delay)设置延迟时间，则输入输出依赖关系不再生效，任务在延迟结束后被调度。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_task_attr_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-task-attr-t)* attr | 指向任务属性的指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [const ffrt_task_attr_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-task-attr-t)* attr | 指向任务属性的指针。 |
...
| --- | --- |
| [ffrt_task_attr_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-task-attr-t)* attr | 指向任务属性的指针。 |
| [ffrt_qos_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量) qos | 待设置的QoS等级，取值参考[ffrt_qos_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量)。 |

#### [h2]ffrt_task_attr_get_qos()
...
| 类型 | 说明 |
| --- | --- |
| FFRT_C_API [ffrt_qos_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量) | QoS等级，默认返回ffrt_qos_default。 |

#### [h2]ffrt_task_attr_set_delay()
...
#### [h2]ffrt_task_attr_set_queue_priority()
...
| --- | --- |
| [ffrt_task_attr_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-task-attr-t)* attr | 指向任务属性的指针。 |
| [ffrt_queue_priority_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_queue_priority_t) priority | 并发队列任务的执行优先级，取值参考[ffrt_queue_priority_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_queue_priority_t)；在同一并发队列内，高优先级任务优先于低优先级任务被调度。超出合法范围的值会被静默忽略。 |

#### [h2]ffrt_task_attr_get_queue_priority()
...
| 类型 | 说明 |
| --- | --- |
| FFRT_C_API [ffrt_queue_priority_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_queue_priority_t) | 并发队列任务的优先级。 |

#### [h2]ffrt_task_attr_set_stack_size()
...
#### [h2]ffrt_task_attr_get_stack_size()
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_qos_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量) qos | 任务待更新的QoS等级，取值参考[ffrt_qos_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量)。 |

返回：

| 类型 | 说明 |
| --- | --- |
| FFRT_C_API int | QoS更新成功，或新QoS与当前QoS相同时返回0； QoS映射未注册、当前任务为空，或当前任务非通用类型任务（即非通过[ffrt_submit_base](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_base)或[ffrt_submit_h_base](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_h_base)提交的任务）时返回1。 |
...
| 类型 | 说明 |
| --- | --- |
| FFRT_C_API [ffrt_qos_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量) | 任务QoS。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_function_kind_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_function_kind_t) kind | 函数执行结构体类型。通用类型适用于通过[ffrt_submit_base](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_base)或[ffrt_submit_h_base](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_h_base)提交的任务， 队列类型适用于通过并发队列提交接口提交的任务。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_function_header_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-function-header-t)* f | 任务执行体封装的指针，需通过[ffrt_alloc_auto_managed_function_storage_base](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_alloc_auto_managed_function_storage_base)申请内存，且必须包含任务销毁回调函数。 |
| [const ffrt_deps_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-deps-t)* in_deps | 指向输入依赖的指针。 |
| [const ffrt_deps_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-deps-t)* out_deps | 指向输出依赖的指针。 |
| [const ffrt_task_attr_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-task-attr-t)* attr | 指向任务属性的指针。 |
...
| 类型 | 说明 |
| --- | --- |
| FFRT_C_API [ffrt_task_handle_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-task-handle-t) | 任务提交成功时返回非空的任务句柄； 否则返回空指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_function_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_function_t) func | 待执行的任务函数。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_task_handle_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-task-handle-t) handle | 任务句柄，由[ffrt_submit_h_base](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_h_base)或[ffrt_submit_h_f](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-task-h#ffrt_submit_h_f)返回。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_task_handle_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-task-handle-t) handle | 任务句柄。 |
...
| 参数项 | 描述 |
| --- | --- |
| [const ffrt_deps_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-deps-t)* deps | 指向依赖任务列表的指针。阻塞当前任务，直到该依赖列表引用的所有任务执行完毕。 |
