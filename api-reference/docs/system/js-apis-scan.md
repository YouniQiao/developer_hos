...
![](./img/note_3.0-zh-cn.png) 本模块首批接口从API version 20开始支持。
...
| --- | --- | --- | --- |
| type | 'scanDeviceFound' | 是 | 事件类型。 |
| callback | Callback | 否 | 回调函数，返回扫描仪设备发现信息。 |
...
| --- | --- | --- | --- |
| type | 'scanDeviceSync' | 是 | 事件类型。 |
| callback | Callback | 否 | 回调函数，返回扫描仪设备同步信息。 |
