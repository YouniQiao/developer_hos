...
| 类型 | 说明 |
| --- | --- |
| [TimeService_ErrCode](#timeservice_errcode) | 返回TIMESERVICE_ERR_OK表示成功。 返回TIMESERVICE_ERR_INTERNAL_ERROR表示获取系统参数失败。 返回TIMESERVICE_ERR_INVALID_PARAMETER表示timeZone为NULL指针或时区名称（不包括结束字符（'\0'））的大小大于或等于len。 |
