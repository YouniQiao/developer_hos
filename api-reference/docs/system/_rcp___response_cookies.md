...
| bool [secure](#secure) | 响应Cookie安全属性。true表示此cookie是通过安全连接返回的，false表示此cookie不是通过安全连接返回的。 |
| bool [httpOnly](#httponly) | 响应Cookie httpOnly属性。true表示不可通过页面脚本等活动内容访问cookie，false表示表示可以通过页面脚本等活动内容访问cookie。 |
