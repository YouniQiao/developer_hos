...
| supportedUAFVersions | Array | 是 | 否 | 认证器支持的UAF协议版本。 |
...
| attestationTypes | Array | 是 | 否 | 支持的鉴别类型列表。 目前支持TAG_ATTESTATION_BASIC_FULL(0x3E07)。 |
...
| tcDisplayPNGCharacteristics | Array | 是 | 否 | 交易确认显示支持的PNG集合。 |
| icon | string | 是 | 否 | 认证器的PNG图标。长度由不同PNG图片Base64编码长度限制。 |
| supportedExtensionIDs | Array | 是 | 否 | UAF协议扩展标识符的支持列表。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| supportedUAFVersions | Array | 否 | 否 | 客户端支持的FIDO UAF协议版本列表。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [common.Context](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-context#context) | 是 | 当前ability的应用上下文信息。 |

返回值:

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回包含当前设备软件支持的认证器数据的对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回包含签名后响应数据的UAFMessage消息对象。 |
