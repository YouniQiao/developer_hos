...
content_hash: "ebf1b86715e5"
synced_at: "2026-08-29T18:17:01.333398"
---
...
1. 检查源文件路径。 确保路径不为空、不包含“../”等相对路径，并且位于正确的应用沙箱路径下。
...
1. 检查目标路径。 确保路径不为空、不包含“../”等相对路径，并且位于正确的应用沙箱路径下。
...
1. 检查源文件是否为有效且完整的ZIP文件。 如果源文件通过网络下载，请确保文件下载完成后再调用decompressFile接口。文件已损坏时，请重新生成或获取ZIP文件。
