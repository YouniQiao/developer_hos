...
| SMB_FILE_SEND | 0x0F000001 | SMB(Samba)外发事件。 **起始版本：** 6.1.0(23) |
| KIA_PRE_OPEN | 0x1C000014 | KIA文件秒开事件。 **起始版本：** 6.1.0(23) |
| HDC_DEBUG | 0x27000100 | HDC(HarmonyOS Device Connector)调测文件事件。 **起始版本：** 6.1.0(23) |
| HDC_DEBUG_INTERCEPTED | 0x27000101 | HDC(HarmonyOS Device Connector)调测拦截事件。 **起始版本：** 6.1.0(23) |
| USER_SPACE_DATA_TRANSFER | 0x2F000000 | 多用户空间数据互传事件。 **起始版本：** 6.1.0(23) |
| USER_SPACE_DATA_TRANSFER_POLICY | 0x2F000001 | 多用户空间互换审核策略事件。 **起始版本：** 6.1.0(23) |
| SERIAL_PORT_ACCESS | 0x30000100 | 串口访问审计事件。 **起始版本：** 6.1.0(23) |
...
| callback | Callback | 是 | 用于接收审计事件的回调函数。 |
...
| callback | Callback | 否 | 用于接收审计数据的回调函数。如果传入了callback，则取消该callback的订阅，否则取消所有callback的订阅。 |
...
![](./img/note_3.0-zh-cn.png) 需要通过[newClient](#newclient)构造实例。
...
![](./img/note_3.0-zh-cn.png) 需要通过[newAuthClient](#newauthclient)构造实例。
