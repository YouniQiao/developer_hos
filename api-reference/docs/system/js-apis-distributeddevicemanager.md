...
| 类型 | 说明 |
| --- | --- |
| Array | 返回可信设备列表，包含设备标识、名称、类型和网络标识等信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数。当获取可信设备列表成功时，err为undefined，data为获取到的可信设备列表；失败时，err为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，resolve时返回分布式设备基本信息列表，reject时返回错误信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| discoverParam | {[key: string]: Object;} | 是 | 发现配置参数。用于指定发现的目标类型。 discoverTargetType: 发现目标类型，取值为1，表示设备类型。传入其他值时不生效。 |
| filterOptions | {[key: string]: Object;} | 否 | 发现设备过滤信息。可选。默认值为undefined，表示发现未上线的设备。会携带以下key值（不传入某个key时，表示不按该维度进行筛选）： availableStatus(0-1)：筛选发现设备的可用状态，传入超出范围的值时不生效。 -0：设备离线，客户端需要通过调用bindTarget绑定设备。 -1：设备已在线，客户端可以进行连接。 discoverDistance(0-100)：根据传入的距离值，发现本地周围指定距离内的设备，单位为cm。传入超出范围的值时不生效。使用WiFi进行设备发现时不支持该参数，若传入则不生效。 authenticationStatus(0-1)：根据不同的认证状态发现设备，传入超出范围的值时不生效： -0：设备未认证，客户端需要通过调用bindTarget进行认证绑定。 -1：设备已认证，可用于分布式业务。 authorizationType(0-2)：根据不同的授权类型发现设备，传入超出范围的值时不生效： -0：根据临时协商的会话密钥认证的设备。 -1：基于同账号密钥进行身份验证的设备。 -2：基于不同账号凭据密钥认证的设备。 |
...
| bindParam | {[key: string]: Object;} | 是 | 认证参数。支持传入以下key值： bindType（number，必填）：绑定的类型，取值为1，表示PIN码认证。传入其他值时返回参数错误。 targetPkgName（string，可选）：绑定目标的包名，不传入时默认使用调用方应用的Bundle名称。 appName（string，可选）：尝试绑定目标的应用名称，用于在认证界面展示发起绑定的应用名称，不传入时不展示。 appOperation（string，可选）：应用要绑定目标的原因，用于向用户展示绑定目的，不传入时不展示。 customDescription（string，可选）：绑定操作的详细补充说明，用于向用户展示更具体的操作描述，不传入时不展示。 |
| callback | AsyncCallback | 是 | 认证结果回调。当认证成功时，err为undefined，data为包含deviceId的对象；当认证失败时，err为错误对象。 |
...
| callback | Callback | 是 | 注册设备状态的回调方法，返回设备状态和设备信息。 |
...
| callback | Callback | 否 | 指示要取消注册的设备状态回调，返回设备状态和设备信息。如果指定该参数则取消对应callback，否则取消所有已注册的deviceStateChange回调。 |
...
| callback | Callback | 是 | 注册设备发现的回调方法，返回发现的设备信息（DeviceBasicInfo）。 |
...
| callback | Callback | 否 | 指示要取消注册的设备发现回调，返回发现的设备信息。如果指定该参数则取消对应callback，否则取消所有已注册的discoverSuccess回调。 |
...
| callback | Callback | 是 | 注册设备名称改变的回调方法，返回变更后的设备名称。 |
...
| callback | Callback | 否 | 指示要取消注册设备名称改变的回调方法。如果指定该参数则取消对应callback，否则取消所有已注册的deviceNameChange回调。 |
...
| callback | Callback | 是 | 注册设备发现失败的回调方法，返回设备发现失败的原因。回调参数reason为数值类型，表示发现失败的错误原因码，具体取值可参见[设备管理错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-device-manager)。 |
...
| callback | Callback | 否 | 指示要取消注册的设备发现失败回调。如果指定该参数则取消对应callback，否则取消所有已注册的discoverFailure回调。 |
