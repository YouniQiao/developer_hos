...
content_hash: "39a342e2bd56"
synced_at: "2026-07-28T16:50:41.483591"
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数。当获取可信设备列表成功，err为undefined，data为获取到的Array；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回分布式设备基本信息列表。 |
...
| callback | AsyncCallback | 是 | 认证结果回调。 |
...
| callback | Callback | 是 | 指示要注册的设备状态回调，返回设备状态和设备信息。 |
...
| callback | Callback | 否 | 指示要取消注册的设备状态回调，返回设备状态和设备信息。 |
...
| callback | Callback | 是 | 注册设备发现的回调方法。 |
...
| callback | Callback | 否 | 指示要取消注册的设备发现回调，返回设备状态和设备信息。 |
...
| callback | Callback | 是 | 注册设备名称改变的回调方法。 |
...
| callback | Callback | 否 | 指示要取消注册设备名称改变的回调方法。 |
...
| callback | Callback | 是 | 注册设备发现失败的回调方法。 |
...
| callback | Callback | 否 | 指示要取消注册的设备发现失败回调。 |
