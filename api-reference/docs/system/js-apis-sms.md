...
content_hash: "3bf6202516cc"
synced_at: "2026-08-29T18:16:54.151210"
---
...
| callback | AsyncCallback | 是 | 获取短信实例的回调函数。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 6开始支持，从API version 10开始废弃。建议使用[sendShortMessage](#smssendshortmessage10)替代。
...
| --- | --- | --- | --- |
| options | [SendMessageOptions](#sendmessageoptions) | 是 | 发送短信的参数和回调，参考[SendMessageOptions](#sendmessageoptions)。 |
| callback | AsyncCallback | 是 | 发送短信的回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 获取发送短信的默认SIM卡槽ID的回调函数。 - 0：卡槽1 - 1：卡槽2 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 获取默认短信SIM的SIM ID的回调函数。 与SIM卡绑定，从1开始递增。 无卡时返回值为-1。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | 以Promise形式返回发送短信的默认SIM卡ID： 与SIM卡绑定，从1开始递增。 无卡时返回值为-1。 |
...
| pdu | Array | 否 | 否 | SMS消息中的协议数据单元(PDU)。 |
...
| content | string | Array | 否 | 否 | 如果内容是字符串，则这是一条文本短信。如果内容是字节数组，则这是一条数据短信。 |
...
| sendCallback | AsyncCallback | 否 | 是 | 短信发送结果回调，返回短信发送的结果，参考[ISendShortMessageCallback](#isendshortmessagecallback)。发送数据短信时，此项必填；发送文本短信时，此项可选，不填写则不会收到短信发送结果的回调通知。 |
| deliveryCallback | AsyncCallback | 否 | 是 | 短信送达结果回调，返回短信递送报告，参考[IDeliveryShortMessageCallback](#ideliveryshortmessagecallback)。发送数据短信时，此项必填；发送文本短信时，此项可选，不填写则不会收到短信送达报告的回调通知。 |
