...
![](./img/caution_3.0-zh-cn.png) 导入tag模块编辑器报错，在某个具体设备型号上能力可能超出工程默认设备定义的能力集范围，如需要使用此部分能力需额外配置自定义syscap，参考[syscap开发指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/syscap)。
...
#### [h2]IsoDepTag.getHistoricalBytes9+
...
#### [h2]IsoDepTag.getHiLayerResponse9+
...
#### [h2]IsoDepTag.isExtendedApduSupported9+
...
#### [h2]NdefMessage.getNdefRecords9+
...
| **类型** | **说明** |
| --- | --- |
| Promise | Promise对象。返回从NDEF标签中读取到的Message数据对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数，返回从NDEF标签中读取到的Message信息。 |
...
| --- | --- | --- | --- |
| msg | [NdefMessage](#ndefmessage9) | 是 | NDEF Message数据对象。 |
| callback | AsyncCallback | 是 | 回调函数。当NDEF Message数据对象写入成功时，err为undefined，否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当NDEF标签设置为只读成功时，err为undefined，否则为错误对象。 |
...
#### [h2]NdefTag.getNdefTagTypeString9+
...
#### [h2]MifareClassicTag.authenticateSector9+
...
| key | number[] | 是 | 用于扇区验证的密钥（6字节）。 |
| isKeyA | boolean | 是 | isKeyA标志。true 表示KeyA，false 表示KeyB。 |
| callback | AsyncCallback | 是 | 回调函数。当身份验证成功时，err为undefined，否则为错误对象。 |
...
#### [h2]MifareClassicTag.readSingleBlock9+
...
| --- | --- | --- | --- |
| blockIndex | number | 是 | 要读取的块索引，从0开始。 |
| callback | AsyncCallback | 是 | 以callback形式异步返回读取到的块数据。 |
...
#### [h2]MifareClassicTag.writeSingleBlock9+
...
| blockIndex | number | 是 | 要写入的块索引，从0开始。 |
| data | number[] | 是 | 要写入的数据，大小必须是16个字节。 |
| callback | AsyncCallback | 是 | 回调函数。当向块存储写入内容成功时，err为undefined，否则为错误对象。 |
...
#### [h2]MifareClassicTag.incrementBlock9+
...
| callback | AsyncCallback | 是 | 回调函数。当对块增加指定数值成功时，err为undefined，否则为错误对象。 |
...
#### [h2]MifareClassicTag.decrementBlock9+
...
| blockIndex | number | 是 | 要被运算的块索引，从0开始。 |
| value | number | 是 | 要减少的数值，非负数。 |
| callback | AsyncCallback | 是 | 回调函数。当对块减少指定数值成功时，err为undefined，否则为错误对象。 |
...
#### [h2]MifareClassicTag.transferToBlock9+
...
| --- | --- | --- | --- |
| blockIndex | number | 是 | 被操作的块的索引，从0开始。 |
| callback | AsyncCallback | 是 | 回调函数。当临时寄存器的值转移到指定块成功时，err为undefined，否则为错误对象。 |
...
#### [h2]MifareClassicTag.restoreFromBlock9+
...
| --- | --- | --- | --- |
| blockIndex | number | 是 | 被操作的块的索引，从0开始。 |
| callback | AsyncCallback | 是 | 回调函数。当复制指定块内容到临时寄存器成功时，err为undefined，否则为错误对象。 |
...
#### [h2]MifareClassicTag.getSectorCount9+
...
#### [h2]MifareClassicTag.getBlockCountInSector9+
...
#### [h2]MifareClassicTag.getTagSize9+
...
#### [h2]MifareClassicTag.isEmulatedTag9+
...
#### [h2]MifareClassicTag.getBlockIndex9+
...
#### [h2]MifareClassicTag.getSectorIndex9+
...
#### [h2]MifareUltralightTag.readMultiplePages9+
...
| **类型** | **说明** |
| --- | --- |
| Promise | Promise对象。以Promise形式返回读取的4页的数据，共16字节。 |
...
| --- | --- | --- | --- |
| pageIndex | number | 是 | 要读取页面的索引，从0开始。 |
| callback | AsyncCallback | 是 | 以callback形式异步返回页操作结果。返回读取到的数据，共16字节。 |
...
#### [h2]MifareUltralightTag.writeSinglePage9+
...
| pageIndex | number | 是 | 要写入页面的索引，从0开始。 |
| data | number[] | 是 | 要写入页面的数据内容，必须是4个字节大小。 |
| callback | AsyncCallback | 是 | 回调函数。当写入数据成功时，err为undefined，否则为错误对象。 |
...
#### [h2]MifareUltralightTag.getType9+
...
| --- | --- | --- | --- |
| message | [NdefMessage](#ndefmessage9) | 是 | 格式化成功时要写入的Ndef消息。可以为null，为null时仅格式化标签，不写入内容。 |
| callback | AsyncCallback | 是 | 回调函数。当NDEF消息写入标签成功时，err为undefined，否则为错误对象。 |
...
#### [h2]NdefFormatableTag.formatReadOnly9+
...
| --- | --- | --- | --- |
| message | [NdefMessage](#ndefmessage9) | 是 | 格式化成功时要写入的NDEF消息。可以为null，为null时仅格式化标签，不写入内容。 |
| callback | AsyncCallback | 是 | 回调函数。当NDEF消息写入NDEF标签成功时，err为undefined，否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回BarCode类型的 tag。 |
