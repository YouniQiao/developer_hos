...
synced_at: "2026-06-24T20:51:18.411239"
---
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| preKeys | Array | 否 | 否 | 修饰键（包括 Ctrl、Shift 和 Alt）集合，数量范围[1, 4]，无顺序要求。 例如，Ctrl+Shift+Esc中，Ctrl+Shift称为修饰键。 |
| finalKey | number | 否 | 否 | 被修饰键，除修饰键和Meta键以外的按键，详细按键介绍请参见[@ohos.multimodalInput.keyCode (键值)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-keycode)。 例如，Ctrl+Shift+Esc中，Esc称为被修饰键。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回所有系统快捷键的列表。 |
...
| callback | Callback | 是 | 回调函数，返回满足条件的组合按键输入事件。 |
...
| type | string | 是 | 事件类型，固定取值为'hotkeyChange'。 |
| hotkeyOptions | [HotkeyOptions](#hotkeyoptions) | 是 | 快捷键选项。 |
| callback | Callback | 否 | 需要取消订阅的回调函数。若缺省，则取消当前应用快捷键选项已订阅的所有回调函数。 |
...
| callback | Callback | 是 | 回调函数，返回按键事件。订阅不同的按键事件需要使用不同的callback，否则订阅不生效。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件类型，固定取值为'keyPressed'。 |
| callback | Callback | 否 | 需要取消订阅的回调函数。若缺省，则取消当前已订阅的所有回调函数。 |
