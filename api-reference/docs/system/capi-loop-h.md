...
| 名称 | 描述 |
| --- | --- |
| [ffrt_loop_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-loop-t) | loop句柄。 |
...
| [FFRT_C_API int ffrt_loop_epoll_ctl(ffrt_loop_t loop, int op, int fd, uint32_t events, void *data, ffrt_poller_cb cb)](#ffrt_loop_epoll_ctl) | 管理loop上的监听事件。 不建议在cb中调用exit函数，可能导致未定义行为。 |
| [FFRT_C_API ffrt_timer_t ffrt_loop_timer_start(ffrt_loop_t loop, uint64_t timeout, void* data, ffrt_timer_cb cb, bool repeat)](#ffrt_loop_timer_start) | 在ffrt loop上启动定时器。 不建议在cb中调用exit函数，可能导致未定义行为。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_queue_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-queue-t) queue | 并发队列。 |

返回：

| 类型 | 说明 |
| --- | --- |
| FFRT_C_API [ffrt_loop_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-loop-t) | 创建成功返回ffrt_loop_t对象， 创建失败返回空指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_loop_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-loop-t) loop | loop对象。 |
...
| [ffrt_poller_cb](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_poller_cb) cb | 事件变化时触发的回调函数。 |
...
| void* data | 事件变化时触发的回调函数的入参。 |
| [ffrt_timer_cb](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_timer_cb) cb | 事件变化时触发的回调函数。 |
...
| 类型 | 说明 |
| --- | --- |
| FFRT_C_API [ffrt_timer_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量) | 返回定时器句柄。 |
