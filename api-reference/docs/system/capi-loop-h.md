...
content_hash: "65318a16f290"
synced_at: "2026-07-28T16:51:04.464665"
---
...
| 名称 | 描述 |
| --- | --- |
| [ffrt_loop_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-loop-t) | loop句柄，用于标识不同的loop。 |
...
| [FFRT_C_API int ffrt_loop_run(ffrt_loop_t loop)](#ffrt_loop_run) | 启动一次loop循环。该函数会独占调用线程，在当前调用线程中同步运行事件循环，直到调用[ffrt_loop_stop](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-loop-h#ffrt_loop_stop)后才会返回。 |
| [FFRT_C_API void ffrt_loop_stop(ffrt_loop_t loop)](#ffrt_loop_stop) | 停止loop循环。调用后，正在执行[ffrt_loop_run](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-loop-h#ffrt_loop_run)的线程将停止循环并返回。 |
...
| [FFRT_C_API ffrt_timer_t ffrt_loop_timer_start(ffrt_loop_t loop, uint64_t timeout, void* data, ffrt_timer_cb cb, bool repeat)](#ffrt_loop_timer_start) | 在ffrt loop上启动定时器。超时后调用回调函数；若repeat为true，则周期性重复触发。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_queue_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-queue-t) queue | 队列。 |

返回：

| 类型 | 说明 |
| --- | --- |
| FFRT_C_API [ffrt_loop_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-loop-t) | loop创建成功时返回非空的loop句柄； 否则返回空指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ffrt_loop_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-loop-t) loop | loop句柄。 |
...
| [ffrt_poller_cb](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_poller_cb) cb | 当目标fd被轮询到时执行的用户回调函数。 |
...
| void* data | 传递给cb的用户数据。 |
| [ffrt_timer_cb](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#ffrt_timer_cb) cb | 超时后执行的用户回调函数。 |
...
| 类型 | 说明 |
| --- | --- |
| FFRT_C_API [ffrt_timer_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量) | 定时器句柄；若loop或cb为空则返回-1。 |
...
| --- | --- |
| [ffrt_loop_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt-ffrt-loop-t) loop | loop句柄。 |
| [ffrt_timer_t](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-type-def-h#变量) handle | 定时器句柄，由[ffrt_loop_timer_start](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-loop-h#ffrt_loop_timer_start)返回。 |
