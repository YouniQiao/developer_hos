...
| 名称 | 类型 | 说明 |
| --- | --- | --- |
| ShapeInfo | [ShapeInfo](#shapeinfo) | 一笔成形识别结果对象，包含识别的图像的基本信息。 |

本模块提供以下方法。

| 方法名称 | 说明 |
| --- | --- |
| [processTouchEvent](#processtouchevent) | 传递触摸事件。 |
| [getPathFromString](#getpathfromstring) | 从给定的形状字符串中提取形状信息。 |
| [notifyAreaChange](#notifyareachange) | 通知控件大小变化。 |
| [setPauseTime](#setpausetime) | 设置触发识别的暂停时间，单位：ms。 |
| [release](#release) | 销毁识别工具。 |
| [onShapeRecognized](#onshaperecognized) | 注册识别完成时的回调方法。使用callback异步回调。 |
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| callback | Callback | 是 | 回调函数，返回一笔成形识别结果对象，包含识别的图像的基本信息。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| [InstantShapeGenerator](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pen-instantsshapegenerator) | 一笔成形工具方法类实例。 |
