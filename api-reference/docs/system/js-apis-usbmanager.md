...
content_hash: "a6a5b5553da3"
synced_at: "2026-07-09T00:59:39.692017"
---
...
![](./img/note_3.0-zh-cn.png) 三方应用没有权限获取serial字段读取设备序列号，需要通过[requestRight](#usbmanagerrequestright)申请权限后，自行发起控制传输获取。
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，获取传输或接收到的数据块大小。失败返回其它错误码如下： - -1：驱动异常。 |
...
| 类型 | 说明 |
| --- | --- |
| Array> | 只读的USB配件列表。当前仅支持列表中包含1个USB配件。 |
...
![](./img/note_3.0-zh-cn.png) 本接口调用后会重置此前设置的配置和替换接口，请在调用之前确认相关业务已结束。
...
![](./img/note_3.0-zh-cn.png) 从 API version 9开始支持，从API version 12开始废弃。建议使用 [usbControlTransfer](#usbmanagerusbcontroltransfer12) 替代。
...
![](./img/note_3.0-zh-cn.png) 主机控制器按照Endpoint类型调度。
...
| interfaces | Array | 否 | 否 | 配置支持的接口属性。 |
...
| callback | AsyncCallback | 否 | 否 | 传输完成时的回调信息。 |
...
![](./img/note_3.0-zh-cn.png) 从 API version 9开始支持，从API version 18开始废弃。建议使用 [USBDeviceRequestParams](#usbdevicerequestparams12) 替代。
