...
content_hash: "7c9c0285e037"
synced_at: "2026-08-29T18:17:12.408528"
---
...
| callback | Callback | 是 | 回调函数，用于接收设备静止姿态（支架态）状态信息。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件类型。固定传入'steadyStandingDetect'，表示设备静止姿态（支架态）感知。 |
| callback | Callback | 否 | 要注销的回调函数，需与订阅时传入的回调函数一致。若不填，则取消当前监听该事件的所有回调函数。 |
