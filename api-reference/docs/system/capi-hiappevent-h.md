...
content_hash: "d1d98ec64e8d"
synced_at: "2026-07-09T01:00:04.219020"
---
...
| [HiAppEvent_Watcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-watcher) | HiAppEvent_Watcher | 接收应用事件的监听器。用于对应用事件进行监听和处理。 |
...
| [HiAppEvent_Watcher* OH_HiAppEvent_CreateWatcher(const char* name)](#oh_hiappevent_createwatcher) | - | 创建一个用于监听应用事件的监听器。 **注意**：创建的监听器不再使用后，必须通过[OH_HiAppEvent_DestroyWatcher](#oh_hiappevent_destroywatcher)接口进行销毁。 |
...
![](./img/caution_3.0-zh-cn.png) 创建的监听器不再使用后，必须通过[OH_HiAppEvent_DestroyWatcher](#oh_hiappevent_destroywatcher)接口进行销毁。
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent_Watcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-watcher)* | 接口调用成功时返回指向的新建监听器的指针，当name为空指针时返回nullptr。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent_Watcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-watcher)* watcher | 指向监听器的指针（即OH_HiAppEvent_CreateWatcher接口返回的指针）。 |
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent_Processor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-processor)* | 接口调用成功时返回指向的新建处理者的指针，name参数异常时返回nullptr。 |
