---
title: "hiappevent.h"
upstream_id: "harmonyos-references/capi-hiappevent-h"
catalog: "harmonyos-references"
content_hash: "9e5af378563b"
synced_at: "2026-07-28T16:51:23.165243"
---
...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| [HiAppEvent_AppEventInfo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-appeventinfo) | HiAppEvent_AppEventInfo | 单个事件信息，包含事件领域、事件名称、事件类型和事件携带的用JSON格式字符串表示的自定义参数列表。 |
| [HiAppEvent_AppEventGroup](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-appeventgroup) | HiAppEvent_AppEventGroup | 一组事件信息，用于管理和组织具有相同名称的事件信息。该结构体包含事件组的名称，按名称分组的单个事件信息数组，事件数组的长度。 |
| [ParamListNode*](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-paramlistnode8h) | ParamList | 事件参数列表节点。用于组织和管理事件参数列表信息，通过ParamListNode可以构建参数链表，支持多参数事件的参数传递。 |
| [HiAppEvent_Watcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-watcher) | HiAppEvent_Watcher | 接收应用事件的事件观察者。用于对应用事件进行监听和处理。 |
| [HiAppEvent_Processor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-processor) | HiAppEvent_Processor | 应用事件上报的处理者。用于事件的上报和管理，开发者可自定义数据处理配置，满足不同的数据处理需求。 |
| [HiAppEvent_Config](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-config) | HiAppEvent_Config | 设置系统事件自定义规格的配置对象。可以用来自定义系统事件规格参数，参数列表设置可参考应用事件的[宏定义](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-param-h#宏定义)。 |
...
| [typedef void (*OH_HiAppEvent_OnTrigger)(int row, int size)](#oh_hiappevent_ontrigger) | OH_HiAppEvent_OnTrigger | 事件观察者收到事件后，若事件观察者中未设置OH_HiAppEvent_OnReceive回调，将保存该事件。 当保存的事件满足通过OH_HiAppEvent_SetTriggerCondition设定的条件后，将触发该回调。回调结束后，当新保存的事件消息再次满足设定的条件后，将再次进行回调。 |
...
| [HiAppEvent_Watcher* OH_HiAppEvent_CreateWatcher(const char* name)](#oh_hiappevent_createwatcher) | - | 创建一个用于监听应用事件的事件观察者。 **注意**：创建的事件观察者不再使用后，必须通过[OH_HiAppEvent_DestroyWatcher](#oh_hiappevent_destroywatcher)接口进行销毁。 |
...
| [int OH_HiAppEvent_SetTriggerCondition(HiAppEvent_Watcher* watcher, int row, int size, int timeOut)](#oh_hiappevent_settriggercondition) | - | 用于设置事件观察者OH_HiAppEvent_OnTrigger回调的触发条件。 分别可以从事件观察者新接收事件数量、新接收事件大小、onTrigger触发超时时间，设置触发条件。调用方应至少保证从一个方面设置触发条件。 |
...
| [int OH_HiAppEvent_SetWatcherOnTrigger(HiAppEvent_Watcher* watcher, OH_HiAppEvent_OnTrigger onTrigger)](#oh_hiappevent_setwatcherontrigger) | - | 用于设置事件观察者onTrigger回调的接口。 如果未设置OnReceive回调或已将其设置为nullptr，则将保存观察者接收到的应用事件。当保存的应用事件满足onTrigger回调的触发条件时，将调用onTrigger回调。 |
...
| [int OH_HiAppEvent_AddWatcher(HiAppEvent_Watcher* watcher)](#oh_hiappevent_addwatcher) | - | 添加事件观察者的接口，事件观察者开始监听系统消息。 **注意**：OH_HiAppEvent_AddWatcher接口涉及I/O操作。在对性能敏感的业务场景中，开发者应根据实际需要确定该接口是在主线程还是在子线程中调用。 订阅接口OH_HiAppEvent_AddWatcher传入的名称name是唯一的，相同的name，后一次调用会覆盖前一次的订阅。 |
...
| [HiAppEvent_Processor* OH_HiAppEvent_CreateProcessor(const char* name)](#oh_hiappevent_createprocessor) | - | 创建一个用于处理应用事件上报的处理者。 **注意**：创建的处理者不再使用后，必须通过[OH_HiAppEvent_DestroyProcessor](#oh_hiappevent_destroyprocessor)接口进行销毁。 |
...
| [int OH_HiAppEvent_SetEventConfig(const char* name, HiAppEvent_Config* config)](#oh_hiappevent_seteventconfig) | - | 事件相关的配置参数设置方法。 不同的事件有不同的配置项，目前仅支持以下事件： MAIN_THREAD_JANK（参数配置详见[主线程超时事件检测](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-mainthreadjank-events#oh_hiappevent_seteventconfig接口参数设置说明)） MAIN_THREAD_JANK_V2（参数配置详见[主线程超时事件检测](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-mainthreadjank-events#oh_hiappevent_seteventconfig接口参数设置说明)） EVENT_APP_CRASH（参数配置详见[崩溃事件介绍](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-crash-events#oh_hiappevent_seteventconfig接口参数设置说明)），从API version 24开始支持该事件。 |
| [int OH_HiAppEvent_ReportFrameworkMemAnomaly(enum OH_HiAppEvent_FrameworkType frameworkType, const char* frameworkVersion, const char* description)](#oh_hiappevent_reportframeworkmemanomaly) | - | 报告应用框架内存占用异常的信息。 该接口的调用频率限制为：1分钟最多能成功调用1次，超过频率限制会返回错误码HIAPPEVENT_REPORT_FREQUENCY_EXCEEDED。 当应用检测到应用框架内存占用异常，并且调用该接口返回操作成功时： 1. 若开发者已经订阅了事件领域domain为“HIVIEWDFX”，且事件名称names为“FW_MEM_ANOMALY”的应用事件，则应用中将会收到应用框架内存占用异常信息的回调。 2. 若开发者未订阅该应用事件，则应用中将不会收到应用框架内存占用异常信息的回调。 |
...
| HIAPPEVENT_REPORT_FREQUENCY_EXCEEDED = -300 | 报告频率超限。 **起始版本：** 26.0.0 |
...
#### [h2]OH_HiAppEvent_FrameworkType
...
| [const struct HiAppEvent_AppEventGroup](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-appeventgroup)* appEventGroups | 按照不同事件名称分组的事件组数组。 |
| uint32_t groupLen | 事件组数组的长度。 |

#### [h2]OH_HiAppEvent_OnTrigger()
...
#### [h2]OH_HiAppEvent_CreateParamList()
...
![](./img/caution_3.0-zh-cn.png) 创建的指向参数列表对象的指针不再使用后，必须通过[OH_HiAppEvent_DestroyParamList](#oh_hiappevent_destroyparamlist)接口进行销毁。
...
#### [h2]OH_HiAppEvent_DestroyParamList()
...
#### [h2]OH_HiAppEvent_AddBoolArrayParam()
...
#### [h2]OH_HiAppEvent_AddInt8ArrayParam()
...
#### [h2]OH_HiAppEvent_AddInt16Param()
...
#### [h2]OH_HiAppEvent_AddInt16ArrayParam()
...
#### [h2]OH_HiAppEvent_AddInt32Param()
...
#### [h2]OH_HiAppEvent_AddInt32ArrayParam()
...
#### [h2]OH_HiAppEvent_AddInt64Param()
...
#### [h2]OH_HiAppEvent_AddInt64ArrayParam()
...
#### [h2]OH_HiAppEvent_AddFloatParam()
...
#### [h2]OH_HiAppEvent_AddFloatArrayParam()
...
#### [h2]OH_HiAppEvent_AddDoubleParam()
...
#### [h2]OH_HiAppEvent_AddDoubleArrayParam()
...
#### [h2]OH_HiAppEvent_AddStringParam()
...
#### [h2]OH_HiAppEvent_AddStringArrayParam()
...
| 参数项 | 描述 |
| --- | --- |
| const char* name | 配置项名称。名称可填[DISABLE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-cfg-h#disable)和[MAX_STORAGE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-cfg-h#max_storage)。 |
| const char* value | 配置项值。如果配置项名称是[DISABLE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-cfg-h#disable)，值可以填“true”或者“false”； 如果配置项名称是[MAX_STORAGE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-cfg-h#max_storage)，配额值字符串只由数字字符和大小单位字符（单位字符支持[b|k|kb|m|mb|g|gb|t|tb]，不区分大小写）构成。 配额值字符串必须以数字开头，后面可以选择不传单位字符（默认使用byte作为单位），或者以单位字符结尾。 |
...
#### [h2]OH_HiAppEvent_CreateWatcher()
...
![](./img/caution_3.0-zh-cn.png) 创建的事件观察者不再使用后，必须通过[OH_HiAppEvent_DestroyWatcher](#oh_hiappevent_destroywatcher)接口进行销毁。
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent_Watcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-watcher)* | 接口调用成功时返回指向的新建事件观察者的指针，当name为空指针时返回NULL。 |

#### [h2]OH_HiAppEvent_DestroyWatcher()
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent_Watcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-watcher)* watcher | 指向事件观察者的指针（即OH_HiAppEvent_CreateWatcher接口返回的指针）。 |

#### [h2]OH_HiAppEvent_SetTriggerCondition()
...
#### [h2]OH_HiAppEvent_SetAppEventFilter()
...
#### [h2]OH_HiAppEvent_SetWatcherOnTrigger()
...
| --- | --- |
| [HiAppEvent_Watcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-watcher)* watcher | 指向事件观察者的指针（即OH_HiAppEvent_CreateWatcher接口返回的指针）。 |
| [OH_HiAppEvent_OnTrigger](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#oh_hiappevent_ontrigger) onTrigger | 需要设置的回调。 |
...
#### [h2]OH_HiAppEvent_SetWatcherOnReceive()
...
| --- | --- |
| [HiAppEvent_Watcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-watcher)* watcher | 指向事件观察者的指针（即OH_HiAppEvent_CreateWatcher接口返回的指针）。 |
| [OH_HiAppEvent_OnReceive](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#oh_hiappevent_onreceive) onReceive | 回调函数的函数指针。 |
...
#### [h2]OH_HiAppEvent_TakeWatcherData()
...
| [OH_HiAppEvent_OnTake](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#oh_hiappevent_ontake) onTake | 回调函数指针，事件通过调用该函数返回事件信息。 |
...
![](./img/caution_3.0-zh-cn.png) OH_HiAppEvent_AddWatcher接口涉及I/O操作。在对性能敏感的业务场景中，开发者应根据实际需要确定该接口是在主线程还是在子线程中调用。
...
#### [h2]OH_HiAppEvent_RemoveWatcher()
...
![](./img/caution_3.0-zh-cn.png) 创建的处理者不再使用后，必须通过[OH_HiAppEvent_DestroyProcessor](#oh_hiappevent_destroyprocessor)接口进行销毁。
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent_Processor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-processor)* | 接口调用成功时返回指向的新建处理者的指针，name参数异常时返回NULL。 |

#### [h2]OH_HiAppEvent_SetReportRoute()
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent_Processor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-processor)* processor | 指向处理者的指针（即OH_HiAppEvent_CreateProcessor接口返回的指针）。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功； [HIAPPEVENT_PROCESSOR_IS_NULL](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：processor入参为空； [HIAPPEVENT_INVALID_PARAM_VALUE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：参数值无效； [HIAPPEVENT_INVALID_UID](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：用户标识无效； [HIAPPEVENT_INVALID_PARAM_VALUE_LENGTH](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：参数值长度无效。 具体可参考[HiAppEvent_ErrorCode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功； [HIAPPEVENT_PROCESSOR_IS_NULL](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：processor入参为空； [HIAPPEVENT_INVALID_PARAM_VALUE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：参数值无效； [HIAPPEVENT_INVALID_UID](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：用户标识无效。 具体可参考[HiAppEvent_ErrorCode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)。 |
...
| 类型 | 说明 |
| --- | --- |
| int64_t | 调用成功时返回处理者唯一ID，大于0； [HIAPPEVENT_PROCESSOR_IS_NULL](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：processor入参为空； [HIAPPEVENT_INVALID_PARAM_VALUE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：参数值无效； [HIAPPEVENT_OPERATE_FAILED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：数据处理者名称未找到或注册失败； [HIAPPEVENT_INVALID_UID](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：用户标识无效。 具体可参考[HiAppEvent_ErrorCode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)。 |
...
#### [h2]OH_HiAppEvent_RemoveProcessor()
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功； [HIAPPEVENT_PROCESSOR_NOT_FOUND](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：事件处理者不存在； [HIAPPEVENT_OPERATE_FAILED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：操作失败； [HIAPPEVENT_INVALID_UID](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：用户标识无效。 具体可参考[HiAppEvent_ErrorCode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)。 |
...
![](./img/caution_3.0-zh-cn.png) 创建的指向设置系统事件自定义规格的配置对象的指针不再使用后，必须通过[OH_HiAppEvent_DestroyConfig](#oh_hiappevent_destroyconfig)接口进行销毁。
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent_Config](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-config)* | 指向设置系统事件自定义规格的配置对象的指针。 |

#### [h2]OH_HiAppEvent_DestroyConfig()
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent_Config](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-hiappevent-config)* config | 指向配置对象的指针（即OH_HiAppEvent_CreateConfig接口返回的指针）。 |

#### [h2]OH_HiAppEvent_SetConfigItem()
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功； [HIAPPEVENT_EVENT_CONFIG_IS_NULL](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：传入的指向配置对象的指针为空； [HIAPPEVENT_INVALID_PARAM_VALUE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：设定的配置项无效。 具体可参考[HiAppEvent_ErrorCode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)。 |
...
EVENT_APP_CRASH（参数配置详见[崩溃事件介绍](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-crash-events#oh_hiappevent_seteventconfig接口参数设置说明)），从API version 24开始支持该事件。
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：接口调用成功； [HIAPPEVENT_INVALID_PARAM_VALUE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)：设置的参数无效。 具体可参考[HiAppEvent_ErrorCode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)。 |

#### [h2]OH_HiAppEvent_ReportFrameworkMemAnomaly()
...
该接口的调用频率限制为：1分钟最多能成功调用1次，超过频率限制会返回错误码HIAPPEVENT_REPORT_FREQUENCY_EXCEEDED。

当应用检测到应用框架内存占用异常，并且调用该接口返回操作成功时：

1. 若开发者已经订阅了事件领域domain为“HIVIEWDFX”，且事件名称names为“FW_MEM_ANOMALY”的应用事件，则应用中将会收到应用框架内存占用异常信息的回调。
...
| 参数项 | 描述 |
| --- | --- |
| enum [OH_HiAppEvent_FrameworkType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#oh_hiappevent_frameworktype) frameworkType | 应用框架类型。 |
...
| 类型 | 说明 |
| --- | --- |
| int | HIAPPEVENT_SUCCESS：操作成功。 HIAPPEVENT_INVALID_PARAM_VALUE：参数值无效。 HIAPPEVENT_OPERATE_FAILED：系统/应用事件写入失败，或获取时间戳失败。 HIAPPEVENT_REPORT_FREQUENCY_EXCEEDED：报告频率超限。 具体可参考[HiAppEvent_ErrorCode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hiappevent-h#hiappevent_errorcode)。 |
