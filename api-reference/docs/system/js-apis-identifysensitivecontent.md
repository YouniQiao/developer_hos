...
content_hash: "d344a1461f1f"
synced_at: "2026-07-28T16:50:24.548429"
---
...
| identifyPolicies | Array | 是 | 用于识别敏感内容的策略数组。每个Policy定义识别规则（标签、关键字、正则表达式），系统将根据这些规则扫描文件内容并返回匹配结果。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回敏感内容识别的结果。成功时返回匹配结果数组，异常返回错误码。 |
...
| keywords | Array | 否 | 否 | 表示关键字集合，用于匹配文件中的敏感关键字。 系统会在文件内容中搜索这些关键字，匹配时区分大小写，匹配成功则返回识别结果。Array最大50，每个元素最大30字节。 |
