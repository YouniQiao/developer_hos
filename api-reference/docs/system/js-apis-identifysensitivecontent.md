...
![](./img/note_3.0-zh-cn.png) 本模块首批接口从API version 21开始支持。后续版本的新增接口采用上角标单独标记接口的起始版本。
...
| identifyPolicies | Array | 是 | 识别的策略。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象。返回敏感内容识别的结果。 |
...
| keywords | Array | 否 | 否 | 表示关键字集合。Array最大50，每个元素最大30字节。 |
