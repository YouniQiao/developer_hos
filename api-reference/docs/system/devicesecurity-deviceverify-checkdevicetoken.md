...
| [设备真实性证明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/device-attestation-intro) | 是通过在设备侧的TEE中创建一对公私钥，并对该密钥进行证明后把公钥保存在应用服务器，需要开发者在应用服务器中保存证明后的公钥，并维护公钥的生命周期，对开发者的集成要求相对较高。 支持防重放，后续的业务请求无需与设备真实性证明服务器交互，对业务的处理时延无明显的影响。 |
...
![](./img/note_3.0-zh-cn.png) Authorization格式：Bearer后面拼接空格，再拼接获取的鉴权信息。令牌生成[示例代码](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/devicesecurity-deviceverify-token#示例代码)。
...
![](./img/note_3.0-zh-cn.png) 开发者在构造消息体时，消息体需要在外层包一层data结构，详情参考如下调用示例。
