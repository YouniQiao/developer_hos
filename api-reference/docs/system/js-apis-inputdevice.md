...
content_hash: "0a8b49a5002a"
synced_at: "2026-07-28T16:51:06.981486"
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数。当获取成功，err为undefined，data为所有输入设备的ID列表（ID是输入设备的唯一标识）；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回所有输入设备的ID列表。ID是输入设备的唯一标识。 |
...
| callback | AsyncCallback | 是 | 回调函数。当获取成功，err为undefined，data为输入设备信息（包括输入设备ID、名称、支持的输入能力等）；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回输入设备信息，包括输入设备ID、名称、支持的输入能力、物理地址、版本信息及产品信息等。 |
...
| listener | Callback | 是 | 回调函数，返回输入设备热插拔事件。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 输入设备的事件类型，固定值为'change'。 |
| listener | Callback | 否 | 取消监听的回调函数，缺省时取消所有输入设备热插拔事件的监听。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 8 开始支持，从API version 9 开始废弃，建议使用[inputDevice.getDeviceList](#inputdevicegetdevicelist9)替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数。当获取成功，err为undefined，data为所有输入设备的ID列表；否则为错误对象。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 8 开始支持，从API version 9 开始废弃，建议使用[inputDevice.getDeviceInfo](#inputdevicegetdeviceinfo9)替代。
...
| --- | --- | --- | --- |
| deviceId | number | 是 | 输入设备的唯一标识，同一个物理设备反复插拔或重启，设备ID可能会发生变化。 |
| callback | AsyncCallback | 是 | 回调函数。当获取成功，err为undefined，data为输入设备信息；否则为错误对象。 |
...
| --- | --- | --- | --- |
| deviceId | number | 是 | 输入设备的唯一标识，同一个物理设备反复插拔或重启，设备ID可能会发生变化。 |
| keys | Array | 是 | 需要查询的键值，最多支持5个按键查询。 |
| callback | AsyncCallback> | 是 | 回调函数。当查询成功，err为undefined，data为按键支持查询结果（数组元素与keys参数一一对应，true表示支持，false表示不支持）；否则为错误对象。 |
...
| --- | --- | --- | --- |
| deviceId | number | 是 | 输入设备的唯一标识，同一个物理设备反复插拔或重启，设备ID可能会发生变化。 |
| keys | Array | 是 | 需要查询的键值，最多支持查询5个按键。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回查询结果。true表示支持，false表示不支持。 |
...
| 类型 | 说明 |
| --- | --- |
| Array | 返回查询结果。true表示支持，false表示不支持。 |
...
| --- | --- | --- | --- |
| deviceId | number | 是 | 输入设备的唯一标识，同一个物理设备反复插拔或重启，设备ID可能会发生变化。 |
| callback | AsyncCallback | 是 | 回调函数。当查询成功，err为undefined，data为输入设备的键盘类型；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回查询结果，true表示功能键使能，false表示功能键未使能。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回距离上次系统输入事件的时间间隔，单位为微秒（μs）。 |
...
| sources | Array | 否 | 否 | 输入设备的输入能力。包括键盘、鼠标、触摸屏、轨迹球、触控板、操纵杆等。 |
| axisRanges | Array | 否 | 否 | 输入设备的轴信息。 |
