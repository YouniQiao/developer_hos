---
title: "@ohos.busManager.serial (串口通信管理)"
upstream_id: "harmonyos-references/js-apis-busmanager-serial"
catalog: "harmonyos-references"
content_hash: "a90a42fb0aed"
synced_at: "2026-08-14T15:54:40.934484"
---
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回串口设备列表。 |

错误码：

以下错误码的详细介绍参见[通用错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal)和[串口管理错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-busmanager-serial)。
...
以下错误码的详细介绍参见[串口管理错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-busmanager-serial)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 是 | 回调函数，返回串口接收到的数据。用于监听串口数据接收事件，注册回调后，串口接收到数据时会触发该回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 否 | 回调函数。传入callback时，取消指定的串口数据接收监听；不传入callback时，清除所有串口数据接收监听。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回CTS信号状态，返回true表示可以发送数据，返回false表示不可以发送数据。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回DSR信号状态；true表示数据设备就绪；false表示数据设备未就绪。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 是 | 回调函数，串口断开时触发。用于监听串口断开事件，注册回调后，串口设备断开连接时会触发该回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 否 | 回调函数，需先通过onDisconnect()注册回调后才能取消。传入callback时，取消指定的串口断开事件监听；不传入callback时，清除所有串口断开事件监听。 |
