---
title: "SDK编译校验错误码"
upstream_id: "harmonyos-references/errorcode-sdk-compile"
catalog: "harmonyos-references"
content_hash: "9e404fc6415d"
synced_at: "2026-08-29T18:11:55.435619"
---
...
参考API文档说明：[apiAvailable](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-device-info#deviceinfoapiavailable)。
...
4. 参数中包含了除数字、点和括号以外的非法字符，例如：apiAvailable("12处理步骤

1. 核对apiAvailable接口的参数格式要求，确认当前参数类型（整数版本号、M.S.F版本号或版本范围表达式）。
2. 根据参数类型修正版本号： 整数格式：取值范围为1~25，例如：apiAvailable(12)。
3. M.S.F格式：M>=26且M错误信息
...
1. 格式不符合规范：版本号需满足以下格式之一： M.S.F带括号格式：如：6.0.0(20)
2. M.S.F格式，如：26.0.0
3. 版本号不在允许列表中：格式正确但未在sdkApiVersionMap.json中匹配到对应条目。
...
sdkApiVersionMap.json文件位于DevEco Studio安装目录下sdk\default\hms\ets\build-tools\ts-checker-hooks\目录中。
...
2. 按照运行时的版本号规范修正版本号： 整数格式：取值范围1~999，例如：@Available({minApiVersion: "12"})、@Available({minApiVersion: "26"})、@Available({minApiVersion: "100"})。
3. M.S.F格式：M为1~99的整数，S和F为0~99的整数，例如：@Available({minApiVersion: "26.0.0"})。
4. 确保版本号不含非法字符，且格式完整（M.S.F必须为三段式）。
5. 参考API文档说明：[Available](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-annotation#available)。
...
1. @Available装饰器中指定的操作系统名称合法但与项目运行时不匹配，例如：项目运行时为HarmonyOS，但装饰器中指定了@Available({minApiVersion: "HarmonyOS"})。
...
2. 将@Available装饰器中的操作系统名称替换为与项目运行时匹配的正确名称，例如：项目运行时为HarmonyOS时，使用@Available({minApiVersion: "HarmonyOS"})。
3. 确认操作系统名称拼写正确，且为当前运行时所支持的名称。
4. 参考API文档说明：[Available](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-annotation#available)。
