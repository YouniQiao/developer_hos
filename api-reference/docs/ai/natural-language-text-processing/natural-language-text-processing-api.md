...
| wordTag | string | 否 | 否 | 词性。从getWordSegment方法中获取到的结果中的词性，词性分类参考[wordTag](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/natural-language-word-tag-api)。 |
...
| type | [EntityType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/natural-language-entity-type-api) | 否 | 否 | 实体类别。 |
| jsonObject | string | 否 | 否 | Entity返回参数说明。详情参考[jsonObject](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/natural-language-json-object-api)。 |
...
| entityConfig | [EntityConfig](#entityconfig) | 否 | 实体配置项：实体类别。 默认全选。推荐按需加载实体类别，以提高应用性能。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回实体的结果集合。 |
...
![](./img/note_3.0-zh-cn.png) 当开发者需要提前初始化自然语言处理引擎，以减少首次调用文本处理能力（如分词、实体识别）的时延时，可以主动调用此方法。在实际使用中，如果对性能没有特殊要求，可以不调用此接口，系统会在首次使用时自动初始化。
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，true表示初始化成功，false表示初始化失败。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，true表示释放引擎成功，false表示释放引擎失败。 |
