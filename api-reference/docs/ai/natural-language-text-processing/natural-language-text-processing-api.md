...
还提供实体抽取功能，通过对用户输入的文本进行实体识别。然后依据Kit中的实体类别来进行分类，其中用户可以根据实体类别列表中的类别来进行选择。输出结果中包含实体的类别、实体在原文本中的位置、实体的原文本以及实体解析后的其他字段。实体字段内容可参考[EntityType](#entitytype)详情。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| entityTypes | [EntityType](#entitytype)[] | 否 | 是 | 实体的类别。 默认全选。 |
...
| type | [EntityType](#entitytype) | 否 | 否 | 实体类别。 |
...
![](./img/note_3.0-zh-cn.png) 此初始化接口非必须调用。在主动使用时，可以提前初始化该功能，减少首次调用文本处理能力的时延。
