...
upstream_id: "harmonyos-references/capi-neural-network-runtime-h"
catalog: "harmonyos-references"
content_hash: "aec8d05074c7"
synced_at: "2026-07-09T01:01:44.551080"
---
...
| 名称 | 描述 |
| --- | --- |
| [NN_QuantParam *OH_NNQuantParam_Create()](#oh_nnquantparam_create) | 创建一个[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)量化参数实例。 |
| [OH_NN_ReturnCode OH_NNQuantParam_SetScales(NN_QuantParam *quantParams, const double *scales, size_t quantCount)](#oh_nnquantparam_setscales) | 设置[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)的缩放系数。 参数**quantCount**是张量中量化参数的数量，例如对于per-channel量化，**quantCount**就是通道数量。 |
| [OH_NN_ReturnCode OH_NNQuantParam_SetZeroPoints(NN_QuantParam *quantParams, const int32_t *zeroPoints, size_t quantCount)](#oh_nnquantparam_setzeropoints) | 设置[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)的零点。 参数**quantCount**是张量中量化参数的数量，例如对于per-channel量化，**quantCount**就是通道数量。 |
| [OH_NN_ReturnCode OH_NNQuantParam_SetNumBits(NN_QuantParam *quantParams, const uint32_t *numBits, size_t quantCount)](#oh_nnquantparam_setnumbits) | 设置[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)的量化位数。 参数**quantCount**是张量中量化参数的数量，例如对于per-channel量化，**quantCount**就是通道数量。 |
| [OH_NN_ReturnCode OH_NNQuantParam_Destroy(NN_QuantParam **quantParams)](#oh_nnquantparam_destroy) | 销毁[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)实例。 当设置[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)实例到一个[NN_Tensor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-tensor)中后，如果不再使用该实例，需要销毁它以避免内存泄漏。 如果**quantParams**或***quantParams**是空指针，那么该接口仅打印警告日志，不会执行销毁操作。 |
| [OH_NNModel *OH_NNModel_Construct(void)](#oh_nnmodel_construct) | 创建[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)类型的模型实例，搭配OH_NNModel模块提供的其他接口，完成模型实例的构造。 |
| [OH_NN_ReturnCode OH_NNModel_AddTensorToModel(OH_NNModel *model, const NN_TensorDesc *tensorDesc)](#oh_nnmodel_addtensortomodel) | 向模型实例中添加张量。 Neural Network Runtime模型中的数据节点和算子参数均由模型的张量构成。 |
...
| [OH_NN_ReturnCode OH_NNModel_SetTensorQuantParams(OH_NNModel *model, uint32_t index, NN_QuantParam *quantParam)](#oh_nnmodel_settensorquantparams) | 设置张量的量化参数，参考[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)。 |
| [OH_NN_ReturnCode OH_NNModel_SetTensorType(OH_NNModel *model, uint32_t index, OH_NN_TensorType tensorType)](#oh_nnmodel_settensortype) | 设置张量的类型，参考[OH_NN_TensorType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-type-h#oh_nn_tensortype)。 |
...
| [void OH_NNModel_Destroy(OH_NNModel **model)](#oh_nnmodel_destroy) | 销毁模型实例。调用[OH_NNModel_Construct](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_construct)创建的模型实例需要调用该接口主动销毁，否则将造成内存泄漏。 如果model为空指针或者*model为空指针，该接口仅打印警告日志，不执行销毁操作。 |
...
| [OH_NN_ReturnCode OH_NNModel_AddTensor(OH_NNModel *model, const OH_NN_Tensor *tensor)](#oh_nnmodel_addtensor) | 向模型实例中添加张量。 Neural Network Runtime模型中的数据节点和算子参数均由模型的张量构成，该接口根据tensor，向model实例中添加张量。 |
...
| [OH_NN_ReturnCode OH_NNExecutor_SetOutput(OH_NNExecutor *executor,uint32_t outputIndex,void *dataBuffer,size_t length)](#oh_nnexecutor_setoutput) | 设置模型单个输出的内存。 该接口将dataBuffer指向的内存与outputIndex指定的输出绑定，内存的长度由length指定。 |
...
| [void OH_NNExecutor_DestroyInputMemory(OH_NNExecutor *executor, uint32_t inputIndex, OH_NN_Memory **memory)](#oh_nnexecutor_destroyinputmemory) | 释放[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)实例指向的输入内存。 调用[OH_NNExecutor_AllocateInputMemory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnexecutor_allocateinputmemory)创建的内存实例，需要主动调用该接口进行释放，否则将造成内存泄漏。 |
| [void OH_NNExecutor_DestroyOutputMemory(OH_NNExecutor *executor, uint32_t outputIndex, OH_NN_Memory **memory)](#oh_nnexecutor_destroyoutputmemory) | 释放[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)实例指向的输出内存。 调用[OH_NNExecutor_AllocateOutputMemory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnexecutor_allocateoutputmemory)创建的内存实例，需要主动调用该接口进行释放，否则将造成内存泄漏。outputIndex和memory的对应关系需要和创建内存实例时保持一致。 如果memory或*memory为空指针，该接口仅打印警告日志，不执行释放操作。 |
| [OH_NN_ReturnCode OH_NNExecutor_SetInputWithMemory(OH_NNExecutor *executor,uint32_t inputIndex,const OH_NN_Tensor *tensor,const OH_NN_Memory *memory)](#oh_nnexecutor_setinputwithmemory) | 将[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)实例指向的硬件共享内存，并指定为单个输入使用的内存。 |
| [OH_NN_ReturnCode OH_NNExecutor_SetOutputWithMemory(OH_NNExecutor *executor,uint32_t outputIndex,const OH_NN_Memory *memory)](#oh_nnexecutor_setoutputwithmemory) | 将[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)实例指向的硬件共享内存，并指定为单个输出使用的内存。 |
...
| 类型 | 说明 |
| --- | --- |
| [NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam) * | 指向[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)实例的指针，如果创建失败就返回NULL。 |

#### [h2]OH_NNQuantParam_SetScales()
...
| 参数项 | 描述 |
| --- | --- |
| [NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam) *quantParams | 指向[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)实例的指针。 |
...
#### [h2]OH_NNQuantParam_SetZeroPoints()
...
| 参数项 | 描述 |
| --- | --- |
| [NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam) **quantParams | 指向[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)实例的二级指针。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) * | 返回一个指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的指针，如果创建失败就返回NULL。 |

#### [h2]OH_NNModel_AddTensorToModel()
...
| 参数项 | 描述 |
| --- | --- |
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) *model | 指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的指针。 |
| [const NN_TensorDesc](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-tensordesc) *tensorDesc | [NN_TensorDesc](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-tensordesc)张量的指针，[NN_TensorDesc](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-tensordesc)指定了添加到模型实例中张量的属性。 |
...
#### [h2]OH_NNModel_SetTensorQuantParams()
...
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) *model | 指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的指针。 |
| uint32_t index | 张量的索引值。 |
| [NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam) *quantParam | 指向[NN_QuantParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-nn-quantparam)的指针。 |
...
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) *model | 指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的指针。 |
| uint32_t index | 张量的索引值。 |
| [OH_NN_TensorType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-type-h#oh_nn_tensortype) tensorType | 张量类型。 |
...
| --- | --- |
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) *model | 指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的指针。 |
| [OH_NN_OperationType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-type-h#oh_nn_operationtype) op | 指定添加的算子类型，取值请参考[OH_NN_OperationType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-type-h#oh_nn_operationtype)的枚举值。 |
| [const OH_NN_UInt32Array](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-uint32array) *paramIndices | OH_NN_UInt32Array实例的指针，设置算子的参数张量索引。 |
| [const OH_NN_UInt32Array](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-uint32array) *inputIndices | OH_NN_UInt32Array实例的指针，指定算子的输入张量索引。 |
| [const OH_NN_UInt32Array](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-uint32array) *outputIndices | OH_NN_UInt32Array实例的指针，设置算子的输出张量索引。 |
...
#### [h2]OH_NNModel_SpecifyInputsAndOutputs()
...
| --- | --- |
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) *model | 指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的指针。 |
| [const OH_NN_UInt32Array](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-uint32array) *inputIndices | OH_NN_UInt32Array实例的指针，指定算子的输入张量。 |
| [const OH_NN_UInt32Array](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-uint32array) *outputIndices | OH_NN_UInt32Array实例的指针，指定算子的输出张量。 |
...
| 参数项 | 描述 |
| --- | --- |
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) **model | 指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的二级指针。模型实例销毁后，该接口会将*model主动设置为空指针。 |

#### [h2]OH_NNModel_GetAvailableOperations()
...
| --- | --- |
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) *model | 指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的指针。 |
| size_t deviceID | 指定查询的硬件ID，通过[OH_NNDevice_GetAllDevicesID](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-core-h#oh_nndevice_getalldevicesid)获取。 |
| const bool **isSupported | 指向bool数组的指针。调用该接口时，要求(*isSupported)为空指针，否则返回[OH_NN_INVALID_PARAMETER](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-type-h#oh_nn_returncode)。 |
...
| --- | --- |
| [OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel) *model | 指向[OH_NNModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnmodel)实例的指针。 |
| [const OH_NN_Tensor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-tensor) *tensor | [OH_NN_Tensor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-tensor)张量的指针，tensor指定了添加到模型实例中张量的属性。 |
...
- 动态形状输入的场景：在构图阶段，由于动态输入的形状不确定，调用该接口时，要求tensor.dimensions中的每个值必须大于0，以确定执行计算阶段输入的形状。设置形状时，只允许调整数值为-1的维度。 假设在构图阶段，输入A的维度为[-1, 224, 224, 3]，调用该接口时，只能调整第一个维度的尺寸，如：[3, 224, 224, 3]。调整其他维度将返回[OH_NN_INVALID_PARAMETER](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-type-h#oh_nn_returncode)。
...
| uint32_t inputIndex | 输入的索引值，与调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时输入数据的顺序一致。 假设调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时，inputIndices为{1, 5, 9}，则在设置输入的阶段，三个输入的索引值分别为{0, 1, 2}。 |
| [const OH_NN_Tensor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-tensor) *tensor | 设置输入数据对应的张量。 |
...
| --- | --- |
| [OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor) *executor | 指向[OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor)实例的指针。 |
| uint32_t outputIndex | 输出的索引值，与调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时输出数据的顺序一致。 假设调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时，outputIndices为{4, 6, 8}，则在设置输出内存时，三个输出的索引值分别为{0, 1, 2}。 |
...
#### [h2]OH_NNExecutor_AllocateInputMemory()
...
| --- | --- |
| [OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor) *executor | 指向[OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor)实例的指针。 |
| uint32_t inputIndex | 输入的索引值，与调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时输入数据的顺序一致。 假设调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时，inputIndices为{1, 5, 9}，则在申请输入内存时，三个输入的索引值分别为{0, 1, 2}。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory) * | 指向[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)实例的指针，如果创建失败就返回NULL。 |

#### [h2]OH_NNExecutor_AllocateOutputMemory()
...
| --- | --- |
| [OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor) *executor | 指向[OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor)实例的指针。 |
| uint32_t outputIndex | 输出的索引值，与调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时输出数据的顺序一致。 假设调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时，outputIndices为{4, 6, 8}，则在申请输出内存时，三个输出的索引值分别为{0, 1, 2}。 |
| size_t length | 申请的内存字节。 |
...
#### [h2]OH_NNExecutor_DestroyInputMemory()
...
| --- | --- |
| [OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor) *executor | 指向[OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor)实例的指针。 |
| uint32_t inputIndex | 输入的索引值，与调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时输入数据的顺序一致。 假设调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时，inputIndices为{1, 5, 9}，则在释放输入内存时，三个输入的索引值分别为{0, 1, 2}。 |
| [OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory) **memory | 指向[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)实例的二级指针。共享内存释放后，该接口将*memory主动设置为空指针。 |

#### [h2]OH_NNExecutor_DestroyOutputMemory()
...
| --- | --- |
| [OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor) *executor | 指向[OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor)实例的指针。 |
| uint32_t outputIndex | 输出的索引值，与调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时输出数据的顺序一致。 假设调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时，outputIndices为{4, 6, 8}，则在释放输出内存时，三个输出的索引值分别为{0, 1, 2}。 |
| [OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory) **memory | 指向[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)实例的二级指针。共享内存释放后，该接口将*memory主动设置为空指针。 |

#### [h2]OH_NNExecutor_SetInputWithMemory()
...
| --- | --- |
| [OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor) *executor | 指向[OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor)实例的指针。 |
| uint32_t inputIndex | 输入的索引值，与调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时输入数据的顺序一致。 假设调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时，inputIndices为{1, 5, 9}，则在指定输入的共享内存时，三个输入的索引值分别为{0, 1, 2}。 |
| [const OH_NN_Tensor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-tensor) *tensor | 指向[OH_NN_Tensor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-tensor)的指针，设置单个输入所对应的张量。 |
| [const OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory) *memory | 指向[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)的指针。 |
...
#### [h2]OH_NNExecutor_SetOutputWithMemory()
...
| 参数项 | 描述 |
| --- | --- |
| [OH_NNExecutor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nnexecutor) *executor | 执行器。 |
| uint32_t outputIndex | 输出的索引值，与调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时输出数据的顺序一致。 假设调用[OH_NNModel_SpecifyInputsAndOutputs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neural-network-runtime-h#oh_nnmodel_specifyinputsandoutputs)时，outputIndices为{4, 6, 8}，则在指定输出的共享内存时，三个输出的索引值分别为{0, 1, 2}。 |
| [const OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory) *memory | 指向[OH_NN_Memory](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-neuralnetworkruntime-oh-nn-memory)的指针。 |
