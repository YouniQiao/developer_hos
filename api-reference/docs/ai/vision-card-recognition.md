...
content_hash: "a955c8e58cde"
synced_at: "2026-07-28T16:53:17.366615"
---

# CardRecognition（卡证识别控件）

卡证识别控件提供身份证、行驶证、驾驶证、护照、银行卡、港澳居民来往内地通行证、台湾居民来往大陆通行证的结构化识别服务，并支持自动分类功能，系统可自动判断所属卡证类型并返回结构化信息和卡证图片信息。

起始版本： 5.0.0(12)

![](./img/note_3.0-zh-cn.png) 使用该控件会创建弹窗，并以全模态形式展示。因此，该控件被拉起或退出时均会触发接入页面的生命周期变化，拉起时会触发页面的onPageHide，退出时则触发页面的onPageShow。
...
| 名称 | 类型 | 必填 | 装饰器类型 | 说明 |
| --- | --- | --- | --- | --- |
| supportType | [CardType](#cardtype) | 是 | - | 表示需要识别的卡证类型。当前仅支持中国境内（香港特别行政区、澳门特别行政区、中国台湾除外）相关卡证的识别。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| cardSide | [CardSide](#cardside) | 否 | - | 表示需要识别的卡证页面。默认值：DEFAULT。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| callback(deprecated) | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback) | 是 | - | 卡证识别结果的返回。callback和onResult参数同时配置时只有callback生效。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 **说明：** 从5.1.1(19)开始废弃。建议使用onResult替代。 |
| onResult | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback) | 是 | - | 卡证识别结果的返回。callback和onResult参数同时配置时只有callback生效。 **元服务API：** 从版本5.1.1(19)开始，该接口支持在元服务中使用。 **起始版本：** 5.1.1(19) |
| cardRecognitionConfig | [CardRecognitionConfig](#cardrecognitionconfig) | 否 | - | 卡证识别配置项。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| CARD_AUTO | 0 | 自动识别卡证类型，不预设类型。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| CARD_ID | 1 | 身份证。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| CARD_BANK | 2 | 银行卡。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| CARD_PASSPORT | 3 | 护照。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| CARD_DRIVER_LICENSE | 4 | 驾驶证。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| CARD_VEHICLE_LICENSE | 5 | 行驶证。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| CARD_MAINLAND_TRAVEL_PERMIT_HK_MO | 6 | 港澳居民来往内地通行证。 **元服务API：** 从版本6.1.1(24)开始，该接口支持在元服务中使用。 **起始版本：** 6.1.1(24) |
| CARD_MAINLAND_TRAVEL_PERMIT_TW | 7 | 台湾居民来往大陆通行证。 **元服务API：** 从版本6.1.1(24)开始，该接口支持在元服务中使用。 **起始版本：** 6.1.1(24) |
...
![](./img/note_3.0-zh-cn.png) 从5.1.1(19)开始废弃。建议使用[CardRecognitionResult](#cardrecognitionresult)替代。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| defaultShootingMode | [ShootingMode](#shootingmode) | 否 | 是 | 拍摄模式，默认值：ShootingMode.MANUAL。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
...
| cardContentConfig | [CardContentConfig](#cardcontentconfig) | 否 | 是 | 卡证内容配置。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| bankCard | [BankCardConfig](#bankcardconfig) | 否 | 是 | 识别银行卡的具体配置。 **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| idCard | [IdCardConfig](#idcardconfig) | 否 | 是 | 识别身份证的具体配置。 **元服务API：** 从版本5.0.2(14)开始，该接口支持在元服务中使用。 **起始版本**：5.0.2(14)。 |
...
| cardNumImageUri | string | 银行卡号图片uri。 **起始版本**：6.1.1(24)。 |
| isCropped | string | 是否成功裁剪卡证图片。 取值范围如下。 "1"：成功裁剪，此时返回的cardImageUri为不含背景的卡证图片。 "0"：未成功裁剪，此时返回的cardImageUri为含背景的卡证图片。 **起始版本**：6.1.1(24)。 |
| luhnCheckResult | string | 银行卡号是否符合卢恩算法的校验结果。 取值范围如下。 "-1"：未进行校验。 "0"：不符合卢恩算法。 "1"：符合卢恩算法。 **起始版本**：6.1.1(24)。 |
