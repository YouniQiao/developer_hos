...
| cornerPoints | Array | 是 | 否 | 以顺时针返回的单词外框位置点列表，数组第一个元素表示左上角。 |
...
| cornerPoints | Array | 是 | 否 | 以顺时针返回该文本行的外框位置点列表，数组第一个元素表示左上角。 |
| words | Array | 是 | 否 | 该行包含的单词信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回所支持的[语言类型列表](#语言类型列表)。 |
