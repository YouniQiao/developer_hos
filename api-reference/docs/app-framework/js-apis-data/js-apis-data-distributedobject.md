...
| fields | Array | 是 | 标识对象变更的属性名。属性名可自定义，要求字符串非空且长度不超过128字节。 |
...
| callback | AsyncCallback | 是 | 回调函数。当加入session成功，err为undefined，否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当退出sessionid成功，err为undefined，否则为错误对象。 |
...
| callback | (sessionId: string, fields: Array) => void | 是 | 变更回调对象实例。 sessionId：标识变更对象的sessionId； fields：标识对象变更的属性名。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件类型，固定为'change'，表示数据变更。 |
| callback | (sessionId: string, fields: Array) => void | 否 | 需要删除的数据变更回调，若不设置则删除该对象所有的数据变更回调。 sessionId：标识变更对象的sessionId； fields：标识对象变更的属性名。 |
...
| callback | AsyncCallback | 是 | 回调函数。当保存成功，err为undefined，data为SaveSuccessResponse（包含sessionId、version、deviceId等信息）；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回SaveSuccessResponse，包含sessionId、version、deviceId等信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当撤回保存成功，err为undefined，data为RevokeSaveSuccessResponse（包含sessionId信息）；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回RevokeSaveSuccessResponse，包含sessionId。 |
...
| callback | AsyncCallback | 是 | 回调函数。当绑定数据库成功，err为undefined，否则为错误对象。 |
...
![](./img/caution_3.0-zh-cn.png) 在设置资产时必须保证assetKey存在且对应文件为资产类型文件，否则无法保证对端能接收到此次设置的资产。
...
![](./img/caution_3.0-zh-cn.png) 在设置资产时必须保证assetsKey存在且对应文件为资产类型文件，否则无法保证对端能接收到此次设置的资产。
...
| uris | Array | 是 | 待设置的新资产数组的uri集合，表示资产数组内每个资产存放的分布式路径。数组中元素的数量为[1, 50]，元素uri必须为真实存在的资产对应的分布式路径。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[distributedDataObject.create](#distributeddataobjectcreate9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，暂无替代接口。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[setSessionId](#setsessionid9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[on('change')](#onchange9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[off('change')](#offchange9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[on('status')](#onstatus9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[off('status')](#offstatus9)替代。
