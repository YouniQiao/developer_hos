...
本模块对标准化数据类型进行了抽象定义与描述，用于统一表示和管理各类数据类型的层级与归属关系（如JPEG归属于IMAGE、IMAGE归属于MEDIA等），便于跨模块/跨应用的一致化数据交互。详细设计原理参见[UTD预置列表](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uniform-data-type-list)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| typeId11+ | string | 否 | 否 | 标准化数据类型的ID（即[UTD预置列表](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uniform-data-type-list)中各类型对应的UTD-ID），也可以是自定义UTD。自定义UTD建议使用反向域名格式（如'com.example.mytype'）。 |
| belongingToTypes11+ | Array | 否 | 否 | 标准化数据类型所归属的类型typeId列表。 |
...
| filenameExtensions12+ | Array | 否 | 否 | 标准化数据类型所关联的文件名后缀列表。 |
| mimeTypes12+ | Array | 否 | 否 | 标准化数据类型所关联的多用途互联网邮件扩展类型列表。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 所指定的标准化数据类型（即[UTD预置列表](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uniform-data-type-list)中各类型对应的UTD-ID或自定义UTD-ID）。 |
...
| belongsTo | string | 否 | 要查询的标准化数据类型所归属类型ID，用于限定查询范围。当需要查询特定归属类型下的数据类型时传入此参数，无默认值，若不传入此参数则只按照文件后缀名称查询[标准化数据类型ID](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uniform-data-type-descriptors)。 |
...
| 类型 | 说明 |
| --- | --- |
| Array | 返回与给定文件后缀名以及归属类型ID（如果设置了belongsTo参数）匹配的标准化数据类型ID列表，如果要查询的标准化数据类型不存在则返回根据入参按指定规则生成的动态类型列表。 |
...
| 类型 | 说明 |
| --- | --- |
| Array | 返回与MIME类型名称以及归属类型ID（如果设置了belongsTo参数）匹配的标准化数据类型ID列表，如果要查询的标准化数据类型不存在则返回根据入参按指定规则生成的动态类型列表。 |
