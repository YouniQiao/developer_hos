...
content_hash: "13bd43128fca"
synced_at: "2026-09-23T18:54:52.446934"
---
...
开发者在使用[模块描述](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-webview)中的WebviewController、WebMessagePort、WebDownloadItem、NativeMediaPlayerHandler、NativeMediaPlayerBridge、WebSchemeHandlerRequest等核心类时，均需依赖本模块中的枚举类型来配置行为参数或解析返回结果。当开发者需要精细控制Web组件的运行模式、查询页面状态、处理下载任务、接入原生媒体播控或优化加载体验时，应查阅本模块中对应的枚举定义。

![](./img/note_3.0-zh-cn.png) 本模块首批接口从API version 9开始支持，后续版本的新增接口，则采用上角标单独标记接口的起始版本。
...
| IMAGE | 0 | 图片类型的资源。 |
| CSS | 1 | CSS类型的资源。 |
| CLASSIC_JS | 2 | 通过标签加载的JavaScript资源。 |
| MODULE_JS | 3 | 通过标签加载的JavaScript资源。 |
...
| ERR_DURATION_OUT_OF_RANGE23+ | -6 | 在[BlanklessLoadingParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-webview-i#blanklessloadingparam23)设置的插帧持续时间超出范围。 **模型约束：** 此接口仅可在Stage模型下使用。 |
| ERR_EXPIRATION_TIME_OUT_OF_RANGE23+ | -7 | 在[BlanklessLoadingParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-webview-i#blanklessloadingparam23)设置的历史帧失效时间超出范围。 **模型约束：** 此接口仅可在Stage模型下使用。 |
