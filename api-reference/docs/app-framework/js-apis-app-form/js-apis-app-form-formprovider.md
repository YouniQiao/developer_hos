...
content_hash: "dc70fa46d366"
synced_at: "2026-08-29T18:16:11.739453"
---
...
| callback | AsyncCallback | 是 | 回调函数。设置结果的回调，成功时error为undefined。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 20开始，如果卡片刷新的数据通过共享内存更新，刷新数据总大小不超过10MB，刷新图片数量不超过20张。API version 19及之前的版本，图片文件数量上限为5张，每张限制内存2MB，超出限制的图片会显示异常。
...
| callback | AsyncCallback | 是 | 回调函数。更新结果的回调，成功时error为undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数。返回查询到的卡片信息。 |
...
| callback | AsyncCallback> | 是 | 回调函数。返回查询到符合条件的卡片信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象。返回查询到符合条件的卡片信息。 |
...
![](./img/note_3.0-zh-cn.png) 如果parameters参数没有填完整或者指定的卡片不存在，就会默认展示[form_config.json](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-configuration#卡片配置)中配置的默认卡片。
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回卡片相对屏幕左上角的位置信息和卡片尺寸信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回符合条件的卡片信息，包括卡片名称、尺寸等。 |
...
| moduleName | string | 是 | 指定卡片的moduleName，需与[form_config.json](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-configuration#配置文件字段说明)中配置的module名称一致。需与abilityName、formName配合使用，三者必须同时匹配才能定位到对应卡片。 |
