...
content_hash: "6029b0e115db"
synced_at: "2026-08-29T18:15:41.513908"
---
...
| [OH_ArkUI_TextController](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-textcontroller) | OH_ArkUI_TextController | 定义文本组件的控制器，用于在Native侧对文本组件进行控制和交互。控制器需要先通过节点属性绑定到文本组件，才能进行后续操作。一个控制器只能绑定到一个文本组件，绑定后该控制器将管理对应文本组件的样式和内容。可通过[OH_ArkUI_TextController_Create](#oh_arkui_textcontroller_create)创建控制器对象，创建后必须在使用完毕后调用[OH_ArkUI_TextController_Destroy](#oh_arkui_textcontroller_destroy)接口销毁对象以释放资源，二者必须成对使用，否则会导致内存泄漏。创建控制器后，可通过节点属性将控制器绑定到文本组件，然后使用[OH_ArkUI_TextController_SetStyledString](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-type-h#oh_arkui_textcontroller_setstyledstring)等接口设置文本组件的属性字符串，实现对文本内容的动态管理和样式控制。适用于需要在Native层操作文本组件的场景。 |
...
| 类型 | 说明 |
| --- | --- |
| [ArkUI_TextMarqueeOptions*](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-textmarqueeoptions) | 返回指向文本跑马灯模式配置项对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_TextMarqueeOptions_Dispose](#oh_arkui_textmarqueeoptions_dispose)销毁。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH_ArkUI_TextDataDetectorConfig*](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-textdatadetectorconfig) | 指向[OH_ArkUI_TextDataDetectorConfig](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-textdatadetectorconfig)对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_TextDataDetectorConfig_Destroy](#oh_arkui_textdatadetectorconfig_destroy)销毁。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH_ArkUI_FontWeightConfigs*](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-fontweightconfigs) | 指向文本字体粗细配置对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_FontWeightConfigs_Destroy](#oh_arkui_fontweightconfigs_destroy)销毁。当配置对象为空指针时，不应用默认值，文本字体粗细行为与父组件保持一致。 |
...
![](./img/note_3.0-zh-cn.png) 若该对象已被SetFontWeightConfigs设置到FontConfigs中，应在销毁FontConfigs之后再销毁FontWeightConfigs，或在确保FontConfigs不再使用后再销毁FontWeightConfigs，以避免FontConfigs持有无效指针。
...
| 类型 | 说明 |
| --- | --- |
| [OH_ArkUI_FontConfigs*](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-fontconfigs) | 返回指向文本字体配置对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_FontConfigs_Destroy](#oh_arkui_fontconfigs_destroy)销毁。 |
...
![](./img/note_3.0-zh-cn.png) 销毁FontConfigs前，应先销毁通过SetFontWeightConfigs设置的FontWeightConfigs对象（如果不再使用），或确保FontWeightConfigs的生命周期长于FontConfigs，以避免悬空指针问题。
...
| [OH_ArkUI_FontWeightConfigs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-fontweightconfigs)* fontWeightConfigs | 文本字体粗细配置。当该配置不为空指针时，若用户未显式设置，各项配置将使用默认值（可变字重调节默认为禁用，文本字体粗细跟随设备字体粗细级别更新默认为启用）；字重weight可通过相关接口设置，取值范围为[100, 900]。当该配置为空指针时，不应用上述默认值，文本字体粗细行为与父组件保持一致。 |
...
| 参数项 | 描述 |
| --- | --- |
| [OH_ArkUI_FontConfigs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-fontconfigs)* option | 指向文本字体配置对象的指针。若未设置或设置时为空指针，则返回空指针。 |

返回：

| 类型 | 说明 |
| --- | --- |
| [OH_ArkUI_FontWeightConfigs*](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-fontweightconfigs) | 文本字体粗细配置。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH_ArkUI_TextController*](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-textcontroller) | 指向文本组件控制器对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_TextController_Destroy](#oh_arkui_textcontroller_destroy)销毁。 |
