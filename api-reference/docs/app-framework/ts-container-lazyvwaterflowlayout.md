...
content_hash: "4916d17df521"
synced_at: "2026-09-23T18:53:34.445944"
---

# LazyVWaterFlowLayout

LazyVWaterFlowLayout用于实现支持懒加载的瀑布流布局，适用于展示大量高度不一的列表项场景，如图片墙、商品列表等。通过懒加载机制，该组件仅加载可视区域及附近内容，减少内存占用和渲染开销，提升滚动流畅度。该组件应位于竖直方向的[List](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-list)、[Scroll](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-scroll)或[WaterFlow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-waterflow)组件下，并支持通过[FlowItem](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-flowitem)、[LazyColumnLayout](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-lazycolumnlayout)、自定义组件或[NodeContainer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-nodecontainer)组件封装后使用。
...
创建垂直方向的LazyVWaterFlowLayout组件。该组件应位于竖直方向的[List](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-list)、[Scroll](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-scroll)或[WaterFlow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-waterflow)组件下，并支持通过[FlowItem](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-flowitem)、[LazyColumnLayout](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-lazycolumnlayout)、自定义组件或[NodeContainer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-nodecontainer)组件封装后使用。不建议设置高度、高度约束或宽高比，设置后会导致显示异常。
...
- value设置为string类型时，可设置当前LazyVWaterFlowLayout的列数、固定列宽或最小列宽值。典型取值及对应含义如下，使用效果请参考[示例3](#示例3设置自适应列数)： 1. columnsTemplate('1fr 1fr 2fr')可将LazyVWaterFlowLayout分为3列，组件宽度分为4等份，第1列占1份，第2列占1份，第3列占2份。 2. columnsTemplate('repeat(auto-fit, track-size)')是设置最小列宽值为track-size，自动计算列数和实际列宽。 3. columnsTemplate('repeat(auto-fill, track-size)')是设置固定列宽值为track-size，自动计算列数。 4. columnsTemplate('repeat(auto-stretch, track-size)')是设置固定列宽值为track-size，使用[columnsGap](#columnsgap)作为最小列间距，自动计算列数和实际列间距。 其中，repeat、auto-fit、auto-fill、auto-stretch为关键字。track-size为列宽，支持的单位包括px、vp、%或有效数字，默认单位为vp，track-size至少包括一个有效列宽。 auto-fit模式和auto-stretch模式只支持track-size为一个有效列宽值，并且auto-stretch模式中的track-size只支持px、vp和有效数字，不支持%。auto-fill模式支持一个或多个有效列宽，如columnsTemplate('repeat(auto-fill, 20)')、columnsTemplate('repeat(auto-fill, 20 80px)')。 非repeat形式的模板串中每项仅支持'数字+fr'、'数字+px'、'数字+%'三种格式，不支持vp（如columnsTemplate('100vp 100vp')）。需要按固定vp尺寸自动计算列数时，应使用repeat(auto-fill, track-size)。 设置为'0fr'时，该列的列宽为0，不显示子组件。设置为其他非法值时，子组件显示为固定1列。
- value设置为ItemFillPolicy类型时，将根据LazyVWaterFlowLayout组件宽度对应的[断点类型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-development-grid-layout#栅格容器断点)确定列数。例如，ItemFillPolicy的fillType属性设置为PresetFillType.BREAKPOINT_DEFAULT时，在组件宽度属于sm及更小的断点区间时显示2列，属于md断点区间时显示3列，属于lg及更大的断点区间时显示5列，且每列均为1fr（表示每列占用1等份可用宽度）。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string | [ItemFillPolicy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#itemfillpolicy22) | undefined | 是 | LazyVWaterFlowLayout的列数、固定列宽、最小列宽值或断点填充策略。 |

#### [h2]columnsGap

columnsGap(value: LengthMetrics | undefined): T

设置列与列的间距。默认值为LengthMetrics.vp(0)，设置为小于0的值时，按LengthMetrics.vp(0)处理。与[columnsTemplate](#columnstemplate)的repeat(auto-stretch, track-size)模式配合使用时，该值作为最小列间距，系统会自动计算实际列间距和列数。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [LengthMetrics](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-graphics#lengthmetrics12) | undefined | 是 | 列与列的间距。 默认值：LengthMetrics.vp(0) 取值范围：[0, +∞) 设置为小于0的值时，按LengthMetrics.vp(0)处理。 方法入参为undefined时，恢复为默认值（LengthMetrics.vp(0)）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [LengthMetrics](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-graphics#lengthmetrics12) | undefined | 是 | 行与行的间距。 默认值：LengthMetrics.vp(0) 取值范围：[0, +∞) 设置为小于0的值时，按LengthMetrics.vp(0)处理。 方法入参为undefined时，恢复为默认值（LengthMetrics.vp(0)）。 |
...
![](./img/note_3.0-zh-cn.png) 头部组件位于容器顶部区域，通常用于展示标题、分组说明或其他固定在内容前方的元素。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| builder | [CustomBuilder](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#custombuilder8) | undefined | 是 | 头部组件构造函数。 入参为undefined时，不设置头部组件，如果已有头部组件，也会被移除。 |
...
![](./img/note_3.0-zh-cn.png) 尾部组件位于容器底部区域，通常用于展示补充信息、加载状态或其他固定在内容后方的元素。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| builder | [CustomBuilder](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#custombuilder8) | undefined | 是 | 尾部组件构造函数。 入参为undefined时，不设置尾部组件，如果已有尾部组件，也会被移除。 |
...
![](./img/note_3.0-zh-cn.png) 由于浮点数计算精度，设置sticky后，在滚动过程中可能产生缝隙，可以通过[pixelRound](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-pixelroundforcomponent#pixelround)指定当前组件向下像素取整解决该问题。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sticky | [StickyStyle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-list#stickystyle9枚举说明) | undefined | 是 | 头部组件和尾部组件的吸附模式。sticky属性可设置为StickyStyle.Header（头部组件吸顶）、StickyStyle.Footer（尾部组件吸底）、StickyStyle.BOTH（同时支持头部组件吸顶和尾部组件吸底）或StickyStyle.None（不启用吸附效果）。 入参为undefined时，恢复为默认值StickyStyle.None。 未通过该接口设置时，默认头部组件不吸顶、尾部组件不吸底。 |
...
#### [h2]onVisibleIndexesChange

onVisibleIndexesChange(callback: OnVisibleIndexesChangeCallback | undefined): T

设置onVisibleIndexesChange回调函数。当LazyVWaterFlowLayout可视区域内子组件索引发生变化时触发回调，返回可视区域内子组件的起始索引和结束索引。

![](./img/note_3.0-zh-cn.png) 当父组件设置主轴方向尺寸时，LazyVWaterFlowLayout按照父组件可视区域进行懒加载。此时onVisibleIndexesChange回调中start返回当前可视区域起始位置子组件的索引值，end返回当前可视区域结束位置子组件的索引值。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [OnVisibleIndexesChangeCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-scrollable-common#onvisibleindexeschangecallback) | undefined | 是 | 回调函数，当可见区域内的子组件索引发生变化时触发。 入参为undefined时，取消监听。 |
...
```
 ![](./img/zh-cn_image_0000002739892770.png)

#### [h2]示例2（设置头部组件或尾部组件及吸附效果）

该示例通过[Scroll](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-scroll)嵌套LazyVWaterFlowLayout，并通过[header](#header)、[footer](#footer)、[sticky](#sticky)实现瀑布流顶部和底部吸附效果。滚动过程中header吸附在可视区域顶部，footer吸附在可视区域底部。
...
```
 ![](./img/zh-cn_image_0000002739732892.gif)
...
```
 ![](./img/zh-cn_image_0000002769332241.gif)
