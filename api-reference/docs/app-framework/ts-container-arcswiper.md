...
content_hash: "a54db4ad863e"
synced_at: "2026-07-28T16:43:32.812418"
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| index | Optional | 是 | 当前在容器中显示的子组件的索引值。 当index值为undefined时，按取值为0处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| style | Optional | 是 | 弧形圆点指示器样式。 - ArcDotIndicator：弧形圆点指示器属性及功能。 - boolean：是否启用弧形圆点指示器。设置为true启用，false不启用。 默认值：true 默认类型：ArcDotIndicator |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| duration | Optional | 是 | 子组件切换的动画时长。 默认值：400 单位：毫秒。传入负数时按默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isVertical | Optional | 是 | 是否为纵向滑动。 true: 纵向滑动；false: 横向滑动。 默认值：false |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| disabled | Optional | 是 | 是否禁用组件滑动切换功能。设置为true禁用，false不禁用。 默认值：false |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sensitivity | Optional | 是 | 旋转表冠的灵敏度。设置不同灵敏度级别可调整表冠滚动的响应速度。 默认值：CrownSensitivity.MEDIUM |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| edgeEffect | Optional | 是 | 边缘滑动效果。通过ArcSwiperController接口控制翻页时，回弹效果不生效。 默认值：EdgeEffect.Spring |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| disabled | Optional | 是 | 是否关闭特殊动效效果。 true：关闭特殊动效效果；false：不关闭特殊动效效果。 传入参数非法时，按false处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | [FinishAnimationHandler](#finishanimationhandler) | 否 | 动画结束的回调。 默认值：不传入时无回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| direction | Optional | 是 | 设置弧形指示器的方向。 默认值：ArcDirection.SIX_CLOCK_DIRECTION，6点钟方向。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| color | Optional | 是 | 设置弧形指示器中，未选中导航点的颜色。 默认值：'#A9FFFFFF' |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| color | Optional | 是 | 设置弧形指示器中，选中导航点的颜色。 默认值：'#FF5EA1FF' |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| color | Optional | 是 | 设置弧形指示器被长按时，弧形指示器的颜色。 默认值：'#FF404040' |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| color | Optional | 是 | 设置弧形指示器的遮罩渐变色。 起始颜色默认值：'#00000000' 结束颜色默认值：'#FF000000' |
...
| event | [SwiperAnimationEvent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-swiper#swiperanimationevent10对象说明) | 是 | 动画相关信息，包括主轴方向上当前显示元素和目标元素相对ArcSwiper起始位置的位移，以及离手速度。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | Optional | 是 | 当前显示元素的索引回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | Optional | 是 | 切换动画结束时触发该回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | Optional | 是 | 在页面跟手滑动过程中，逐帧触发该回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| transition | Optional | 是 | ArcSwiper自定义切换动画相关信息，包含timeout和transition两个属性。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| timeout | number | 否 | 是 | ArcSwiper自定义切换动画超时时间。从页面执行默认动画（页面滑动）至移出视窗外的第一帧开始计时，如果到达该时间后，开发者仍未调用[SwiperContentTransitionProxy](#swipercontenttransitionproxy)的[finishTransition](#finishtransition)接口通知ArcSwiper组件此页面的自定义动画已结束，组件将强制结束该页面的自定义动画，并立即在该页面节点下渲染树。 单位：ms 默认值：0。 |
...
```
 ![](./img/zh-cn_image_0000002686087945.gif)
...
```
 ![](./img/zh-cn_image_0000002685928117.gif)
