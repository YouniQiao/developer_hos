...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_StyledString_Destroy](#oh_arkui_styledstring_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 调用此方法压入的样式，必须在使用完毕后调用[OH_ArkUI_StyledString_PopTextStyle](#oh_arkui_styledstring_poptextstyle)将其出栈。未出栈的样式会持续影响后续添加的文本排版。典型使用流程：PushTextStyle → AddText → PopTextStyle。
...
![](./img/note_3.0-zh-cn.png) 此方法必须在[OH_ArkUI_StyledString_PushTextStyle](#oh_arkui_styledstring_pushtextstyle)之后调用，用于将之前压入的样式从栈中移除。对空栈调用此方法不产生效果。
...
![](./img/note_3.0-zh-cn.png) [OH_Drawing_Typography](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-drawing-oh-drawing-typography)对象的生命周期由应用管理，当应用销毁该对象时，应同步调用[NODE_TEXT_CONTENT_WITH_STYLED_STRING](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h-nodeattributetype-text#node_text_content_with_styled_string)对应的reset方法进行置空，避免野指针崩溃风险。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_StyledString_Descriptor_Destroy](#oh_arkui_styledstring_descriptor_destroy)销毁它。
...
| const char* html | 待转换为属性字符串的HTML格式字符串，当前支持转换的HTML标签范围：、、、
、、、、、、、、、、、、、、、、、、、、、、。 |
...
![](./img/note_3.0-zh-cn.png) 被清除后属性样式取TextEditor组件对应类型属性的默认值（如清除TextStyle后取TextEditor的字体默认样式）。
...
![](./img/note_3.0-zh-cn.png) 调用该接口会立即触发通过[OH_ArkUI_CustomSpan_RegisterOnDrawCallback](#oh_arkui_customspan_registerondrawcallback)注册在自定义绘制Span上的回调函数。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_TextStyle_Destroy](#oh_arkui_textstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_SpanStyle_Destroy](#oh_arkui_spanstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 此操作会替换[OH_ArkUI_SpanStyle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-spanstyle)对象中已设置的其他类型样式。一个SpanStyle对象只能持有一种类型的样式，设置新类型样式后，之前已设置的其他类型样式（如已设置的GestureStyle、ParagraphStyle等）将被清除。
...
![](./img/note_3.0-zh-cn.png) 此操作会替换[OH_ArkUI_SpanStyle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-oh-arkui-spanstyle)对象中已设置的其他类型样式。一个SpanStyle对象只能持有一种类型的样式，设置新类型样式后，之前已设置的其他类型样式（如已设置的TextStyle、ParagraphStyle等）将被清除。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_LeadingMarginSpanDrawInfo_Destroy](#oh_arkui_leadingmarginspandrawinfo_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_ParagraphStyle_Destroy](#oh_arkui_paragraphstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_GestureStyle_Destroy](#oh_arkui_gesturestyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_TextShadowStyle_Destroy](#oh_arkui_textshadowstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_DecorationStyle_Destroy](#oh_arkui_decorationstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_BaselineOffsetStyle_Destroy](#oh_arkui_baselineoffsetstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_LetterSpacingStyle_Destroy](#oh_arkui_letterspacingstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_LineHeightStyle_Destroy](#oh_arkui_lineheightstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 与[OH_ArkUI_LineHeightStyle_SetLineHeightMultiple](#oh_arkui_lineheightstyle_setlineheightmultiple)同时设置时，仅lineHeightMultiple生效，lineHeight不生效。lineHeightMultiple小于0时不生效，此时使用lineHeight设置行高。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_LineSpacingStyle_Destroy](#oh_arkui_linespacingstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 与[OH_ArkUI_LineHeightStyle_SetLineHeightMultiple](#oh_arkui_lineheightstyle_setlineheightmultiple)同时设置时，仅lineHeightMultiple生效，行间距不生效。lineHeightMultiple小于0时不生效，此时行间距正常生效。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_BackgroundColorStyle_Destroy](#oh_arkui_backgroundcolorstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_UrlStyle_Destroy](#oh_arkui_urlstyle_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_UserDataSpan_Destroy](#oh_arkui_userdataspan_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_CustomSpan_Destroy](#oh_arkui_customspan_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 当该对象不再使用时，调用[OH_ArkUI_ImageAttachment_Destroy](#oh_arkui_imageattachment_destroy)销毁它。
...
![](./img/note_3.0-zh-cn.png) 与[OH_ArkUI_ImageAttachment_SetResource](#oh_arkui_imageattachment_setresource)同时设置时，后设置的生效。
...
![](./img/note_3.0-zh-cn.png) 与[OH_ArkUI_ImageAttachment_SetPixelMap](#oh_arkui_imageattachment_setpixelmap)同时设置时，后设置的生效。
...
![](./img/note_3.0-zh-cn.png) 与[OH_ArkUI_ImageAttachment_SetDrawingColorFilter](#oh_arkui_imageattachment_setdrawingcolorfilter)同时设置时，后设置的生效。
...
![](./img/note_3.0-zh-cn.png) 与[OH_ArkUI_ImageAttachment_SetColorFilter](#oh_arkui_imageattachment_setcolorfilter)同时设置时，后设置的生效。
...
![](./img/note_3.0-zh-cn.png) 此属性仅在通过[OH_ArkUI_ImageAttachment_SetResource](#oh_arkui_imageattachment_setresource)设置图片源为资源地址时生效。
...
#### [h2]OH_ArkUI_ParagraphStyle_SetLinearGradient()
...
#### [h2]OH_ArkUI_ParagraphStyle_GetLinearGradient()
...
#### [h2]OH_ArkUI_ParagraphStyle_SetRadialGradient()
...
#### [h2]OH_ArkUI_ParagraphStyle_GetRadialGradient()
...
#### [h2]OH_ArkUI_ParagraphStyle_SetTailIndents()
...
![](./img/note_3.0-zh-cn.png) 所有输入指针参数必须由调用者负责分配、管理和释放。
...
#### [h2]OH_ArkUI_ParagraphStyle_GetTailIndents()
