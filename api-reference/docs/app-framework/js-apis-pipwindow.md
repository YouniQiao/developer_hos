...
content_hash: "d24fb264d7e6"
synced_at: "2026-08-29T18:12:35.866478"
---
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回当前画中画的可见状态。true表示前台可见，false表示前台不可见（收入侧边栏）。画中画生命周期不为[STARTED](#pipstate)时调用本接口总是返回false。 |
...
| callback | Callback | 是 | 返回当前画中画的可见状态。true表示前台可见，false表示前台不可见（收入侧边栏）。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件类型，固定为'activeStatusChange'，即画中画可见状态变化事件。 |
| callback | Callback | 否 | 返回当前画中画的可见状态。true表示前台可见，false表示前台不可见（收入侧边栏）。如果未传入参数，解除type为'activeStatusChange'的所有回调。 |
