...
![](./img/note_3.0-zh-cn.png) 该接口支持在ArkTS卡片中使用，但是ArkTS卡片中暂不支持组件预览。
...
![](./img/note_3.0-zh-cn.png) PreviewParams中只支持使用与定义参数类型相匹配的入参，否则所有的@Preview的参数都将被置为默认值。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| title | string | 否 | 是 | 组件预览标题，默认为自定义组件名称。 |
...
| deviceType | string | 否 | 是 | 组件预览渲染的设备类型，默认为Phone。设备类型枚举值参考[deviceTypes标签](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#devicetypes标签)。 |
