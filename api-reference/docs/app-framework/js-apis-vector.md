...
upstream_id: "harmonyos-references/js-apis-vector"
catalog: "harmonyos-references"
content_hash: "926def64b12d"
synced_at: "2026-08-03T17:09:33.984227"
---
...
API version 9开始，该接口不再维护，推荐使用接口[ArrayList](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arraylist)。
...
| vector | [Vector](#vector) | 否 | 当前调用replaceAllElements方法的实例对象，默认值为当前实例对象。 |
...
| value | T | 是 | 当前遍历到的元素。 |
| index | number | 否 | 当前遍历到的下标值。 |
| vector | [Vector](#vector) | 否 | 当前调用forEach方法的实例对象，默认值为当前实例对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| fromIndex | number | 是 | 起始下标，取值范围：0 ≤ fromIndex 返回值：

| 类型 | 说明 |
| --- | --- |
| Vector | 返回包含从起始下标到终止下标（不包括终止下标）元素的Vector对象实例。 |
...
| 类型 | 说明 |
| --- | --- |
| Vector | 返回与原实例内容相同的克隆Vector对象实例，修改克隆实例不影响原实例。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| index | number | 是 | 查找的下标值，取值范围：0 ≤ index 返回值：
...
| 类型 | 说明 |
| --- | --- |
| IterableIterator | 返回一个迭代器，用于遍历Vector实例中的元素。 |
