...
content_hash: "abaa3ce3bf30"
synced_at: "2026-09-17T18:50:25.516687"
---
...
1. 检查Want中的bundleName、moduleName和abilityName是否正确。
2. 确认被调用方（服务端）的Ability类型与调用接口是否匹配。对于ServiceExtensionAbility，应使用[connectServiceExtensionAbility()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#connectserviceextensionability)方法连接。同时需要确保[module.json5配置文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file)中extensionAbilities的type设置为与接口匹配的service。
...
当应用受到企业设备管理[Enterprise Device Manager](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/mdm-kit-admin)管控时，方法将返回该错误码。
...
1. 启动Ability时传入的[Want](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-want#约束限制)数据过大。
2. 设备解锁前拉起非系统应用。
3. 调用openAtomicService拉起元服务时，安装未完成前用户点击取消，应用市场返回user_cancel错误码，导致安装失败。
4. 开发者无法处理的系统内部错误。包括但不限于：内部对象为空指针、处理超时、IPC跨进程通信失败、包管理获取应用信息失败、系统服务获取失败、启动的Ability实例已达到上限等。
5. 隐式拉起时未安装应用市场App。

处理步骤

1. 对于启动Ability失败时，可以检查传入的[Want](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-want#约束限制)数据是否过大。
...
1. 查看[对应Extension类型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/extensionability-overview)严格模式开启条件。
...
该应用没有在app.json5配置文件[multiAppMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-configuration-file#multiappmode标签)标签中配置应用分身字段，导致该应用不支持分身模式，调用[getCurrentAppCloneIndex](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-applicationcontext#applicationcontextgetcurrentappcloneindex12)接口时返回该错误码。

处理步骤

参考[应用多实例的配置方法](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/multiinstance)，在app.json5配置文件中配置multiAppMode标签，开启应用分身功能后，再调用[getCurrentAppCloneIndex](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-applicationcontext#applicationcontextgetcurrentappcloneindex12)接口。
...
调用[startAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#startability)、[startAbilityForResult](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#startabilityforresult)等启动Ability接口时，如果目标应用不支持应用多开，则返回该错误码。

处理步骤

调用[startAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#startability)、[startAbilityForResult](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#startabilityforresult)等启动Ability接口时，确保目标应用支持应用多开，并在app.json5配置文件中配置[multiAppMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-configuration-file#multiappmode标签)标签开启应用分身功能。
...
指定的[APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)不存在时，返回该错误码。

可能原因

应用的实例中不存在该[APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)指定的实例。

处理步骤

确保传入的[APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)是一个有效值。
...
[APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)和[CREATE_APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)不支持同时指定。当指定[CREATE_APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)的同时指定[APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)，返回该错误码。
...
参数[APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)和[CREATE_APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)只支持二选一。
...
只允许应用使用[CREATE_APP_INSTANCE_KEY](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)创建自己的实例，不允许应用间启动时为其他应用创建实例。否则，返回该错误码。
...
在[module.json5配置文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file)的[abilities标签](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#abilities标签)中，当前UIAbility的allowSelfRedirect字段取值为“false”。

处理步骤

- 如果允许使用App Linking拉起当前UIAbility，开发者需要在[module.json5配置文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file)将[abilities标签](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#abilities标签)的allowSelfRedirect字段设置为true。
...
调用方不在目标应用的[appIdentifierAllowList](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#extensionabilities标签)中时，返回该错误码。

可能原因

[startAppServiceExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#startappserviceextensionability20)、[stopAppServiceExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#stopappserviceextensionability20)接口调用方的app-identifier不在目标[AppServiceExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-appserviceextensionability)的[appIdentifierAllowList](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#extensionabilities标签)中。

处理步骤

将接口调用方的app-identifier配置在目标[AppServiceExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-appserviceextensionability)的[appIdentifierAllowList](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#extensionabilities标签)中。
...
使用[connectAppServiceExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#connectappserviceextensionability20)接口时服务端还未启动且当前应用无权限拉起目标服务。

处理步骤

1. 等待服务端启动后重新连接。
2. 由当前应用拉起目标服务时，需要将接口调用方的app-identifier配置在目标[AppServiceExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-appserviceextensionability)的[appIdentifierAllowList](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#extensionabilities标签)中。
...
根据需要调整超时时间。超时时间的设置可参见[设置启动参数](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-startup#设置启动参数)。
...
1. 进程中存在未完成的[onNewProcessRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilitystage#onnewprocessrequest11)请求。
2. 当启动模式为[specified](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-launch-type#specified启动模式)的UIAbility运行在独立进程时，当前进程中存在未完成的[onAcceptWant](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilitystage#onacceptwant)请求。
