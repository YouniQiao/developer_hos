...
| repeat | [RepeatMode](#repeatmode) | 否 | 是 | 设置被切割的图片在边框上的重复方式。 默认值：RepeatMode.Stretch |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| angle | number | string | 否 | 是 | 线性渐变的起始角度。以12点钟方向为0度，顺时针旋转为正向角度。 默认值：180 角度为字符串时，支持数值加单位的组合格式，单位仅支持'deg'、'grad'、'rad'、'turn'，例如'90deg'、'180grad'、'3.14rad'、'0.25turn'。 **说明：** 设置direction后该属性不生效。 |
...
| colors | Array | 否 | 否 | 指定渐变色颜色和其对应的百分比位置的数组，设置非法颜色直接跳过。 |
...
通过[slider](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-components-basic-slider)接口动态调整[borderImage](#borderimage)接口中属性值。
...
通过[borderImage](#borderimage)接口中的slice、width和outset属性值使用[LocalizedEdgeWidths](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#localizededgewidths12)类型。
