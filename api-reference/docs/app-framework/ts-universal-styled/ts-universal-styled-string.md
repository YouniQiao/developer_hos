...
| styles | Array | 否 | 属性字符串初始化选项。 **说明：** start为异常值时，按默认值0处理； 当length为异常值时，length等于属性字符串在start后的实际长度； 当StyledStringKey与StyledStringValue不匹配时，styles不生效。 |
...
| 类型 | 说明 |
| --- | --- |
| Array | 各样式对象的数组。 **说明：** 当指定范围属性字符串未设置任何样式，则返回空数组。 当start和length越界或者必填传入undefined时，会抛出异常； 当styledKey传入异常值或undefined时，会抛出异常。 当styledKey为CustomSpan时，返回的是创建CustomSpan时传入的样式对象，即修改该样式对象也会影响实际的显示效果。 |
...
| --- | --- |
| | 段落，分隔文本段落。 |
| | 行内文本，支持样式设置。API version 17及之前，设置的background-color属性转换不生效。 |
| | 插入图片。 |
| | 加粗文本。 |
|
20+ | 换行。 |
...
| | 斜体文本。 **起始版本：** 26.0.0 |
| | 斜体文本。 **起始版本：** 26.0.0 |
| | 缩小字号标签。字号缩放为父容器字号属性的0.8倍，支持嵌套叠加。 **起始版本：** 26.0.0 |
| | 一级标题。 **起始版本：** 26.0.0 |
| | 二级标题。 **起始版本：** 26.0.0 |
| | 三级标题。 **起始版本：** 26.0.0 |
| | 四级标题。 **起始版本：** 26.0.0 |
...
![](./img/note_3.0-zh-cn.png) 当start和length越界或者必填传入undefined时，会抛出异常；
...
![](./img/note_3.0-zh-cn.png) 样式的最小颗粒度是StyledStringValue，如果设置了多个相同的StyledStringValue，只有最后一次设置会生效。如设置两个属性不同的TextStyle，则只有第二次设置的TextStyle生效。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onClick | Callback | 否 | 是 | 设置点击事件。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| onLongPress | Callback | 否 | 是 | 设置长按事件。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| onTouch20+ | Callback | 否 | 是 | 设置触摸事件。 **元服务API：** 从API version 20开始，该接口支持在元服务中使用。 |
...
| options20+ | [DecorationOptions](#decorationoptions20) | 是 | 是 | 获取属性字符串的文本装饰线样式的额外配置选项。 **元服务API：** 从API version 20开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| textShadow | Array | 是 | 否 | 获取属性字符串的文本阴影。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ShadowOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-image-effect#shadowoptions对象说明) | Array | 是 | 文本阴影设置项。 |
...
| colorFilter15+ | [ColorFilterType](#colorfiltertype15) | 是 | 是 | 获取属性字符串的图片颜色滤镜效果。 **元服务API：** 从API version 15开始，该接口支持在元服务中使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| attachment | Optional | 是 | PixelMap类型或[ResourceStr](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resourcestr)类型图片设置项。 |
...
该示例通过[fromHtml](#fromhtml)（从API version 12开始）、[toHtml](#tohtml14)（从API version 14开始）接口，将HTML中strong、b20+、em20+、i20+、u20+、del20+、s20+、a20+、sub20+、sup20+标签及其style属性中的background-color转换为属性字符串并转回HTML。
