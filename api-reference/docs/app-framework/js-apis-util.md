...
content_hash: "caefcc8b2811"
synced_at: "2026-09-04T18:09:34.445737"
---
...
![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，从API version 9开始废弃，建议使用[util.format9+](#utilformat9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，从API version 9开始废弃，建议使用[util.errnoToString9+](#utilerrnotostring9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，从API version 9开始废弃，此接口不可用，建议使用[util.promisify9+](#utilpromisify9)替代。
...
#### [h2]setMultithreadingDetectionEnabled23+
...
![](./img/note_3.0-zh-cn.png) 此接口在执行时会暂停所有VM线程运行以获取内存信息。由于需要等待所有VM线程暂停，高负载场景下调用此接口的耗时可能较高。
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，解析为HeapMemoryInfo对象数组，每个对象包含线程ID、线程名称、堆类型和堆对象大小。此方法可以获取local堆和shared堆的内存信息。 |
...
#### [h2]enableLocalHandleDetection24+
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 是 | 垃圾回收后内存达到预警阈值时触发的回调函数，字符串参数表示内存压力事件的类型。目前事件的类型有三种取值，"LocalHeapMemPressure"，"SharedHeapMemPressure"，"ProcessHeapMemPressure"。 |
...
![](./img/note_3.0-zh-cn.png) 该接口会正常解析值为\0的字节，将其转换为Unicode字符\u0000（空字符），不会导致解码中断或错误。
...
#### [h2]decodeWithStream(deprecated)

decodeWithStream(input: Uint8Array, options?: DecodeWithStreamOptions): string

将输入参数解码后输出对应文本。当input是一个空数组时，返回undefined。

![](./img/note_3.0-zh-cn.png) 从API version 9开始支持，从API version 12开始废弃，建议使用[decodeToString12+](#decodetostring12)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，从API version 9开始废弃，建议使用[create9+](#create9)替代。
...
#### [h2]decode(deprecated)

decode(input: Uint8Array, options?: { stream?: false }): string

通过输入参数解码后输出对应文本。

![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，从API version 9开始废弃，建议使用[decodeToString12+](#decodetostring12)替代。
...
#### [h2]encodeInto(deprecated)

encodeInto(input: string, dest: Uint8Array): { read: number; written: number }

将生成的utf-8编码文本写入dest数组。

![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，从API version 9开始废弃，建议使用[encodeIntoUint8Array9+](#encodeintouint8array9)替代。
...
#### [h2]encode(deprecated)

encode(input?: string): Uint8Array

将输入字符串编码后输出对应的Uint8Array字节数组。

![](./img/note_3.0-zh-cn.png) 从API version 7开始支持，从API version 9开始废弃，建议使用[encodeInto9+](#encodeinto9)替代。
...
#### [h2]parseRationalNumber9+

static parseRationalNumber(numerator: number,denominator: number): RationalNumber

创建具有给定分子和分母的RationalNumber实例。

![](./img/note_3.0-zh-cn.png) 该接口要求参数numerator和denominator必须是整数类型。如果传入的参数是小数类型，不会进行拦截，但是会输出错误信息："parseRationalNumber: The type of Parameter must be integer"。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| numerator | number | 是 | 分子，整数类型。取值范围：-Number.MAX_VALUE 返回值：
...
#### [h2]createRationalFromString8+

static createRationalFromString(rationalString: string): RationalNumber

使用给定的字符串创建RationalNumber对象。

![](./img/note_3.0-zh-cn.png) 该接口要求参数rationalString是字符串格式。如果传入的参数是小数类型字符串格式，不会进行拦截，但是会输出错误信息："createRationalFromString: The type of Parameter must be integer string"。
...
![](./img/note_3.0-zh-cn.png) 该接口要求参数number1和number2必须是整数类型。如果传入的参数是小数类型，不会进行拦截，但是会输出错误信息："getCommonFactor: The type of Parameter must be integer"。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| number1 | number | 是 | 整数类型。-Number.MAX_VALUE 返回值：
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[parseRationalNumber9+](#parserationalnumber9)替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| numerator | number | 是 | 分子，整数类型。取值范围：-Number.MAX_VALUE 示例：

```
let rationalNumber = new util.RationalNumber(1,2);
```

#### [h2]compareTo(deprecated)

compareTo(another: RationalNumber): number​

比较当前的RationalNumber对象与给定的对象。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[compare9+](#compare9)替代。
...
#### [h2]getCommonDivisor(deprecated)

static getCommonDivisor(number1: number,number2: number): number

获取两个指定整数的最大公约数。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[getCommonFactor9+](#getcommonfactor9)替代。
...
![](./img/note_3.0-zh-cn.png) 若此回调方法在[clear9+](#clear9)、[updateCapacity9+](#updatecapacity9)接口调用之后触发执行，传入的key和value参数类型为MapIterator，可参照示例二进行后续操作。
...
| 类型 | 说明 |
| --- | --- |
| IterableIterator | 返回一个迭代器对象，用于按插入顺序遍历当前缓冲区中的所有键值对[key, value]。 |
...
| 类型 | 说明 |
| --- | --- |
| IterableIterator | 返回一个键值对形式的二维数组。 |
...
| options12+ | [Type](#type10) | 否 | 从API version 12开始支持该参数，表示对应的编码格式。 此参数可选，可选值为：util.Type.BASIC和util.Type.BASIC_URL_SAFE，默认值为：util.Type.BASIC。 util.Type.BASIC表示Base64编码。 util.Type.BASIC_URL_SAFE表示Base64URL编码。 |
...
| options10+ | [Type](#type10) | 否 | 从API version 10开始支持该参数，表示对应的编码格式。 此参数可选，可选值为：util.Type.BASIC，util.Type.MIME，util.Type.BASIC_URL_SAFE 和util.Type.MIME_URL_SAFE，默认值为：util.Type.BASIC。 - 当参数取值为util.Type.BASIC，表示Base64编码，返回值没有回车符、换行符。 - 当参数取值为util.Type.MIME，表示使用Base64编码。如果返回值超过76个字符，则会在每76个字符处进行换行，并以'\r\n'结束每行。如果返回值少于76个字符，则会抛出异常。 - 当参数取值为util.Type.BASIC_URL_SAFE，表示Base64URL编码，返回值没有回车符、换行符。 - 当参数取值为util.Type.MIME_URL_SAFE，表示Base64URL编码，返回值每一行不超过76个字符，而且每行以'\r\n'符结束。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。编码成功时resolve为新分配的Uint8Array对象，编码失败时reject为错误对象。 |
...
| options10+ | [Type](#type10) | 否 | 从API version 10开始支持该参数，表示对应的编码格式。 此参数可选，可选值为：util.Type.BASIC，util.Type.MIME，util.Type.BASIC_URL_SAFE和util.Type.MIME_URL_SAFE，默认值为：util.Type.BASIC。 - 当参数取值为util.Type.BASIC，表示Base64编码，返回值没有回车符、换行符。 - 当参数取值为util.Type.MIME，表示Base64编码，返回值每一行不超过76个字符，而且每行以'\r\n'符结束。 - 当参数取值为util.Type.BASIC_URL_SAFE，表示Base64URL编码，返回值没有回车符、换行符。 - 当参数取值为util.Type.MIME_URL_SAFE，表示Base64URL编码，返回值每一行不超过76个字符，而且每行以'\r\n'符结束。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。编码成功时resolve为Base64编码后的字符串，编码失败时reject为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | 返回异步解码后新分配的Uint8Array对象。 |
...
![](./img/note_3.0-zh-cn.png) 从 API version 8开始支持，从API version 14开始废弃。没有替代接口。
...
#### [h2]isBoxedPrimitive(deprecated)

isBoxedPrimitive(value: Object): boolean

检查value是否为Boolean、Number、String或Symbol对象类型。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 14开始废弃，没有替代接口。
...
![](./img/note_3.0-zh-cn.png) AutoFinalizer<T>需要和AutoFinalizerCleaner<T>一起使用，只实现该接口类没有任何功能。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| obj | [AutoFinalizer](#autofinalizert22) | 是 | 实现了AutoFinalizer接口的对象，其onFinalization方法会在该对象被回收时调用。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache9+](#lrucache9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.constructor9+](#constructor9-3)替代。
...
#### [h2]updateCapacity(deprecated)

updateCapacity(newCapacity: number): void

将缓冲区容量更新为指定容量，如果newCapacity小于或等于0，则抛出异常。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.updateCapacity9+](#updatecapacity9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.toString9+](#tostring9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.getCapacity9+](#getcapacity9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.clear9+](#clear9)替代。
...
#### [h2]getCreateCount(deprecated)

getCreateCount(): number

获取createDefault()返回值的次数。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.getCreateCount9+](#getcreatecount9)替代。
...
#### [h2]getMissCount(deprecated)

getMissCount(): number

获取查询值不匹配的次数。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.getMissCount9+](#getmisscount9)替代。
...
#### [h2]getRemovalCount(deprecated)

getRemovalCount(): number

获取从缓冲区中逐出值的次数。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.getRemovalCount9+](#getremovalcount9)替代。
...
#### [h2]getMatchCount(deprecated)

getMatchCount(): number

获取查询值匹配成功的次数。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.getMatchCount9+](#getmatchcount9)替代。
...
#### [h2]getPutCount(deprecated)

getPutCount(): number

获取将值添加到缓冲区的次数。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.getPutCount9+](#getputcount9)替代。
...
#### [h2]isEmpty(deprecated)

isEmpty(): boolean

检查当前缓冲区是否为空。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.isEmpty9+](#isempty9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.get9+](#get9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.put9+](#put9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.values9+](#values9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.keys9+](#keys9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.remove9+](#remove9)替代。
...
#### [h2]afterRemoval(deprecated)

afterRemoval(isEvict: boolean,key: K,value: V,newValue: V): void

删除值后执行后续操作。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.afterRemoval9+](#afterremoval9)替代。
...
#### [h2]contains(deprecated)

contains(key: K): boolean

检查当前缓冲区是否包含指定的键。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.contains9+](#contains9)替代。
...
#### [h2]createDefault(deprecated)

createDefault(key: K): V

当缓冲区中未匹配到指定键时，执行后续处理逻辑，参数表示未匹配到的键，返回与该键关联的值，默认返回undefined。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.createDefault9+](#createdefault9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.entries9+](#entries9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[LRUCache.[Symbol.iterator]9+](#symboliterator9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper9+](#scopehelper9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.constructor9+](#constructor9-4)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.toString9+](#tostring9-1)替代。
...
#### [h2]intersect(deprecated)

intersect(range: Scope): Scope

获取给定范围和当前范围的交集。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.intersect9+](#intersect9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.intersect9+](#intersect9-1)替代。
...
#### [h2]getUpper(deprecated)

getUpper(): ScopeType

获取当前范围的上限。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.getUpper9+](#getupper9)替代。
...
#### [h2]getLower(deprecated)

getLower(): ScopeType

获取当前范围的下限。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.getLower9+](#getlower9)替代。
...
#### [h2]expand(deprecated)

expand(lowerObj: ScopeType,upperObj: ScopeType): Scope

创建并返回包括当前范围和给定下限和上限的并集。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.expand9+](#expand9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.expand9+](#expand9-1)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.expand9+](#expand9-2)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.contains9+](#contains9-1)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.contains9+](#contains9-2)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[ScopeHelper.clamp9+](#clamp9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[Base64Helper9+](#base64helper9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[Base64Helper.constructor9+](#constructor9-5)替代。
...
#### [h2]encodeSync(deprecated)

encodeSync(src: Uint8Array): Uint8Array

将输入的Uint8Array字节数组进行Base64编码，返回编码后的Uint8Array数组。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[Base64Helper.encodeSync9+](#encodesync9)替代。
...
#### [h2]encodeToStringSync(deprecated)

encodeToStringSync(src: Uint8Array): string

将输入的Uint8Array字节数组进行Base64编码，返回编码后的字符串结果。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[Base64Helper.encodeToStringSync9+](#encodetostringsync9)替代。
...
#### [h2]decodeSync(deprecated)

decodeSync(src: Uint8Array | string): Uint8Array

通过输入参数解码后输出对应文本。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[Base64Helper.decodeSync9+](#decodesync9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[Base64Helper.encode9+](#encode9)替代。
...
#### [h2]encodeToString(deprecated)

encodeToString(src: Uint8Array): Promise<string>

通过输入参数异步编码后输出对应文本。

![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[Base64Helper.encodeToString9+](#encodetostring9)替代。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 9开始废弃，建议使用[Base64Helper.decode9+](#decode9)替代。
