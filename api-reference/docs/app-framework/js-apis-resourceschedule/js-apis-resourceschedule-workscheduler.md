...
| callback | AsyncCallback | 是 | 回调函数。如果workId有效，则返回从WorkSchedulerService获取的任务，否则抛出异常。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，如果workId有效，则返回从WorkSchedulerService获取的任务，否则抛出异常。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 9开始支持，从API version 10开始废弃，建议使用[obtainAllWorks10+](#workschedulerobtainallworks10)替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数，获取成功时，err为undefined，否则为错误对象。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Array | 延迟任务列表，如果已添加延迟任务到执行队列，则返回当前应用所有的延迟任务列表；否则返回空列表。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数，获取成功时，返回当前应用所有的延迟任务列表，否则抛出异常。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回当前应用所有的延迟任务。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回true表示指定任务的最后一次执行超时，false表示未超时。 |
...
![](./img/note_3.0-zh-cn.png) WorkInfo参数设置时需遵循以下规则：
