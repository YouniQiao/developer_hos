...
| swipeActionOptions | [HdsSwipeActionOptions](#hdsswipeactionoptions) | [SwipeActionOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-listitem#swipeactionoptions9对象说明) | 否 | - | 动效横滑内容展示。 HdsSwipeActionOptions是HdsListItem封装后的横滑动效类型，SwipeActionOptions支持用户自定义使用ListItem的横滑动效类型。 |
...
| menuStyle | [MenuStyle](#menustyle) | 否 | @Prop | ListItem预览菜单样式。 **起始版本：** 6.1.0(23) |
...
| listItemStateStyles | [HdsListItemStateStylesOptions](#hdslistitemstatestylesoptions) | 否 | @Prop | ListItem多态样式。 **起始版本：** 26.0.0 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| icons | Array | 否 | 是 | 配置除删除按钮之外其他三个按钮样式。 |
| deleteIconOptions | [DeleteIconOptions](#deleteiconoptions) | 否 | 是 | 配置删除按钮样式。 |
| fullDeleteOptions | [FullDeleteOptions](#fulldeleteoptions) | 否 | 是 | 配置划动距离超过划出组件大小后的行为。 |
| deleteTriggerType | [SwipeDeleteTriggerType](#swipedeletetriggertype) | 否 | 是 | 配置横滑删除的触发类型。 默认值：SwipeDeleteTriggerType.NORMAL_TRIGGER。 **起始版本：** 6.1.0(23) |
| onStateChange | [OnStateChangeCallback](#onstatechangecallback) | 否 | 是 | 列表滑出状态变化回调。 **起始版本：** 6.1.0(23) |
...
| accessibilityRole | [AccessibilityRoleType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-accessibility#accessibilityroletype18枚举说明) | 否 | 是 | 图标的无障碍组件类型。 可以根据应用诉求，修改组件类型，用于控制无障碍模式下对组件的朗读方式和朗读内容。 默认值：AccessibilityRoleType.ROLE_NONE。 **起始版本：** 6.1.0(23) |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| selectedBackgroundColor | [ResourceColor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resourcecolor) | 否 | 是 | 选中态背景颜色。 默认值：Color.Transparent。 |
| normalBackgroundColor | [ResourceColor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#resourcecolor) | 否 | 是 | 正常态背景颜色。 默认值：Color.Transparent。 |
