...
- 类型为ring、scale-ring时，支持如下属性： 名称 类型 默认值 必填 描述 clockwise boolean true 否 圆环形进度条是否采用顺时针。 默认值：true，表示圆环形进度条采用顺时针。
- 类型为arc、eclipse5+时，支持如下属性： 名称 类型 默认值 必填 描述 percent number 0 否 当前进度。取值范围为0-100。
...
| 名称 | 类型 | 默认值 | 必填 | 描述 |
| --- | --- | --- | --- | --- |
| color | | #ff007dff | 否 | 设置进度条的颜色。 |
| stroke-width | | 4px | 否 | 设置进度条的宽度。 |
| background-color | | - | 否 | 设置进度条的背景色。 |
| secondary-color | | - | 否 | 设置次级进度条的颜色。 |

type=circular

| 名称 | 类型 | 默认值 | 必填 | 描述 |
| --- | --- | --- | --- | --- |
| color | | - | 否 | loading型进度条上的圆点颜色。 |

type=ring, scale-ring

| 名称 | 类型 | 默认值 | 必填 | 描述 |
| --- | --- | --- | --- | --- |
| color | | | - | 否 | 环形进度条的颜色，ring类型支持线性渐变色设置。 线性渐变色仅支持两个颜色参数设置格式，如color = linear-gradient(#ff0000, #00ff00)。 |
| background-color | | - | 否 | 环形进度条的背景色。 |
| secondary-color | | - | 否 | 环形次级进度条的颜色。 |
| stroke-width | | 10px | 否 | 环形进度条的宽度。 |
| scale-width | | - | 否 | 带刻度的环形进度条的刻度粗细，类型为scale-ring生效。 |
