...
| callback | [AsyncLockCallback](#asynclockcallback) | 是 | 获取锁后要调用的函数。 |
| mode | [AsyncLockMode](#asynclockmode) | 是 | 锁的操作模式。 |
| options | [AsyncLockOptions](#asynclockoptions) | 是 | 锁的操作选项。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | 回调执行后解决的Promise，或者在超时情况下被拒绝。 |
...
| signal | [AbortSignal](#abortsignal)|null | 否 | 否 | 用于终止异步操作的对象。当signal.aborted为true时，锁请求将被丢弃；当signal.aborted为false时，请求会继续等待获取锁；当signal为null时，请求正常排队运行。默认为 null。 |
