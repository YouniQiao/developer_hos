---
title: "拖拽排序"
upstream_id: "harmonyos-references/ts-universal-attributes-drag-sorting"
catalog: "harmonyos-references"
content_hash: "ffe38007671c"
synced_at: "2026-07-28T16:42:26.607273"
---
...
拖拽排序数据移动回调。当父容器组件为[List](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-list)或[Grid](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-grid)，并且ForEach/LazyForEach/Repeat每次迭代都生成一个ListItem或GridItem组件时才生效。调用后开启拖拽排序功能；拖拽排序离手后，如果数据位置发生变化，将触发handler回调，上报数据移动起始索引号和目标索引号。需要在回调中修改数据源，并确保数据仅顺序发生变化，才能正常执行落位动画。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | Optional | 是 | 拖拽排序数据移动回调。当数据位置因拖拽发生变化时触发，需在回调中根据起始索引号和目标索引号修改数据源。 |
...
拖拽排序数据移动回调。当父容器组件为[List](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-list)或[Grid](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-grid)，并且ForEach/LazyForEach/Repeat每次迭代都生成一个ListItem或GridItem组件时才生效。调用后开启拖拽排序功能；拖拽排序离手后，如果数据位置发生变化，将触发handler回调，上报数据移动起始索引号和目标索引号。需要在回调中修改数据源，并确保数据仅顺序发生变化，才能正常执行落位动画。与[onMove](#onmove)相比，新增eventHandler参数，可监听长按、开始拖拽、经过其他组件、拖拽结束等拖拽阶段事件。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onLongPress | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback) | 否 | 是 | 长按时触发的回调，不设置时不触发该回调。 - 参数index为长按时当前目标的索引号，取值范围是[0, 数据源长度-1]。 |
| onDragStart | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback) | 否 | 是 | 在页面跟手滑动开始时触发的回调，不设置时不触发该回调。 - 参数index为拖拽开始时当前目标的索引号，取值范围是[0, 数据源长度-1]。 |
| onMoveThrough | [OnMoveHandler](#onmovehandler) | 否 | 是 | 在页面跟手滑动过程中经过其他组件时触发的回调，不设置时不触发该回调。 - 参数from为拖拽起始索引号，参数to为当前经过的目标索引号，取值范围均是[0, 数据源长度-1]。 |
| onDrop | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback) | 否 | 是 | 在页面跟手滑动结束时触发的回调，不设置时不触发该回调。 - 参数index为拖拽结束时当前目标的索引号，取值范围是[0, 数据源长度-1]。 |

#### 示例

#### [h2]示例1（List使用OnMove进行拖拽）
...
#### [h2]示例2（List使用OnMove进行拖拽，并设置拖拽事件回调）
...
#### [h2]示例3（Grid规则布局使用ForEach的onMove进行拖拽，并设置拖拽事件回调）
...
```
 ![](./img/zh-cn_image_0000002655848402.gif)

#### [h2]示例4（Grid不规则布局使用ForEach的onMove进行拖拽，并设置拖拽事件回调）

从API版本26.0.0开始，以下示例展示了ForEach在Grid组件设置拖拽效果后触发的回调事件，Grid里存在不规则的GridItem。应用可通过[irregularIndexes](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-grid#gridlayoutoptions10对象说明)设置哪些索引是不规则节点，通过修改对应索引的rectSize调整该GridItem所占的行列数。
...
```
 ![](./img/zh-cn_image_0000002686087831.gif)

#### [h2]示例5（Grid不规则布局使用LazyForEach的onMove进行拖拽，并设置拖拽事件回调）
...
```
 ![](./img/zh-cn_image_0000002685928003.gif)

#### [h2]示例6（Grid不规则布局使用Repeat的onMove进行拖拽，并设置拖拽事件回调）
...
```
 ![](./img/zh-cn_image_0000002656008324.gif)
