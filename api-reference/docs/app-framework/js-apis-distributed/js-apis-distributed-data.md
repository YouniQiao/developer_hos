...
| callback | AsyncCallback | 是 | 回调函数。返回创建的KVManager对象实例。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回创建的KVManager对象实例。 |
...
| callback | AsyncCallback | 是 | 回调函数。返回创建的KVStore数据库实例。 |
...
| --- | --- | --- | --- |
| appId | string | 是 | 所调用数据库方的包名。 |
| callback | AsyncCallback | 是 | 回调函数。返回所有创建的KvStore数据库的storeId。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回所有创建的KvStore数据库的storeId。 |
...
#### [h2]on('distributedDataServiceDie')8+
...
#### [h2]off('distributedDataServiceDie')8+
...
| deathCallback | Callback | 否 | 取消订阅的函数。如不设置callback，则取消所有已订阅的函数。 |
...
| kvStoreType | [KVStoreType](#kvstoretype) | 否 | 设置要创建的数据库类型，默认为DEVICE_COLLABORATION，即多设备协同数据库。 **系统能力：** SystemCapability.DistributedDataManager.KVStore.Core |
| securityLevel | [SecurityLevel](#securitylevel) | 否 | 设置数据库安全级别(S1-S4)。 **系统能力：** SystemCapability.DistributedDataManager.KVStore.Core |
| schema8+ | [Schema](#schema8) | 否 | 设置定义存储在数据库中的值，默认为undefined，即不使用schema。 **系统能力：** SystemCapability.DistributedDataManager.KVStore.DistributedKVStore |
...
![](./img/note_3.0-zh-cn.png) 其中deviceId通过调用deviceManager.getTrustedDeviceListSync方法得到。deviceManager模块的接口均为系统接口，仅系统应用可用。
...
| listener | Callback | 否 | 取消订阅的函数。如不设置callback，则取消所有订阅的函数。 |
...
| syncCallback | Callback> | 否 | 取消订阅的函数。如不设置callback，则取消所有订阅的函数。 |
...
![](./img/note_3.0-zh-cn.png) 其中deviceIds为DeviceInfo中的networkId，通过调用deviceManager.getTrustedDeviceListSync方法得到。deviceManager模块的接口均为系统接口，仅系统应用可用。
...
| callback | AsyncCallback | 是 | 回调函数，返回匹配给定条件的字符串值。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回匹配给定条件的字符串值。 |
...
| --- | --- | --- | --- |
| query | [Query](#query8) | 是 | 表示查询对象。 |
| callback | AsyncCallback | 是 | 回调函数，返回与指定Query对象匹配的KvStoreResultSet对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回与指定Query对象匹配的KvStoreResultSet对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回与指定Query对象匹配的结果数。 |
