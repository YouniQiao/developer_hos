...
content_hash: "39a43e8098c5"
synced_at: "2026-08-29T18:16:17.795162"
---
...
1. 排查是否为以下场景：HAR模块开启[混淆配置](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/har-package#混淆配置)、中间码HAR、字节码HAR、跨HAP/HSP包。这四种场景下通过$r('app.xxx.xxx').id获取的资源ID为-1，推荐使用[getStringByName()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-resource-manager#getstringbyname9)等方法通过名称获取资源。
2. 检查传入的资源ID在HAP/HSP包中是否存在。可以参考[restool工具](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/restool)的dump命令，先输出HAP/HSP的资源信息，然后在输出结果中搜索是否存在传入的资源ID。
...
检查传入的资源名称在HAP/HSP包中是否存在。可以参考[restool工具](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/restool)的dump命令，先输出HAP/HSP的资源信息，然后在输出结果中搜索是否存在传入的资源名称。
...
检查使用$string:xxx、$integer:xxx等方式引用其他资源时是否存在循环引用的情况。
...
检查应用进程的[应用沙箱目录](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-sandbox-directory)下是否包含系统资源。
...
1. 检查overlay路径是否正确。
2. 检查应用进程的[应用沙箱目录](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-sandbox-directory)下是否包含传入的overlay路径。
