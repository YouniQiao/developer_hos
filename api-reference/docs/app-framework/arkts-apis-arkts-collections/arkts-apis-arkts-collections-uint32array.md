...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| buffer | [ArrayBuffer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-arraybuffer) | 是 | 否 | ArkTS Uint32Array底层使用的[ArrayBuffer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-arraybuffer)对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| elements | Iterable | 是 | 可迭代数字集合，用于构造ArkTS Uint32Array对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| array | ArrayLike | ArrayBuffer | 是 | 用于构造ArkTS Uint32Array的对象。当参数类型是ArrayBuffer时buffer所占的字节数需是4的整数倍。当参数类型是ArrayLike时，将复制其中的元素并创建新的底层[ArrayBuffer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-arraybuffer)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| arrayLike | ArrayLike | 是 | 用于构造ArkTS Uint32Array的ArrayLike对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| arrayLike | Iterable | 是 | 用于构造ArkTS Uint32Array的可迭代对象。 |
...
```
let uint32Array: collections.Uint32Array = collections.Uint32Array.of(1, 2, 3, 4);
console.info(uint32Array.toString()); // 预期输出：1,2,3,4
```
...
测试ArkTS Uint32Array中是否存在满足指定条件的元素。
...
| 类型 | 说明 |
| --- | --- |
| number | ArkTS Uint32Array中给定元素的第一个索引；没有找到，则返回-1。 |
...
返回ArkTS Uint32Array实例中最后一次出现searchElement的索引，如果不包含该元素，则返回-1。
...
| 类型 | 说明 |
| --- | --- |
| number | ArkTS Uint32Array中给定元素的最后一个索引；没有找到，则返回-1。 |
...
从指定的位置截取数组，返回一个新的、基于相同ArkTS ArrayBuffer的ArkTS Uint32Array对象。修改返回的subarray或原始ArkTS Uint32Array会互相影响，因为它们共享同一份底层[ArrayBuffer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-arraybuffer)数据。如需创建独立副本，请使用[slice()](#slice)方法。
...
| fromIndex | number | 否 | 开始搜索的索引，如果fromIndex 返回值：
