...
content_hash: "8d3d28f29860"
synced_at: "2026-08-04T17:00:49.559291"
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| elements | Iterable | 是 | 可迭代数字集合，用于构造ArkTS Uint8Array对象。集合中的数值取值范围为[0, 255]，超出值被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| array | ArrayLike | ArrayBuffer | 是 | 用于构造ArkTS Uint8Array的对象。传入ArrayLike时，数值取值范围为[0, 255]，超出值被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| arrayLike | ArrayLike | 是 | 用于构造ArkTS Uint8Array的ArrayLike对象。其中数值元素的取值范围为[0, 255]，超出值被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
...
| --- | --- | --- | --- |
| arrayLike | ArrayLike | 是 | 用于构造ArkTS Uint8Array的ArrayLike对象。其中数值元素的取值范围为[0, 255]，超出值被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
| mapFn | [TypedArrayFromMapFn](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-types#typedarrayfrommapfn) | 是 | 映射函数将ArrayLike中的每个元素从类型T映射为number类型，用于创建ArkTS Uint8Array。其中数值元素的取值范围为[0, 255]，超出值被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| arrayLike | Iterable | 是 | 用于构造ArkTS Uint8Array的可迭代对象。其中数值元素的取值范围为[0, 255]，超出值被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
| mapFn | [TypedArrayFromMapFn](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-types#typedarrayfrommapfn) | 否 | 映射函数。如果省略，则不对元素进行加工处理，直接使用原始数据创建Uint8Array。其中数值元素的取值范围为[0, 255]，超出值被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| target | number | 是 | 目标起始位置的下标，如果target 返回值：
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| predicate | [TypedArrayPredicateFn](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-types#typedarraypredicatefn) | 是 | 用于测试的断言函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| predicate | [TypedArrayPredicateFn](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-types#typedarraypredicatefn) | 是 | 用于元素过滤的断言函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| predicate | [TypedArrayPredicateFn](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-types#typedarraypredicatefn) | 是 | 用于元素查找的断言函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callbackFn | [TypedArrayForEachCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-types#typedarrayforeachcallback) | 是 | 用于对每个元素执行的回调函数。 |
...
| fromIndex | number | 否 | 从前到后搜索的起始下标。默认值为0。如果fromIndex 返回值：
...
| --- | --- | --- | --- |
| searchElement | number | 是 | 待查找索引的元素。 |
| fromIndex | number | 否 | 从后到前搜索的起始下标。默认值为ArkTS Uint8Array的长度减1。如果fromIndex 返回值：
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callbackFn | [TypedArrayMapCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-types#typedarraymapcallback) | 是 | 回调函数，对ArkTS Uint8Array中的每个元素执行转换操作，返回值作为对应位置的新元素，返回值若超出取值范围[0, 255]，会被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| array | ArrayLike | 是 | 用于设置的ArrayLike对象。数值元素的取值范围为[0, 255]，超出值被截断为低8位（例如256的二进制是100000000，截断后是00000000）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| compareFn | [TypedArrayCompareFn](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-arkts-collections-types#typedarraycomparefn) | 否 | 用于确定元素顺序的函数。默认使用升序排序。 |
...
| fromIndex | number | 否 | 开始搜索的索引，默认值为0。如果fromIndex 返回值：
...
| 类型 | 说明 |
| --- | --- |
| IterableIterator | 包含ArkTS Uint8Array中每个元素键值对的迭代器对象。 |
...
| 类型 | 说明 |
| --- | --- |
| IterableIterator | 包含ArkTS Uint8Array中每个元素的键（下标）的迭代器对象。 |
...
| 类型 | 说明 |
| --- | --- |
| IterableIterator | 包含ArkTS Uint8Array中每个元素的值的迭代器对象。 |
...
![](./img/note_3.0-zh-cn.png) 本接口不支持在.ets文件中使用（和本文中其他迭代器方法不同，其他迭代器方法没有此限制）。
