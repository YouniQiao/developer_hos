...
| encoding | [BufferEncoding](#bufferencoding) | 否 | 编码格式（当fill参数为string时，才有意义）。默认值：'utf8'。 |
...
| encoding | [BufferEncoding](#bufferencoding) | 否 | 编码格式（string参数为string类型时才有意义）。默认值：'utf8'。 |
...
| 错误码ID | 错误信息 |
| --- | --- |
| 10200001 | The value of "length" is out of range. It must be >= 0 and 示例：
...
| 错误码ID | 错误信息 |
| --- | --- |
| 10200001 | The value of "[byteOffset/length]" is out of range. It must be >= [left range] and 示例：
...
| length | number | 是 | 字节长度（此入参仅在object的valueOf()返回值为ArrayBuffer时生效，取值范围：0 返回值：
...
| targetStart | number | 否 | target实例中开始的偏移量。取值范围：>= 0且返回值：
...
| 错误码ID | 错误信息 |
| --- | --- |
| 10200001 | The value of "[targetStart/targetEnd/sourceStart/sourceEnd]" is out of range. It must be >= 0 and 示例：
...
| targetStart | number | 否 | target实例中开始写入的偏移量。取值范围：>= 0且返回值：
...
| offset | number | 否 | 起始偏移量。取值范围：>= 0且返回值：
...
| 错误码ID | 错误信息 |
| --- | --- |
| 10200001 | The value of "[offset/end]" is out of range. It must be >= 0 and 示例：
...
| encoding | [BufferEncoding](#bufferencoding) | 否 | 字符编码格式（value参数为string时才有意义）。默认值：'utf8'。 |
...
| 类型 | 说明 |
| --- | --- |
| [Buffer](#buffer) | 返回新的Buffer对象。当start 示例：
...
| 类型 | 说明 |
| --- | --- |
| IterableIterator | 返回包含Buffer中每个字节值的迭代器。 |
...
| offset | number | 否 | 偏移量。取值范围：>= 0且返回值：
...
| 错误码ID | 错误信息 |
| --- | --- |
| 10200001 | The value of "[offset/length]" is out of range. It must be >= 0 and 示例：
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | 是 | 写入Buffer的数据。取值范围：-128 返回值：
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | 是 | 写入Buffer的数据。取值范围：0 返回值：
...
| --- | --- | --- | --- |
| value | number | 是 | 写入Buffer的数据。 |
| offset | number | 否 | 偏移量。默认值为0。取值范围：0 返回值：
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，resolve返回包含Blob数据的ArrayBuffer，reject返回错误信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，resolve返回以utf8解码后的字符串，reject返回错误信息。 |
