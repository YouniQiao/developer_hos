...
![](./img/note_3.0-zh-cn.png) 针对maxWidth、maxHeight、minWidth、minHeight属性：
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| windowRect | [Rect](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-i#rect7) | 否 | 否 | 窗口尺寸，窗口在屏幕上的实际位置和大小。 **元服务API：** 从API version 15开始，该接口支持在元服务中使用。 |
| windowAlpha | number | 否 | 是 | 窗口透明度。有效值范围为[0.0, 1.0]，0.0表示完全透明，1.0表示完全不透明。默认值是-1.0，表示未查询到窗口透明度或者查询失败。 说明：本透明度非窗口背景色透明度（可通过[setWindowBackgroundColor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setwindowbackgroundcolor9)接口设置）。 **起始版本：** 26.0.0 **元服务API：** 从API版本26.0.0开始，该接口支持在元服务中使用。 **模型约束：** 此接口仅可在Stage模型下使用。 |
...
窗口截图动画配置，仅支持在调用[maximizeWithOptions()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#maximizewithoptions)或[recover()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#recover)接口时配置。
...
设备行为差异： 该接口在Phone设备的非[自由多窗模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/window-terminology#free-windows自由多窗模式)、Tablet设备的非[自由多窗模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/window-terminology#free-windows自由多窗模式)且非电脑模式下可正常调用，在其他设备、[自由多窗模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/window-terminology#free-windows自由多窗模式)或电脑模式下调用不生效也不报错。
...
| needAnimation23+ | boolean | 否 | 是 | 是否启用窗口创建动效。 传入true时，跟随系统默认动效。传入false时，表示关闭窗口创建动效，仅在[自由窗口状态](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/window-terminology#freeform-window自由窗口)的情况下生效。 此参数不填时，默认为undefined，跟随系统默认动效。 **模型约束：** 此接口仅可在Stage模型下使用。 |
