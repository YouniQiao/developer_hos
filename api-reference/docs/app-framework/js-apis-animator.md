...
content_hash: "a41d4d2228e5"
synced_at: "2026-07-28T16:40:59.245701"
---
...
![](./img/note_3.0-zh-cn.png) 从API version 6开始支持，从API version 9开始废弃，建议使用[create](#createdeprecated)替代。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onFrame12+ | (progress: number) => void | 否 | 否 | 接收到帧时回调。 progress表示动画的当前值。取值范围为[AnimatorOptions](#animatoroptions)定义的[begin, end]，默认取值范围为[0, 1]。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 **模型约束：** 此接口仅可在Stage模型下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| begin | number | 是 | 动画插值起点。 **说明：** 会影响[onFrame](#属性)回调的入参值。 |
| end | number | 是 | 动画插值终点。 **说明:** 会影响[onFrame](#属性)回调的入参值。 |
...
```
 ![](./img/zh-cn_image_0000002685927843.gif)
...
```
 ![](./img/zh-cn_image_0000002656008164.gif)
...
```
 ![](./img/zh-cn_image_0000002655848244.gif)
