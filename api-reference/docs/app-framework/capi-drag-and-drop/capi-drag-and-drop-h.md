...
| char* key | 返回数据加载成功启动后分配的key值，用于接收key值的字符数组长度不小于[UDMF_KEY_BUFFER_LEN](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-udmf-h#udmf_key_buffer_len)。 |
| unsigned int keyLen | key字符串的长度，不应小于[UDMF_KEY_BUFFER_LEN](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-udmf-h#udmf_key_buffer_len)定义的长度，用于确保key值能够被完整接收。 |
