...
content_hash: "5d80304066ed"
synced_at: "2026-08-07T15:56:13.909210"
---
...
```
 ![](./img/zh-cn_image_0000002698142269.png)
...
- 应用可注册多个监听回调，按照后注册先执行的顺序触发，当某个监听回调消费智慧手势事件后，即返回值[Class (GestureHandlingResolution)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-gesturehandlingresolution).isConsumed为true时，后续监听回调不再执行。
- 当应用重复注册相同回调时，只会保存首次注册的回调，重复注册不生效。
- 回调返回值必须是合法的[Class (GestureHandlingResolution)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-gesturehandlingresolution)实例，否则本次改写不生效。
...
```
 ![](./img/zh-cn_image_0000002668302604.png)
...
```
 ![](./img/zh-cn_image_0000002668462484.png)
