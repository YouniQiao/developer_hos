...
![](./img/note_3.0-zh-cn.png) buffer是开发者根据需要自定义大小的缓冲区，用于临时存储生成的XML文本。在使用过程中必须确保缓冲区足以容纳生成的文本内容。
...
![](./img/note_3.0-zh-cn.png) 该接口必须在[startElement](#startelement)之后调用，用于为当前已开启的元素设置属性。在元素开始标记写入之前调用此接口将产生无效XML。
...
![](./img/note_3.0-zh-cn.png) 该接口对所添加数据不做标准XML校验处理，请确保所添加的数据符合标准XML规范。例如不允许添加数字开头的元素名称。
...
![](./img/note_3.0-zh-cn.png) 调用该接口前必须先调用[startElement](#startelement)接口写入元素开始标记。
...
![](./img/note_3.0-zh-cn.png) 该接口应在[startElement](#startelement)之前调用，为即将开启的元素设置命名空间前缀。调用顺序：先调用setNamespace设置命名空间，再调用startElement开启元素。
...
![](./img/note_3.0-zh-cn.png) 该接口对所添加数据不做标准XML校验处理，请确保所添加的数据符合标准XML规范。例如不允许在CDATA标签中添加包含"]]>"字符串的数据。
...
![](./img/note_3.0-zh-cn.png) 使用该类构造的对象无需自行创建ArrayBuffer，程序动态扩容，可以不断添加XML元素，最终序列化结果字符串长度上限为100000。
...
![](./img/note_3.0-zh-cn.png) 该接口必须在[startElement20+](#startelement20)之后调用，用于为当前已开启的元素设置属性。
...
- 调用该接口后须调用[endElement20+](#endelement20)写入元素结束标记，以确保节点正确闭合。
- 该接口对所添加数据不做标准XML校验处理，请确保所添加的数据符合标准XML规范。例如不允许添加数字开头的元素名称。
...
![](./img/note_3.0-zh-cn.png) 调用该接口前必须先调用[startElement20+](#startelement20)接口写入元素开始标记。
...
![](./img/note_3.0-zh-cn.png) 该接口应在[startElement20+](#startelement20)之前调用，为即将开启的元素设置命名空间前缀。调用顺序：先调用setNamespace设置命名空间，再调用startElement开启元素。
...
![](./img/note_3.0-zh-cn.png) 该接口必须在[startElement20+](#startelement20)之后、[endElement20+](#endelement20)之前调用，用于设置当前元素的文本内容。
...
![](./img/note_3.0-zh-cn.png) 从API version 8开始支持，从API version 14开始废弃，建议使用[parseXml14+](#parsexml14)替代。
...
| tokenValueCallbackFunction | (eventType: [EventType](#eventtype), value: [ParseInfo](#parseinfo)) => boolean | 否 | 是 | 解析元素事件类型([EventType](#eventtype))和[ParseInfo](#parseinfo)属性，默认值为undefined，表示不解析。 **元服务API**：从API version 11开始，该接口支持在元服务中使用。 |
...
![](./img/note_3.0-zh-cn.png) 标签内的空白事件深度与标签的深度保持一致。
...
当解析器在XML文本开始解析时触发的回调函数。该回调函数需要开发者自行实现。具体使用示例可见[characters24+](#characters24)。
...
![](./img/note_3.0-zh-cn.png) 当可读流结束时触发此回调。在stream中调用push()，传入null值，从而触发该回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| elementName | string | 是 | 解析器回传的元素名称（不包含命名空间前缀）。例如，对于，elementName为"child"。 |
...
| qName | string | undefined | 是 | 解析器回传的元素限定名（包含命名空间前缀）。例如，对于，qName为"ns2:child"。如果元素没有命名空间则qName为undefined。 |
| attributes | Map | 是 | 解析器回传的元素的属性映射表，键为属性名（可能包含命名空间前缀，如"ns2:attrA"），值为属性值。 |
