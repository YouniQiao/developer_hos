...
| callback | AsyncCallback | 否 | 是 | 返回添加卡片是否成功的结果回调。返回为0表示卡片添加成功，非0表示卡片添加失败，失败时请参考[卡片错误码信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-form)进行排查。 |
...
![](./img/note_3.0-zh-cn.png) 仅在 style配置为空或不配置时，使用默认的图标和menu文字。
