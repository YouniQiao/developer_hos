...
![](./img/note_3.0-zh-cn.png) 仅type为SURFACE时有效。
...
![](./img/note_3.0-zh-cn.png) 当配置libraryname参数时，[点击事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-click)、[触摸事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-touch)、[挂载卸载事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-show-hide)、[按键事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-key)、[焦点事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-focus-event)、[鼠标事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-mouse-key)仅响应C-API侧事件接口。
...
![](./img/note_3.0-zh-cn.png) 使用自定义组件节点创建XComponent组件时，因为onLoad回调触发时机早于[onSurfaceCreated](#onsurfacecreated12)，所以在onLoad回调中调用[getXComponentSurfaceId](#getxcomponentsurfaceid9)获取surfaceId会失败，建议在[onSurfaceCreated](#onsurfacecreated12)回调中获取。
...
![](./img/note_3.0-zh-cn.png) rect参数中的offsetX/offsetY不设置或传入异常值时，Surface显示区域相对于XComponent左上角x/y轴的偏移效果默认按照居中显示。
...
![](./img/note_3.0-zh-cn.png) 仅当XComponent组件未设置libraryname参数时，会进行该回调。
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，无返回结果。用于获取AI分析是否成功执行。 |
...
![](./img/note_3.0-zh-cn.png) rotationOptions未配置时，默认XComponent持有Surface在屏幕旋转时不锁定方向，跟随屏幕进行旋转。
...
| 类型 | 说明 |
| --- | --- |
| Required | 获取XComponent持有Surface在屏幕旋转时是否锁定方向的设置。 |
