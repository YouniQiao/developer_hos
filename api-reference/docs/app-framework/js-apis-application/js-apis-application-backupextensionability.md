...
content_hash: "9d56c17512a6"
synced_at: "2026-08-29T18:16:07.018859"
---
...
备份恢复框架在备份时向应用传递扩展参数，支持应用根据参数执行自定义备份处理。与onBackup相比，onBackupEx支持接收扩展参数、返回处理结果并支持异步操作。需要处理调用方扩展参数或返回备份结果时使用onBackupEx，仅需执行基础备份逻辑时使用onBackup。使用Promise异步回调。
...
| 类型 | 说明 |
| --- | --- |
| string | Promise | 返回应用执行自定义备份操作的信息，返回值为JSON格式字符串，包含type、errorCode和errorInfo字段。type、errorCode和errorInfo由应用根据业务处理结果自行填充。 异步返回Promise。 同步返回string。 |

![](./img/note_3.0-zh-cn.png) 同步处理业务场景中，推荐使用示例如下。
...
```
 ![](./img/note_3.0-zh-cn.png) 异步处理业务场景中，推荐使用示例如下。
...
Extension生命周期回调，在执行恢复数据时回调，由开发者实现自定义恢复数据处理。与onRestore相比，onRestoreEx支持接收扩展参数、返回处理结果并支持异步操作。需要处理调用方扩展参数或返回恢复结果时使用onRestoreEx，仅需执行基础恢复逻辑时使用onRestore。使用Promise异步回调。
...
| 类型 | 说明 |
| --- | --- |
| string | Promise | 返回应用执行自定义恢复操作的信息，返回值为JSON格式字符串，包含type、errorCode和errorInfo字段。type、errorCode和errorInfo由应用根据业务处理结果自行填充。 异步返回Promise。 同步返回string。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scenario | number | 是 | 系统根据当前操作场景传入该参数，开发者根据参数值执行对应清理逻辑。 scenario = 1表示当前为备份场景。 scenario = 2表示当前为恢复场景。 |
