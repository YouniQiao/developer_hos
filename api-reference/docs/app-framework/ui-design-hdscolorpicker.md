...
| options | [HdsColorPickerOptions](#hdscolorpickeroptions) | 否 | @Param | 颜色选择器组件选项。 |
...
| tabs | [HdsColorPickerTabType](#hdscolorpickertabtype)[] | 否 | 是 | 设置颜色选择器的标签页类型列表。 默认值：[HdsColorPickerTabType.GRID, HdsColorPickerTabType.SPECTRUM, HdsColorPickerTabType.SLIDERS]。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| favoritesList | Array | 是 | 更新后的收藏颜色列表，颜色值支持十六进制ARGB标准颜色格式（如'FFFF0000'）。 |
