...
![](./img/note_3.0-zh-cn.png) 其中devices通过调用[deviceManager.getAvailableDeviceListSync](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-distributeddevicemanager#getavailabledevicelistsync)方法得到。

调用[sync](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbstore#sync)接口同步数据库时，在入参谓词中调用inDevices接口以选择设备。如果不调用inDevices接口，则默认连接组网内所有的设备。
...
| value | [ValueType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-t#valuetype) | 是 | 指示要与谓词匹配的值，长度不超过1024字节。 |
...
配置模糊查询条件，指定field列的模糊匹配条件。该方法等同于SQL语句中的"LIKE"。
...
配置谓词条件，表示字段field的值必须在给定的value列表内。该方法等同于SQL语句中的"IN"。

![](./img/note_3.0-zh-cn.png) value集合不能为空。如果传入空集，此条件将失效，导致操作针对所有数据（如全量查询、更新或删除）。请在调用前判断value是否为空集，避免误操作。
...
| args | Array | 否 | 条件中使用的参数，用来替换条件语句中的占位符，不传时默认为空数组。 |
