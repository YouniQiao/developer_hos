...
content_hash: "0989fc2644bc"
synced_at: "2026-09-23T18:53:08.716628"
---
...
| 14800000 | Inner error. 适用版本：12+ |
| 14800014 | The target instance is already closed. 适用版本：12+ |
| 14800015 | The database does not respond. 适用版本：12+ |
| 14800021 | SQLite: Generic error. 适用版本：12+ |
| 14800023 | SQLite: Access permission denied. 适用版本：12+ |
| 14800024 | SQLite: The database file is locked. 适用版本：12+ |
| 14800025 | SQLite: A table in the database is locked. 适用版本：12+ |
| 14800026 | SQLite: The database is out of memory. 适用版本：12+ |
...
| callback | AsyncCallback | 是 | 回调函数。当插入数据成功，err为undefined，data为行ID；否则为错误对象。 |
...
| 14800011 | The current operation failed because the database is corrupted. 适用版本：12+ |
| 14800014 | The target instance is already closed. 适用版本：12+ |
| 14800015 | The database does not respond. 适用版本：12+ |
| 14800021 | SQLite: Generic error. Possible causes: Insert failed or the updated data does not exist. 适用版本：12+ |
...
| 14800029 | SQLite: The database is full. 适用版本：12+ |
| 14800030 | SQLite: Unable to open the database file. 适用版本：12+ |
| 14800031 | SQLite: TEXT or BLOB exceeds size limit. 适用版本：12+ |
| 14800032 | SQLite: Abort due to constraint violation. 适用版本：12+ |
| 14800033 | SQLite: Data type mismatch. 适用版本：12+ |
| 14800034 | SQLite: Library used incorrectly. 适用版本：12+ |
...
| --- | --- | --- | --- |
| table | string | 是 | 指定的目标表名，不能为空字符串。 |
| values | Array | 是 | 表示要插入到表中的一组数据。 |
| callback | AsyncCallback | 是 | 回调函数。当批量插入成功，err为undefined，data为插入的数据个数；否则为错误对象。 |
...
| values | Array | 是 | 要插入到表中的一组数据。注意：空数组、含有重复资产数据会抛出参数错误。 |
...
| conflict | [ConflictResolution](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-e#conflictresolution10) | 否 | 指定冲突解决模式。默认为ON_CONFLICT_NONE。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回受影响的数据集。 |
...
| --- | --- | --- | --- |
| table | string | 是 | 要插入的目标表名。注意：正确的表名不应包含空格、逗号和星号，不能以点开头和结尾等，否则会抛出参数错误。 |
| values | Array | 是 | 表示要插入到表中的一组数据。注意：空数组、含有重复资产数据会抛出参数错误。 |
| config | [ReturningConfig](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#returningconfig23) | 是 | 指定返回值的配置信息。 |
| conflict | [ConflictResolution](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-e#conflictresolution10) | 否 | 指定冲突解决模式。默认为ON_CONFLICT_NONE。 |
...
| 类型 | 说明 |
| --- | --- |
| [Result](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#result23) | 返回受影响的数据集。 |
...
| callback | AsyncCallback | 是 | 回调函数。当更新数据成功，err为undefined，data为受影响的行数；否则为错误对象。 |
...
| callback | AsyncCallback | 是 | 回调函数。当删除数据成功，err为undefined，data为受影响的行数量；否则为错误对象。 |
...
| callback | AsyncCallback | 是 | 回调函数。当查询成功，err为undefined，data为ResultSet对象；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回ResultSet对象。 |
...
| --- | --- | --- | --- |
| predicates | [RdbPredicates](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates) | 是 | RdbPredicates的实例对象指定的查询条件。 |
| columns | Array | 否 | 表示要查询的列。如果值为空，则查询应用于所有列。默认值为空。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| [ResultSet](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-resultset) | 返回ResultSet对象。 |
...
| --- | --- | --- | --- |
| predicates | [RdbPredicates](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates) | 是 | RdbPredicates的实例对象指定的查询条件。 |
| columns | Array | 否 | 表示要查询的列。如果值为空，则查询该表的所有列。默认值为空。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | 返回LiteResultSet对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [LiteResultSet](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-literesultset) | 返回LiteResultSet对象。 |
...
| bindArgs | Array | 否 | SQL语句中参数的值。该值与sql参数语句中的占位符相对应。当sql参数语句完整时，该参数不填。默认值为空数组。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回LiteResultSet对象。 |
...
![](./img/note_3.0-zh-cn.png) 其中device通过调用[deviceManager.getAvailableDeviceListSync](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-distributeddevicemanager#getavailabledevicelistsync)方法得到。
...
| --- | --- | --- | --- |
| sql | string | 是 | 指定要执行的SQL语句，不能为空字符串。 |
| bindArgs | Array | 是 | SQL语句中参数的值。该值与sql参数语句中的占位符相对应。当sql参数语句完整时，该参数不填。默认值为空数组。 |
| callback | AsyncCallback | 是 | 回调函数。当查询成功，err为undefined，data为ResultSet对象；否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sql | string | 是 | 指定要执行的SQL语句，不能为空字符串。 不能为空字符串。 必须使用有效的SQL语句。否则在使用ResultSet时可能会抛出错误码。 |
| bindArgs | Array | 否 | SQL语句中参数的值。该值与sql参数语句中的占位符相对应。默认值为空数组。 |
...
| --- | --- | --- | --- |
| predicates | [RdbPredicates](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates) | 是 | RdbPredicates的实例对象指定的查询条件。 |
| columns | Array | 否 | 表示要查询的列。如果值为空，则查询应用于所有列。默认值为空数组。 |
...
| --- | --- | --- | --- |
| sql | string | 是 | 指定要执行的SQL语句，不能为空字符串。 |
| callback | AsyncCallback | 是 | 回调函数。当执行SQL成功，err为undefined，否则为错误对象。 |
...
| --- | --- |
| 401 | Parameter error. Possible causes: 1. Mandatory parameters are left unspecified; 2. Incorrect parameter types; 3. Parameter verification failed. |
| 801 | Capability not supported the sql(attach,begin,commit,rollback etc.). 适用版本：12+ |
| 14800000 | Inner error. |
| 14800011 | The current operation failed because the database is corrupted. 适用版本：12+ |
| 14800014 | The target instance is already closed. 适用版本：12+ |
| 14800015 | The database does not respond. 适用版本：12+ |
...
| --- | --- | --- | --- |
| sql | string | 是 | 指定要执行的SQL语句，不能为空字符串。 |
| args | Array | 否 | SQL语句中参数的值。该值与SQL参数语句中的占位符相对应。当SQL参数语句完整时，该参数不填。默认值为空数组。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回SQL执行后的结果。 |
...
| --- | --- | --- | --- |
| sql | string | 是 | 指定要执行的SQL语句，不能为空字符串。 |
| args | Array | 否 | SQL语句中参数的值。该值与SQL参数语句中的占位符相对应。该参数不填，或者填null或undefined，都认为是SQL参数语句完整，默认值为空数组。 |
...
| callback | AsyncCallback | 是 | 回调函数。当获取修改时间成功，err为undefined，data为ModifyTime对象；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | 返回ModifyTime类型的Promise对象，表示数据最后的修改时间。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [TransactionOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#transactionoptions14) | 否 | 表示事务对象的配置信息，默认值为DEFERRED。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回事务对象。 |
...
| callback | AsyncCallback | 是 | 回调函数。当备份成功，err为undefined，否则为错误对象。 |
...
| 401 | Parameter error. Possible causes: 1. Mandatory parameters are left unspecified; 2. Incorrect parameter types; 3. Parameter verification failed. |
| 14800000 | Inner error. |
| 14800010 | Failed to open or delete the database by an invalid database path. 适用版本：12+ |
| 14800011 | The current operation failed because the database is corrupted. 适用版本：12+ |
| 14800014 | The target instance is already closed. 适用版本：12+ |
| 14800015 | The database does not respond. 适用版本：12+ |
| 14800021 | SQLite: Generic error. 适用版本：12+ |
...
| callback | AsyncCallback | 是 | 回调函数。当恢复成功，err为undefined，否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| tables | Array | 是 | 要设置的分布式数据库的表名。 |
| callback | AsyncCallback | 是 | 回调函数。当设置分布式列表成功，err为undefined，否则为错误对象。 |
...
| config | [DistributedConfig](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#distributedconfig10) | 否 | 表的分布式配置信息。不传入时默认autoSync为false，需要调用[cloudSync](#cloudsync10-3)接口触发端云同步。 |
...
| callback | AsyncCallback | 是 | 回调函数。当获取分布式表名成功，err为undefined，data为远程设备的分布式表名；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回远程设备的分布式表名。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象。返回同步结果。string：设备ID；number：每个设备同步状态，0表示成功，1表示失败。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mode | [SyncMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-e#syncmode) | 是 | 同步模式。该值可以是relationalStore.SyncMode.SYNC_MODE_PUSH、relationalStore.SyncMode.SYNC_MODE_PULL。 |
| predicates | [RdbPredicates](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates) | 是 | 约束同步数据和设备。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象。返回SyncResult数组。 |
...
| progress | Callback | 是 | 用来处理数据库同步详细信息的回调函数。 |
| callback | AsyncCallback | 是 | 回调函数。当同步成功，err为undefined，否则为错误对象。 |
...
![](./img/note_3.0-zh-cn.png) [CloudSyncConfig](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#cloudsyncconfig)中仅支持以下谓词：

- [beginWrap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#beginwrap)
- [endWrap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#endwrap)
- [or](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#or)
- [and](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#and)
- 以下谓词的数据字段类型[ValueType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-t#valuetype)仅支持number类型的整数和string： [equalTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#equalto)
- [notEqualTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#notequalto)
- [in](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#in)
- [notIn](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#notin)

以下谓词的数据字段类型[ValueType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-t#valuetype)仅支持number类型的整数：

- [greaterThan](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#greaterthan)
- [lessThan](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#lessthan)
- [greaterThanOrEqualTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#greaterthanorequalto)
- [lessThanOrEqualTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates#lessthanorequalto)
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [CloudSyncConfig](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#cloudsyncconfig) | 是 | 云同步配置。 |
| progress | Callback | 是 | 进度回调函数，返回ProgressDetails实例对象。 |
...
| event | string | 是 | 取值为'dataChange'，表示数据更改。 |
| type | [SubscribeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-e#subscribetype) | 是 | 订阅类型。 |
| observer | Callback> | Callback> | 是 | 回调函数。 当type为SUBSCRIBE_TYPE_REMOTE，observer类型需为Callback>，其中Array为数据库中的数据发生改变的对端设备ID。 当type为SUBSCRIBE_TYPE_CLOUD，observer类型需为Callback>，其中Array为数据库中的数据发生改变的云端账号。 当type为SUBSCRIBE_TYPE_CLOUD_DETAILS，observer类型需为Callback>，其中Array为数据库端云同步过程的详情。 当type为SUBSCRIBE_TYPE_LOCAL_DETAILS，observer类型需为Callback>，其中Array为本地数据库中的数据更改的详情。 |
...
| observer | Callback | 是 | 回调函数。当进程间或本进程数据变更时触发回调。 |
...
| progress | Callback | 是 | 用于返回[ProgressDetails](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#progressdetails10)结果的回调函数。 |
...
| observer | Callback | 是 | 回调函数。用于返回数据库中SQL执行时间的统计信息。 |
...
| observer | Callback | 是 | 回调函数。用于返回SQL执行时出现的异常信息。 |
...
订阅SQL统计信息。使用[createTransaction](#createtransaction14)创建的事务进行相关操作（[Transaction](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-transaction)），只会在事务结束（COMMIT/ROLLBACK）时通知一次统计信息。
...
| observer | Callback | 是 | 回调函数。用于返回数据库执行SQL的时间。 |
...
| event | string | 是 | 取值为'dataChange'，表示数据更改。 |
| type | [SubscribeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-e#subscribetype) | 是 | 订阅类型。 |
| observer | Callback>| Callback> | 否 | 回调函数。 当type为SUBSCRIBE_TYPE_REMOTE，observer类型需为Callback>，其中Array为数据库中的数据发生改变的对端设备ID。 当type为SUBSCRIBE_TYPE_CLOUD，observer类型需为Callback>，其中Array为数据库中的数据发生改变的云端账号。 当type为SUBSCRIBE_TYPE_CLOUD_DETAILS，observer类型需为Callback>，其中Array为数据库端云同步过程的详情。 当type为SUBSCRIBE_TYPE_LOCAL_DETAILS，observer类型需为Callback>，其中Array为本地数据库中的数据更改的详情。 当observer没有传入时，表示取消当前type类型下所有数据变更的事件监听。 |
...
| observer | Callback | 否 | 回调函数。该参数存在，则取消指定Callback监听回调，否则取消该event事件的所有监听回调。 |
...
| observer | Callback | 否 | 回调函数，表示订阅时的回调函数。该参数存在，则取消指定Callback监听回调，否则取消该event事件的所有监听回调。 |
...
| callback | AsyncCallback | 是 | 回调函数。当清理成功，err为undefined，否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象。返回分离后剩余附加的数据库的数量。 |
...
![](./img/note_3.0-zh-cn.png) 加密参数变更需谨慎，在完成rekeyEx操作后，getRdbStore时必须使用新的参数来打开数据库，否则可能会导致开库失败。
