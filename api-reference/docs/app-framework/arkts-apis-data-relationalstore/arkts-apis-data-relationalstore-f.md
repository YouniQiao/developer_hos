...
| callback | AsyncCallback | 是 | 回调函数。当获取RdbStore成功，err为undefined，data为RdbStore对象；否则为错误对象。 |
...
删除成功后，建议将数据库对象置为null。建立数据库时，若在[StoreConfig](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#storeconfig)中配置了自定义路径，则调用此接口进行删库无效，必须使用 [deleteRdbStore](#relationalstoredeleterdbstore10) 接口进行删库。
...
| callback | AsyncCallback | 是 | 回调函数。当删除数据库成功，err为undefined，否则为错误对象。 |
...
删除成功后，建议将数据库对象置为null。建立数据库时，若在[StoreConfig](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#storeconfig)中配置了自定义路径，则调用此接口进行删库无效，必须使用 [deleteRdbStore](#relationalstoredeleterdbstore10-1) 接口进行删库。
...
| --- | --- | --- | --- |
| predicates | [RdbPredicates](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-rdbpredicates) | 是 | 与指定字段匹配的谓词。 |
| columns | Array | 否 | 要查询的列；如果不指定此参数，则查询所有列。 |
