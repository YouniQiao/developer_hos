...
| callback | Callback | 是 | 回调函数，返回当前音频返听的状态。 |
...
| 类型 | 说明 |
| --- | --- |
| Array | 返回支持返听的音频输入输出设备数组。 如果没有可用的输入输出设备组合，则返回空数组。 |
...
| 类型 | 说明 |
| --- | --- |
| [AudioDevicePair](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-i#audiodevicepair) | null | 返回系统推荐的音频输入输出设备组合。 如果没有可用的输入输出设备组合，则返回null。 |
