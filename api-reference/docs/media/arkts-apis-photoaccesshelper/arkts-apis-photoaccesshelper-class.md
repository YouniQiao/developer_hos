...
| assetCompatibleCapability24+ | [AssetCompatibleCapability](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-i#assetcompatiblecapability24) | 否 | 是 | 资产兼容性能力配置。 **模型约束：** 此接口仅可在Stage模型下使用。 **元服务API：** 从API version 24开始，该接口支持在元服务中使用。 |
| preferredCompatibleMode | [PreferredCompatibleMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-e#preferredcompatiblemode) | 否 | 是 | 资产兼容性模式配置。 **模型约束：** 此接口仅可在Stage模型下使用。 **元服务API：** 从API版本26.0.0开始，该接口支持在元服务中使用。 **起始版本：** 26.0.0 |
...
| movingPhotoBadgeStates22+ | Array | 否 | 否 | 返回图库选择的媒体文件动态照片状态数组。 当isMovingPhotoBadgeShown为true时，movingPhotoBadgeStates携带动态照片状态，反之为空。 **元服务API：** 从API version 22开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| mimeTypeArray | Array | 否 | 否 | PhotoPicker可供用户选择媒体文件的过滤类型。数组长度最大为10，因此支持最多十种指定类型。 过滤类型参考MIME类型定义，例如：“image/jpeg”、“video/mp4”等。 |
...
| value | Array | 否 | 是 | 不同谓词所需匹配的值。 非条件谓词如and、or、beginWrap、endWrap等不涉及该字段。 限制最大长度为10，超出则取前10个值。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| authorizedUris | Array | 否 | 是 | 返回已创建并授予保存权限的uri列表。 |
| invalidUris | Array | 否 | 是 | 返回可能被删除、隐藏或重命名的无效uri列表。 |
