...
synced_at: "2026-06-24T20:51:56.975529"
---
...
![](./img/note_3.0-zh-cn.png) SDK始终使用UMP（Universal MIDI Packet）格式进行数据传输，无论选择何种协议。此枚举定义连接的是数据行为和语义，而不是数据结构。MT（Message Type，消息类型）是UMP数据包的消息类型标识，不同MT值对应不同类型的MIDI消息。
