...
synced_at: "2026-06-24T20:52:33.745996"
---
...
| playlistLoopMode | [PlaylistLoopMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#playlistloopmode) | 否 | 是 | 在播放媒体列表时，设置循环模式。默认值为PLAYLIST_LOOP_MODE_ALL，表示循环播放列表中的所有项目。 **起始版本：** 26.0.0 **元服务API：** 从API Version 26.0.0开始，该接口支持在元服务中使用。 **模型约束：** 此接口仅可在Stage模型下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| filter | [TrackSelectionFilter](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-i#trackselectionfilter) | 是 | 轨道选择过滤器。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| src | [MediaSource](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-mediasource) | 是 | 要添加的媒体源。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mediaType | [MediaType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#mediatype8) | 是 | 媒体类型枚举。 **API version 12-19**：仅支持设置MEDIA_TYPE_AUD。 **API version 20及以后**：增加支持设置MEDIA_TYPE_VID。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回播放器信息PlaybackInfo。 |
...
| mode | [SwitchMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#switchmode12) | 否 | 切换轨道的模式。 取值约束：该模式仅适用于视频轨道的切换。 默认值：SMOOTH模式，在片段末尾进行切换，以确保视频播放的连续性。**仅在DASH/HLS协议网络流视频轨切换时生效。** 从API版本26.0.0开始支持HLS协议网络流视频。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回播放器当前已加载的时间区间段的列表。 时间区间段以播放时间轴上的[start, end]位置表示，单位为毫秒。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise> | Promise对象，返回播放器当前可跳转的时间区间段的列表。 时间区间段以播放时间轴上的[start, end]位置表示，单位为毫秒。 |
...
| payloadTypes | Array | 是 | SEI信息的订阅负载类型数组。当前仅支持负载类型为5，即payloadType = 5。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 是 | 事件触发时调用的回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 否 | 当事件触发时调用的回调函数。若未指定此参数，则取消订阅该事件的所有回调函数。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回当前播放器的指标信息PlaybackMetrics。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback> | 是 | 上报的指标事件信息的方法。使用callback异步回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback> | 否 | 上报的指标事件信息的方法。使用callback异步回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 否 | 回调函数，返回上报基于时间的元数据。默认值为取消订阅该事件的所有回调函数。 |
