---
title: "Interface (AVAdsController)"
upstream_id: "harmonyos-references/arkts-apis-media-avadscontroller"
catalog: "harmonyos-references"
content_hash: "0c92628a081b"
synced_at: "2026-08-29T18:17:40.306439"
---

# Interface (AVAdsController)

广告内容控制接口，用于管理广告播放控制器中的广告资源及监听广告事件，支持添加和移除广告源、跳过当前广告、禁用剩余广告等，适用于需要在视频播放过程中插入和管理广告内容的场景。通过[createAVAdsController()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-f#mediacreateavadscontroller)创建实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| src | [MediaSource](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-mediasource) | 是 | 要插入到主内容中播放的媒体源。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回添加到广告控制器中的媒体源ID，removeAdsMediaSource接口可用该ID移除对应的广告源。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [OnAdsEventLoadingErrorHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-t#onadseventloadingerrorhandle) | 是 | 广告内容加载失败的处理函数。由使用方实现。 第一个参数用于传递广告ID，第二个参数用于传递失败原因。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [OnAdsEventAdsStartedHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-t#onadseventadsstartedhandle) | 是 | 广告内容开始播放时的处理函数。常用于从主内容播放界面切换到广告播放界面的场景。 第一个参数表示正在播放的广告ID，第二个参数表示广告的时长，单位为毫秒（ms）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 是 | 广告跳过的处理函数。常用于恢复主内容播放。参数为被跳过的广告ID。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 是 | 广告播放完成的处理函数。常用于恢复主内容播放。参数为播放完成的广告ID。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [OnAdsEventLoadingErrorHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-t#onadseventloadingerrorhandle) | 否 | 广告内容加载失败的处理函数。 传入指定回调时，仅取消订阅该回调；不传入该参数时，默认取消订阅该事件的所有回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [OnAdsEventAdsStartedHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-t#onadseventadsstartedhandle) | 否 | 广告内容开始播放时的处理函数。常用于从主内容播放界面切换到广告播放界面的场景。 传入指定回调时，仅取消订阅该回调；不传入该参数时，默认取消订阅该事件的所有回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 否 | 广告跳过的处理函数。 传入指定回调时，仅取消订阅该回调；不传入该参数时，默认取消订阅该事件的所有回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback | 否 | 广告播放完成的处理函数。 传入指定回调时，仅取消订阅该回调；不传入该参数时默认取消订阅该事件的所有回调函数。 |
