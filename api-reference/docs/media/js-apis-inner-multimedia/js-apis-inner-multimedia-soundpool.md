...
- 在使用SoundPool实例的方法时，建议开发者注册相关回调，主动获取当前状态变化。 [on('loadComplete')](#onloadcomplete)：监听资源加载完成。建议开发者监听此回调以确保音频在加载完成后进行播放。
...
该方法不支持加载rawfile目录资源，需要通过[load(fd: number, offset: number, length: number, callback: AsyncCallback): void](#load-2)或者[load(fd: number, offset: number, length: number): Promise](#load-3)实现。
...
| callback | AsyncCallback | 是 | 异步音频资源加载返回的资源id，有效值大于0。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回资源的id，有效值大于0。 |
...
| callback | AsyncCallback | 是 | 获取回调的soundID，有效值大于0。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回soundID，有效值大于0。 |
...
| callback | AsyncCallback | 是 | 获取回调的音频流ID，有效值大于0。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回音频流ID，有效值大于0。 |
...
| callback | AsyncCallback | 是 | 回调函数。当音频池stop回调成功，err为undefined，否则为错误对象。 |
...
| callback | AsyncCallback | 是 | 回调函数。当音频池setPriority方法回调成功，err为undefined，否则为错误对象。 |
...
| callback | AsyncCallback | 是 | 回调函数。当音频池setRate方法回调成功，err为undefined，否则为错误对象。 |
...
| callback | AsyncCallback | 是 | 回调函数。当音频池setVolume方法回调成功，err为undefined，否则为错误对象。 |
...
| --- | --- | --- | --- |
| soundID | number | 是 | 资源ID，通过load方法获取。 |
| callback | AsyncCallback | 是 | 回调函数。当音频池unload方法回调成功，err为undefined，否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。当音频池release方法回调成功，err为undefined，否则为错误对象。 |
...
| callback | Callback | 是 | 回调函数，返回对应资源加载完成的资源ID。 |
...
| callback | Callback | 是 | 回调函数，返回播放完成的音频的streamId。 |
...
| callback | Callback | 是 | 异步'playFinished'的回调方法。 |
...
| callback | Callback | 是 | 回调函数，返回错误事件回调方法。在使用播放器的过程中发生错误时，提供错误信息[ErrorInfo](#errorinfo20)。 |
...
| callback | Callback | 否 | 错误事件回调方法。在使用播放器的过程中发生错误时，提供错误信息[ErrorInfo](#errorinfo20)，不设置callback时不提供相关信息。 |
