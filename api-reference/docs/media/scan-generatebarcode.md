...
content_hash: "dc6d90ac04fb"
synced_at: "2026-07-28T16:52:06.099079"
---
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| scanType | scanCore.[ScanType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/scan-scancore#scantype) | 否 | 否 | 指定要生成的码类型。 |
...
| level | [ErrorCorrectionLevel](#errorcorrectionlevel) | 否 | 是 | 纠错水平，默认值为LEVEL_H。此参数只在生成QR码时有效。 |
| backgroundColor | number | 否 | 是 | 生成码图背景颜色，HEX格式颜色，默认为白色（0xFFFFFF）。 |
| pixelMapColor | number | 否 | 是 | 生成码图颜色，HEX格式颜色，默认为黑色（0x000000）。 |

![](./img/note_3.0-zh-cn.png) 生成码参数建议：

- 码图颜色和背景 建议使用默认颜色和背景：黑色码图、白色背景。如果码图颜色和背景对比度较小会影响识别。
- 码图最小边距 建议使用默认最小边距1，单位：px，取值范围：[1, 10]。
- 码图大小 1. 生成QR Code、Data Matrix、Aztec类型的码图时，输入的width和height值相同且均大于等于200小于等于4096，单位：px，否则生成的码图过小会影响识别。 2. 生成EAN-8、EAN-13、UPC-A、UPC-E、Codabar、Code 39、Code 93、Code 128、ITF-14、PDF417类型的码图时，建议输入的width和height值比例为2:1，并且width值需大于400，单位：px，否则生成的码图会过小影响识别。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| content | string | 是 | 码内容字符串。参数限制请参见[createBarcode](#createbarcode)的表1-content参数限制条件。 |
| options | [CreateOptions](#createoptions) | 是 | 码图生成的配置参数。 |
| callback | AsyncCallback | 是 | 回调函数。当码图生成成功，err为undefined，data为生成的码图对象image.PixelMap；否则为错误对象。 |
