...
<strong>例外情况：</strong>必须对外提供且无利用风险的应用组件，可以不设置权限。
...
<strong>第一种方法：</strong>在加载URL页面前，校验当前页面的URL是否在白名单内，仅允许白名单内的URL调用。要同时校验协议、域名、PATH等，白名单范围控制的越小越安全。
