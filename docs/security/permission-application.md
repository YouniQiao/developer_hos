...
* 实施策略：如果用户拒绝了某项权限申请，应用不应再次弹窗请求该权限。应在页面的适当位置添加明确的提示，指导用户开启权限或退出当前需要该权限的场景，直到用户重新触发时再引导其完成授权。例如，权限申请被拒绝后，按钮上方显示提示文字，具体效果见下图：
  ...
  ![](./img/permission-application/permission-application-01.png "点击放大")
...
* 正例1（以示例代码为例，效果图如下）
  ...
  ![](./img/permission-application/permission-application-02.png "点击放大")
...
3. 不允许：点击后应用无法获取该权限，且再次调用requestPermissionsFromUser()方法无法拉起该权限设置弹框。
   ...
   ![](./img/permission-application/permission-application-03.png "点击放大")

具体流程图如下：

![](./img/permission-application/permission-application-04.png "点击放大")
...
![](./img/permission-application/permission-application-05.png "点击放大")
...
[应用代码混淆](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-app-code-ob "应用代码混淆")

[网络连接安全配置](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-network-ca-security "网络连接安全配置")