---
format: md
title: "如何给新增的module在线签名"
original_url: /docs/FAQ/faqs-deveco-studio/faqs-compiling-and-building/faqs-compiling-and-building-52
upstream_id: FAQ/faqs-deveco-studio/faqs-compiling-and-building/faqs-compiling-and-building-52
last_sync: 2026-06-07
sync_hash: 15f7a38b
---
操作步骤：

1. 连接真机设备，确保[DevEco Studio与真机设备已连接](/docs/tools/coding-debug/ide-run-device)，真机连接成功后如下图所示：

   ![](./img/618834e5.png)
2. 进入 File > Project Structure... > Project > Signing Configs 界面，勾选“Automatically generate signature”。如果是 HarmonyOS 工程，还需勾选“Support HarmonyOS”。若未登录，请先单击 Sign In 进行登录，然后完成签名。

   ![](./img/1664cafd.png "点击放大")

   签名完成后，如下图所示：

   ![](./img/9289ea4b.png "点击放大")
