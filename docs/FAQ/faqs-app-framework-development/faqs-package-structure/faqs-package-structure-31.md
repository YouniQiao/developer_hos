...
upstream_id: FAQ/faqs-app-framework-development/faqs-package-structure/faqs-package-structure-31
...
2. 构建闭源HAR：DevEco Studio支持闭源HAR构建，通过对代码进行编译混淆，生成闭源HAR。在不共享源码的情况下，通过闭源HAR对外提供组件、资源等，可以实现多个模块或多个工程共享组件、资源等。具体实现方式可以参考：[以release模式构建](/docs/tools/coding-debug/ide-hvigor-build-har#section19788284410)。
3. 可以通过开启混淆能力来保护代码资产。代码混淆的使用约束包括：仅支持Stage工程；在编译模式为release模式时生效；模块及其依赖的HAR和HSP均未关闭混淆。
   ...
   * "files"：配置混淆规则文件路径。
   ...
   具体内容可参考链接：[混淆加固](/docs/tools/coding-debug/ide-build-obfuscation)。
