...
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ide-insight-session-time
format: md
---
...
2. 创建Time任务并录制相关数据，操作方法可参考[性能问题定位：深度录制](`https://`developer.huawei.com/consumer/cn/doc/harmonyos-guides/deep-recording)。或在会话区选择<strong>Open File</strong>，导入历史数据。
   ...
   * <strong>User Trace</strong>：用户自定义打点泳道，基于时间轴展示当前时段内用户使用hiTraceMeter接口自定义的打点任务的具体运行情况。
   * <strong>ArkTS Callstack</strong>：方舟运行时函数调用泳道，基于时间轴展示CPU使用率和虚拟机的执行状态，以及当前调用栈名称和调用类型。由于隐私安全政策，已上架应用市场的应用不支持录制此泳道。
   ...
   * <strong>Callstack</strong>：ArkTS和Native混合函数调用泳道。基于时间轴展示各线程的CPU使用率，以及在一段时间内的混合调用栈。调用栈类型会分为开发者或系统的ArkTS以及Native代码两类。由于隐私安全政策，已上架应用市场的应用不支持录制此泳道。
...
3. 在“ArkTS Callstack”泳道和“ArkTS Callstack”子泳道上长按鼠标左键并拖拽，框选要分析的时间段。
   ...
   * 打开页面下方的<strong>Flame Chart</strong>开关，函数调用栈将以火焰图的形式展示。其中，横轴表示函数的执行时长，纵轴表示调用栈的深度。

     ![](./img/note_3.0-zh-cn.png)

     + 火焰图条块支持搜索，搜索结果不匹配的条块会被置灰。
     + “Ctrl+鼠标滚轮”的操作，或单击该区域右上角的![](./img/zh-cn_image_0000002571547264.png)、![](./img/zh-cn_image_0000002571547252.png)可放大和缩小火焰图的时间轴比例，单击![](./img/zh-cn_image_0000002571387624.png)可恢复时间轴比例为初始状态。
     + “Shift+鼠标滚轮”的操作可左右横向调整可视区间，单独操作滚轮可上下纵向调整可视区间。
     + 选中节点，单击该区域右上角的![](./img/zh-cn_image_0000002602066741.png "点击放大")，点击添加面包屑。添加面包屑后，该节点成为根节点，耗时占比为100%，子节点的耗时占比相对于该节点重新计算。
     + 在火焰图中选中任一节点，使用“Alt+鼠标左键”可将该节点左置底并将其占比放大到100%，其上从属节点按同比例放大显示。

     ![](./img/zh-cn_image_0000002602066747.png "点击放大")
   * 在“ArkTS Callstack”子泳道或“Callstack”子泳道上点击<strong>Unfold CallStack</strong>按钮，可以在时间轴上将函数调用栈以冰锥图的形式展示。其中调用栈的先后顺序与实际调用时序保持一致。

     ![](./img/zh-cn_image_0000002571387620.png "点击放大")
4. 在<strong>Callstack</strong>泳道上长按鼠标左键并拖拽，框选要分析的时间段。
   * <strong>Summary</strong>列表展示框选时段内，所有Native线程的CPU占用率的峰值、谷值、平均值。
   * <strong>Callstack</strong>列表展示框选时段内，所有Native线程的函数热点。

   ![](./img/zh-cn_image_0000002571387630.png "点击放大")

   * 悬浮到节点，显示以此节点为根节点，点击添加面包屑。添加面包屑后，该节点成为根节点，耗时占比为100%，子节点的耗时占比相对于该节点重新计算。

   ![](./img/zh-cn_image_0000002602186801.png "点击放大")
5. 在<strong>Callstack</strong>子泳道上长按鼠标左键并拖拽，框选要展示分析的时间段。
   * <strong>Top Down</strong>页签显示所选时间段内的函数栈耗时分布情况，<strong>Heaviest Stack</strong>区域会展示出“Details”区域选择节点所处的耗时最长的完整调用栈。
   * <strong>Bottom UP</strong>页签显示函数列表，展开任一函数节点可查看其调用方及每个调用方的耗时。

   ![](./img/zh-cn_image_0000002571547266.png "点击放大")
6. （可选）在<strong>Details</strong>中双击需要优化的节点（例如耗时超过预期），可快速跳转至对应工程源码，为开发者节省定位代码路径的时间。
...
![](./img/zh-cn_image_0000002602186797.png "点击放大")
...
![](./img/zh-cn_image_0000002571547254.png "点击放大")

您可以通过点击工具栏![](./img/zh-cn_image_0000002602066739.png "点击放大")按钮，导入包含debug信息的so库。
...
![](./img/zh-cn_image_0000002571387622.png "点击放大")
...
![](./img/zh-cn_image_0000002602066735.png)
...
![](./img/zh-cn_image_0000002602186791.png "点击放大")
