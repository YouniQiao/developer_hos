...
1. 在工程中选中模块或文件，右键单击<strong>New &gt; Image Asset</strong>，进入图标配置页面。

   ![](./img/69783cd8.png "点击放大")
2. 需要根据向导配置图标样式、大小等基本信息。
   * <strong>Device</strong>：选择当前配置的图标生效的设备类型。
   * <strong>Icon Type</strong>：展示当前图标的类型。
   * <strong>Name</strong>：配置图标名称。命名支持使用字母、数字、下划线，长度最多128个字符，不支持中文命名。
   * <strong>Foreground Layer</strong>：分层图标资源前景层。可配置下列字段信息：
     + <strong>Path</strong>：选择前景Image存放路径。推荐使用的图标尺寸为1024px\*1024px，保证图标整体的清晰性。
     + <strong>Trim</strong>：选择Yes，将调整图标图形与边框之间的距离，同时会去除图片周围多余的透明空间。
     + <strong>Resize</strong>：拖动滑块，设置图形的缩放比例。

   ![](./img/ea482130.png)

   * <strong>Background Layer</strong>：分层图标资源背景层。请配置下列字段信息：
     + <strong>Asset Type</strong>：设置图标背景类型。可以选择颜色（<strong>Color</strong>）或图像（<strong>Image</strong>）。
     + <strong>Color</strong>：点击色块区域，选择适当的背景色。
     + <strong>Path</strong>：选择背景Image路径。推荐使用的图标尺寸为1024px\*1024px，保证图标整体的清晰性。
     + <strong>Trim</strong>：选择Yes，将调整图标图形与边框之间的距离，同时会去除图片周围多余的透明空间。
     + <strong>Resize</strong>：拖动滑块，设置图形的缩放比例。

   ![](./img/8b5be6ff.png)
3. 点击<strong>Next</strong>，确认图标的存储路径和相应的尺寸信息，图标将默认存放在<strong>resources</strong> 目录下。点击<strong>Finish</strong>完成图标生成。
