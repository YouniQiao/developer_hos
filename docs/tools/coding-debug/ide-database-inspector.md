...
1. 点击菜单栏<strong>View &gt; Tool Windows &gt; Database Inspector</strong>，打开Database Inspector。

   ![](./img/zh-cn_image_0000002571546676.png)

   Database Inspector打开后，页面各区域作用如下：①选择设备，②选择应用包名，③数据库和表信息展示，④SQL执行和数据查看。

   ![](./img/zh-cn_image_0000002571387042.png)
2. 从设备下拉列表中选择设备（设备需已连接）。
3. 选择包名，点击右侧的<strong>Connect to Databases</strong>按钮，即可查看数据库相关信息。（如果使用DevEco Studio 6.0.2 Beta1版本，按钮名称是<strong>Execute</strong>）。

   ![](./img/note_3.0-zh-cn.png)

   设备系统版本低于API 22时，Database Inspector会将数据库下载到本地计算机，界面上显示的是本地计算机路径，设备系统为API 22及以上版本时，界面上显示的是设备上的数据库路径。

   ![](./img/zh-cn_image_0000002602066157.png)
4. 双击数据库表名，右侧区域展示表数据，默认按照20条/页展示。

   ![](./img/zh-cn_image_0000002571546672.png)
5. 左侧区域点击<strong>New Query</strong>后，右侧会出现<strong>SQL Editor</strong>页签，根据需要选择数据库，并输入SQL后，点击<strong>Execute</strong>按钮即可查看或修改数据。

   修改数据后，点击SQL输入框下方的<strong>Refresh Table</strong>刷新页面上的数据。
   ...
   * 数据展示页面不支持可视化修改。
   ...
   ![](./img/zh-cn_image_0000002571546674.png)
