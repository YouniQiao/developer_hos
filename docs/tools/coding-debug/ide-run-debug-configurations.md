...
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ide-run-debug-configurations
format: md
---
...
点击<strong>Run &gt; Edit Configurations &gt; Debugger</strong>，选择相应模块，设置Debug type即可。

![](./img/zh-cn_image_0000002602186461.png)

工程调试类型默认为<strong>Detect Automatically</strong>，关于各调试类型的说明如下表所示：

<strong>表1</strong> 调试类型配置项

| 调试类型 | 调试代码 |
| --- | --- |
| <strong>Detect Automatically</strong> | 新建工程默认调试器选项。根据工程模块及其依赖的模块涉及的编程语言，自动启动对应的调试器。 |
| <strong>ArkTS/JS</strong> | * 调试ArkTS代码 * 调试JS代码 |
| <strong>Native</strong> | 仅调试C/C++代码 |
| <strong>Dual(ArkTS/JS + Native)</strong> | 调试C/C++工程的ArkTS/JS和C/C++代码 |
...
单击<strong>Run &gt; Edit Configurations</strong>，设置指定模块的HAP安装方式，勾选<strong>Keep Application Data</strong>，则表示采用覆盖安装方式，保留应用/元服务缓存数据。

![](./img/zh-cn_image_0000002602066385.png)
...
* Launch Options
  + Launch：指定在安装应用后启动的Ability。
    ...
    ![](./img/zh-cn_image_0000002602066381.png)
    ...
    - Default Ability：默认的EntryAbility。
      * Stage模型：module.json5文件中配置了“skills”属性的第一个ability；若无配置“skills”属性的ability，则取“mainElement”指定的ability（该ability需存在于“abilities”数组内）；若“mainElement”未指定，则取“abilities”数组内的第一个ability。
        ...
        ![](./img/zh-cn_image_0000002602186441.png "点击放大")
...
如果开发者需要配置和管理应用开发环境，以及控制应用程序的行为，可在<strong>Environment Variables</strong>下配置环境变量。

![](./img/zh-cn_image_0000002571387268.png)
...
环境变量配置完成后，需确保环境变量已勾选，勾选后点击<strong>Apply</strong>才可生效。

![](./img/zh-cn_image_0000002571387270.png)
...
单击<strong>Run</strong> <strong>&gt;</strong> <strong>Edit Configurations</strong>，在<strong>General</strong>中，勾选<strong>Auto WebView Debug</strong>。

![](./img/zh-cn_image_0000002602066373.png)

开启后，当检测到设备上有可调试的WebView组件进程时，会在Run面板中打印转发成功的端口，通过浏览器的DevTools工具连接该端口即可进行WebView调试。

![](./img/zh-cn_image_0000002571387292.png)

#### 多模块调试

#### 安装多个模块

如果一个工程中同一个设备存在多个模块（如存在entry和feature模块），且存在模块间的调用时，在调试阶段需要同时安装多个模块的Hap包到设备中。此时，需要在<strong>Deploy Multi Hap/Hsp</strong>中选择多个模块，启动调试时，DevEco Studio会将所有的模块都安装到设备上。

设置方法如下：

单击<strong>Run &gt; Edit Configurations</strong>，在<strong>Deploy Multi Hap/Hsp</strong>中，勾选<strong>Deploy Multi Hap/Hsp Packages</strong>，选择多个模块。

![](./img/zh-cn_image_0000002571387258.png)

#### 自动安装依赖

如果一个工程中entry/feature/HSP模块直接依赖其他HAR/HSP模块（如entry模块依赖HSP模块）及间接依赖其他模块（如entry模块依赖HAR模块，HAR又依赖HSP模块）时，在调试阶段需要同时安装模块包及其所有依赖模块的包到设备中。此时，可以设置<strong>Auto Dependencies</strong>，启动调试时会自动将所有依赖的模块都安装到设备上。

设置方法如下：

单击<strong>Run &gt; Edit Configurations</strong>，在<strong>General</strong>中，勾选<strong>Auto Dependencies。</strong>

![](./img/zh-cn_image_0000002571387288.png)

在Before launch窗格中，您可以点击![](./img/zh-cn_image_0000002571387290.png)添加应用启动前的任务。

![](./img/zh-cn_image_0000002602066387.png)

也可以点击![](./img/zh-cn_image_0000002602186455.png)移除任务。

![](./img/zh-cn_image_0000002571387260.png)

在勾选<strong>Auto Dependencies</strong>后，可以同时勾选<strong>Deploy Multi Hap/Hsp Packages</strong>，从而达到推送所有包的效果。

#### 多设备运行

从DevEco Studio 6.0.2 Beta1版本开始，支持同时在多个设备上运行应用，包括真机和已启动的模拟器。

1. 在设备选择框中，点击<strong>Select Multiple Devices</strong>，弹出多设备选择框。

   ![](./img/zh-cn_image_0000002602066377.png)
2. 选择需要推包运行的设备。

   ![](./img/zh-cn_image_0000002602066405.png)
3. 设备栏会出现Multiple Devices(N)，表示选中N个设备，点击运行按钮即可同时在选中设备上运行应用。

   ![](./img/zh-cn_image_0000002571546908.png)
