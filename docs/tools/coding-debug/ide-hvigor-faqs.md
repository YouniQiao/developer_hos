...
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ide-hvigor-faqs
format: md
---
...
* 如果以上修改没有取得明显的效果，可以使用非并行的模式来执行编译。
  + 在菜单栏点击“File &gt; Settings（macOS为DevEco Studio &gt; Preferences/Settings） &gt; Build, Execution, Deployment &gt; Build Tools &gt; Hvigor”，取消勾选“Execute tasks in parallel mode (may require larger heap size)”。
    ...
    ![](./img/zh-cn_image_0000002602066477.png)
...
* <strong>场景一：</strong>
  ...
  1. 确保XXX是当前的HvigorNode里存在的任务。
     ...
     ![](./img/zh-cn_image_0000002602066539.png)
...
* <strong>场景三：</strong>
  ...
  编译构建时，出现报错“Cannot read properties of undefined(reading 'getPluginId')”<strong>。</strong>
...
* <strong>场景四：</strong>
  ...
  编译构建时，出现报错“Cannot read properties of undefined(reading 'getNeedExecTargetServiceList')”<strong>。</strong>
...
* <strong>场景八：</strong>
  ...
  ![](./img/zh-cn_image_0000002571387378.png)
...
![](./img/zh-cn_image_0000002602186589.png)
...
![](./img/zh-cn_image_0000002602066483.png)
...
![](./img/zh-cn_image_0000002602186587.png)
...
![](./img/zh-cn_image_0000002602066449.png)
...
![](./img/zh-cn_image_0000002602186493.png)
...
![](./img/zh-cn_image_0000002602066459.png)

2.编译工具根据设置的Build Mode选项生成debug标识，如图所示，可以通过此处进行设置。

![](./img/zh-cn_image_0000002571387330.png)
...
![](./img/zh-cn_image_0000002602186551.png)
...
![](./img/zh-cn_image_0000002602066491.png)
...
* <strong>场景一：</strong>
  ...
  1. 在对应cpp目录下新建types/libxxx目录，并在该目录下新增index.d.ts用于声明native的类型符号；新增oh-package.json5配置文件用于校验工具的模块查询。
     ...
     ![](./img/zh-cn_image_0000002602066457.png)

     ![](./img/zh-cn_image_0000002571387376.png)
  2. 在native引用的模块内的oh-package.json5中添加native模块的本地依赖，并根据提示点击<strong>Sync Now</strong>同步工程，下图以entry模块引用native模块为例。

     ![](./img/zh-cn_image_0000002602186523.png)
...
* 场景二：
  ...
  ![](./img/zh-cn_image_0000002571547020.png)
...
* 场景三：
  ...
  ![](./img/zh-cn_image_0000002571387318.png)

  代码中这样引用

  ![](./img/zh-cn_image_0000002602186561.png)这样引用会找不到模块，导致报错。

  <strong>处理措施</strong>

  修改引用方式，改为推荐的引用方式

  ![](./img/zh-cn_image_0000002602066515.png)
* 场景四：
...
* 场景七：
  ...
  ![](./img/zh-cn_image_0000002602066549.png)
  ...
  1. 通过在build目录中搜索'rollup\_plugin\_ignore\_empty\_module\_placeholder'，找到报错的中间文件，并根据中间文件找到对应工程文件。
     ...
     ![](./img/zh-cn_image_0000002602186559.png)

     在输入栏中输入rollup\_plugin\_ignore\_empty\_module\_placeholder，找到问题模块的中间文件。

     ![](./img/zh-cn_image_0000002602066455.png)
  2. 在引用类型文件中通过添加type显式声明符号类型引用：
     ...
     ![](./img/zh-cn_image_0000002602186599.png)
  3. 同时排查是否从d.ts/d.ets中引用值类型符号，禁止在声明文件中声明值变量。

     ![](./img/zh-cn_image_0000002571546990.png)
...
* 方法2：新增x.js对应的声明文件(.d.ts)，并在引用时不指定后缀。
  ...
  ![](./img/zh-cn_image_0000002602186601.png)
...
![](./img/zh-cn_image_0000002602186605.png)

<strong>处理措施</strong>

该问题是由于file1为当前工程外的代码：

![](./img/zh-cn_image_0000002571547062.png)
...
* <strong>场景一：</strong>
  ...
  ![](./img/zh-cn_image_0000002571387436.png)

  <strong>问题确认</strong>
  1. 进入上面报黄色的源码文件中，可以看到依赖有红色告警信息。

     ![](./img/zh-cn_image_0000002571546994.png "点击放大")
  2. 进入包下的oh-package.json5文件，查看依赖配置为devDependencies。

     ![](./img/zh-cn_image_0000002571387402.png)
...
* <strong>场景二：</strong>
  ...
  ![](./img/zh-cn_image_0000002571387334.png)
...
* <strong>场景三：</strong>
  ...
  ![](./img/zh-cn_image_0000002602186553.png)

  ![](./img/zh-cn_image_0000002571387390.png)![](./img/zh-cn_image_0000002571547024.png)<strong>处理措施</strong>
...
* <strong>场景五：</strong>
  ...
  ![](./img/zh-cn_image_0000002571546996.png "点击放大")
  ...
  + 将xxx依赖配置到yyy模块oh\_package.json5的dependencies中。
    ...
    ![](./img/zh-cn_image_0000002602186529.png "点击放大")
...
* <strong>场景六：</strong>
  ...
  请确认当前使用的DevEco Studio和SDK版本是配套的，点击菜单栏<strong>Help &gt; About DevEco Studio</strong>，<strong>Help &gt; About HarmonyOS SDK</strong>分别查看配套的DevEco Studio和SDK版本。

  ![](./img/zh-cn_image_0000002602186611.png)
* <strong>场景七：</strong>
  ...
  ![](./img/zh-cn_image_0000002602066487.png)
  ...
  ![](./img/zh-cn_image_0000002571546978.png)
...
![](./img/zh-cn_image_0000002602066437.png)
...
打开模块下的build-profile.json5文件，在<strong>arguments</strong>参数中添加如下配置。并根据电脑CPU配置，修改compile和link的值。建议compile和link的值之和，设置为CPU核数的一半，如CPU为8核，则compile和link分别设置为2。
...
<strong>示例 CMakeLists.txt</strong>
...
<strong>检查目标文件</strong>

```
nm myLibrary.o | grep myFunction
```

<strong>检查三方库文件</strong>
...
![](./img/zh-cn_image_0000002602066533.png)
...
在工程同步、编译构建过程中，提示<strong>LABEL\_VALUE\_ERROR</strong>错误信息。

![](./img/zh-cn_image_0000002602186609.png)

<strong>解决措施</strong>

该问题是由于config.json文件的资源引用规则变更导致，需要将“label”字段的取值，修改为资源引用方式。

1. 在<strong>resources &gt; base &gt; element</strong>中的string.json中添加对应的字符串信息。
2. 然后在config.json中重新引用该字符串资源。

   ![](./img/zh-cn_image_0000002571547066.png)
...
![](./img/zh-cn_image_0000002571387326.png)
...
1. 在模块下的<strong>resources &gt; base &gt; element</strong>下，点击右键选择<strong>New &gt; Element Resource File</strong>创建资源文件。

   ![](./img/zh-cn_image_0000002602066431.png)
2. 在弹出的对话框中，“File name”开发者可自定义，如color；“Root element”请选择<strong>color。</strong>

   ![](./img/zh-cn_image_0000002602066489.png)

   创建完成后，color.json文件如下图所示：

   ![](./img/zh-cn_image_0000002571387328.png)
3. 将[2](#li124901748185712)创建的color.json文件拷贝至模块的<strong>ohosTest &gt; resources &gt; base &gt; element</strong>目录下。

   ![](./img/zh-cn_image_0000002602066481.png)
4. 在模块的<strong>src &gt; main &gt; module.json5</strong>文件的abilities数组中，添加startWindowIcon和startWindowBackground字段（若缺少任一字段，将出现ERROR: Schema validate failed报错）。其中，startWindowIcon字段索引模块下<strong>resources &gt; base &gt; media</strong>中的图标资源，startWindowBackground字段索引<strong>resources &gt; base &gt; element</strong> <strong>&gt; color.json</strong>中的color。

   ![](./img/zh-cn_image_0000002571547068.png)
5. 在<strong>src &gt; ohosTest &gt; module.json5</strong>文件的abilities数组中，加startWindowIcon和startWindowBackground字段。其中，startWindowIcon字段索引模块ohosTest下<strong>resources &gt; base &gt; media</strong>中的图标资源， startWindowBackground字段索引<strong>resources &gt; base &gt; element</strong> <strong>&gt; color.json</strong>中的color。
...
* keyword：标识当前报错字段的可选配属性，当前报错中包括'required'、'oneOf'、'enum'、'propertyNames'。
  ...
  + oneOf：表示当前配置不符合oneOf要求。通过instancePath已经确认报错出现在abilities标签，在DevEco Studio中，按住Ctrl点击"abilities"跳转到对应的module.json文件，可以查看到必须配置以下两组中的一组。根据对比排查，可识别到因拼写错误导致"name"属性未配置。
    ...
    ![](./img/zh-cn_image_0000002602066523.png)
...
![](./img/zh-cn_image_0000002571546956.png)
...
<strong>密钥库密码</strong>和<strong>密钥密码</strong>是在创建p12文件时由开发者自行输入的，请牢记该密码。DevEco Studio工程的build-profile.json5文件中有记录密码的密文，但签名工具需要输入密码明文，不能直接将build-profile.json5中的值用到签名工具中。
...
![](./img/zh-cn_image_0000002602066467.png)

开发者可通过重新自动签名解决该问题：

1. 点击<strong>File &gt; Project Structure &gt; Project &gt; Signing Configs</strong>，打开签名配置页面。

2. 勾选“Automatically generate signature”，等待重新签名，然后点击<strong>OK</strong>即可。

![](./img/zh-cn_image_0000002571387384.png)
...
![](./img/zh-cn_image_0000002571387410.png)

<strong>解决措施</strong>

请删除报错模块下的.cxx文件夹，然后选中需要构建的模块，执行<strong>Make Module</strong> <strong>$`&#123;moduleName&#125;`</strong>完成单独构建，避免同时构建多个模块。

![](./img/zh-cn_image_0000002571387348.png)
...
4. 将定义的三方so声明给目标。
   ...
   ![](./img/zh-cn_image_0000002602066485.png)
...
点击\<strong\>Build &gt; Make Module $`&#123;libraryName&#125;`</strong>编译构建生成HAR后，发现构建产物中未出现cpp目录下的文件。

![](./img/zh-cn_image_0000002571547054.png)
...
![](./img/zh-cn_image_0000002602066479.png)
...
![](./img/zh-cn_image_0000002571387392.png)
...
![](./img/zh-cn_image_0000002602186581.png "点击放大")

出现编译报错“ninja: error: mkdir(xxx): No such file or directory”，示例如下：

![](./img/zh-cn_image_0000002571547032.png "点击放大")
...
* 方法二：根据object file实际路径长度计算CMAKE\_OBJECT\_PATH\_MAX大小。
  ...
  + 总路径长度 = object file directory长度 + object file长度，object file directory、object file如下图所示，两个长度之和为297字符，以实际为准
    ...
    ![](./img/zh-cn_image_0000002571387356.png "点击放大")
...
![](./img/zh-cn_image_0000002602186563.png "点击放大")

出现编译报错“(is the command line too long?)”，示例如下：

![](./img/zh-cn_image_0000002571387412.png)
...
* 方法二：根据object file实际路径长度计算CMAKE\_OBJECT\_PATH\_MAX大小。
  ...
  + 总路径长度 = object file directory长度 + object file长度，object file directory、object file如下图所示，两个长度之和为297字符，以实际为准
    ...
    ![](./img/zh-cn_image_0000002571387398.png "点击放大")
  + object file中目录部分长度：示例中“\_\_/\_\_/\_\_/\_\_/\_\_/third-party/rn/ReactCommon/react/renderer/textlayoutmanager”长度为74字符，以实际为准
  + cmake哈希值字符数：cmake将长路径转换为哈希值时哈希值的长度，固定为32
...
![](./img/zh-cn_image_0000002571387324.png "点击放大")
...
![](./img/zh-cn_image_0000002571387440.png "点击放大")
...
![](./img/zh-cn_image_0000002571547018.png)![](./img/zh-cn_image_0000002571546982.png)
...
![](./img/zh-cn_image_0000002571546968.png)
...
![](./img/zh-cn_image_0000002602066493.png)<strong>问题确认</strong>
...
![](./img/zh-cn_image_0000002571547064.png "点击放大")
...
![](./img/zh-cn_image_0000002571387382.png "点击放大")
...
![](./img/zh-cn_image_0000002602186525.png "点击放大")
...
![](./img/zh-cn_image_0000002571387420.png)
...
![](./img/zh-cn_image_0000002571387406.png)
...
![](./img/zh-cn_image_0000002571387380.png "点击放大")
...
![](./img/zh-cn_image_0000002602186515.png)
...
![](./img/zh-cn_image_0000002602186517.png)
...
![](./img/zh-cn_image_0000002571387414.png)

<strong>解决方案</strong>

如果文件中不包含export关键字，该文件将视作全局命名空间的一部分，相当于两个文件实质为同一个文件。请添加export关键字使其成为独立命名空间，或者将声明的内容添加到自定义的命名空间中。

#### 如何解决编译报错“ The inferred type of 'xxx' cannot be named without a reference to 'xxx'. This is likely not portable. A type annotation is necessary<strong>.</strong>”的问题
...
![](./img/zh-cn_image_0000002571387340.png "点击放大")
...
![](./img/zh-cn_image_0000002602186487.png)
...
* 方法一：点击编辑器窗口上方的Sync Now。
  ...
  ![](./img/zh-cn_image_0000002571547022.png)
* 方法二：点击工具栏<strong>File &gt; Sync and Refresh Project</strong>。

  ![](./img/zh-cn_image_0000002571547052.png)
* 方法三：如果方法1、2无法解决问题，可以手动删除工程目录下的.hvigor目录后重启执行Sync。

  ![](./img/zh-cn_image_0000002571387336.png)
