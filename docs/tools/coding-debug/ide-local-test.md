...
original_url: /docs/tools/coding-debug/ide-local-test
format: md
upstream_id: tools/coding-debug/ide-local-test
...
1. 在工程目录下打开待测试模块（支持HAP、HAR、HSP模块）下的ets文件，将光标置于代码中任意位置，单击<strong>右键 &gt; Show Context Actions</strong> <strong>&gt; Create Local Test</strong>或快捷键<strong>Alt+Enter（macOS为Option+Enter） &gt; Create Local Test</strong>创建测试类。

   ![](./img/zh-cn_image_0000002602186741.png)
2. 在弹出的Create Local Test窗口，输入或选择如下参数。
   * <strong>Testing library</strong>：测试类型，默认为DECC-ArkTSUnit。
...
|  |  |
| --- | --- |
| ![](./img/zh-cn_image_0000002602186735.png) | ![](./img/zh-cn_image_0000002571547208.png) |
| 目录级 | 文件级 |
| ![](./img/zh-cn_image_0000002571547196.png) | ![](./img/zh-cn_image_0000002602186753.png) |
| 套件级 | 方法级 |

以文件级别为例，在工程目录中，选中文件，单击<strong>右键 &gt; Run'测试文件名称'</strong>，执行测试。

![](./img/zh-cn_image_0000002602186737.png)

也可以通过如下方式，执行Local Test：

* 在工具栏主菜单单击<strong>Run &gt; Run'测试名称'</strong>。
* 在DevEco Studio的右上角，选择一项测试任务的配置，然后单击右侧的![](./img/zh-cn_image_0000002602066699.png)按钮，执行Local Test。

  ![](./img/zh-cn_image_0000002602066681.png)

执行完测试任务后，查看测试结果。

![](./img/zh-cn_image_0000002571547206.png)
...
* 方式一：在工程目录中，选中文件，单击<strong>右键 &gt; Run '测试文件名称' with Coverage</strong>，以覆盖率统计模式执行测试任务。

  ![](./img/zh-cn_image_0000002602186755.png)

* 方式二：在DevEco Studio的右上角，选择测试任务，然后单击右侧的![](./img/zh-cn_image_0000002571387572.png)按钮，执行测试。

  ![](./img/zh-cn_image_0000002571387582.png)
...
![](./img/zh-cn_image_0000002571387584.png)
...
2. 在<strong>Run/Debug Configurations</strong>界面，单击<strong>+</strong>按钮，在弹出的下拉菜单中，单击<strong>Local Test</strong>。

   ![](./img/zh-cn_image_0000002602066673.png)
3. 根据实际情况，配置Local Test的运行参数。 然后单击<strong>OK</strong>，完成配置。

   ![](./img/zh-cn_image_0000002571547204.png)
