...
1. 在升级之前，请务必备份好ohpm-repo私仓工具中的历史数据，避免因升级操作失误，导致数据丢失。备份的内容包括[`deploy_root`](./ide-upgrade-110_to_2xx#li10435216234)ohpm-repo部署根目录内数据，db元数据以及store三方包数据，详细可参考[数据备份](./ide-ohpm-repo-data-backup) 。

2. 如果您想要改变db元数据和store三方包的存储方式，可在正确升级后参考[数据迁移文档指导](./ide-ohpm-repo-data-migration)修改。
...
5. （可选项）新版本如果需要使用新的部署目录`new_deploy_root`，需要手动迁移数据。
   ...
   * 修改新版本ohpm-repo配置文件：打开新版本ohpm-repo的解压根目录，进入conf目录下，修改配置文件config.yaml中配置项deploy\_root为新的部署目录`new_deploy_root`。
