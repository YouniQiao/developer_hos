...
1. 解压ohpm-repo工具包。
   ...
   ![](./img/309de391.png)
2. 请将ohpm-repo工具包解压目录中bin目录的路径配置到[系统环境变量](./ide-ohpm-repo-faq#section24117279211)path中，执行如下查询命令:

   ```
   ohpm-repo -v
   ```

   终端输出版本号（如：2.0.0），则表示安装包解压无问题。如有报错，请参考[FAQ](./ide-ohpm-repo-faq#section82-执行命令-ohpm-repo-command报错-ohpm-repo-不存在或者-command-命令不存在)解决。

   ![](./img/947615d7.png)

   针对Linux和Mac系统，建议使用bash作为命令行界面。
3. 进入ohpm-repo解压目录的conf目录中，修改配置文件config.yaml：
   * 检查[listen](./ide-ohpm-repo-configuration#zh-cn_topic_0000001745376470_listen)配置，默认配置为localhost:8088 ，表示仅支持监听本机地址；如果希望其他机器通过ip/域名访问，则建议修改listen配置为ohpm-repo部署机器的ip地址：

     ```
     listen: <部署ohpm-repo机器的ip>:8088
     ```
   * 检查[deploy\_root](./ide-ohpm-repo-configuration#zh-cn_topic_0000001745376470_deploy_root)配置：如果选择不配置，会存储在默认地址中。禁止该路径配置为ohpm-repo解压根目录。
   * 数据存储db模块使用mysql：
   ...
   * 文件存储store模块使用sftp，sftp配置最多只能设置3个：
     ...
     1、ohpm-repo文件的存储路径为： ``<sftp服务器配置的存储根目录>`` +``<store配置的[path](./ide-ohpm-repo-configuration#zh-cn_topic_0000001745376470_li1275312401171146)路径>``，其中path只支持相对路径，必须以/开头。例如sftp服务器存储根目录为/user/sftp/data，store中[path](./ide-ohpm-repo-configuration#zh-cn_topic_0000001745376470_li1275312401171146)配置的路径为/source，故最终ohpm-repo文件存储路径为/user/sftp/data/source。

     2、多实例部署ohpm-repo时，必须配置反向代理服务器，转发客户端请求到部署的多个ohpm-repo实例服务器中，故[store.config.server](./ide-ohpm-repo-configuration#zh-cn_topic_0000001745376470_store)必须手动配置为反向代理服务器的域名/ip地址，且需要配置[use\_reverse\_proxy](./ide-ohpm-repo-configuration#section1074004784011)值为true。
...
![](./img/da6bbbb0.png "点击放大")

部署成功后可执行ohpm-repo start启动ohpm-repo。

![](./img/fb9bf1d1.png "点击放大")
...
在进行该配置前需要将ohpm-repo工具bin目录配置到[系统环境变量](./ide-ohpm-repo-faq#section24117279211)path中。
