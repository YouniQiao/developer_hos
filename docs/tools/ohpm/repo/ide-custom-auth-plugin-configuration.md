...
2. 进入ohpm-repo解压根目录，把模板文件中[tsconfig.json 文件](./ide-custom-auth-plugin-template#section14188258114612)移动到ohpm-repo解压根目录。
3. 建议将模板文件中[CustomAuth.ts](./ide-custom-auth-plugin-template#section14072085013)文件存入ohpm-repo解压根目录的plugins文件夹内。
...
2. 编译插件文件。
   ...
   * 命令成功执行后会在ohpm-repo解压根目录的plugins/outDir文件夹中生成编译后的文件CustomAuth.js。
     ...
     如果CustomAuth.ts没有存放在ohpm-repo解压根目录的plugins内，请先修改[tsconfig.json 文件](./ide-custom-auth-plugin-template#section14188258114612)中include和outDir参数，前者指定待编译插件代码的存储目录，后者指定编译完成后文件的输出位置，然后再在ohpm-repo解压根目录执行编译命令tsc。
