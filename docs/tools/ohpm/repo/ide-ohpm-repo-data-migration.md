...
title: "数据迁移"
format: md
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ide-ohpm-repo-data-migration---
...
1. 为保证数据不丢失，请务必先进行[数据备份](./ide-ohpm-repo-data-backup)。
...
请参考[数据备份指导文档](./ide-ohpm-repo-data-backup)进行操作。
...
1. 导出DB数据
   ...
   使用[export\_userinfo](./ide-ohpm-repo-export-userinfo)命令导出下面10张数据表的数据，并且导出加密组件，在命令执行目录生成打包export\_userInfo\_xxx.zip文件。
...
2. 导出已上架的包列表
   ...
   使用[export\_pkginfo](./ide-ohpm-repo-export-pkginfo)命令，导出已上架的包列表。
...
3. 批量下载包文件
   ...
   使用[batch\_download](./ide-ohpm-repo-batch-download)命令，从ohpm-repo配置的store存储目录中批量下载包文件。

   使用第2步生成的pkgInfo\_xxxx.json作为[batch\_download](./ide-ohpm-repo-batch-download)命令参数，批量下载har或tgz包，在命令执行目录生成batch\_download\_xxx.zip文件。
...
4. 导出包权限数据
   ...
   ohpm-repo 5.4.0版本开始，支持使用[export\_pkgPermission](./ide-ohpm-repo-export-pkgpermission)命令，导出包权限数据。
...
2. 导入DB数据。
   ...
   使用[import\_userinfo](./ide-ohpm-repo-import-userinfo)命令将export\_userInfo\_xxx.zip中的数据导入数据库。
   ...
   ![](./img/56eecd92.png "点击放大")
3. 批量上传包到ohpm-repo配置的store目录。

   使用[batch\_publish](./ide-ohpm-repo-batch-publish)命令，将批量下载生成的batch\_download\_xxx.zip中的包依次发布到ohpm-repo。
...
4. 导入包权限数据。
   ...
   ohpm-repo 5.4.0版本开始，支持使用[import\_pkgPermission](./ide-ohpm-repo-import-pkgpermission)命令，导入包权限数据。导入时有[三种导入模式](./ide-ohpm-repo-import-pkgpermission#section8679814134119)，请根据不同模式的处理规则选择适合的导入模式。
