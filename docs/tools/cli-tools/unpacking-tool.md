...
| 类名 | 接口原型 | 类型 | 接口详细描述 |
| --- | --- | --- | --- |
| UncompressEntrance | UncompressResult parseApp(String appPath, ParseAppMode parseMode, String hapName) | Java接口 | 接口功能：根据参数解析app包的pack.info信息。  输入参数：appPath app包路径，parseMode 解析模式枚举（ALL/HAP\_LIST/HAP\_INFO），hapName hap包名（parseMode为HAP\_INFO时需要配置）。  返回值：UncompressResult。 |
| UncompressEntrance | UncompressResult parseApp(InputStream input, ParseAppMode parseMode, String hapName) | Java接口 | 接口功能：根据参数解析app包的pack.info信息。  输入参数：input app文件流，parseMode 解析模式枚举（ALL/HAP\_LIST/HAP\_INFO），hapName hap包名（parseMode为HAP\_INFO时需要配置）。  返回值：UncompressResult。 |
...
| buildVersion | String | 标识App中的[buildVersion](./app-configuration-file.md#配置文件标签)信息。 | 从API version 23开始支持。 |
...
| isModuleAbcCompressed | boolean | 标识[modules.abc](./ide-compile-build.md)文件是否为压缩状态。true表示压缩状态，false表示非压缩状态。 | NA |
| requiredDeviceFeatures | Map&lt;String, List&lt;String&gt;&gt; | 标识当前Module运行所需要的特定的设备特性。详细请参见[requiredDeviceFeatures标签](./module-configuration-file.md#requireddevicefeatures标签) | 从API version 24开始支持。 |
...
| reason | String | 当申请的权限为user\_grant权限时此字段必填，描述申请权限的原因。 | NA |
| usedScene | UsedScene结构体（见下述UsedScene） | 描述权限使用的场景和时机。场景类型有：ability、调用时机（when），可配置多个ability。 | NA |
| reasons | HashMap&lt;String, String&gt; | 当申请的权限为user\_grant权限时此字段必填，描述申请权限的原因。 | NA |
