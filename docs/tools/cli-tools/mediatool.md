...
original_url: https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/mediatool
format: md
---
...
该命令需要root权限，能够将设备&lt;path-to-local-media-file&gt;路径下的图片视频文件推入媒体库中保存。支持保存图片、视频和音频文件。文件在媒体库中会保留原有的名字。&lt;path-to-local-media-file&gt;可以为文件夹，mediatool会将文件夹里的所有文件置入媒体库中。保存成功后会打印成功置入的资源的uri。
...
该命令需要root权限，能够将&lt;resource-uri&gt;指定uri对应的媒体库内资源信息以csv格式打印出来。
