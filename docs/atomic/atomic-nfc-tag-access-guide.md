---
title: "NFC标签读写开发指南"
original_url: /docs/dev/atomic-dev/atomic-nfc/atomic-nfc-tag-access-guide
format: md
upstream_id: dev/atomic-dev/atomic-nfc/atomic-nfc-tag-access-guide
...
NFC标签读写完整的API说明以及示例代码请参考：[NFC标签接口](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nfctag)。
  ...
  if (!error) {
    ...
    try {
      isoDep.transmit(cmdData).then((response : number[]) => {
        ...
        hilog.error(0x0000, 'testTag', 'readerModeCb isoDep.transmit errCode: ' + err.code + ', errMessage: ' + err.message);
        return;
      });
    } catch (busiError) {
      hilog.error(0x0000, 'testTag', 'readerModeCb isoDep.transmit errCode: ' + (busiError as BusinessError).code + ', errMessage: ' + (busiError as BusinessError).message);
      return;
    }
  } else {
...
export default class EntryAbility extends UIAbility {
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    ...
    try {
      ...
      hilog.error(0x0000, 'testTag', 'getTagInfo errCode: ' + (error as BusinessError).code + ', errMessage: ' + (error as BusinessError).message);
      return;
    }
    ...
    for (let i = 0; i < tagInfo.technology.length; i++) {
      if (tagInfo.technology[i] == tag.ISO_DEP) {
        try {
          ...
          hilog.error(0x0000, 'testTag', 'getIsoDep errCode: ' + (error as BusinessError).code + ', errMessage: ' + (error as BusinessError).message);
          return;
        }
      }
      // use other technology to access this nfc tag if necessary.
    ...
    try {
        ...
        hilog.error(0x0000, 'testTag', 'isoDep.connect errCode: ' + (error as BusinessError).code + ', errMessage: ' + (error as BusinessError).message);
        return;
    }
    if (!isoDep.isConnected()) {
    ...
    try {
      isoDep.transmit(cmdData).then((response : number[]) => {
        ...
        hilog.error(0x0000, 'testTag', 'isoDep.transmit errCode: ' + err.code + ', errMessage: ' + err.message);
        return;
      });
    } catch (busiError) {
      hilog.error(0x0000, 'testTag', 'isoDep.transmit errCode: ' + (busiError as BusinessError).code + ', errMessage: ' + (busiError as BusinessError).message);
      return;
    }
  }
}
