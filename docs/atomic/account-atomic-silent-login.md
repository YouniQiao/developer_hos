...
登录能力支持Phone、Tablet、PC/2in1设备，并且从5.1.1(19)版本开始，新增支持TV设备。
...
1. 调用登录API阶段（序号1-3）：
   ...
   2. 如华为账号已登录，且API调用成功，元服务能获取到Authorization Code等登录结果。注意：如华为账号未登录，元服务会获取到[1001502001 用户未登录华为账号](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-error-code#section1001502001-用户未登录华为账号)错误码，再根据需要自行处理。
