...
format: md
upstream_id: dev/atomic-dev/atomic-nfc/atomic-nfc-hce-guide
...
NFC卡模拟完整的API说明以及示例代码请参考：[NFC卡模拟接口](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cardemulation)。
...
async function hceCommandCb(error : BusinessError, hceCommand : number[]) {
  if (!error) {
    ...
    hceService.transmit(responseData).then(() => {
      ...
      hilog.error(0x0000, 'testTag', 'hceService transmit Promise errCode: ' + err.code + ', errMessage: ' + err.message);
    });
  } else {
    hilog.error(0x0000, 'testTag', 'hceCommandCb errCode: ' + error.code + ', errMessage: ' + error.message);
  }
}
  ...
  onForeground() {
    ...
    if (hceElementName != undefined) {
      try {
        ...
        hilog.error(0x0000, 'testTag', 'hceService.start errCode: ' + error.code + ', errMessage: ' + error.message);
      }
    }
  }
  ...
  onBackground() {
    ...
    if (hceElementName != undefined) {
      try {
        ...
        hilog.error(0x0000, 'testTag', 'hceService.stop errCode: ' + (error as BusinessError).code + ', errMessage: ' + (error as BusinessError).message);
      }
    }
  }
}
