...
（1）如果您的元服务内需要构建账号体系时，必须使用华为账号[静默登录](https://developer.huawei.com/consumer/cn/doc/atomic-guides/account-atomic-silent-login#场景介绍)能力。

（2）如果您需要将用户与已注册账号关联时，为用户同步历史数据资产，可以向用户申请[获取手机号](https://developer.huawei.com/consumer/cn/doc/atomic-guides/account-guide-atomic-get-phone-overview)。

（3）元服务内不允许出现显性登录页面（含提示语等），即不允许出现“登录/注册”等字样，更多的账号相关规范详见[账号最佳设计实践](https://developer.huawei.com/consumer/cn/doc/design-guides/accounts-0000001967444380)。
