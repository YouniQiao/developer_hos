...
1. 应用启动时延（APP_START_RESPONSE_TIME/APP_START_COMPLETE_TIME）数据采集说明如下：
...
1. 页面切换时延（PAGE_SWITCH_COMPLETE_TIME）数据采集说明如下：
...
1. 列表滑动帧率（LIST_SWIPE_FPS）数据采集说明如下：
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| metrics | Array | 否 | 否 | 被测性能指标列表。 |
| actionCode | Callback> | 否 | 否 | 测试代码段。 |
| resetCode | Callback> | 否 | 是 | 测试结束环境重置代码段。默认为空，框架运行时不执行此代码段。 |
...
| roundValues | Array | 是 | 否 | 被测性能指标的各轮测量数据值。当数据采集失败时返回-1。 |
