...
2. 调用本模块接口和常量时请使用[canIUse("SystemCapability.Communication.NFC.Tag")](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/init#caniuse)判断设备是否支持NFC能力，否则可能导致应用运行稳定性问题，参考[nfc-tag开发指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/nfc-tag-access-guide)。
...
![](./img/note_3.0-zh-cn.png) 从 API version 7 开始支持，从 API version 9 开始废弃，建议使用[tag.getNfcA](#taggetnfca9)替代。
...
![](./img/note_3.0-zh-cn.png) 从 API version 7 开始支持，从 API version 9 开始废弃，建议使用[tag.getNfcB](#taggetnfcb9)替代。
...
![](./img/note_3.0-zh-cn.png) 从 API version 7 开始支持，从 API version 9 开始废弃，建议使用[tag.getNfcF](#taggetnfcf9)替代。
...
![](./img/note_3.0-zh-cn.png) 从 API version 7 开始支持，从 API version 9 开始废弃，建议使用[tag.getNfcV](#taggetnfcv9)替代。
...
| elementName | [ElementName](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bundlemanager-elementname) | 是 | 所属应用读卡的页面信息（至少包含bundleName、abilityName这两项的赋值），不可以为空。 |
| callback | AsyncCallback | 否 | 前台读卡监听回调函数，返回读到的Tag信息。不填该参数则取消订阅该type对应的读卡回调。 |
...
| discTech | number[] | 是 | 前台应用指定的NFC读卡技术类型，至少指定一种读卡技术类型。每个number值表示所支持技术类型的常量值型，根据number值设置NFC读卡轮询的Tag技术类型（仅包含[NFC_A](#常量), [NFC_B](#常量), [NFC_F](#常量), [NFC_V](#常量)。 |
...
| elementName | [ElementName](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bundlemanager-elementname) | 是 | 所属应用读卡的页面信息（至少包含bundleName、abilityName这两项的赋值）。 |
| callback | Callback | 否 | 前台读卡监听回调函数，返回读到的Tag信息。不填该参数则取消订阅该type对应的读卡回调。 |
...
| **类型** | **说明** |
| --- | --- |
| [NdefMessage](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nfctech#ndefmessage9) | NDEF标签的Message，详见NDEF技术规范《NFCForum-TS-NDEF_1.0》。 |
...
| **参数名** | **类型** | **必填** | **说明** |
| --- | --- | --- | --- |
| ndefRecords | [NdefRecord](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nfctag#ndefrecord9)[] | 是 | NDEF标签的Record列表，详见NDEF技术规范《NFCForum-TS-NDEF_1.0》。 |
...
| supportedProfiles(deprecated) | number[] | 否 | 否 | 支持的技术类型。 **说明：** 从API version 7开始支持，从API version 9开始废弃，使用[tag.TagInfo#technology](#taginfo)替代。 |
...
| **名称** | **类型** | **值** | **说明** |
| --- | --- | --- | --- |
| NFC_A | number | 1 | NFC-A (ISO 14443-3A)技术。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| NFC_B | number | 2 | NFC-B (ISO 14443-3B)技术。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| ISO_DEP | number | 3 | ISO-DEP (ISO 14443-4)技术。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| NFC_F | number | 4 | NFC-F (JIS 6319-4)技术。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| NFC_V | number | 5 | NFC-V (ISO 15693)技术。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| NDEF | number | 6 | NDEF技术。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| NDEF_FORMATABLE9+ | number | 7 | 可以格式化的NDEF技术。 **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
