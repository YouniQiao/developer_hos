...
| 名称 | 描述 |
| --- | --- |
| bool [autoRedirect](#autoredirect) | 是否自动遵循HTTP重定向响应。true表示开启自动重定向，false表示不开启自动重定向。默认为true。 |
...
| bool [assumesHTTP3Capable](#assumeshttp3capable) | 是否假定目标服务器支持HTTP/3。true代表假定目标服务器支持HTTP/3，false代表不假定。默认值为false。 |
