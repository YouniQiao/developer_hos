---
title: "EmailInfo"
original_url: /docs/distribute/agc/agc-help-auth-api-0000002273777077/agc-help-auth-api-emailinfo-0000002238618028
format: md
upstream_id: distribute/agc/agc-help-auth-api-0000002273777077/agc-help-auth-api-emailinfo-0000002238618028
last_sync: 2026-06-07
sync_hash: d864cbb6
---
Email账号信息类，用于更新用户当前Email账号信息的操作。

#### Parameters

| Name | Type | Description |
| --- | --- | --- |
| email | string | Email账号。 |
| verifyCode | string | Email验证码。 |
| lang | string | 验证码文字语言，例如中文：“zh\_CN”。所有支持的语言类型请参考[认证服务验证码语言类型](/docs/distribute/agc/agc-help-appendix-0000002312305161/agc-help-auth-verifycode-langtype-0000002532176920)。 |
