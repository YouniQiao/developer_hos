---
title: "创建测试态服务"
original_url: https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-teststate-service-0000002382856313
---
...
2. 进入“HarmonyOS”页签，您可通过包名、应用名称、应用类型等信息进行筛选，然后在应用列表中点击您的应用/元服务名称。
   ...
   ![](../img/agc-help-create-teststate-service-0000002382856313_1.png)
3. 左侧菜单栏选择“近场服务 > 近场管理”，进入近场管理主界面。选择“服务管理”页签，点击“新建”。

   ![](../img/agc-help-create-teststate-service-0000002382856313_2.png "点击放大")

#### 配置服务基本信息

进入“新建服务”页面，在“基本信息”区域配置服务基本信息。

![](../img/agc-help-create-teststate-service-0000002382856313_3.png)

| 配置项 | 说明 | |
| --- | --- | --- |
| 名称 | 近场服务名称，长度为1~128个字符。 | |
| 状态 | 服务的状态。   * 草稿：点击“保存”后，服务状态变更为“草稿”。 * 已上线：测试态服务不需要人工审核，点击“提交”后，服务状态变更为“已上线”。 * 已下线：您自行点击“下线”或者由华为侧强制下线。 | |
| 消息渠道 | 向用户推送应用/元服务卡片的渠道。  当前测试态近场服务仅支持通过“小艺建议”渠道推送卡片，**请选择“小艺建议”**。 | |
| 开放范围 | 此配置项决定您创建的是测试态近场服务还是全网态近场服务，**请选择“测试”**。 | |
| 测试方式 | 当前仅支持“自有真机测试”方式，使用您指定的真实手机而非模拟器进行测试。  使用自有真机测试之前，须确保应用已有“正在测试”状态的[邀请测试](https://developer.huawei.com/consumer/cn/doc/app/agc-help-invite-test-0000002270829393)版本或在架状态的正式版本。 | |
| 选择应用程序 | 在应用程序包列表中选择已注册意图的应用版本。 | |
| 测试用户 | 选择参与测试的用户列表，用户列表的用户数之和不可超过50个。 | |
...
![](../img/agc-help-create-teststate-service-0000002382856313_4.png)

* POI列表中仅展示状态为“已激活”且使用场景为“测试”的POI。
* 被其他已上线全网态近场服务使用的POI，不展示在POI待选列表中。

![](../img/agc-help-create-teststate-service-0000002382856313_5.png)
...
![](../img/agc-help-create-teststate-service-0000002382856313_6.png)
...
* 模板卡片-图文版（适合信息量少，有1个跳转热区的卡片）
  ...
  ![](../img/agc-help-create-teststate-service-0000002382856313_7.png)
...
* 模板卡片-双按钮版（适合有2个跳转热区的卡片）
  ...
  ![](../img/agc-help-create-teststate-service-0000002382856313_8.png)

  | 配置项 | 说明 |
  | --- | --- |
  | 主题图 | 带内容的底图，建议不要太抢眼以免妨碍卡片阅读。传1张，尺寸444\*444px，格式为JPG/PNG，大小1MB以内。 |
  | 按钮图标 | 热区按钮的背景图。传2张，作为2个热区的按钮背景图。尺寸90\*90px，格式为JPG/PNG，大小100KB以内。 |
...
* 模板卡片-单按钮版（适合有1个跳转热区的卡片）
  ...
  ![](../img/agc-help-create-teststate-service-0000002382856313_9.png)

  | 配置项 | 说明 |
  | --- | --- |
  | 主题图 | 带内容的底图。传1-3张，尺寸444\*444px，格式JPG/PNG，大小1MB以内。顶部114px或底部126px范围内避免放置重要信息，建议将重要信息放在卡片的中间位置。传多张时以轮播效果进行展示。为保证更好的卡片展示效果，建议您至少传2张主题图。 |
...
* 预置卡片
  ...
  ![](../img/agc-help-create-teststate-service-0000002382856313_10.png)

  | 配置项 | 说明 |
  | --- | --- |
  | 卡片样式 | 当前“小艺建议”渠道仅支持展示2\*2，2\*4样式的卡片内容。  系统会在页面加载初始化时判断是否有满足条件的卡片样式，如不满足，页面将给出提示信息。 |
  | 意图名称 | 选择您已在应用程序包中定义的意图名称。  **若您切换了应用程序包，需重新选择意图名称****。** |
...
* 模板卡片-图文版
  ...
  ![](../img/agc-help-create-teststate-service-0000002382856313_11.png)
* 模板卡片-双按钮版

  ![](../img/agc-help-create-teststate-service-0000002382856313_12.png)
* 模板卡片-单按钮版

  ![](../img/agc-help-create-teststate-service-0000002382856313_13.png)
* 预置卡片

  ![](../img/agc-help-create-teststate-service-0000002382856313_14.png)

#### [h2]时段设置

默认选择“不启用”，全时段推送服务内容。如果您希望根据用户行为习惯，在指定的时间段内推送服务内容，可以选择“启用”前的单选框以开启定时推送功能。这样，在非指定时间段内，系统将不会向用户推送服务内容，从而避免打扰用户。

![](../img/agc-help-create-teststate-service-0000002382856313_15.png)
...
![](../img/agc-help-create-teststate-service-0000002382856313_16.png)

“开放范围”为“测试”的近场服务自更新时间起，若超过90天，将自动下线，且该服务关联的测试任务将被全部删除。

1. 服务配置完成后，点击页面顶部的“提交”。

   ![](../img/agc-help-create-teststate-service-0000002382856313_17.png)
2. 测试态服务不需要审核，服务状态为“已上线”，点击“前往测试”即可进入近场测试服务界面开始测试。

   ![](../img/agc-help-create-teststate-service-0000002382856313_18.png)
