...
original_url: https://developer.huawei.com/consumer/cn/doc/service/faq-0000002105172802
format: md
---
...
* AirTouch标签信息鉴权失败。
  ...
  请检查[管理台配置](https://developer.huawei.com/consumer/cn/doc/service/create-service-0000002105172798#ZH-CN_TOPIC_0000002105172798__li98981858184115)的标签是否已删除/失效/挂起。
* 网络较差，加载服务超时。

  请在网络状况良好的场景下重试。
* 跳转地址均失效。

  请检查[跳转地址](https://developer.huawei.com/consumer/cn/doc/service/create-service-0000002105172798#ZH-CN_TOPIC_0000002105172798__li17961112110137)是否配置正确，AirTouch会尝试依次跳转所有配置地址，直到有一个跳转成功则不再进行后面的尝试。如果所有地址均失效，则会跳转默认兜底链接，但是如果兜底链接配置格式如下，则不会进行跳转。
