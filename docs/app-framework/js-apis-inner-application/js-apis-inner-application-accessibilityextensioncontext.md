...
| customComponentType18+ | string | 否 | 是 | 自定义组件类型。与元素的[AccessibilityRoleType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-accessibility#accessibilityroletype18枚举说明)类型所对应。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| targetNames | Array | 是 | 设置关注应用的包名，服务接收关注应用的无障碍事件，默认接收所有应用的无障碍事件，取消关注应用则传空数组。 |
...
| --- | --- | --- | --- |
| targetNames | Array | 是 | 设置关注应用的包名，服务接收关注应用的无障碍事件，默认接收所有应用的无障碍事件，取消关注应用则传空数组。 |
| callback | AsyncCallback | 是 | 回调函数，如果设置关注的目标包名失败，则AsyncCallback中err有数据返回。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数，返回当前对应的焦点元素。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数，返回指定窗口的根节点元素。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数，返回指定屏幕的所有窗口。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 9开始支持，从API version 10开始废弃，建议使用[injectGestureSync](#injectgesturesyncdeprecated)替代。
...
| --- | --- | --- | --- |
| gesturePath | [GesturePath](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-accessibility-gesturepath#gesturepath) | 是 | 表示手势的路径信息。 |
| callback | AsyncCallback | 是 | 回调函数，表示注入手势执行结果的回调。 |
...
![](./img/note_3.0-zh-cn.png) 从API version 10开始支持，从API version 12开始废弃，系统不再开放相关能力。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数，返回节点元素的所有属性名称。 |
...
| --- | --- | --- | --- |
| attributeName | ElementAttributeKeys | 是 | 表示属性的名称。 |
| callback | AsyncCallback | 是 | 回调函数，返回根据节点属性名称获取的属性值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback> | 是 | 回调函数，返回节点元素支持的所有操作名称。 |
...
| --- | --- | --- | --- |
| actionName | string | 是 | 表示属性的名称，取值参考[Action](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-accessibility#action)。 |
| callback | AsyncCallback | 是 | 回调函数，表示执行指定操作的回调。 |
...
| type | string | 是 | 固定为'content',表示查找的类型为节点元素内容。 |
| condition | string | 是 | 表示查找的条件。 |
| callback | AsyncCallback> | 是 | 回调函数，返回满足指定查询关键字的所有节点元素。 |
...
| type | string | 是 | 固定为'focusType'，表示查询的类型为节点的焦点元素类型。 |
| condition | [FocusType](#focustype) | 是 | 表示查询焦点元素的类型。 |
| callback | AsyncCallback | 是 | 回调函数，返回满足指定查询焦点元素类型的节点元素。 |
...
| callback | AsyncCallback | 是 | 回调函数，返回满足指定查询下一焦点元素方向的节点元素。 |
