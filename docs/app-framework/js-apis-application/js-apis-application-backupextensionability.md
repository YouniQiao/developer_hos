...
| 类型 | 说明 |
| --- | --- |
| string | Promise | 返回应用执行自定义备份操作的信息，包含备份结果和报错信息，返回值为Json格式。 异步返回Promise对象。 同步返回string。 |
...
| 类型 | 说明 |
| --- | --- |
| string | Promise | 返回应用执行自定义恢复操作的信息，包含恢复结果和报错信息，返回值为Json格式。 异步返回Promise对象。 同步返回string。 |
