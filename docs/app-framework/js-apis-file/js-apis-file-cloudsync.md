...
synced_at: "2026-06-24T20:50:12.962985"
---
...
![](./img/note_3.0-zh-cn.png) 以下同步状态发生变更时，如果应用注册了同步过程事件监听，则通过回调通知应用。
...
| callback | Callback | 是 | 回调函数。同步过程事件。 |
...
| callback | Callback | 否 | 回调函数。同步过程事件， 默认值为null。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。异步启动端云同步。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。异步停止端云同步。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback | 是 | 回调函数。异步获取上次同步时间。 |
...
| callback | Callback | 是 | 回调函数。云文件下载过程事件。 |
...
| callback | Callback | 是 | 回调函数。云文件批量缓存过程事件。 |
...
| --- | --- | --- | --- |
| event | string | 是 | 取消订阅的事件类型，取值为'progress'（同步过程事件）。 |
| callback | Callback | 否 | 回调函数。云文件下载过程事件。若填写，将视为取消指定的回调函数；否则为取消当前订阅的所有回调函数。 |
...
| callback | Callback | 否 | 回调函数。云文件批量缓存过程事件。如果填写此参数，将取消指定的回调函数；否则，将取消当前订阅的相同事件类型的所有回调函数。 |
...
| --- | --- | --- | --- |
| uri | string | 是 | 待下载文件uri。 |
| callback | AsyncCallback | 是 | 回调函数。异步启动云文件下载。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uris | Array | 是 | URI列表，一次调用最多支持传入400个URI，超过报错22400004。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回启动的云文件批量缓存任务的ID。 |
...
| --- | --- | --- | --- |
| uri | string | 是 | 待下载文件uri。 |
| callback | AsyncCallback | 是 | 回调函数。异步停止云文件下载。 |
...
| callback | Callback | 是 | 回调函数，返回更改的数据。 |
...
| isDirectory | Array | 否 | 否 | 指示更改的URI是否为目录。true：是目录。false：非目录。 |
| uris | Array | 否 | 否 | 需要更改的URI列表。 |
...
| callback | Callback | 是 | 回调函数，返回下载进度。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回历史版本临时存储文件的URI。 |
...
#### [h2]replaceFileWithHistoryVersion20+
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回本地文件和云端文件的冲突标志，true表示冲突，false表示不冲突。 |
