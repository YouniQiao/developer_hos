...
![](./img/note_3.0-zh-cn.png) 不支持focusable、disabled属性。
...
| 名称 | 类型 | 默认值 | 必填 | 描述 |
| --- | --- | --- | --- | --- |
| margin | | 0 | 否 | 使用简写属性设置所有的外边距属性，该属性可以有1到4个值。 |
| margin-[left|top|right|bottom] | | 0 | 否 | 使用简写属性设置左、上、右、下外边距属性，类型length，单位px，默认值0。 |
| color | | #08000000 | 否 | 设置分割线颜色。 |
| stroke-width | | 1 | 否 | 设置分割线宽度。 |
...
| flex | number | - | 否 | 规定了分割线如何适应父组件中的可用空间，用来设置组件的flex-grow。 仅父容器为、、时生效。 |
| flex-grow | number | 0 | 否 | 设置分割线的伸展因子，指定父组件容器主轴方向上剩余空间（容器本身大小减去所有flex项加起来的大小）的分配系数，0为不伸展。 仅父容器为、、时生效。 |
| flex-shrink | number | 1 | 否 | 设置分割线的收缩因子，flex元素仅在默认宽度之和大于容器的时候才会发生收缩，0为不收缩。 仅父容器为、、时生效。 |
| flex-basis | | - | 否 | 设置分割线在主轴方向上的初始大小。 仅父容器为、、时生效。 |
