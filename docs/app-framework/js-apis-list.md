...
![](./img/caution_3.0-zh-cn.png) 在List中使用[index]的方式获取元素可能导致未定义结果，推荐使用get()方法。
...
| list | List | 否 | 当前调用replaceAllElements方法的实例对象，默认值为当前实例对象。 |
...
| value | T | 是 | 当前遍历到的元素。 |
| index | number | 否 | 当前遍历到的下标值，默认值为0。 |
| List | List | 否 | 当前调用forEach方法的实例对象，默认值为当前实例对象。 |
