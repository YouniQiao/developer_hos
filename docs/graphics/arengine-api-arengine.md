...
| ARENGINE_FEATURE_TYPE_SEMANTIC_DENSE | 4 | 高精几何重建特性。 |
| ARENGINE_FEATURE_TYPE_SEMANTIC | 5 | 平面及物体语义特性。 |
...
| **名称** | **值** | **说明** |
| --- | --- | --- |
| NORMAL | 0 | 正常模式。 当添加的图片数量超过默认的最多图片数量（50）时，AR Engine会报告错误，错误信息见[1009200011](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-ar-engine#section1009200011-添加的图片数量超过最大数量)。 |
...
![](./img/note_3.0-zh-cn.png) 这个操作是耗时的，建议在后台线程中运行。
...
| **类型** | **说明** |
| --- | --- |
| Promise | Promise对象，返回ArrayBuffer对象。 ArrayBuffer是一个缓冲区，这个缓冲区包含了增强图像数据库的序列化数据。 |
...
| **类型** | **说明** |
| --- | --- |
| Promise | Promise对象，返回ARAddAugmentedImageResult对象。 ARAddAugmentedImageResult是一个表示添加增强图像结果的数据。 |
...
| points | Array | 是 | 否 | 表示点云中所有点的坐标以及坐标对应的置信度数组。 |
...
| vertexData | Array | 是 | 否 | 当前立方体的顶点数据。 对应立方体的8个顶点。索引从立方体后表面开始，按逆时针方向排列。 |
...
![](./img/note_3.0-zh-cn.png) 由于此函数并没有释放锚点[ARAnchor](#aranchor)，开发者需要通过调用[ARAnchor.release](#aranchorrelease)来释放锚点。
...
| **类型** | **说明** |
| --- | --- |
| Promise | Promise对象，返回true表示设备的闪光灯已开启，返回false表示设备闪光灯未开启。 |
...
| **类型** | **说明** |
| --- | --- |
| Promise | Promise对象，返回true表示设备的闪光灯已关闭，返回false表示设备闪光灯未关闭。 |
...
![](./img/note_3.0-zh-cn.png) 这个函数不会释放锚点，开发者需要通过调用[ARAnchor.release](#aranchorrelease)方法来释放锚点。
...
| planes | Array | 是 | 否 | 图像组件的基本结构参数列表。 |
...
| **类型** | **说明** |
| --- | --- |
| Array | 返回一个高精几何重建对象数据的立方体数据信息的列表。 |
...
| **类型** | **说明** |
| --- | --- |
| Promise | Promise对象，返回ARAugmentedImageDatabase对象。 |
