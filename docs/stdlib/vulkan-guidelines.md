...
libnative_buffer.so
```
 ![](./img/note_3.0-zh-cn.png) 在程序中通过dlopen函数链接libvulkan.so动态库时不需要在CMake中增加依赖，否则会导致符号冲突。
...
2. **获取OHNativeWindow**。 OHNativeWindow需要从XComponent组件中获取，下面提供一份从XComponent组件中获取OHNativeWindow的代码示例，XComponent模块的具体使用方法请参考[XComponent模块的介绍文档](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/napi-xcomponent-guidelines)。 ets/pages/Index.ets中增加一个XComponent组件。 
```
XComponent({
    id: 'xcomponentId',
    type: 'surface',
...
3. 从XComponent组件中获取OHNativeWindow。 
```
