...
![](./img/7160dd3a.gif "点击放大")

**图2** 10000条数据量下ForEach和LazyForEach最佳实践滑动对比
![](./img/722ea104.gif "点击放大")
...
* 方式一，循环渲染
  ...
  通过[循环渲染（ForEach）](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-rendering-control/arkts-rendering-control-foreach)从数组中获取数据，并为每个数据项创建相应的组件，可减少代码复杂度。
...
* 方式二，数据懒加载
  ...
  通过[数据懒加载（LazyForEach）](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-rendering-control/arkts-rendering-control-lazyforeach)从提供的数据源中按需迭代数据，并在每次迭代过程中创建相应的组件。
...
详细的使用规则可以参考ForEach的[使用建议](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-rendering-control/arkts-rendering-control-foreach#使用建议)和LazyForEach的[使用限制](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-rendering-control/arkts-rendering-control-lazyforeach#使用限制)。
...
* 对比案例1：使用ForEach对List列表进行加载
  ...
  ```
<div class="source-link-wrapper"><a class="source-link" href="https://gitcode.com/harmonyos_samples/list-optimization/blob/master/entry/src/main/ets/pages/ForEachListPage.ets#L36-L101">ForEachListPage.ets</a></div>
* 对比案例2：使用LazyForEach对List列表进行加载
  ...
  ```
<div class="source-link-wrapper"><a class="source-link" href="https://gitcode.com/harmonyos_samples/list-optimization/blob/master/entry/src/main/ets/pages/LazyForEachListPage.ets#L38-L118">LazyForEachListPage.ets</a></div>
...
| **Lazy**ForEach对比指标 | 10条数据 | 100条数据 | 1000条数据 | 10000条数据 |
| --- | --- | --- | --- | --- |
...
```
<div class="source-link-wrapper"><a class="source-link" href="https://gitcode.com/HarmonyOS_Samples/list-optimization/blob/master/entry/src/main/ets/pages/LazyForEachListPage.ets#L39-L119">LazyForEachListPage.ets</a></div>
...
```
<div class="source-link-wrapper"><a class="source-link" href="https://gitcode.com/harmonyos_samples/list-optimization/blob/master/entry/src/main/ets/pages/prefetching.ets#L2-L74">prefetching.ets</a></div>

在应用列表界面，创建DataSourcePrefetching和[BasicPrefetcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-prefetcher#basicprefetcher)对象。BasicPrefetcher提供了一种智能数据预取算法，以根据屏幕上可见区域的实时变化和预取持续时间的变化来决定应预取哪些数据项。在List的onScrollIndex回调中，调用BasicPrefetcher的visibleAreaChanged()方法，传入List的可见区域起始坐标。在可见区域变化时，实时更新预取的范围。
...
```
<div class="source-link-wrapper"><a class="source-link" href="https://gitcode.com/harmonyos_samples/list-optimization/blob/master/entry/src/main/ets/pages/basicPre.ets#L2-L44">basicPre.ets</a></div>
...
| --- | --- | --- |
| ![](./img/966d82fc.gif "点击放大") | ![](./img/db4b0a18.gif "点击放大") | ![](./img/e241f4b0.gif "点击放大") |
...
```
<div class="source-link-wrapper"><a class="source-link" href="https://gitcode.com/HarmonyOS_Samples/list-optimization/blob/master/entry/src/main/ets/components/ReusableArticleCardView.ets#L18-L98">ReusableArticleCardView.ets</a></div>
...
```
<div class="source-link-wrapper"><a class="source-link" href="https://gitcode.com/harmonyos_samples/list-optimization/blob/master/entry/src/main/ets/pages/LazyForEachListPage.ets#L40-L120">LazyForEachListPage.ets</a></div>

![](./img/7267e937.png)

需要注意的是复用组件中有@Builder自定义构建函数时，状态变量推荐使用[按引用传递](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-ui-paradigm-basic-syntax/arkts-extend-components/arkts-builder#按引用传递参数)。@Builder装饰的函数默认按值传递，当传递的参数为状态变量时，状态变量的改变不会引起@Builder方法内的UI刷新。
