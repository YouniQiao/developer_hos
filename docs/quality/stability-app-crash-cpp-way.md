...
* use after free：指使用已经被释放的内存，比如函数返回局部变量的引用、指针释放后未置空并继续使用等。
  ...
  ```
<SourceLink name="CppCrashQuestionCheck1.cpp" url="https://gitcode.com/harmonyos_samples/BestPracticeSnippets/blob/master/CppCrash/entry/src/main/cpp/CppCrashQuestionCheck1.cpp#L26-L40" />
* 栈溢出：如递归调用，析构函数相互调用，特殊的栈（信号栈）中使用大块栈内存。
  ...
  ```
<SourceLink name="CppCrashQuestionCheck2.cpp" url="https://gitcode.com/harmonyos_samples/BestPracticeSnippets/blob/master/CppCrash/entry/src/main/cpp/CppCrashQuestionCheck2.cpp#L26-L46" />
