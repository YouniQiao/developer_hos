---
title: "前台不可见动效问题分析"
original_url: /docs/quality/frontend-invisible-animation-analysis
upstream_id: quality/frontend-invisible-animation-analysis
...
当开发者进行应用[功耗基础质量测试](/docs/quality/power-basic-quality-test)或提交应用上架审核时，若遇到应用存在不可见动效的问题，可参考以下步骤进行分析。
...
该故障类型优化建议，参见[不可见组件低功耗建议](/docs/quality/low-power-consumption-suggestions)。
...
该故障类型的优化建议，请参考[Buffer低功耗优化](/docs/quality/buffer-power-optimization)。

## 定位根因

获取ArkUI组件id后，开发者可充分利用DevEco Studio提供的ArkUI Inspector工具进行[布局分析](/docs/tools/coding-debug/ide-arkui-inspector)，定位更多组件信息。结合代码逻辑，与问题复现时的组件状态对比，重点关注以下信息：
