...
[《应用性能体验建议》](/docs/experience-suggestions/performance/performance-delay#section118706211961)指出，应用或元服务内点击操作响应时延应&lt;=100ms。为了保证操作响应及时，提供极致流畅体验，需要分析从手势抬手到渲染上屏这段时间内应用执行的耗时操作，并针对性地优化相关逻辑。

**图1** 点击响应起止点示意图
![](./img/0c7c85dd.png "点击放大")
...
![](./img/81bd8d03.png "点击放大")

如上图所示，分析点击响应时延问题通常需要以下步骤：

1. 性能体检：使用性能检测工具AppAnalyzer检测和诊断应用是否存在性能问题。
2. 确定响应时延：根据检测工具AppAnalyzer检测的结果，确定响应时延的耗时，判断是否符合[《时延体验建议》](/docs/experience-suggestions/performance/performance-delay)中的规范。
...
2. 点击方法名，可跳转定位至PageJumpSceneUseCase3.ets页面中的aboutToAppear()方法处，代码中执行了耗时方法。
   ...
   ```
3. 点击优化建议下的跳转链接[分析UI主线程高耗时函数](/docs/quality/zhenlv#section117831333645)，即可获取相应的优化建议。

### UI线程应用自定义组件创建耗时长

1. 获得检测结果后，点击详情报告中的“点击响应时延”，可以查看UI线程应用自定义组件创建耗时检测结果。

   ![](./img/7e49cb8b.png "点击放大")
2. 可点击源文件定位到创建耗时的UI组件，根据提供的可能故障原因，去对UI组件进行相应优化修改，减少该UI组件自身创建耗时。

   ![](./img/e40f6165.png "点击放大")
...
3. 分解时间段：点击响应的整体时延拆解后，主要分为输入阶段、应用阶段和渲染阶段。
   ...
   | 首帧响应时延 | 起点 | 终点 | 基线(ms) |
   | --- | --- | --- | --- |
   | 输入阶段 | mmi\_service对应的service report(type为up) | 应用DispatchTouchEvent的起点(type=1) | 8 |
...
相关原理及案例参考[长列表加载丢帧优化](/docs/quality/best-practices-long-list)。
...
具体的使用场景和实现方案参考[动态加载](/docs/dev/app-dev/application-framework/arkts/arkts-runtime/arkts-runtime-module/arkts-dynamic-import)。
...
* **延迟执行资源释放操作**
  ...
  将资源关闭和释放操作放在setTimeout函数中执行，使其延迟到系统相对空闲的时刻进行，可以避免在程序忙碌时段占用关键资源，提升整体性能及响应能力。具体的使用场景和实现方案参考[延迟执行资源释放操作](/docs/quality/application-latency-optimization-cases#section8783201923819)。

* **减小拖动识别距离**

  应用识别拖动手势事件时需要设置合理的拖动距离，设置不合理的拖动距离会导致滑动不跟手、响应时延慢等问题。针对此类问题可以通过设置distance大小来解决。具体的使用场景和实现方案参考[减小拖动识别距离](/docs/quality/application-latency-optimization-cases#section1116134115286)。
...
具体的使用场景和实现方案参考[转场动画场景案例](/docs/quality/application-latency-optimization-cases#section17886155414385)和[动画时延场景案例](/docs/quality/application-latency-optimization-cases#section36181326113013)。
