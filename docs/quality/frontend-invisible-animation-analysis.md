---
title: "前台不可见动效问题分析"
original_url: https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-frontend-invisible-animation-analysis
---
...
<MergedTable
  headers={['问题大类', '细分类别', '根因', '影响程度', '是否触发图层空跑']}
  rows={[
    ...
    [{ text: '应用进程下发的冗余绘制指令', rowSpan: 5 }, 'ArkUI组件存在绘制渲染任务', '1. 组件离开屏幕后，依然在绘制刷新。 2. 非必要节点属性变更导致执行布局等刷新任务。', '中，会对UI、RS进程产生影响，一些场景会被系统兜底，仅影响RS。', '可能。'],
    ...
    [{ text: '使用Vsync方法执行绘制任务或较重的业务逻辑。', rowSpan: 2 }, '不恰当使用系统提供的DisplaySync方法执行绘制任务。', '中，UI进程中有持续的DisplayVsync任务。', '可能。'],
...
![](./img/59bc7c09.png "点击放大")![](./img/d37a9ce3.png "点击放大")
