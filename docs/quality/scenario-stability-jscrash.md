...
1. 提取日志关键信息
   * 错误类型（如Unexpected xxx in JSON）

     Unexpected end Text in JSON：JSON字符串提前结束，通常是由于缺少&#125;、]、"或键值对不完整导致的。

     Unexpected number in JSON：Number格式不符合JSON规范，例如前导0（如0123）等非法写法。
