...
| LIFECYCLE\_TIMEOUT | Ability生命周期切换超时。 |
...
![](./img/4bd530ec.png "点击放大")![](./img/c2c41a44.png "点击放大")解决方案：
...
![](./img/2d441068.png)![](./img/ddeb4b2f.png)
...
![](./img/903b2275.png)**案例五：应用内存泄漏导致频繁GC**
...
![](./img/c4036ef4.png)为了进一步验证上述猜想，结合GC日志做进一步分析。从日志可以看出，应用分配的ArrayBuffer内存已经达到1.4G，且GC仍无法释放，再加上NativeBindingSize，总的内存大小已经接近3G。根据虚拟机的GC内存触发策略，当Native内存超过2G后，每次分配都会尝试去触发GC。

![](./img/f67afad0.png)解决方案：通过上面分析，该问题实际就由freeze问题转换成了内存泄漏问题。应用内存泄漏一般可以分为两种：一种是js内存泄漏，另一种是Native内存泄漏。对于js内存泄漏，可以同HeapSnapshot工具生成内存快照，辅助问题定位，参考文档[ArkTS内存泄漏分析：Snapshot分析](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-insight-session-snapshot)。对于Native内存泄漏，DevEco Profiler提供了基础的内存场景分析Allocation，参考文档[Native内存泄漏分析：Allocation分析](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-insight-session-allocations)。
...
![](./img/85547f82.png)High Priority Event的数量过多

![](./img/8df65717.png)Immediate Priority Event的数量过多。
...
![](./img/9fba1bd4.png)后续观察kernel stack，发现代码逻辑进入内核的代码，且等锁。
...
![](./img/34703326.png)进一步分析日志，发现此时整机CPU负载达到60%以上，而hiviewx为后台应用，且GC线程本身优先级较低，在高负载场景下，无法调度到CPU资源，导致执行超时。

![](./img/83bb6338.png "点击放大")
...
![](./img/197c4059.png)从3s和6s栈可以看出，栈顶都是在libark\_jsruntime.so但栈顶不同。由于无法提供trace，只能根据堆栈和日志分析。
...
![](./img/62dd161f.png)6S栈

![](./img/1a717b62.png)定位过程：
...
![](./img/c726d4e0.png "点击放大")![](./img/cf76b038.png)
...
![](./img/fe47d9cf.png)定位过程：

瞬时栈相关调用为vm加载abc接口。

通过trace信息看到主线程主要耗时在IO操作：

![](./img/b7c40f46.png)![](./img/bd2a4640.png)
