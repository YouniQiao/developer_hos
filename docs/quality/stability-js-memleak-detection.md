...
2. 设置Snapshot泳道。
   ...
   单击任务左上角的![](./img/b1e2a6b4.png)进行泳道的新增和删除，再次单击此按钮可关闭设置并生效。
3. 开始录制后可观察Memory泳道的内存使用情况，在需要定位的时刻单击任务左上角的![](./img/0358f200.png)启动一次快照。

   “ArkTS Snapshot”泳道的紫色区块表示一次快照完成。

   ![](./img/75939914.png)

   * 在任务录制过程中，单击分析窗口左上角的![](./img/06557ea4.png)可启动内存回收机制。
...
* 带![](./img/58bfcaf8.png)标识的对象，表示其为全局对象，可以通过全局window对象直接访问。
...
![](./img/1a45564c.png)在“Snapshot”的“Comparison”页签中，可进行两次快照的差异比较，比较内容包括新增数、删除数、个数增量、分配大小、释放大小、大小增量等等。通过不断对比，可快速分析和定位内存问题的具体位置。
...
![](./img/d88944c8.png)可以导入与heapsnapshot文件匹配的.jsleaklist文件，展示jsleakwatcher监控采集到的内存泄漏对象。
...
```
<div class="source-link-wrapper"><a class="source-link" href="https://gitcode.com/harmonyos_samples/BestPracticeSnippets/blob/bacbf85d70037d5aad5457a63ce3cb1e9bce283b/MemoryDetection/entry/src/main/ets/pages/MemoryLeakDetection.ets#L7-L21">MemoryLeakDetection.ets</a></div>
