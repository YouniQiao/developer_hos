---
title: "帧率问题分析"
original_url: /docs/quality/zhenlv
upstream_id: /docs/quality/zhenlv
...
《应用性能体检建议》中针对滑动帧率问题的指标有[应用或元服务内滑动操作响应快](/docs/experience-suggestions/performance/performance-delay#section1767913186810)、[应用或元服务应用内滑动过程流畅](/docs/experience-suggestions/performance/performance-frame-rate#section159268494256)，当前AppAnalyzer工具已支持（或等效支持）上述指标项。
...
![](./img/7c973158.png "点击放大")

**图4** Render Service卡顿导致丢帧的故障模型

![](./img/3a807b64.png "点击放大")
...
在进行规则体检或场景化体检之前，先要确保[DevEco Studio与真机设备已连接](/docs/tools/coding-debug/ide-run-device)，并根据[应用/元服务签名](/docs/tools/coding-debug/ide-signing)章节进行签名，再编译生成HAP或HSP。

1. 在DevEco Studio中启动AppAnalyzer工具，详细请参考：[AppAnalyzer](/docs/quality/performance-detection#section135451444171)。

   ![](./img/808a1b64.png "点击放大")
...
* 如果函数的单次调用耗时长（查看平均耗时），说明是函数本身耗时长，开发者自行优化函数，将函数放到子线程或者进行缓存，开发者可参考[其他主线程优化思路](/docs/quality/time-optimization-of-the-main-thread#section4365993361)。

  ![](./img/5fa50307.png "点击放大")
* 如果函数本身耗时不长，但是函数调用次数多（例如在高频回调里打日志等操作）

  ![](./img/0816d7e4.png "点击放大")

  确认函数是不是每次都要调用，能否通过一些全局或者缓存的方式降低调用次数，避免高频回调，开发者可参考[高频回调场景](/docs/quality/time-optimization-of-the-main-thread#section10112623611)。
* 如果方法名aboutToBeDeleted调用次数过多，如下图所示：

  ![](./img/7002b98a.png "点击放大")

  此时需要点击**总耗时**列的超链接，打开trace向前排查主线程是否有阻塞。如下图所示也是因为前面的主线程阻塞导致的，因为destroy只会发生在idle，如果主线程阻塞一直没有idle，就会积压；

  ![](./img/b3c8b628.png "点击放大")

  解决方案：可以采用并行化，或者缓存的方式来优化业务逻辑。开发者可参考[其他主线程优化思路](/docs/quality/time-optimization-of-the-main-thread#section4365993361)。

  设置子线程优先级的方法有两种，具体可参考：[Priority](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-taskpool#priority)和[QoS开发指导](/docs/dev/app-dev/system/qos-guidelines)。
...
![](./img/49405963.png "点击放大")

**问题3：图片纹理过大**

![](./img/385e1e7d.png "点击放大")
...
![](./img/d4fb816d.png "点击放大")

接下来通过Trace查看每一帧的具体耗时，点击![](./img/4e7d79a9.png "点击放大")图标跳转到卡顿帧应用侧Trace详情，如下图所示。开发者，可以点击泳道信息区的收藏按钮，将应用帧处理的泳道收藏置顶，防止上下文信息丢失。

![](./img/662b86c8.png "点击放大")
...
![](./img/2c69db28.png "点击放大")

其它函数的详细调用和耗时情况在此不再一一列举。

**选择优化方案**

选择优化方案需要一些经验的积累，开发者可以参考一些[性能优化](/docs/quality/performance-optimization)的最佳实践，来选择相应的优化方法。
...
![](./img/788dcec3.png "点击放大")

### 主线程中执行冗余和耗时操作

避免在主线程中执行冗余或耗时操作，这可能导致UI渲染阻塞，引起界面卡顿或掉帧，尤其在高频回调中。参考[主线程耗时操作优化指导](/docs/quality/time-optimization-of-the-main-thread)。
...
* 尽量减少布局的嵌套层数，[合理使用布局](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-improve-layout-performance)，使用[相对布局 (RelativeContainer)](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-layout-development/arkts-build-layout/arkts-layout-development-relative-layout)来减少层级。
* 使用[组件复用](/docs/quality/best-practices-long-list#section36781044162218)减少组件的重复创建与渲染。
...
* 使用LazyForEach加载长列表，具体优化方法可以参考[优化长列表加载慢丢帧问题](/docs/quality/best-practices-long-list)。
* 使用系统提供的动画接口，避免动画丢帧。
* 优化主线程中的耗时操作，具体可以参考[主线程耗时操作优化指导](/docs/quality/time-optimization-of-the-main-thread)。
