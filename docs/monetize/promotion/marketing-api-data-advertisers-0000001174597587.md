...
original_url: /docs/monetize/promotion/marketing-api-data-advertisers-0000001174597587
format: md
upstream_id: monetize/promotion/marketing-api-data-advertisers-0000001174597587
last_sync: 2026-06-07
sync_hash: e9decb2f
---
# 广告主数据

您通过本接口获取广告主维度的基本投放数据，包括花费、点击量、点击均价等。

![](./img/note_3.0-zh-cn_d2d34541302d.png) 

本接口不支持查询合约数据。

- <strong>请求地址</strong>：请根据您的就近区域进行选择

  <strong>亚非拉：</strong>https://ads-dra.cloud.huawei.com/openapi/v2/reports/advertiser/query

  <strong>俄罗斯 :</strong> https://ads-drru.cloud.huawei.ru/openapi/v2/reports/advertiser/query

  <strong>欧洲：</strong>https://ads-dre.cloud.huawei.com/openapi/v2/reports/advertiser/query
...
- <strong>请求参数：</strong>
  ...
  | time\_granularity | string | 否 | "STAT\_TIME\_GRANULARITY\_DAILY"天粒度，详见[时间粒度](/docs/monetize/promotion/marketing-api-appendix1-0000001174597591#section482444972216)。 |
  ...
  | is\_abroad | Boolean | 是 | 是否为非中国大陆账户，取值为：  true：非中国大陆账户  false：中国大陆账户。 |
  | flow\_resource | integer | 否 | 投放网络。 |
  | campaign\_type | integer | 否 | 计划类型：1：展示广告；2：关键词广告；3：推送广告；4：商品类广告。 |
  | index\_screen\_list | List &lt;Struct1&gt; | 否 | 指标筛选。 |
  ...
  index\_screen\_list (Struct1)参数
  ...
  | type | integer | 否 | 筛选指标类型，取值为：  大于等于，只需要赋值low\_value，type=1  小于等于，只需要赋值up\_value，type=2  介于，需要赋值low\_value up\_value，type=0。 |
  ...
  - <strong>响应参数</strong>
    ...
    | stat\_datetime | string | 是 | 数据起始时间  格式：YYYYMMDDHH  请求中的时间粒度，决定时间精度。 |
    ...
    | pay\_amount\_normalized | float | 否 | 归一化的付费金额。 |
    | venus\_form\_submit\_count | integer | 否 | 维纳斯表单提交量。 |
    | venus\_form\_submit\_cost | float | 否 | 维纳斯表单提交成本。 |
    ...
    | attribution\_income\_iaa | float | 否 | IAA |
    | roas | float | 否 | roas |
