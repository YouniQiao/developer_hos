...
original_url: /docs/monetize/promotion/bp-hm-callback-0000002513681587
format: md
upstream_id: monetize/promotion/bp-hm-callback-0000002513681587
...
1. 根据[创建API客户端](#section1040426172715)中获取的“客户端ID”和“密钥”调用[获取Token](/docs/monetize/promotion/bp-hm-token-0000002481641726)接口到华为[AppGallery Connect平台](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html#/)进行鉴权，鉴权通过后将获得用于访问AppGallery Connect API的Access Token。
2. 通过已获取的Access Token作为请求参数，调用[回传用户行为数据接口](/docs/monetize/promotion/bp-hm-backapi-0000002513681589)进行数据回传。
