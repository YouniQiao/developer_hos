...
2. 在鲸鸿动能广告平台创建资产并添加关联。
   ...
   2. 设置应用关联信息：
      ...
      - 如果您未选择智能跟踪，您需要通过转化跟踪平台将数据回传到鲸鸿动能广告平台，且需要[手动创建转化事件](#手动创建转化事件)并进行[应用测试](#应用测试)，如果没有测试按钮，您需要等待应用产生自然转化事件或者您手动触发转化事件，系统收到回传数据后，转化指标状态变为”已启用“，您才能创建oCPC任务。
...
4. 确认鲸鸿动能广告平台收到转化跟踪数据。
   ...
   | AppsFlyer | - 投放试投放任务，手动触发转化事件 - [手动创建转化事件](#手动创建转化事件)并[手动联调](#应用测试) |
   | Adjust | - 投放试投放任务，手动触发转化事件 - [手动创建转化事件](#手动创建转化事件)并[手动联调](#应用测试) |
   | Sizmek | / |
   | Kochava | - 投放正式广告 - 投放试投放任务，手动触发转化事件 - 等待应用产生自然转化事件或者手动触发转化事件 |
   | Tenjin | - 投放试投放任务，手动触发转化事件 - [手动创建转化事件](#手动创建转化事件)并[手动联调](#应用测试) |
   | Airbridge | - 投放试投放任务，手动触发转化事件 - [手动创建转化事件](#手动创建转化事件)并[手动联调](#应用测试) |
   | Branch | - 投放试投放任务，手动触发转化事件 - [手动创建转化事件](#手动创建转化事件)并[手动联调](#应用测试) |
...
2. 设置事件信息。
   ...
   - <strong>事件类别：</strong>指的是您可以跟踪的转化事件，详情可参考[事件概览](/docs/monetize/promotion/tracking-gaishu-0000001139892539#三资产事件概览)。
