...
format: md
upstream_id: monetize/monetization/agd_pro_sdk_andriod_sdk-video-flow-0000001397733821
...
1. 构造请求AdSlot。
   ...
   | darkMode | 否 | int | 深色模式开关。  注意：  如果Android版本低于10，则开启深色模式开关，也无法生效。因为Android 10以下版本不支持深色模式。  取值范围：  * 1：深色模式 * 0：浅色模式 * -1：默认不使用深色模式，即跟随系统颜色模式。 默认值：-1  具体darkMode对应的深色模式开关逻辑说明，请参见[深色模式开关参数说明](#ZH-CN_TOPIC_0000001397733821__zh-cn_topic_0000001338103450_zh-cn_topic_0000001328328856_zh-cn_topic_0000001356838857_zh-cn_topic_0000001356578689_zh-cn_topic_0000001354444445_p8787923403)。 |
   ...
   | mediaExtra | 否 | JSONObject | 个性化参数。  此字段属于透传字段。请参见[mediaExtra参数说明](#ZH-CN_TOPIC_0000001397733821__zh-cn_topic_0000001338103450_zh-cn_topic_0000001328328856_zh-cn_topic_0000001356838857_zh-cn_topic_0000001356578689_zh-cn_topic_0000001354444445_p1171111125435)。 |
   ...
   **透传字段mediaExtra参数具体说明如下表所示。**
   ...
   | personalize | `String(JSON)` | 媒体传递的个性化标识PersonalizeInfo对象的JSON字符串。  具体personalize参数说明，请参见[personalize参数说明](#ZH-CN_TOPIC_0000001397733821__zh-cn_topic_0000001338103450_zh-cn_topic_0000001328328856_zh-cn_topic_0000001356838857_zh-cn_topic_0000001356578689_zh-cn_topic_0000001354444445_p16235951184419)。 |
...
```
adsContext.loadVideoFeedAds(adSlot, new TemplateLoadListener() {
    @Override
...
1. 配置媒体交互回调事件监听器。
   ...
   ![](./img/agd_pro_sdk_andriod_sdk-video-flow-0000001397733821_0.png)

   交互回调事件、视频回调事件必须在调用**render**之前注册，否则会影响广告**view**获取和事件上报。
...
2. 配置视频回调事件监听器。
   ...
   ```
   // 以获取到的首条广告为例
   ads.get(0).setVideoAdListener(new VideoAdListener() {
       @Override
       ...
       @Override
       public void onVideoRenew() {
           // 视频重播回调

       }

       @Override
       public void onVideoProgress(long current, long duration) {
           // 视频播放进度回调，current指当前播放进度，duration指总进度
