...
- **承载协议：** HTTPS POST
- **接口方向：** 开发者服务器 -> IAP服务器
- **接口URL：** {rootUrl}/harmony/v1/application/user/orders/query 说明：rootUrl具体请参见[站点信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/iap-rest-common-statement#站点信息)。
...
| productIdList | 否 | List | 用于查询指定商品ID的历史购买记录，可以指定最多10个商品ID。 |
| subGroupIdList | 否 | List | 用于查询指定订阅组ID的历史购买记录，可以指定最多10个订阅组ID。 |
...
| jwsPurchaseOrderList | 否 | List | 已购订单相关状态信息的JWS格式数据的数组。 可参见[返回结果验签](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/iap-verifying-signature)处理，验签通过后解码获取相关订单状态信息，具体请参见[PurchaseOrderPayload](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/iap-server-data-model#purchaseorderpayload)说明。 |
