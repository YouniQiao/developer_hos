...
| 参数 | 是否必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| appIds | 否 | List | 待解绑游戏的APP ID列表： - 若是开发者级别绑定，则为绑定发起的APP。 - 若是游戏级别绑定，则为绑定涉及的APP。 |
...
4. 将待验签字符串使用游戏公钥按照RSA算法（SHA256WithRSA/PSS）进行签名，并与sign参数中的签名字符串进行一致性验证。 ![](./img/note_3.0-zh-cn.png) 游戏公钥获取请参见[获取游戏密钥](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/gameservice-key)。 Java示例代码如下： 
```
