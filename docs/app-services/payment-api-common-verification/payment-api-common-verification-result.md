...
- **承载协议：** HTTPS POST
- **接口方向：** 开发者服务器 -> 华为支付服务器
- **接口URL：** https://petalpay-developer.cloud.huawei.com.cn/api/v1/realname/verification/result
...
| 参数 | 是否必选 | 类型 | 说明 |
| --- | --- | --- | --- |
| verifyResultId | 是 | String | 实名信息验证ID。[应用端接口](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-realnameservice#startrealnameverification)返回的用户实名信息验证ID。 |
...
| verifyResult | 是 | Boolean | 实名验证结果。 |
| failedReason | 否 | String | 开发者预期可以感知的验证错误原因。 - ID_TYPE_NOT_SUPPORT：证件类型不支持（当前华为支付开放实名验证能力仅支持身份证） - RARE_WORD_NOT_SUPPORT：生僻字场景不支持 （如姓名中携带了生僻字） |
