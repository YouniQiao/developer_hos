...
获取用户点击事件需使用[voipCall.on](#voipcallonvoipcalluievent)在业务流程开始时提前订阅voipCallUiEvent事件，可于业务流程结束后使用[voipCall.off](#voipcalloffvoipcalluievent)结束订阅。
...
| voipCallUiEvent | [VoipCallUiEvent](#voipcalluievent) | 否 | 否 | 应用内通话事件。 |
| errorReason | [ErrorReason](#errorreason) | 否 | 否 | 应用内通话错误码。 |
...
| --- | --- | --- | --- | --- |
| callId | string | 否 | 否 | 应用内通话唯一ID。 |
| voipCallType | [VoipCallType](#voipcalltype) | 否 | 否 | 应用内通话类型。 |
| userName | string | 否 | 否 | 应用内通话用户昵称。 |
| userProfile | image.[PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap) | 否 | 否 | 用户头像图片，需导入图像处理模块，详情见[PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap)。 支持传入的最大图片大小为221x221px，推荐传入的图片大小为112x112px。通过 [PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap)的[getPixelBytesNumber](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap#getpixelbytesnumber7)接口获取到的图片大小要小于196608bit。 |
| abilityName | string | 否 | 否 | 接听后需加载的应用界面ability名称。 |
| voipCallState | [VoipCallState](#voipcallstate) | 否 | 否 | 应用内通话状态。 |
...
| callback | Callback | 是 | 回调函数，返回通话事件详细信息对象。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 通话事件类型，参数固定为“voipCallUiEvent”。 |
| callback | Callback | 否 | 回调函数，返回通话事件详细信息对象。若不填，则取消当前应用监听该事件的所有回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| voipCallAttribute | [VoipCallAttribute](#voipcallattribute) | 是 | 应用内通话详细信息如用户头像、用户昵称、通话唯一标识等，详情请参见[VoipCallAttribute](#voipcallattribute)。（应用上报去电时，VoipCallAttribute中的voipCallState属性必须为VOIP_CALL_STATE_DIALING。） |

返回值：

| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回错误码类型。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| voipCallAttribute | [VoipCallAttribute](#voipcallattribute) | 是 | 应用内通话详细信息如用户头像、用户昵称、通话唯一标识等，详情请参见[VoipCallAttribute](#voipcallattribute)。 |
...
| callAudioEvent | [CallAudioEvent](#callaudioevent) | 是 | VoIP开/关静音、扬声器事件，详情请参见[CallAudioEvent](#callaudioevent)。 |
...
| callState | [VoipCallState](#voipcallstate) | 是 | 通话状态，参考[VoipCallState](#voipcallstate)。 |
...
| callId | string | 是 | 呼叫ID。 |
| callState | [VoipCallState](#voipcallstate) | 是 | 通话状态，参考[VoipCallState](#voipcallstate)。 |
| callType | [VoipCallType](#voipcalltype) | 是 | 通话类型，参考[VoipCallType](#voipcalltype)。 |
...
| --- | --- | --- | --- |
| callId | string | 是 | 应用内通话唯一ID。 |
| voipCallFailureCause | [VoipCallFailureCause](#voipcallfailurecause) | 是 | 来电消息建立失败原因。 |
