...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| utd | string | 否 | 否 | 统一数据类型，参考[@ohos.data.uniformTypeDescriptor (标准化数据定义与描述)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-data-uniformtypedescriptor) 建议开发者传入精准的数据类型，有助于匹配到精确的目标应用，参见：[Share Kit体验规范](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/share-access-utd) 。 |
...
![](./img/note_3.0-zh-cn.png) 设备屏幕宽度较小时，不会以悬浮形态展示，而是以模态形式/弹窗形式显示。
...
| generatedImageUri | string | 否 | 否 | 生成的分享图uri路径 (应用文件URI)，用于写入绘图结果或预览。开发者需要在该路径下创建一个空的图片文件，支持的图片文件类型包括PNG、JPG。 应用文件URI，参见：[应用文件分享](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/share-app-file) **说明：** 沙箱路径可通过[fileUri.getUriFromPath](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-fileuri#fileurigeturifrompath)方法获取文件URI。 |
...
| readingShareStyle | [ReadingShareRecordStyle](#readingsharerecordstyle) | 否 | 是 | 阅读分享模版数据类型， 默认STYLE_HIGHLIGHT_CONTENT。 |
...
| coverImageUri | string | 否 | 是 | 封面图uri路径（应用文件URI）（如：书籍封面图路径）。readingShareStyle为STYLE_HIGHLIGHT_TITLE生效。 应用文件URI，参见：[应用文件分享](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/share-app-file)。 |
...
![](./img/note_3.0-zh-cn.png) 请勿随意修改返回值want数据中的参数，可能会导致未知的错误。
...
| 类型 | 说明 |
| --- | --- |
| Promise | Promise对象，返回分享目标应用的want信息。 |
