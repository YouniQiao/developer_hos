...
| 类型 | 说明 |
| --- | --- |
| [ParseResult](#parseresult) | ParseResult枚举类型。 |
...
| --- | --- | --- | --- |
| path | string | 是 | 文件路径。 |
| format | [ImageFormat](#imageformat) | 是 | 图片枚举类型。 |
...
| 类型 | 说明 |
| --- | --- |
| [Bookmark](#bookmark) | 成功创建的书签，并返回Bookmark信息。 |
...
| **参数名** | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| bookmark | [Bookmark](#bookmark) | 是 | 要移除的书签对象。 |
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| bookmark | [Bookmark](#bookmark) | 是 | 书签的信息。 |
| parent | [Bookmark](#bookmark) | 是 | 父类书签信息，可以传null。 |
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| info | [HeaderFooterInfo](#headerfooterinfo) | 是 | 页眉页脚的信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [HeaderFooterInfo](#headerfooterinfo) | 获取到的页眉页脚信息。 |
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| info | [WatermarkInfo](#watermarkinfo) | 是 | 水印的信息。 |
| startIndex | number | 是 | 起始页，必须大于等于0，0为起始页。 |
| endIndex | number | 是 | 结束页，小于总页数。 |
| oddPages | boolean | 是 | 奇数页是否添加，true表示是，false表示否。 |
| evenPages | boolean | 是 | 偶数页是否添加，true表示是，false表示否。 |
...
| 类型 | 说明 |
| --- | --- |
| [WatermarkInfo](#watermarkinfo) | 获取PDF文档水印信息。 |
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| info | [BackgroundInfo](#backgroundinfo) | 是 | 背景的信息。 |
| startIndex | number | 是 | 起始页，必须大于等于0，0为起始页。 |
| endIndex | number | 是 | 结束页，小于总页数。 |
| oddPages | boolean | 是 | 奇数页是否添加，true表示是，false表示否。 |
| evenPages | boolean | 是 | 偶数页是否添加，true表示是，false表示否。 |
...
| 类型 | 说明 |
| --- | --- |
| [BackgroundInfo](#backgroundinfo) | 获取PDF文档背景信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [image.PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap) | 合并后PDF页面的image.PixelMap。 |
...
| listener | [SearchKeyCallback](#searchkeycallback) | 是 | 每页搜索完毕后执行的回调函数 |
| options | [SearchOptions](#searchoptions) | 否 | 搜索设置项 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| results | [SearchResultData](#searchresultdata)[] | 是 | 搜索的结果 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [AnnotationType](#annotationtype) | 否 | 否 | 批注类型。 |
...
| 类型 | 说明 |
| --- | --- |
| [PdfAnnotationInfo](#pdfannotationinfo) | PDF页面批注。 |
...
| border | [PdfBorder](#pdfborder) | 否 | 是 | 线框类型。 |
| flag | [AnnotationFlag](#annotationflag) | 否 | 是 | 批注显示类型。 |
...
| action | [PdfAction](#pdfaction) | 否 | 是 | 页面链接跳转，PDF文档内跳转到相应页面和超链接跳转（如：网页地址），目前只支持获取链接，暂不支持添加或编辑链接。 **起始版本：** 5.1.0(18) |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| quadPoints | Array | 否 | 否 | 下划线区域的PdfPoint数组。 每个链接标注有4*n个点，每组4个点分别是： 第1个点：矩形左上角的点。 第2个点：矩形的右上点。 第3个点：矩形左下角的点。 第4个点：矩形的右下角点。 说明：传参和取值会有差异，底层对数据做了处理。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| quadPoints | Array | 否 | 否 | 删除线区域的PdfPoint数组。 每个链接标注有4*n个点，每组4个点分别是： 第1个点：矩形左上角的点。 第2个点：矩形的右上点。 第3个点：矩形左下角的点。 第4个点：矩形的右下角点。 说明：传参和取值会有差异，底层对数据做了处理。 |
...
| 类型 | 说明 |
| --- | --- |
| [PdfDocument](#pdfdocument) | PdfDocument对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| annotationInfo | [PdfAnnotationInfo](#pdfannotationinfo) | 是 | 文档批注类。 |

返回值：

| 类型 | 说明 |
| --- | --- |
| [PdfAnnotation](#pdfannotation) | 文档批注。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| annotation | [PdfAnnotation](#pdfannotation) | 是 | 文档批注。 |
| annotationInfo | [PdfAnnotationInfo](#pdfannotationinfo) | 是 | 文档批注类。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| boxtype | [BoxType](#boxtype) | 是 | 页边界。 |
| rect | [PdfRect](#pdfrect) | 是 | 矩形。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| rotation | [RotationAngle](#rotationangle) | 是 | 旋转角度枚举值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| matrix | [PdfMatrix](#pdfmatrix) | 是 | 坐标变换矩阵，用于在渲染时对页面内容做缩放、平移、旋转等。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| matrix | [PdfMatrix](#pdfmatrix) | 是 | 坐标变换矩阵，用于在渲染时对页面内容做 缩放、平移、旋转等。 |
...
![](./img/note_3.0-zh-cn.png) 系统版本请使用5.0.0.126(SP8)及其以上的版本，低于此版本该API不可用。
...
| style | [TextStyle](#textstyle) | 是 | 文本Style。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| object | [GraphicsObject](#graphicsobject) | 是 | 图形对象。 |
...
![](./img/note_3.0-zh-cn.png) 该接口用于从文本类型的页面中提取原始文本数据，返回值为包含转义字符（如\r\n等）的String类型字符串。
...
| fontInfo | [FontInfo](#fontinfo) | 否 | 否 | 字体信息 |
...
| charRects | Array | 否 | 否 | 获取字符的区域列表，每个区域用矩形的left、right、top、bottom描述位置。 |
| charUnicodes | Array | 否 | 否 | 获取字符的Unicode。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | [DestInfo](#destinfo) | 是 | 书签的跳转信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [BookmarkInfo](#bookmarkinfo) | 书签信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | [BookmarkInfo](#bookmarkinfo) | 是 | 书签信息。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [PdfActionType](#pdfactiontype) | 否 | 否 | 批注链接跳转类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| destInfo | [DestInfo](#destinfo) | 否 | 否 | 跳转信息。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| fitMode | [FitMode](#fitmode) | 否 | 否 | 跳转到目标时的页面适合模式。 |
| pageIndex | number | 否 | 否 | 页面索引，大于等于0，小于总页数，0为起始页。 |
| left | number | 否 | 是 | 视图矩形左边界距页面左边的距离，必须大于等于0，单位为Points（一英寸等于72Points），默认值：0。仅在FitMode为FIT_MODE_XYZ时生效。 |
| top | number | 否 | 是 | 视图矩形上边界距页面上边的距离，必须大于等于0，单位为Points（一英寸等于72Points），默认值：0。仅在FitMode为FIT_MODE_XYZ或FIT_MODE_VERTICAL时生效。 |
| right | number | 否 | 是 | 视图矩形右边界距页面右边的距离，必须大于等于0，单位为Points（一英寸等于72Points），默认值：0。仅在FitMode为FIT_MODE_RECT时生效。 |
| bottom | number | 否 | 是 | 视图矩形下边界距页面底边的距离，必须大于等于0，单位为Points（一英寸等于72Points），默认值：0。仅在FitMode为FIT_MODE_RECT时生效。 |
| zoom | number | 否 | 是 | 缩放比例，1表示100%，必须大于0，小于等于5，默认值：1。仅在FitMode为FIT_MODE_XYZ时生效。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| HIGHLIGHT_INVERT | 1 | 反转注释区域内的颜色。 |
| HIGHLIGHT_OUTLINE | 2 | 显示注释的边框轮廓。 |
| HIGHLIGHT_PUSH | 3 | 将注释外观覆盖在页面内容上。 |
| HIGHLIGHT_TOGGLE | 4 | 切换显示状态，仅用于表单控件。 |
...
| BOX_BLEED | 2 | 印刷出血区域，超出裁切线的部分，防止裁切后出现白边。 |
| BOX_TRIM | 3 | 修剪后完成页面的预期尺寸。 |
| BOX_ART | 4 | 页面核心内容区域，排除页眉、页脚等辅助信息。 |
...
| --- | --- | --- |
| OBJECT_TEXT | 1 | 文本对象。 |
| OBJECT_PATH | 2 | 路径对象，包含线条和形状。 |
| OBJECT_IMAGE | 3 | 图像对象。 |
| OBJECT_SHADING | 4 | 着色对象，包含渐变等填充效果。 |
| OBJECT_FORM | 5 | 表单对象，可复用的内容组。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| PDF_FONT_CID_FONT_CHARSET | 0x100000 | CID字符集，用于中日韩等多字节字符。 |
| PDF_FONT_ANSI_CHARSET | 0 | ANSI字符集，西欧字符。 |
| PDF_FONT_DEFAULT_CHARSET | 1 | 默认字符集。 |
| PDF_FONT_SYMBOL_CHARSET | 2 | 符号字符集。 |
| PDF_FONT_SHIFT_JIS_CHARSET | 128 | Shift JIS字符集，日文。 |
| PDF_FONT_HANGUL_CHARSET | 129 | 韩文字符集。 |
| PDF_FONT_GB2312_CHARSET | 134 | GB2312字符集，简体中文。 |
| PDF_FONT_CHINESE_BIG5_CHARSET | 136 | 繁体中文BIG5字符集。 |
| PDF_FONT_THAI_CHARSET | 222 | 泰文字符集。 |
| PDF_FONT_EAST_EUROPE_CHARSET | 238 | 东欧字符集。 |
| PDF_FONT_RUSSIAN_CHARSET | 204 | 俄文字符集，西里尔字母。 |
| PDF_FONT_GREEK_CHARSET | 161 | 希腊字符集。 |
| PDF_FONT_TURKISH_CHARSET | 162 | 土耳其字符集。 |
| PDF_FONT_VIETNAMESE_CHARSET | 163 | 越南文字符集。 |
| PDF_FONT_HEBREW_CHARSET | 177 | 希伯来文字符集。 |
| PDF_FONT_ARABIC_CHARSET | 178 | 阿拉伯文字符集。 |
| PDF_FONT_BALTIC_CHARSET | 186 | 波罗的海字符集。 |
