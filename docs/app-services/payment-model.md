---
title: "数据模型说明"
upstream_id: "harmonyos-references/payment-model"
catalog: "harmonyos-references"
synced_at: "2026-06-24T20:53:47.659550"
---
...
| referer | 否 | String | referer链接。跳转微信支付时，如传入了referer并且referer有效，则优先使用。 **说明：** 传递的referer链接有效性校验： - 域名不为空且必须是HTTPS协议。 - 只保留协议和域名（若传入https://xxx.com/serach?xxx=123&yyy=456，则只保留https://xx.come）。 - 最小长度1，最大长度2048。 |
...
| **参数** | **是否必选** | **参数类型** | **描述** |
| --- | --- | --- | --- |
| platCoupons | 否 | List | 平台券信息（可参见[查询用户可用券](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-api-common-promotion-service-inquiry)接口获取）。 |
...
| [sceneParams](#sceneparams) | 是 | Map | 场景扩展参数，用于匹配平台券的使用条件。部分参数可参考[设备信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-device-info) |
...
| **参数** | **是否必选** | **参数类型** | **描述** |
| --- | --- | --- | --- |
| useMethod | 是 | List | 核销方式。 - FASTAPP：应用/元服务 |
| useAppId | 否 | List | 核销方式为线上应用/元服务核销时必填。列表最大长度为8。每个元素最小长度为1，最大长度为32。 |
...
| couponCodes | 否 | List | 此批次下的券Code列表，用于创建批次时外部指定Code，或结果返回时返回券的Code。 |
...
| useMethod | 是 | List | 核销方式。 - FASTAPP：应用/元服务 |
| useAppId | 否 | List | 核销方式为应用/元服务核销时必填。最大支持配置8个。每个appId的最小长度为1，最大长度为32。 |
...
| batchNo | 是 | String | 批次号。 |
| couponCodes | 否 | List | 此批次下的券Code列表，用于创建批次时外部指定Code，或结果返回时返回券的Code。列表最大长度为10，每个元素最小长度为1，最大长度为32。 |
...
| irregularyTime | 否 | List | 无规律的有效时间段。 |
...
| **参数** | **是否必填** | **参数类型** | **描述** |
| --- | --- | --- | --- |
| weekDay | 否 | List | 可用星期数，0代表周日，当填写dayTime时，该字段必填。 |
| dayTime | 否 | List | 当天可用时间段，最多不超过2个，具体定义参考CurrentDayTime结构定义。 |
