---
title: "发布与接收频道消息"
original_url: /docs/dev/game-dev/games-gamemme-channelmessage-publish-csharp-mini-0000002359707050
format: md
upstream_id: dev/game-dev/games-gamemme-channelmessage-publish-csharp-mini-0000002359707050
...
1. 调用[GameMediaEngineForMiniGames.PublishRtmChannelMessage](https://developer.huawei.com/consumer/cn/doc/games-references/gamemediaengineforminigames-csharp-minigame-0000002358963772#section19903171145818)方法，向指定频道内的一个或多个订阅用户发布频道消息。
   ...
   * 不指定频道消息接收者时，即向所有订阅了该频道的用户发送消息。当指定频道内部分订阅用户时，则只有对应的订阅用户才能接收到消息。
   ...
   String clientMsgId = engine.PublishRtmChannelMessage(req); // 返回clientMsgId和频道消息绑定
   ```
2. 当发布频道消息时，您可进行相关回调处理。由于游戏多媒体SDK已对回调函数[OnPublishRtmChannelMessage](https://developer.huawei.com/consumer/cn/doc/games-references/igamemmeeventhandlerforminigames-csharp-minigame-0000002392643789#section14274195316409)进行了封装，只需注册OnPublishRtmChannelMessageEvent事件监听，并实现OnPublishRtmChannelMessageCallback委托函数即可。
...
当接收到频道消息时，您可进行相关回调处理。由于游戏多媒体SDK已对回调函数[OnReceiveRtmChannelMessage](https://developer.huawei.com/consumer/cn/doc/games-references/igamemmeeventhandlerforminigames-csharp-minigame-0000002392643789#section10815195494017)进行了封装，只需注册OnReceiveRtmChannelMessageEvent事件监听，并实现OnReceiveRtmChannelMessageCallback委托函数即可。
