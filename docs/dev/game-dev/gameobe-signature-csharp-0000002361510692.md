...
客户端在初始化SDK时，需要实现一个GetSignature签名函数，从服务端获取签名信息然后回调给SDK。示例代码如下:
...
{
      Signature signature = HTTPUtil<Signature>.Post("https://example.com/sign");
      return signature;
}
```
