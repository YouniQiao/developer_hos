...
3. 打开 inspector 端口，连接 devtools 用于调试，其流程如下：在执行JS代码之前，调用OH\_JSVM\_OpenInspector在指定的主机和端口上激活inspector，创建socket。例如OH\_JSVM\_OpenInspectorWithName(env, 123, "test")，创建 tcp socket 及其对应的 unix domain 端口。
4. 调用OH\_JSVM\_WaitForDebugger，等待建立socket连接。
