...
HarmonyOS系统采用ELF文件格式作为全系统的二进制文件格式，具体格式详情，请参考《[System V Application Binary Interface](https://refspecs.linuxfoundation.org/elf/gabi4%2B/contents.html)》。CPU架构相关的格式定义，参考下面对应架构说明。
...
当前通过[DevEco Studio构建NDK工程](/docs/dev/ndk-dev/build-with-ndk-ide)时，不支持armeabi-v7a编译环境。如需要在HarmonyOS中使用该编译环境，需要通过[CMake方式构建](/docs/dev/ndk-dev/build-with-ndk-cmake)。
...
通过SDK CAPI开发native代码的时候，在build/cmake/ohos.toolchain.cmake中定义了HarmonyOS系统一些交叉编译常用的环境变量设置。其中OHOS\_ARCH变量定义了当前目标编译的ABI，可以设置下面三个ABI中的一种，arm64-v8a，armeabi-v7a，x86\_64。
