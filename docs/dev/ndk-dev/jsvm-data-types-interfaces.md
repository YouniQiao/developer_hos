...
JSVM 提供了多种配置选项，允许开发者在执行 OH\_JSVM\_Init 时灵活配置其行为。可以通过 OH\_JSVM\_GetVMInfo 接口获取当前 JSVM 版本所对应的 V8 引擎版本。JSVM 中可支持的选项范围与对应的 V8 引擎版本可支持的选项范围保持一致。OH\_JSVM\_GetVMInfo 接口的使用参考[使用JSVM-API接口获取JSVM API的版本号](/docs/dev/ndk-dev/use-jsvm-about-version)。
...
OH\_JSVM\_CreateCodeCache接口用法可参考[使用code cache加速编译](/docs/dev/ndk-dev/use-jsvm-about-code-cache)。
...
详见[使用 JSVM-API WebAssembly 接口](/docs/dev/ndk-dev/use-jsvm-about-wasm)。
...
具体示例参考[使用JSVM-API接口进行class相关开发](/docs/dev/ndk-dev/use-jsvm-about-class)。
...
[创建和使用虚拟机的启动快照。](/docs/dev/ndk-dev/use-jsvm-create-snapshot)
...
| 接口 | 功能说明 |
| --- | --- |
| OH\_JSVM\_BackgroundDeserialize | 在线程池中反序列化 JSVM\_CodeCache，通过 OH\_JSVM\_ReleaseDeserializeResult 接口释放 JSVM\_DeserializeResult。 |
| OH\_JSVM\_ReleaseDeserializeResult | 当 JSVM\_DeserializeResult 不再被使用时进行释放。 |
