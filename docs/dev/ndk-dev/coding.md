---
title: "代码开发"
original_url: /docs/dev/ndk-dev/coding
format: md
upstream_id: dev/ndk-dev/coding
last_sync: 2026-06-07
sync_hash: 12cf429a
upstream_status: upstream_deleted_pending
---
* **[代码开发概述](/docs/dev/ndk-dev/develop-code-overview)**
* **[C/C++标准库](/docs/dev/ndk-dev/c-cpp)**
* **[使用Node-API实现ArkTS/JS与C/C++语言交互](/docs/dev/ndk-dev/using-napi-interaction-with-cpp)**
* **[使用JSVM-API实现JS与C/C++语言交互](/docs/dev/ndk-dev/jsvm)**
* **[Longque-JS-API](/docs/dev/ndk-dev/longque)**
* **[OpenMP支持](/docs/dev/ndk-dev/openmp)**
* **[资源管理](/docs/dev/ndk-dev/resource-management)**
* **[包管理](/docs/dev/ndk-dev/bundle-management)**
