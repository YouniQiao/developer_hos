...
权限要求：WebAssembly相关接口需要应用拥有JIT权限才能执行，可参考[JSVM 申请JIT权限指导](/docs/dev/ndk-dev/jsvm-apply-jit-profile)申请对应权限。
...
参考 [使用JSVM-API实现JS与C/C++语言交互开发流程](/docs/dev/ndk-dev/use-jsvm-process) 了解 JSVM-API 接口开发流程。本文仅展示接口对应的 C++ 代码。
