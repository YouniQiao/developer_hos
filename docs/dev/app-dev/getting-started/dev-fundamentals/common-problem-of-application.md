---
title: "应用程序包常见问题"
original_url: /docs/dev/app-dev/getting-started/dev-fundamentals/common-problem-of-application
upstream_id: dev/app-dev/getting-started/dev-fundamentals/common-problem-of-application
...
* 通过[bm工具](/docs/dev/app-dev/system/bm-tool)获取指纹信息，使用哈希算法SHA-256生成。
...
* 通过.cer证书文件获取，可以参考[APP备案FAQ](/docs/distribute/app-dist/app-market/x50000/x50130)中HarmonyOS应用/元服务如何获取公钥和签名信息，指纹信息使用哈希算法SHA-1生成。
...
appIdentifier是[Profile文件](/docs/distribute/agc/agc-help-profile-0000002270709473/agc-help-release-profile-0000002248341090)中的一个字段，为应用的唯一标识，在应用签名时生成，其中：

1. 通过DevEco Studio工具[自动签名](/docs/tools/coding-debug/ide-signing#section18815157237)生成，此时的appIdentifier字段是随机生成的，在不同的设备上签名、或者重新签名均会导致appIdentifier字段不一致。
2. 采用手动签名，并通过AppGallery Connect平台申请证书，此时申请[调试Profile](/docs/distribute/agc/agc-help-profile-0000002270709473/agc-help-debug-profile-0000002248181278)或者[发布Profile](/docs/distribute/agc/agc-help-profile-0000002270709473/agc-help-release-profile-0000002248341090)中的appIdentifier字段是固定的，该字段来源于AppGallery Connect创建应用时生成的[APP ID](/docs/distribute/agc/agc-help-app-0000002235710234/agc-help-create-app-0000002247955506#section16423184171915)，由云端统一分配。此时的appIdentifier字段在应用全生命周期中不会发生变化，包括版本升级、证书变更、开发者公私钥变更、应用转移等。

因此，在跨设备调试、跨应用交互调试、或者多用户共同开发且需要共享密钥等要求appIdentifier不变的场景下，推荐使用手动签名，具体场景请参考[使用场景说明](/docs/tools/coding-debug/ide-signing#section54361623194519)。
...
* 通过[bm工具](/docs/dev/app-dev/system/bm-tool)获取。
...
uid是系统中用于[应用沙箱](/docs/dev/app-dev/system/system-security/access-control/access-token-overview#应用沙箱)隔离的唯一标识符，它分配给每个应用进程，确保应用在运行时相互隔离（如文件系统，内存空间等）。
