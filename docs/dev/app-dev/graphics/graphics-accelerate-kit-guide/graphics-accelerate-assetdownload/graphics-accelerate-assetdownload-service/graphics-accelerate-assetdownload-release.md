...
* 游戏资源包支持使用三方CDN或托管至华为CDN：
  ...
  + 若使用华为CDN，将在AppGallery Connect上传资源包。华为CDN对资源包有如下限制：

    - 文件大小：

      * 一个开发者下，所有游戏的所有资源包后台下载任务中，可以上传至华为CDN的资源包总大小不超过300GB。
      * 单个游戏的所有资源包后台下载任务中，可以上传至华为CDN的资源包总大小不超过200GB。
    - 保存时长：上传至华为CDN的资源包最多保留半年。
...
1. 请选择“下载类型”。建议根据应用自身是否内置下载器进行选择。
   ...
   * 应用未内置下载器，建议选择“extension系统托管下载”，表示将由华为提供的系统下载器从华为CDN下载资源包。
   * 应用已内置下载器，建议选择“extension协同下载”，表示将由应用自身下载器从华为CDN下载资源包。
2. “CDN”选择“使用华为CDN”。
...
6. 上传资源包。
   ...
   * 单个游戏的所有资源包后台下载任务中，可以上传至华为CDN的资源包总大小不超过200GB。
   * 上传资源包时，若提示空间已满，建议开发者优先删除草稿任务中的资源文件。
...
8. 提交资源包下载任务。
   ...
   若列表中有“预上线”、“审核中”中的任一状态任务，则无法在其他任务详情页点击“提交申请”。
...
| 2 | 预上线 | 提交申请后的状态，任务可以在这个状态下进行资源包下载测试。预上线状态的任务支持“发布”、“编辑”和“终止”。 |
