...
1. 源端保存迁移数据
   ...
   在源端UIAbility的[onContinue()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiability#oncontinue)接口中创建分布式数据对象并保存数据，执行流程如下：
   ...
   export default class MigrationAbility extends UIAbility {
     ...
     async onContinue(wantParam: Record<string, Object>): Promise<AbilityConstant.OnContinueResult> {
       ...
       try {
         ...
         hilog.error(DOMAIN_NUMBER, TAG, 'Failed to save. Error: ', JSON.stringify(error) ?? '');
       }
