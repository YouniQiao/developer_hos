...
4. 填写[建立域名与应用关联关系](#建立域名与应用关联关系)的网址域名，例如：`https://www.example.com`。`。必须输入精确的域名，不可输入包含特殊字符的模糊网址。
   ...
   不可以在域名后面添加/，即不支持`https://www.example.com/`形`形式。
...
例如，声明应用关联的域名是`www.example.com`，则需进行如下配置。
...
开发者可以通过配置applinking.json以关联多个应用。如果每个应用的module.json5的uris字段配置的都是一样的，那么系统将弹出列表框供用户选择要拉起的目标应用。 为了更好的体验，开发者也可以通过链接的path去区分拉起的目标应用，如链接`https://www.example.com/path1`拉`拉起目标应用1，链接`https://www.example.com/path2`拉`拉起目标应用2。
...
例如：开发者的服务器域名为`www.example.com`，则必须将applinking.json文件放在如下位置：
