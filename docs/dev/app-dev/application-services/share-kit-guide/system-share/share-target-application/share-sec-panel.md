...
format: md
upstream_id: dev/app-dev/application-services/share-kit-guide/system-share/share-target-application/share-sec-panel
...
1. 导入相关模块。
   ...
   ```
2. 目标应用可以使用[ShareExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-shareextensionability)为基类构建分享能力Ability。在Ability被启动后，可以在其[onSessionCreate](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiextensionability#onsessioncreate)回调中获取传入的want参数，将want参数通过[getSharedData](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/share-system-share#getshareddata)解析后得到分享数据。
