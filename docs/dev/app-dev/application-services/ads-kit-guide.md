---
displayed_sidebar: appDevSidebar
title: "Ads Kit（广告服务）"
original_url: /docs/dev/app-dev/application-services/ads-kit-guide
format: md
---


* **[Ads Kit简介](/docs/dev/app-dev/application-services/ads-kit-guide/ads-introduction)**
* **[Ads Kit术语](/docs/dev/app-dev/application-services/ads-kit-guide/ads-kit-glossary)**
* **[开发准备](/docs/dev/app-dev/application-services/development-preparation)**
* **[流量变现服务开发](/docs/dev/app-dev/application-services/ads-kit-guide/ads-publisher-service-dev)**
* **[开放匿名设备标识服务](/docs/dev/app-dev/application-services/ads-kit-guide/oaid-service)**
* **[鲸鸿动能Ads Kit个人数据处理说明](/docs/dev/app-dev/application-services/ads-kit-guide/description-of-personal-data)**
