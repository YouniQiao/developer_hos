...
2. （可选）可根据需要添加环境变量，支持**表单格式**和**JSON格式**两种编辑方式。添加完成后，还可以点击“JSON格式导出”，导出以“函数名称.json”格式命名的环境变量文件，以备后续使用。
   ...
   * 表单格式编辑
     ...
     点击“新增变量”，输入key和value值，如下图中所示，env1为环境变量的key值，test为value值。点击![](./img/269f8be3.png)可将变量删除。
...
| 组成 | 说明 |
| --- | --- |
| 目录树 | 目录树支持如下能力：  - 新增文件夹：选中一个文件或文件夹，点击右上角新增文件夹按钮“![](./img/836dd5ca.png)”。若选中的是文件夹，则新增一个子文件夹。若选中的是文件，则新增一个同级文件夹。  - 新增文件：选中一个文件或文件夹，点击右上角新增文件按钮“![](./img/6e90c337.png)”。若选中的是文件夹，则新增一个子文件。若选中的是文件，则新增一个同级文件。  - 删除文件：选中一个文件或文件夹，点击右侧删除按钮“![](./img/818cfce8.png)”，删除文件或文件夹。不允许删除根目录。  - 重命名：双击文件或文件夹，输入新名称（仅支持字母、数字、下划线和中划线），完毕后按Enter键完成重命名。 |
...
| 最大化 | 点击最大化按钮![](./img/2b8b0cfe.png)，可以最大化在线编辑区，再次点击![](./img/10fc1636.png)按钮或按ESC键退出最大化。 |
