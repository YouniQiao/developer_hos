...
| platform | 输入 | 传入硬件版本以及AI Core中各个硬件单元提供的内存大小。PlatformInfo构造时通过[PlatformAscendC类简介](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-platformascendc-introduction)获取。  PlatformInfo结构定义参数如下，socVersion通过[GetSocVersion](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-getsocversion)获取并透传，各类硬件存储空间大小通过[GetCoreMemSize](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-getcorememsize)获取并透传。  - platform\_ascendc::SocVersion socVersion;  - uint64\_t l1Size = 0;  - uint64\_t l0CSize = 0;  - uint64\_t ubSize = 0;  - uint64\_t l0ASize = 0;  - uint64\_t l0BSize = 0;  不推荐通过直接填值构造PlatformInfo的方式调用构造函数，例如PlatformInfo(, 1024, 1024, ..); |
