...
original_url: /docs/dev/app-dev/ai/neural-network-runtime-kit/neural-network-runtime-kit-introduction
format: md
upstream_id: dev/app-dev/ai/neural-network-runtime-kit/neural-network-runtime-kit-introduction
...
1. **在线构图**：AI推理框架需要调用NNRt的构图接口将推理框架的模型图转换为NNRt内部模型图。而系统内置的MindSpore Lite推理框架（具体可参考[MindSpore Lite Kit](/docs/dev/app-dev/ai/mindspore-lite-kit/deployment/mindspore-lite-guidelines)）通过MindIR模型图对接NNRt。由于MindIR模型图和NNRt内部模型图格式兼容，因此MindSpore Lite无需调用NNRt的构图接口即可对接NNRt。
...
* [MindSpore Lite Kit](/docs/dev/app-dev/ai/mindspore-lite-kit/mindspore-lite-kit-introduction)：HarmonyOS内置的轻量化AI引擎，提供统一推理接口和多后端硬件加速能力，使能手机、PC/2in1、TV等全场景智能应用。
