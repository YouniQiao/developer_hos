...
1. 权重量化。
   ...
   1. 选配量化策略。
      ...
      输出：[dopt\_config.json](#dopt_configjson量化配置文件说明)，权重量化参数trained\_quant\_weight.pth【为阶段二的输入文件】。
      ...
      ```
   2. dopt\_config.json文件手动修改量化策略配置后，再次执行该阶段，进行权重量化。
...
3. 提取量化参数。
   ...
   输出：fake\_quant\_weight.pth+quant\_params\_file，其中fake\_quant\_weight.pth在导出onnx模型时需要替换模型的权重为该文件中的权重。
