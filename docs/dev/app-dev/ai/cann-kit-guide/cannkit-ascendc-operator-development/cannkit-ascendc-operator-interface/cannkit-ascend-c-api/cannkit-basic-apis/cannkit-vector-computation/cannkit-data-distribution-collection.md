...
| 参数名称 | 输入/输出 | 含义 |
| --- | --- | --- |
| dstLocal | 输出 | 目的操作数。  类型为[LocalTensor](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-localtensor)，支持的TPosition为VECIN/VECCALC/VECOUT。  LocalTensor的起始地址需要32字节对齐。  Kirin9020系列处理器，支持的数据类型为：  前n个tensor：uint16\_t、int16\_t、uint32\_t、int32\_t、half、float32\_t  KirinX90系列处理器，支持的数据类型为：  前n个tensor：uint16\_t、int16\_t、uint32\_t、int32\_t、half、float32\_t |
