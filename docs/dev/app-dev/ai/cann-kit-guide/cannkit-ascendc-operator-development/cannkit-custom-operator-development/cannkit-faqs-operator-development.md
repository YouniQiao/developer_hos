...
1. 在kernel侧的CMakeLists中通过add\_ops\_compile\_options针对报错算子添加编译选项“-mllvm -cce-aicore-jump-expand=true”，示例如下。
   ...
   add\_ops\_compile\_options的具体使用方法请参考[支持自定义编译选项](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-operator-project-compilation#支持自定义编译选项)。
2. 重新编译该算子。正常编译无报错。
...
2. 获取日志文件。
   ...
   通过命令行方式，日志落盘地址由[Simulator仿真参数](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-cli-parameters#simulator仿真参数)接口指定，缺省为当前操作路径的debug\_op.log。请根据实际路径打开日志文件。
3. 截取调测命令，重新执行后再分析。
...
2. 获取日志文件。
   ...
   通过命令行方式，日志落盘地址由[NPU调测参数](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-cli-parameters#npu调测参数)接口指定，缺省为当前操作路径的debug\_op.log。请根据实际路径打开日志文件。
