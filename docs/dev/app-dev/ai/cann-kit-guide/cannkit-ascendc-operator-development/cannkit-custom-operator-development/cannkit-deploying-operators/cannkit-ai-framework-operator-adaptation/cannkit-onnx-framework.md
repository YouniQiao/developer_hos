...
2. 使用REGISTER\_CUSTOM\_OP宏，完成DDK算子和ONNX框架的算子映射关系注册。使用方法如下。
   ...
   * REGISTER\_CUSTOM\_OP：注册自定义算子，OpType为算子类型名称，需要与[算子原型定义实现](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-operator-prototype-definition)中的OpType保持一致。
   * [FrameworkType](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-frameworktype)：ONNX代表原始框架为ONNX。
   * [OriginOpType](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-originoptype)：算子在原始框架中的类型。例如自定义算子OpTypeA，对应ONNX算子库版本opset\_version=11，应传入"ai.onnx::11::OpTypeA"，当前支持的ONNX版本范围为9~15。
