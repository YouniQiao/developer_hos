...
format: md
upstream_id: dev/app-dev/ai/mindspore-lite-kit/mindspore-lite-appendix/mindspore-lite-supported-operators
...
| Gather | 沿单一维度收集指定索引位置的元素 | Gather | 不支持uint8类型。不支持QuantType\_QUANT\_NONE量化类型。 |
| GatherD | 将输入tensor中的元素根据索引tensor进行收集 | GatherElements | - |
| GatherNd | 将输入张量的切片聚合成具有indices指定维度的新张量 | GatherND | 从HarmonyOS 6.1.0开始支持空shape推理。 |
