...
* **Execution Subsystem**：包含执行方舟字节码的解释器、快速路径内联缓存以及[文件模块化管理运行](/docs/dev/app-dev/application-framework/arkts/arkts-runtime/arkts-runtime-module/module-principle)。
* **Compiler Subsystem**：包含Stub编译器、基于IR的编译优化框架、AOT静态编译器以及JIT动态编译器（实验中）。
* **Runtime Subsystem**：包含以下ArkTS/TS/JS运行相关的模块。

  + 内存管理：对象分配器与[垃圾回收器](/docs/dev/app-dev/application-framework/arkts/arkts-runtime/gc-introduction)（并发标记和部分内存压缩的CMS-GC和Partial-Compressing-GC）。
