---
title: "社交分享跳转"
original_url: /docs/dev/app-dev/application-framework/social-sharing-redirection
format: md
upstream_id: dev/app-dev/application-framework/social-sharing-redirection
last_sync: 2026-06-07
sync_hash: a05fcf72
---
## 概述

随着内容社交和移动应用的发展，用户越来越习惯于通过社交平台快捷地分享文章、商品或活动等内容，并期待受邀好友能够“一键直达”目标内容。为满足这一需求，系统提供了[App Linking](/docs/dev/app-dev/application-framework/ability-kit/stage-model-development/inter-app-redirection/directional-redirection/app-linking-startup)技术，实现高效、安全且用户体验极佳的社交分享跳转。
...
当用户未安装目标应用且开发者配置了[直达应用市场](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-direct-to-ag)功能时，点击链接将直接跳转到应用市场的应用详情页。

安装完成后，首次打开应用将通过[延迟链接](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-deferredlink)功能自动导航至原始分享内容，无需用户重新搜索或操作。
...
在Web页面可提供“下载”按钮，引导用户安装应用获取更佳体验，安装后仍可通过[延迟链接](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-deferredlink)直达原内容。
...
* [延迟链接](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-deferredlink)还原：即便用户经历先装应用再首次启动，也能还原到用户首次点开的具体内容页，极大提升转化率。
...
1. [在AGC开通App Linking服务](/docs/dev/app-dev/application-framework/ability-kit/stage-model-development/inter-app-redirection/directional-redirection/app-linking-startup#zh-cn_topic_0000001862787784_section189581229144811)。
2. [在开发者网站上关联应用](/docs/dev/app-dev/application-framework/ability-kit/stage-model-development/inter-app-redirection/directional-redirection/app-linking-startup#section6903241628)。
3. [在AGC控制台关联网址域名](/docs/dev/app-dev/application-framework/ability-kit/stage-model-development/inter-app-redirection/directional-redirection/app-linking-startup#section1101111611317)。
4. [在DevEco Studio中配置关联的网址域名](/docs/dev/app-dev/application-framework/ability-kit/stage-model-development/inter-app-redirection/directional-redirection/app-linking-startup#section13808113610362)。
...
社交应用可通过集成[Share Kit（分享服务）](/docs/dev/app-dev/application-services/share-kit-guide/share-introduction)拉起分享面板，用于分享文章详情的App Linking链接。接收方点击链接后可直接跳转至目标应用。详细请参见[分享App Linking直达应用](/docs/dev/app-dev/application-services/share-kit-guide/system-share/share-service-scenarios/share-utd-link#section195231314185618)。
...
为提升用户未安装应用时的体验，可配置[直达应用市场](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-direct-to-ag)功能，引导用户下载安装应用。
...
同时，需要实现[延迟链接](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-deferredlink)功能，确保安装后首次启动可直达内容。
...
当应用未安装且未配置直达应用市场时，系统会打开浏览器Web页面。合理的Web页面开发是完善用户体验的关键环节，特别是与[延迟链接](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-deferredlink)结合使用。
...
详细请参见[User-Agent开发指导](/docs/dev/app-dev/application-framework/arkweb/web-set-attributes-events/web-default-useragent)。
...
当应用已安装时，点击“打开”按钮，可以直接跳转社交应用，直达文章详情页。跳转链接使用前面[配置App Linking服务](#配置app-linking服务)中配置的链接，例如“`https://www.example.com/”`，`，通过设置window.location.href属性进行页面跳转。
...
Web页面与延迟链接的结合是创建完整用户体验的关键。当用户通过Web页面安装应用后，可通过[延迟链接](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-deferredlink)实现首次打开应用时自动跳转到原始内容。
...
* applinking.json配置文件配置错误，请参考[在开发者网站上关联应用](/docs/dev/app-dev/application-framework/ability-kit/stage-model-development/inter-app-redirection/directional-redirection/app-linking-startup#section6903241628)进行配置。
...
3. 开发者请参考[验证应用被拉起效果](/docs/dev/app-dev/application-framework/ability-kit/stage-model-development/inter-app-redirection/directional-redirection/app-linking-startup#section1862871612126)在AGC上手动申请证书与Profile进行手动签名。
4. 重新更换其他网络。
...
1. 适配UA标识。确保Web页面适配了HarmonyOS设备和配套浏览器的UA标识，以正确识别设备并跳转到对应页面或平台。详细请参见[User-Agent开发指导](/docs/dev/app-dev/application-framework/arkweb/web-set-attributes-events/web-default-useragent)。
...
1. 适配UA标识。确保Web B页面适配了HarmonyOS设备和配套浏览器的UA标识，以正确识别设备并跳转到对应页面或平台。详细请参见[User-Agent开发指导](/docs/dev/app-dev/application-framework/arkweb/web-set-attributes-events/web-default-useragent)。
2. 检查Web B页面的资源加载情况。
...
* 检查跳转逻辑。确保Web页面跳转的目标应用正确配置了App Linking。详细请参见[配置App Linking服务](#配置app-linking服务)。
* 检查Web页面“打开”跳转地址的正确性。使用正确的App Linking跳转地址。
* 适配UA标识。确保Web页面适配了HarmonyOS设备和配套浏览器的UA标识，以正确识别设备并跳转到对应页面或平台。详细请参见[User-Agent开发指导](/docs/dev/app-dev/application-framework/arkweb/web-set-attributes-events/web-default-useragent)。
