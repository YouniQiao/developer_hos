...
当状态变量发生变化时，框架会将依赖于该变量的系统组件以及该变量所属的自定义组件标记为“脏”。在后续UI渲染的过程中，会将标脏的节点重新[刷新](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-introduce#触发更新)。
...
在函数调用或变量赋值时，传递的是变量本身，包含[收集依赖](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-introduce#收集依赖)和[触发更新](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-introduce#触发更新)的逻辑。
...
渲染过程是状态管理[触发更新](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-introduce#触发更新)流程中的一个阶段，指组件[标脏](#标脏mark-dirty)完成后，下一帧信号到来时刷新UI的这段流程。在此过程中，框架遍历脏自定义组件列表，调用系统生成的rerender方法，遍历脏系统组件，刷新组件并更新依赖。
