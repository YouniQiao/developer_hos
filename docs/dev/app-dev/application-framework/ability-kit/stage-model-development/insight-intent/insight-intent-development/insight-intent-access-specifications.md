...
| sceneType | string | 否 | 场景类型。  - MORNING\_SCENE：晨间。  - MIDDAY\_SCENE：午间。  - EVENING\_SCENE：晚间。  - NIGHT\_SCENE：夜间。  - WEEKEND\_SCENE：周末。  - FESTIVAL\_SCENE：节假日。  - BIRTHDAY\_SCENE：生日。  - ANNIVERSARY\_SCENE：纪念日。  - DRIVE\_SCENE：驾驶。  - SUBWAY\_SCENE：地铁。  - TRAVEL\_SCENE：出行。  - CITY\_SCENE：跨城。 | "MORNING\_SCENE" |
...
| cabin | string | 否 | 舱位类型。  - 经济舱。  - 头等舱。  - 公务舱。  - 超级经济舱。 | "经济舱" |
...
| seatLevelShow | string | 否 | 座位类型。  - 二等座。  - 二等卧。  - 一等座。  - 一等卧。  - 商务座。  - 硬座。  - 软座。  - 硬卧。  - 软卧。  - 高软。  - 无座。 | "二等座" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| mapType | string | 否 | 地图类型。  - Satellite：卫星地图。  - Standard：标准地图（默认）。  - 3D：3D地图。 | "Satellite" |
...
| entityLogoURL | string | 是 | 执行结果主图片。 | "https://example.com/bike\_image.jpg" |
| entityDisplayName | string | 是 | 执行结果展示名称（主标题）。 | "北京市东城区xxx街道" |
| entityDescription | string | 否 | 执行结果实体描述。 | - |
| statusImageUrl | string | 否 | 执行结果状态图片。 | "https://example.com/status\_image.jpg" |
...
| inOutStatus | string | 否 | 进出港状态，缺省出港。  - Arr：入港。  - Dep：出港（缺省值）。 | "Dep" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| operationType | string | 是 | 操作类型。  - Lock：上锁。  - Unlock：解锁。 | "Lock" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| operationType | string | 是 | 操作类型。  - Lock：上锁。  - Unlock：解锁。 | "Unlock" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| infoType | string | 否 | 获取信息类型。  - CarInfo：默认，车辆信息（含里程，电量）。  - TirePressureInfo：胎压信息（设备名称+胎压信息）。 | "CarInfo" |
...
| tableType | number | 否 | 餐桌类型。  - 1：小桌型。  - 2：中桌型。  - 3：大桌型。  - 4：包间类型。 | 3 |
...
| timeInterval | array | 否 | 用餐时间段。包含开始时间和结束时间的时间戳。  字段类型number。 | [1637393200000, 1637396800020] |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| type | string | 否 | 社保功能。  - "ProofOfPension"：养老金收入证明。  - "PensionPaymentDetails"：养老待遇发放明细。  - "SocialSecurityPaymentRecord"：社保缴纳记录。  - "SocialSecurityBalance"：社保余额。  - "SupplementaryPension"：养老金补缴。  - "PensionInsurance"：养老保险（首页）。  - "SocialSecurityCertificate"：参保证明。 | "ProofOfPension" |
...
| type | string | 否 | 婚姻登记类型。  - "Marriage"：结婚（默认值）。  - "Divorce"：离婚。 | "Marriage" |
...
| billType | number | 否 | 生活缴费费用类型。  - 0：水费。  - 1：电费。  - 2：燃气费。  - 3：话费。  - 4：流量。  - 5：暖气费。 | 0 |
...
| deviceType | string | 否 | 设备的类型。  - 摄像头（默认值）。  - 灯具。 | "摄像头" |
...
| functionType | object | 否 | 设备功能类型，回放/实时画面等。  - Playback：回看录像。  - LiveVideo：实时画面（默认值）。 | "Playback" |
...
| billType | number | 否 | 缴费类型。  - 0：水费。  - 1：电费。  - 2：燃气费。  - 3：话费。  - 4：流量。 | 3 |
...
| --- | --- | --- | --- | --- |
| entityId | string | 否 | 意图实体ID，长度不超过64字符。 | "C10194368" |
| blogCategory | string | 否 | 资讯博客频道分类。如：  - 旅行。  - 生活。  - 军事。  - 科技。  - 文化。  - 美食。  - 汽车。 | "旅行" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| category | string | 否 | 余量分类。  - DataTraffic：数据流量。  - VoiceTraffic：语音流量。  - SMSTraffic：短信流量。  - MobileAccountBalance：话费余额。 | "DataTraffic" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| category | string | 否 | 余量分类。  - DataTraffic：数据流量。  - VoiceTraffic：语音流量。  - SMSTraffic：短信流量。 | "DataTraffic" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| backupType | string | 否 | 备份类型。  - Gallery：相册。  - Contact：通讯录。 | "Contact" |
...
| type | string | 否 | 红包类型。  - Ordinary：普通红包。  - FightingLuck：拼手气红包。  - GoodLuck：好运红包。 | "Ordinary" |
...
| conferenceType | string | 否 | 会议类型。  - Video：视频。  - Audio：音频。 | "Video" |
...
| cardType | string | 否 | 银行卡类型。  - CreditCard：信用卡。  - SavingsCard：储蓄卡。 | "CreditCard" |
...
| repaymentType | string | 否 | 还款类型。  - CreditCard：信用卡。  - ConsumerLoan：消费贷。 | "CreditCard" |
...
| wealthManageType | string | 否 | 理财类型。  - Current：活期理财。  - Regular：定期理财。 | "Current" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| stockName | string | 否 | 股票名称。例如：  - 上证指数。  股票名称和（股票市场&股票代码）二选一填入。 | "上证指数" |
| stockSymbol | string | 否 | 股票代码，全球股票唯一代码。例如：  - 000001：国内上证指数代码。  股票名称和（股票市场&股票代码）二选一填入。 | "SH000001" |
| stockMarket | string | 否 | 股票市场。例如：  - SH：上交所。  - SZ：深交所。  - BJ：北交所。  和股票代码配合使用。 | "SH" |
...
| 参数名 | 类型 | 必选 | 说明 | 数据样例 |
| --- | --- | --- | --- | --- |
| marketArea | string | 否 | 大盘市场区域。  - CN：中国大盘（沪深A股）。  - US：美国大盘。  - HK：香港大盘。 | "CN" |
...
| --- | --- | --- | --- | --- |
| entityId | string | 否 | 意图实体ID，长度不超过64个字符。 | "C10194368" |
| insuranceType | string | 否 | 保险类型。  - Car：车险。  - TrafficCompulsory：交强险。  - Health：健康险。  - Accident：意外险。  - Property：财产险。  - Travel：旅行险。  - Pet：宠物险。  - Enterprise：企业险。 | "Car" |
