...
* **数据库备份**：指对当前数据库的数据库文件进行完整备份。HarmonyOS数据库备份针对数据库全量文件进行完整的备份。
...
HarmonyOS数据库加密时，应用开发者无需传入密钥，只需要设置数据库加密的状态即可。系统会自动帮助开发者将数据库加密，使用[huks通用密钥库系统](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-huks)，完成数据库密钥的生成及加密保护。
