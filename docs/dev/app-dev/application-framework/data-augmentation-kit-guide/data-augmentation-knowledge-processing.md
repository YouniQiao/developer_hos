...
* 通过[开发步骤](#开发步骤)配置knowledge\_schema.json和开库参数后，每次开库都会启动一次知识加工任务。
...
2. 配置数据源库[开库参数](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-i#storeconfig)，根据业务需要预置数据。下文是示例代码片段，仅供参考，具体实现方式请根据业务需要调整。
   ...
   * 建表语句中的表名需要与中"tableName"字段保持一致，列名与"columnName"字段保持一致。
   ...
   ```
3. 可根据业务需要，调用[getStatus()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-knowledgeprocessor-api#getstatus)接口，查询当前的知识加工状态。
   ...
   ```
4. 可根据业务需要，调用[startProcess](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-knowledgeprocessor-api#startprocess)(option: [KnowledgeProcessConfig](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-knowledgeprocessor-api#knowledgeprocessconfig))接口，启动知识加工。
   ...
   ```
5. 可根据业务需要，调用[stopProcess](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-knowledgeprocessor-api#stopprocess)()接口，停止知识加工。
