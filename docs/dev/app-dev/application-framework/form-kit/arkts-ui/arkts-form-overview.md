...
* 卡片渲染服务：用于管理卡片渲染实例，渲染实例与卡片使用方的卡片组件一一绑定。卡片渲染服务根据form\_config.json配置的卡片信息运行widget.abc文件的卡片页面代码进行渲染，并将渲染后的数据发送至卡片使用方对应的卡片组件。
...
* call事件：可以使用call事件拉起应用自身的UIAbility到后台，再通过UIAbility申请对应后台[长时任务](/docs/dev/app-dev/application-framework/arkts/arkts-concurrency/multithread-develop-guide/long-time-task/long-time-task-overview)完成音乐播放等功能。
...
从API version 20开始支持互动卡片，互动卡片提供破框动效能力，并实现人机交互，提升信息提醒、浅层交互和可玩性。具体请参考[互动卡片概述](/docs/dev/app-dev/application-framework/form-kit/arkts-ui/arkts-ui-liveform/arkts-ui-liveform-overview)。

## 模拟器支持情况

本Kit支持模拟器开发，但与真机存在部分能力差异，具体如下：

* 通用差异：详情请参见[模拟器与真机的差异](/docs/tools/coding-debug/ide-emulator-specification)。
* 模拟器中不支持 1 \* 1卡片预览。
* 模拟器中不支持背板透明卡片预览。
* 模拟器中不支持互动卡片预览。
...
* 支持导入[HAR](/docs/dev/app-dev/getting-started/dev-fundamentals/har-package)静态共享包，不支持导入[HSP](/docs/dev/app-dev/getting-started/dev-fundamentals/in-app-hsp)动态共享包。
* 不支持使用native语言开发，不支持加载native so。
* 针对卡片UI页面开发，ArkTS卡片仅支持[声明式开发范式](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-ui-development-overview)的部分组件、事件、动效、数据管理、状态管理和API能力。对于支持在ArkTS卡片UI页面中使用的接口，会添加“卡片能力”的标记，如：从API version 12开始，该接口支持在ArkTS卡片中使用。
...
* DevEco Studio的相关约束请参考[使用约束](/docs/tools/coding-debug/ide-service-widget#section1181172254318)。
