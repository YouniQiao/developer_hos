...
* **媒体会话（[AVSession](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-avsession-avsession)）**
  ...
  本地播放时，应用需要向媒体会话提供播放的媒体信息（如正在播放的歌曲、歌曲的播放状态等），并接收和响应播控中心发出的控制命令（如暂停、下一首等）。具体请参考[本地媒体会话](/docs/dev/app-dev/media/avsession-kit/local-avsession/local-avsession-overview)。
...
* **后台长时任务**
  ...
  应用实现后台播放，需申请后台长时任务，避免应用在投播后被系统后台清理或冻结。具体参考[长时任务](/docs/dev/app-dev/application-framework/background-task-kit/continuous-task)。
