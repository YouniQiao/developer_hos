...
title: "Media Kit简介"
original_url: /docs/dev/app-dev/media/media-kit/media-kit-intro
format: md
upstream_id: dev/app-dev/media/media-kit/media-kit-intro
...
详细流媒体开发流程请参考[流媒体播放开发指导](/docs/dev/app-dev/media/media-kit/media-kit-dev-arkts/media-playback-arkts/streaming-media-playback-development-guide)。
...
1. 应用从XComponent组件获取窗口SurfaceID，获取方式参考[XComponent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-xcomponent)。
...
| 视频源类型 | 说明 |
| --- | --- |
| surface\_yuv | 输入surface中携带的是raw data。 |
| surface\_es | 输入surface中携带的是ES data。 |
...
| --- | --- |
| MIC | 系统麦克风作为音频源输入。 |
| ALL\_PLAYBACK | 系统内录使用作为音频源输入。 |

支持的视频源如下：

| 视频源类型 | 说明 |
| --- | --- |
| SURFACE\_RGBA | 输出Buffer是rgba data。 |
...
支持的音视频源参考[媒体数据解析](/docs/dev/app-dev/media/avcodec-kit/file-muxing-demuxing/audio-video-demuxer)。
...
支持的视频源参考[视频解码](/docs/dev/app-dev/media/avcodec-kit/audio-video-codec/video-decoding)。
...
* 支持转码时降低视频分辨率。
  ...
  目标视频宽、高不能大于源视频宽、高，且不能设置为奇数，详情请参考[设置正确的视频宽高](/docs/dev/app-dev/media/avcodec-kit/audio-video-codec/obtain-supported-codecs#设置正确的视频宽高)。
* 支持的源视频格式：

  当前转码支持的源视频格式依赖于系统解码器和解封装支持的格式，详细情况请参考[AVCodec支持的格式](/docs/dev/app-dev/media/avcodec-kit/avcodec-support-formats#媒体数据解析)。
* 支持的目标视频格式：
...
* 模拟器仅支持播放h264文件格式、RGBA像素格式的视频文件。
