...
支持的编码能力请参考[AVCodec支持的格式](/docs/dev/app-dev/media/avcodec-kit/avcodec-support-formats#音频编码)。

**适用场景**

通常推荐使用异步模式，异步模式请参考[音频编码](/docs/dev/app-dev/media/avcodec-kit/audio-video-codec/audio-encoding)。若需要主动请求buffer去编码PCM，则可以使用同步模式。

* 音频录制

  通过录制传入PCM，然后编码成对应格式的码流，最后封装成所需格式的文件。具体封装方法请参考[媒体数据封装](/docs/dev/app-dev/media/avcodec-kit/file-muxing-demuxing/audio-video-muxer)。
* 音频编辑

  编辑PCM后导出音频文件的场景，需要编码成对应音频格式，最后封装成所需格式的文件。具体封装方法请参考[媒体数据封装](/docs/dev/app-dev/media/avcodec-kit/file-muxing-demuxing/audio-video-muxer)。
