...
**依赖ROI检测识别能力：** 编码器不具备ROI的检测识别能力，所以ROI编码技术生效依赖于开发者输入的ROI信息。开发者可根据业务场景自行设计并实现ROI识别能力，或通过调用系统相机模块原生提供的人脸区域信息，降低开发成本，具体请参考[相机人脸ROI获取示例](/docs/dev/app-dev/media/camera-kit/camera-dev-native/native-camera-metadata#状态监听)。
...
2. 监听相机元数据回调接口获取人脸位置信息。
   ...
   如何注册相机元数据回调可以参考 [相机元数据状态监听](/docs/dev/app-dev/media/camera-kit/camera-dev-arkts/camera-metadata#状态监听)。
...
6. 将ROI信息设置到视频帧NativeBuffer元数据中。
   ...
   绘制之前获取最新的NativeBuffer，并配置ROI信息。绘制完成后可通过eglSwapBuffers把绘制好的数据送到编码器进行编码。
...
5. 在编码输入参数回调中配置ROI信息。
   ...
   视频编码的详细操作步骤请参考[视频编码](/docs/dev/app-dev/media/avcodec-kit/audio-video-codec/video-encoding)开发指南，下面仅针对ROI编码做具体说明。
