---
title: "hidumper"
original_url: /docs/dev/app-dev/system/hidumper
format: md
upstream_id: dev/app-dev/system/hidumper
...
| [-s [SA] -a ["option"]](#获取指定系统服务提供的能力) | 执行单个系统能力的特定选项。  SA：系统能力名称。  option：该系统能力支持的选项。可通过 -s [SA] -a ["-h"] 获取单个系统能力支持的所有选项。 |
| [-e](#获取系统故障日志) | 获取记录的故障日志。输出日志的规格可参考：[CppCrash](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#日志规格)、[JSCrash](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/jscrash-guidelines#日志规格)、[AppFreeze](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines#日志规格)。 |
| [-e --list [process\_name] [-n num] [--since timestamp] [--until timestamp]](#获取异常退出记录列表) | 获取异常退出记录列表。  **说明**：从API version 22开始，支持该参数。 |
| [-e --print [process\_name] [-n num] [--since timestamp] [--until timestamp]](#获取异常退出故障日志) | 获取异常退出故障日志。输出日志的规格可参考：[CppCrash](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#日志规格)、[JSCrash](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/jscrash-guidelines#日志规格)、[AppFreeze](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines#日志规格)。  **说明**：从API version 22开始，支持该参数。 |
| [-e --print record\_id](#获取异常退出故障日志) | 获取指定异常退出记录id的故障日志。输出日志的规格可参考：[CppCrash](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/cppcrash-guidelines#日志规格)、[JSCrash](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/crash-detection/jscrash-guidelines#日志规格)、[AppFreeze](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines#日志规格)。  **说明**：从API version 22开始，支持该参数。 |
...
| [--mem [--prune]](#查询整机内存) | 获取总内存使用情况。如果指定--prune，只导出精简的内存使用情况。  **说明**：从API version 20开始，支持--prune参数。 |
| [--mem pid [--show-ashmem] [--show-dmabuf]](#查询进程内存) | 获取指定pid的进程内存使用情况。  指定--show-ashmem，则补充打印该进程的ashmem使用详细信息。  指定--show-dmabuf，则补充打印DMA内存使用详情信息。  **说明**：  从API version 20开始，支持--show-ashmem、应用进程的--show-dmabuf参数。  从API version 23开始，支持系统服务进程的--show-dmabuf参数。 |
...
| [--mem-smaps pid [-v]](#查询进程内存) | 获取pid内存统计信息，数据来源于/proc/pid/smaps，使用-v指定更多详细信息。（仅支持导出[debug版本应用](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#debug版本应用)）  **说明**：从API version 20开始，支持该参数。 |
| [--mem-jsheap pid [-T tid] [--gc] [--leakobj] [--raw] [--clean]](#查询虚拟机堆内存) | 必选参数pid。触发ArkTS应用JS线程的gc和堆内存快照导出。文件命名格式为：hidumper-jsheap-进程号-JS线程号-时间戳，文件内容为JSON结构的JS堆快照。  指定线程tid时，仅触发该线程的gc和堆内存快照导出。  指定--gc时，仅触发gc，不导出快照。  指定--leakobj时，应用开启泄露检测可获取泄露对象列表。  指定--raw时，堆快照以rawheap格式导出。  指定--clean时，快照导出后触发清理nodeId节点信息。  **说明**：  从API version 19开始，支持--raw参数。  从API version 24开始，支持--clean参数。 |
...
hidumper --mem-smaps [pid] [-v]命令调试的进程应为“使用调试证书签名的应用”，同[debug版本应用](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#debug版本应用)。
...
hidumper --mem-jsheap pid [-T tid] [--gc] [--leakobj] [--raw] [--clean]命令调试的进程应为“使用调试证书签名的应用”，同[debug版本应用](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/performance-analysis-kit-terminology#debug版本应用)。
...
* 可使用hidumper --mem-jsheap pid [-T tid] --raw获取指定进程或指定JS线程的虚拟机堆内存，生成的堆内存文件为rawheap格式，文件命名为hidumper-jsheap-进程号-JS线程号-时间戳.rawheap。rawheap的解析转换可参考使用：[rawheap-translator工具](/docs/dev/app-dev/system/rawheap-translator)。
...
上述生成的文件，可以通过hdc[文件传输](/docs/dev/app-dev/system/hdc#文件传输)命令从设备中获取。
...
以下异常退出原因，可参考[应用终止分析思路和分析步骤](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/appkilled-guidelines#分析思路和分析步骤)进行问题排查处理。
...
仅支持通过[Navigation](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-set-navigation-routing/arkts-navigation-navigation/arkts-navigation-architecture)组件实现页面路由的应用。
...
若想通过Hidebug获取图形内存，请参考[HiDebug能力概述](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/hidebug/hidebug-guidelines)中的相关说明。
