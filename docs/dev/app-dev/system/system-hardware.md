---
title: "硬件"
original_url: /docs/dev/app-dev/system/system-hardware
format: md
---


* **[Car Kit（车服务）](/docs/dev/app-dev/system/system-hardware/car-kit-guide)**
* **[Driver Development Kit（驱动开发服务）](/docs/dev/app-dev/system/system-hardware/driver-development-kit)**
* **[Multimodal Awareness Kit（多模态融合感知服务）](/docs/dev/app-dev/system/system-hardware/multimodal-awareness-kit)**
* **[Pen Kit（手写笔服务）](/docs/dev/app-dev/system/system-hardware/pen-kit-guide)**
* **[Sensor Service Kit（传感器服务）](/docs/dev/app-dev/system/system-hardware/sensor-service-kit)**
* **[Mechanic Kit（机械设备管理服务）](/docs/dev/app-dev/system/system-hardware/mechanic-kit)**
* **[Wear Engine Kit（穿戴服务）](/docs/dev/app-dev/system/system-hardware/wear-engine-kit-guide)**
