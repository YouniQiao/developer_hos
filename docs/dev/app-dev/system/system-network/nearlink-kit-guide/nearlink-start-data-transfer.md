...
1. 数据传输通道不保证链路加密。如需加密数传，需先进行配对流程，通过[startPairing](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/nearlink-remote-device#startpairing)接口发起。
2. 链路是否加密可通过[getAcbState](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/nearlink-remote-device#getacbstate)接口查询，ENCRYPTED状态表示链路已加密。
