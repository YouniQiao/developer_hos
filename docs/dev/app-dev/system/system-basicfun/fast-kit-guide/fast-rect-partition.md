...
其相关定义如下：一个矩形![](./img/27c822ce.png)为二维网格内横纵坐标满足![](./img/ac30b84b.png)的所有单元矩形构成的集合（坐标系说明：X轴从左到右递增，Y轴从上到下递增）。两个矩形相交，当且仅当它们共享至少一个公共的单元矩形。
...
矩形划分求解器运行的时间复杂度为 ![](./img/b5a6c9f7.png)，可以高效处理大规模输入数据。在网格数据处理和空间几何计算等优化场景中，可以使用矩形划分求解器提升区域处理效率，减少冗余空间。
