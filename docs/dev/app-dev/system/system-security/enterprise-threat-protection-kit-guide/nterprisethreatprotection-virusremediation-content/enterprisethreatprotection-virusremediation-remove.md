...
详细接口说明可参考[接口文档](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/enterprisethreatprotection-virusremediation-interface#removeisolatedfile)。
...
1. 导入模块。
   ...
   ```
2. 通过调用接口[removeIsolatedFile](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/enterprisethreatprotection-virusremediation-interface#removeisolatedfile)，实现对指定隔离ID文件的删除。id参数为隔离文件的唯一标识符，可通过调用[queryIsolatedFiles](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/enterprisethreatprotection-virusremediation-interface#queryisolatedfiles)接口获取。
