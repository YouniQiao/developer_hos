...
title: "密钥导入介绍及算法规格"
original_url: /docs/dev/app-dev/system/system-security/huks-kit/huks-local-key-management/huks-key-generation-import/huks-key-import/huks-key-import-overview
format: md
upstream_id: dev/app-dev/system/system-security/huks-kit/huks-local-key-management/huks-key-generation-import/huks-key-import/huks-key-import-overview
...
导出密钥接口返回的[公钥明文材料是按照**X.509**格式封装](/docs/dev/app-dev/system/system-security/huks-kit/huks-local-key-management/huks-concepts#公钥材料格式)，导入加密密钥接口中的密钥材料需满足**LengthData-Data**的格式封装，形如：[(Lengthpart1Datapart1)……(LengthpartnDatapartn)]。
...
导出密钥接口返回的[公钥材料格式](/docs/dev/app-dev/system/system-security/huks-kit/huks-local-key-management/huks-concepts#公钥材料格式)按照X.509格式封装，导入加密密钥接口返回的密钥材料按照**LengthData-Data**的格式封装，分别是[(LengthEncSm4DataEncSm4)(LengthEncImpKeyDataEncImpKey)]。
...
| --- | --- | --- |
| 对称密钥 | - | 密钥字节数据 |
| 非对称密钥-密钥对 | - | [密钥对材料格式](/docs/dev/app-dev/system/system-security/huks-kit/huks-local-key-management/huks-concepts#密钥对材料格式) |
| 非对称密钥-公钥 | ED25519、X25519 | 密钥字节数据，参考[导入X25519密钥公钥](/docs/dev/app-dev/system/system-security/huks-kit/huks-local-key-management/huks-key-generation-import/huks-key-import/huks-import-key-in-plaintext-arkts#导入x25519密钥公钥) |
