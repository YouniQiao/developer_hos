...
| name | string | 线程名。 |
| tid | number | 线程id。 |
| usage | number | 平均负载数值，取值范围：[0，100]，单位：%，具体信息取决于fault\_type故障类型，不同类型记录的平均负载数值含义存在差异。  - 前台CPU高负载异常：多核平均负载，如需换算单核负载需乘以核数；  - 后台CPU高负载异常：多核平均负载，如需换算单核负载需乘以核数；  - 线程CPU高负载异常：单核平均负载。 |
