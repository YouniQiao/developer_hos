...
开发者可以通过设置[EventPolicy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#eventpolicy22) 的参数来开启应用冻屏事件的页面切换日志采集。

| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| appFreezePolicy | [AppFreezePolicy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#appfreezepolicy24) | 否 | 是 | 应用冻屏事件配置策略。 |
...
| priority | number | 实时优先级。读取自/proc/pid/stat的priority的值。  **说明**：从API version 23开始支持。 |
| nice | number | 静态优先级。读取自/proc/pid/stat的nice的值。  **说明**：从API version 23开始支持。 |
| clk | number | 每秒的时钟嘀嗒次数。使用sysconf(\_SC\_CLK\_TCK)获取，获取失败时使用默认值100。通过嘀嗒数除以该值可以计算得到运行时间（单位：秒）。  **说明**：从API version 23开始支持。 |
...
开发者可以通过该接口订阅name为hiAppEvent.event.APP\_FREEZE的应用冻屏事件，具体使用详见[hiAppEvent.setEventParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-hiappevent#hiappeventseteventparam12)。
