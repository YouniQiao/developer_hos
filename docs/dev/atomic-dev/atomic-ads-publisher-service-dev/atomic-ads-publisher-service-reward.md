...
应用上架至华为应用市场（AppGallery）时间超过12小时才可以收到回调。
...
3. 获取要验证的内容。
   ...
   用户观看完激励广告时，鲸鸿动能平台服务端会把需要验证的参数以及keyId和sign传给媒体提供的URL：https://www.example.com/feedback（[即第1点中配置的验证URL](atomic-ads-publisher-service-reward.md#校验服务端验证回调)）。请求体样例：
