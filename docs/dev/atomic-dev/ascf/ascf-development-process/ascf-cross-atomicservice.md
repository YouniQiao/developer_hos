...
元服务兼容模式支持采用公私钥和P12方式签名两种模式，提交发布或运行均会验证签名，ASCF开发助手会对签名配置进行提醒，请按照提示正确完成签名配置。以[手动方式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-signing#section297715173233)完成签名。详情请参见[签名](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-signing)。
