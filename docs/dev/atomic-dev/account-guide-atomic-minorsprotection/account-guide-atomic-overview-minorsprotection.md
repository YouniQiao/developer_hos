...
1. 当元服务接入未成年人模式后，开发者需要在元服务中声明已经支持未成年人模式：
   ...
   * 通过在工程中entry模块的module.json5文件中module属性下配置metadata字段并且配置minors\_mode值为"1"，表示当前元服务已接入未成年人模式。如果开发者没有配置相关字段，或配置值不为"1"，则表示该元服务不支持未成年人模式，当系统开启未成年人模式时，不支持未成年人模式的元服务会被默认禁用。
