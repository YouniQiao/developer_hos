...
Scan Kit为元服务提供了默认界面扫码、图像识码、码图生成三种能力。
...
* 图像识码能力支持Phone、Tablet、Wearable（从26.0.0版本开始支持Phone、Tablet、Wearable）。
...
| 能力 | 限制条件 |
| --- | --- |
| 默认界面扫码能力 | 从26.0.0版本开始，支持使用[scanCore.isDefaultScanSupported](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/scan-scancore#isdefaultscansupported)接口查询当前设备是否支持默认界面扫码；从6.1.0(23)版本开始，标题支持根据[ScanOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/scan-scanbarcode-api#scanoptions)的scanTypes进行动态显示；从6.0.0(20)版本开始，支持悬浮屏、分屏场景。相册扫码只支持单码识别；不支持界面UX添加自定义设置。 |
