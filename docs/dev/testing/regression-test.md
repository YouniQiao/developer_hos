...
测试执行前需先构建测试包，用户利用python的setuptools工具在工程（测试工程构建参考[应用UI测试（基于Python）](/docs/dev/testing/ui-test/python)）根目录下构建setup-regression.py、MANIFEST.in文件，setup-regression.py文件中声明用例，MANIFEST.in文件中声明脚本执行过程中需要用到的aw包、config文件夹下的配置文件及其他的资源文件。
...
![](./img/21d94d76.jpg "点击放大")

**查看报告**

测试完成后，将自动跳转到报告界面。

![](./img/52feb148.png "点击放大")
