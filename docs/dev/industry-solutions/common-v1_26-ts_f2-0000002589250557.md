...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_f2-0000002589250557
---
...
| 场景二 | 如何解决PersistentStorage无法获取持久化的数据的问题？ | 初始化在loadContent之前导致获取失败，详情参考官网行业常见问题：[PersistentStorage无法获取持久化的数据如何解决](https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_9-0000002263991840)。 |
| 场景三 | 如何解决PersistentStorage持久化数据无法删除的问题？ | 存在被注册了的监听，导致无法删除，详情参考官网行业常见问题：[PersistentStorage持久化数据无法删除](https://developer.huawei.com/consumer/cn/doc/architecture-guides/tools-v1_2-ts_228-0000002383644834)。 |

* 场景一：如何解决嵌套对象更新PersistentStorage持久化数据失效的问题？

  在PersistentStorage持久化数据时一般是依赖AppStorage中的相关API进行持久化更新，例如上述问题代码中，通过[@StorageLink](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-appstorage#storagelink)获取的PersistentStorage持久化数据，当UI内修改该数据时，同步修改回PersistentStorage进行本地持久化。所以此时受AppStorage不具备深度观测能力的限制，直接修改嵌套的深层属性，不会更新回PersistentStorage进行本地持久化。修改后完整示例代码如下：
  ...
  ![](./img/8d237110.gif "点击放大")
* 场景二：如何解决PersistentStorage无法获取持久化的数据的问题？

  PersistentStorage和UI实例相关联，持久化操作需要在UI实例初始化成功后（即[loadContent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#loadcontent9)传入的回调被调用时）才可以被调用，由于问题代码中PersistentStorage早于该时机调用，所以会导致持久化失败。详情参考官网行业常见问题：[PersistentStorage无法获取持久化的数据如何解决](https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_9-0000002263991840)。
* 场景三：如何解决PersistentStorage持久化数据无法删除的问题？

  当有页面使用@StorageLink的装饰器注册了监听时，可能存在无法删除的情况，详情参考官网行业常见问题：[PersistentStorage持久化数据无法删除](https://developer.huawei.com/consumer/cn/doc/architecture-guides/tools-v1_2-ts_228-0000002383644834)。
