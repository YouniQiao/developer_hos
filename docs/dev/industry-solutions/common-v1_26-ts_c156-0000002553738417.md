...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_c156-0000002553738417
---
...
1. **API兼容性适配，解决低版本闪退。**
   * **版本判断机制**：
     ...
     HarmonyOS设备独有特性接口，即接口标记为since M.S.F(N)，通过[distributionOSApiVersion](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-device-info#常量)，如6.0.0(20)对应60000。
...
2. **系统能力SysCap检查，检查设备能力差异。**
   * **能力验证**：

     使用[canIUse](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-syscap#caniuse)接口验证设备是否支持特定系统能力。
...
3. **动态资源管理，将不同版本资源分别保存。**
   ...
   * **运行时加载策略**：
     ...
     根据设备版本动态选择资源路径，确保UI和功能适配性。

**4. 分发限制和运行降级。**

* **分发限制配置**：

  在build-profile.json5中通过deviceType字段，如["phone","tablet","2in1"]限制功能仅在支持的设备类型上启用。
* **运行时降级方案**：
