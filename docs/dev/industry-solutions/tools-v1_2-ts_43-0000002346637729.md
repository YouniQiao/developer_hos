...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/tools-v1_2-ts_43-0000002346637729
---
...
* 其他类型文件（例如，db文件、txt文件等，原始文件形式保存）：需要用到资源管理模块resourceManager的常见接口：
  ...
  |  | [getRawFileContent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-resource-manager#getrawfilecontent9) | [getRawFd](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-resource-manager#getrawfd9) |
  | --- | --- | --- |
  ...
  + 文件内容大的文件，进行访问（以数据库db文件，举例说明）：
    ...
    saveFileToSandbox(file: resourceManager.RawFileDescriptor, dbFileName: string) {
      ...
      hilog.info(LogDomain, 'TestShow-', 'Copy Success!');
      fs.close(cacheFile);
    }
    ```
