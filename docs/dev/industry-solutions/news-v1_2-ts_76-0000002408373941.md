...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/news-v1_2-ts_76-0000002408373941
---
...
* 方案一：通过onAreaChange中返回参数获取目标组件相对父元素和页面左上角的坐标位置。
  ...
  | 名称 | 类型 | 说明 |
  | --- | --- | --- |
  | position | [Position](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#position) | 目标元素左上角在以父元素为基准的组件坐标系中的位置。 |
  ...
  ![](./img/9f05a5cd.png "点击放大")

* 方案二：通过inspector.createComponentObserver绑定指定组件，返回对应的监听句柄，结合getRectangleById中返回参数获取目标组件相对父元素和相对于屏幕的信息。

  | 名称 | 类型 | 说明 |
  | --- | --- | --- |
  | localOffset | [Offset](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-componentutils#offset) | 组件相对于父组件信息。 |
