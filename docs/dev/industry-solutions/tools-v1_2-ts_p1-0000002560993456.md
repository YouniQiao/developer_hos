...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/tools-v1_2-ts_p1-0000002560993456
---
...
* 场景一：
  ...
  用[ohpm convert](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-ohpm-convert)将指定ohpm或npm仓库中的某个包或者本地node\_modules目录下的包转换成满足ohpm格式要求的HAR包，并保存至当前工作目录，转换后的包将支持上传至ohpm-repo私仓或OpenHarmony三方库中心仓。
* 场景二：
...
* 场景四：
  ...
  + 动态共享包HSP包：
    ...
    动态共享包HSP包不能直接发布在ohpm-repo内，需要先转化为.tgz包，转换方法见：[编译HSP模块](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hsp#section67683213597)。TGZ包的发布流程同HAR一致。执行 ''ohpm publish <TGZ包路径>'' 命令发布TGZ包，< TGZ 包路径> 指向的文件后缀需为.tgz文件的具体路径。
