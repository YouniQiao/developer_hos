---
title: "Web页面点击无响应"
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/news-v1_2-ts_c15-0000002318280334
---
...
* 不同类型的智能设备，用户可能有不同的交互方式，如通过触摸屏、鼠标、触控板等。如果针对不同的交互方式单独做适配，会增加开发工作量同时产生大量重复代码。为解决这一问题，系统统一了各种交互方式的API，即实现了交互归一。常见的基础输入方式及其在各输入设备上的表现如[下图](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-interaction#section182814229423)所示：
  ...
  ![](./img/bfb36f8c.png "点击放大")

## 问题定位

* 问题一：
  1. 日志过滤ARKWEB-CONSOLE，查看H5页面有没有报错日志输出，获得如下日志，可知UNIApp未被定义。
...
* 问题二：
  ...
  使用Devtools调试Web页面，选择要点击的组件，查看事件侦听器，检查该组件绑定的事件是click还是touch。
