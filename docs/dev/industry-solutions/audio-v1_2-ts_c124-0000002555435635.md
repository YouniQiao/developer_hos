...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/audio-v1_2-ts_c124-0000002555435635
---
...
* 场景一：应用前后台切换自动播放控制，可通过UIAbility生命周期监听结合全局状态管理实现。具体实现思路：
  ...
  2. 在具体的组件中使用[@StorageLink](https://developer.huawei.com/consumer/cn/doc/cangjie-guides/cj-appstorage#storagelink)绑定此状态，并利用[@Watch](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-watch)装饰器监听其变化，从而自动调用VideoController的start()或pause()方法，以同步控制视频的播放与暂停。

     具体实现代码可参考该[指南](https://developer.huawei.com/consumer/cn/doc/architecture-guides/audio-v1_2-ts_9-0000002297910537)。
