...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/tools-v1_2-ts_49-0000002319565646
---
...
* 场景一：用NavigationAttribute把Navigation的title整理成一个公共的动态属性时，程序报错闪退。

  原因：不支持入参或者返回值为CustomBuilder的属性。Navigation的[title](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation#title)方法支持CustomBuilder参数。
  ...
  ```
* 场景二：Text组件使用attributeModifier绑定bindSelectionMenu崩溃，程序报错闪退。

  原因：不支持入参或者返回值为CustomBuilder的属性。Text的[bindSelectionMenu](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-text#bindselectionmenu11)方法第二个入参为CustomBuilder类型。
