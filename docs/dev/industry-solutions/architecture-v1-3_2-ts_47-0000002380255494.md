...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/architecture-v1-3_2-ts_47-0000002380255494
---
...
* **方案一**：通过if/else控制Tabs页面渲染。
  ...
  实现重点如下：
  1. 在渲染过程中需注意渲染的页面位置判定参数的连续性，即下文中tabBuilder(index: number, name: string)组件的index参数需是本页面的位置参数。
  ...
  ![](./img/3c8a4f19.gif "点击放大")
* **方案二**：使用ForEach操作数组进行页面部分隐藏。

  实现重点如下：
  1. 将Tabs栏的数据封装在数组中，使用ForEach遍历展示；
  2. 操作数组来对Tab页面进行部分隐藏。
