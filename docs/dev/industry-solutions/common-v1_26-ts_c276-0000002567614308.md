...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_c276-0000002567614308
---
...
2. 部分三方服务使用appId鉴权，具体的获取方式参考[bundleInfo.signatureInfo.appId。](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bundlemanager-bundleinfo#signatureinfo)正确的appId形式为："包名\_签名信息"。
...
1. 本地调试包获取appId的方式有两种：
   ...
   * 安装应用后通过[bm](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/bm-tool)工具获取。参考命令：

     hdc shel bm dump -n {bundlename} | grep appId
2. 上架包获取appId的方式也有两种。
...
1. 获取appId，如：com.example.demo\_BWashfosahfsoafjsoafhj\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*ASDASAFdsafdDFAawWS=;
2. 去除包名信息，获取签名信息保存为txt文件，如sign.txt。
3. Windows系统下可通过**certutil -hashfile sign.txt SHA256**命令获取，Mac系统下可通过**shasum -a 256 sign.txt**命令获取。
...
2. 签名信息是唯一的，通过签名信息能很好的完成业务的鉴权，一般表现形式是公钥指纹或appId。部分业务通过[fingerprint](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bundlemanager-bundleinfo#signatureinfo)鉴权，但是使用的签名证书发生变化时，fingerprint也会发生变化。因此涉及到该业务要及时关注fingerprint的值是否正确。
