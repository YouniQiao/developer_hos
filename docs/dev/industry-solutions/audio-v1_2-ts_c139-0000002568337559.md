...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/audio-v1_2-ts_c139-0000002568337559
---
...
struct Index {
  ...
  async aboutToAppear(): Promise<void> {
    ...
    try {
      ...
      // 解码图片文件为PixelMap
      let imageSource: image.ImageSource = image.createImageSource(fileData.buffer.slice(0));
  ...
  async rotatePixelmap() {
    ...
    try {
      ...
      console.error(`failed to ratae image, Cause: ${JSON.stringify(err)}`);
    } finally {
      if (oldPixelMap) {
        oldPixelMap.release();
      }
