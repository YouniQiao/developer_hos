...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/insurance-v1_2-ts_c96-0000002446028233
---
...
1. 从faultlogger目录下获取到应用的JS Crash故障日志，故障原因是自定义错误类Error，故障信息为Internal error. UI execution context not found，错误码为100001。
   ...
   ```
2. 异常信息UI execution context not found表示没有获取到UI的执行上下文。排查栈顶函数gotoLoginPage是否直接使用了router跳转页面，此情况可能导致[UI上下文不明确](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-global-interface#ui上下文不明确)的问题。
...
* 方式一：
  ...
  通过使用[Class (UIContext)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-uicontext)中的[getRouter](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-uicontext#getrouter)方法获取当前UI上下文关联的[Class (Router)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-router)对象，再通过该对象调用对应方法。可参考[pushUrl](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-router#pushurl)中的示例，此页面路由方式不推荐，建议使用方式二。
* 方式二：
