...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_494-0000002547627959
---
...
* 场景二：
  ...
  Hvigor在初始化时会读取环境变量HVIGOR\_USER\_HOME，若其值包含空格，会导致路径解析失败，进而引发构建异常。
* 场景三：
...
* 场景二：
  ...
  避免使用含空格的路径，参考官方文档[自定义.hvigor目录路径](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-path)，通过设置HVIGOR\_USER\_HOME环境变量指定一个不含空格的路径，防止因为路径中有空格导致Hvigor初始化失败。
* 场景三：

  配置受信任的CA证书或通过禁用SSL证书验证临时规避。
