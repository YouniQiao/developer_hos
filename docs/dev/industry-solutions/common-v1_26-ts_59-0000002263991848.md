...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_59-0000002263991848
---
...
* [customScan.start](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/scan-customscan-api#section747366165913)：启动扫码相机流，使用Callback异步回调获取扫码结果、相机预览流（YUV-图像格式NV21基于4:2:0采样）。
* [customScan.rescan](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/scan-customscan-api#section19244173211169)：触发一次重新扫码。如果扫描结果不是预期结果，可以调用此接口触发下一次扫描。本接口必须在启动相机流start接口后，stop接口之前使用，未启动相机流调用会抛出内部错误的异常。仅对start接口的Callback异步回调有效，Promise异步回调接口无效。
* 自定义持续扫码时序图如下：

  ![](./img/0409aab6.png "点击放大")
...
2. 在start接口的异步回调中通过变量控制显示扫描结果和重新扫描按钮，点击重新扫描按钮即可调用rescan接口再次触发重新扫码功能。

   注意：本案例进入页面之后需要扫码才能出现重新扫码页面，重新扫码响应快速，因此点击重新扫码之前建议将摄像头远离码图。
   ...
   struct CustomScanPage {
     ...
     @State scanResult: string = ''; // 扫码结果
     @State isShowResult: boolean = false; // 是否正在扫码
     ...
     async onPageShow() {
       ...
       // 多码扫码识别时enableMultiMode设置为true，单码扫码识别时enableMultiMode设置为false
