...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_105-0000002378041989
---

## 问题现象

socket.TCPSocketServer订阅'connect'事件，off掉之后，重新订阅，回调方法不生效。
...
function buildRespString(content: string) {
  let result: string = '';
  let bodyContent =
    `<html><head><title>HTTP服务器模拟</title></head><body><h1>浏览器发送的请求信息</h1><pre><h2>${content}</h2></pre></body></html>`;
