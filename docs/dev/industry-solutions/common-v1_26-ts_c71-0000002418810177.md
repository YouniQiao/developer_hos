---
title: "启动应用时发生闪退"
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_c71-0000002418810177
---
...
* [JS Crash异常](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-app-crash-js-way)根据不同的异常场景，在Reason字段进行了分类，分为Error、TypeError、SyntaxError、ReferenceError、RangeError等错误类型。

  JS Crash日志包含异常原因及异常堆栈等关键定位信息：
...
* [CppCrash](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-fault-type#section169359583920)应用崩溃指的是C/C++运行时崩溃，CppCrash进程崩溃检测基于操作系统信号机制。
  ...
  | 二级分类 | 信号字符串 | 解释 | 触发原因 |
  | --- | --- | --- | --- |
  | 1 | ILL\_ILLOPC | 非法操作码异常。 | 发生在执行不被CPU支持的指令时，或者在尝试执行特权指令时。 |
...
故障信号是SIGILL(ILL\_ILLOPC)：

* 异常信息是：Signal:SIGILL(ILL\_ILLOPC)@000000000000000000。
