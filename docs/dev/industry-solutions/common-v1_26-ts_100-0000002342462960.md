---
title: "Image加载本地图片"
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_100-0000002342462960
---
...
* 方案二、通过本地路径直接引用。
  + 将图片放在ets目录下的任意位置（如ets/images/）。

    新建工程或模块时，默认配置enable字段并且值为false，即默认不打包ets目录下的资源文件。需要在entry/build-profile.json5中将"copyCodeResource": {"enable": false}的false改为true。详细参考：[resOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-build-profile#section754823013348)资源编译配置项。
  + 特点：
...
A：可以通过[ohos\_apng](https://gitee.com/openharmony-sig/ohos_apng)库来实现。
