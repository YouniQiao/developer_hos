---
title: "不拉起相机实现无感拍照"
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/photo-v1_2-ts_c48-0000002601670573
---

## 问题现象

教育类APP需要在考试界面定时拍摄，以核验答题者是否有作弊行为，但是因为不想打断考试过程，因此想要在不拉起预览界面的情况下完成拍照，请问是否有可行的方案？
...
业务场景要求不拉起相机实现无感拍照，主要是不能拉起XComponent预览界面，因此可以在双路预览方案的基础上，砍掉XComponent那一路送显的流，仅通过ImageReceiver那一路流去获取并处理相机拍摄到的帧数据。
...
struct Index {
  ...
  private period: number = 10; // 定义提取视频帧时间间隔
  private canIProcess: boolean = false; // 是否需要处理ImageReceiver回图
  ...
  onImageArrival(receiver: image.ImageReceiver): void {
    ...
    receiver.on('imageArrival', () => {
      ...
      receiver.readNextImage((err: BusinessError, nextImage: image.Image) => {
        ...
        // 如果当前状态为无需处理，则在释放资源后直接跳过以减轻能耗负载。
        if (!this.canIProcess) {
          // 注意跳过处理时必须在这里手动释放资源，否则会因为消费端数据堆积而阻塞bufferQueue队列轮转
          nextImage.release();
          return;
        }
        this.canIProcess = false; // 如果当前需要处理，则先将状态位更新为无需处理后再处理当前帧数据，防止处理时间过长导致状态更新不及时
        // 解析图像内容
        nextImage.getComponent(image.ComponentType.JPEG, async (err: BusinessError, imgComponent: image.Component) => {
          ...
          if (imgComponent.byteBuffer) {
            ...
            await this.packToData(pixelMap!); // 保存图片
            await pixelMap.release(); // 释放资源
          } else {
  ...
  async initCamera(): Promise<void> {
    ...
    try {
      ...
      // 筛选预览Profile。这里筛选分辨率宽高比1:1，且分辨率>=1080的Profile，开发者可根据业务需要自定义筛选逻辑。
...
Q：代码中canIProcess参数的作用是什么？是否非有不可？

A：on('imageArrival')回调中涉及到PixelMap的创建和处理，这些操作耗费资源比较严重，如果持续处理会对性能造成不小的负担，因此使用canIProcess配合定时器的方式，隔一段时间处理一帧，这样处理既能即可以节约性能，又与业务场景相契合。考虑到考试场景可能耗时较久，建议还是参考这个处理思路，但如果业务处理逻辑不需要做繁杂的PixelMap处理，也可以考虑移除这套控制逻辑。
