...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/traffic-v1_1-ts_5-0000002346853189
---
...
* 正常传输数据包：
  ...
  ![](./img/9a284e6e.png "点击放大")
* TCP粘包：

  是指发送方发送的若干数据，在接收方的缓冲区里被组合到一起后读取，后面数据包的头部“粘着”前面数据包的尾部，当单个包比较小时，多个包可能会被合并成一个数据包，所以称为粘包。

  粘包示意图：

  ![](./img/073de6c6.png "点击放大")
* TCP拆包：

  是指一个比较大的数据包，在接收方缓冲区被拆分成多个数据包来读取，这些多个数据包组合起来才是正确的数据包。

  拆包和粘包示意图：

  ![](./img/28757f03.png "点击放大")
