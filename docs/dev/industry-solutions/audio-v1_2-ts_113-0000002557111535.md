...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/audio-v1_2-ts_113-0000002557111535
---
...
* 开发者可以通过[AVScreenCapture](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-h)模块的C API，采集设备内外的音视频数据源。开发者可以设置屏幕录制所需要的音频参数[OH\_AudioInfo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avscreencapture-oh-audioinfo)包括内录和麦克风音频[OH\_AudioCaptureInfo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avscreencapture-oh-audiocaptureinfo)。
...
2. 创建并配置内录与外录音频编码器。创建媒体文件封装器，并添加内录与外录的音频轨道。
   ...
   void CAVScreenCaptureToStream::InitMuxerAndEncoder(int32_t outputFd)
   {
       ...
       // 音频编码器
       audioEncoderInner_ = std::make_unique<AudioEncoder>(); // 内录音频编码器
       audioEncoderMic_ = std::make_unique<AudioEncoder>();   // 外录音频编码器
       // 媒体文件封装器
   ...
   ```
3. 监听录屏数据获取回调[OH\_AVScreenCapture\_OnBufferAvailable](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-base-h#oh_avscreencapture_onbufferavailable)获取设备内部与麦克风采集的原始音频数据。
...
5. 录制完成后，调用ffmpeg指令编辑录制生成的音频文件，混合内录和外录音频轨道，生成内录与外录混合后的音频文件。
   ...
   /**
    * 调用ffmpeg指令混合两路音频
    * @param inputFilePath 输入音频文件沙箱路径
    * @param outputFilePath 输出音频文件沙箱路径
    */
   async mixAudioTrack(inputFilePath: string, outputFilePath: string) {
     ...
     // 混音命令，将原始音频文件的两路音频轨道合成一路，生成混音后的音频文件。
     let cmd =
