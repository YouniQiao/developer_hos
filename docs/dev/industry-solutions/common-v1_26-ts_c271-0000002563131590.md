---
title: "Button组件背景色设置"
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_c271-0000002563131590
---
...
* **场景四**：Button组件可以设置不透明度属性达到置灰效果，如设置不透明度属性opacity(0.4)实现与禁用控制属性enabled(false)时的相同样式效果。
  ...
  ![](./img/7f9eca55.png "点击放大")
* **场景五**：定义调色板数据源，用数组存储自定义颜色，通过@State响应式状态管理维护当前选中颜色的索引，实现状态与试图的双向联动。用Flex和ForEach组件完成调色板布局。给按钮和调色板都绑定点击事件，点击按钮时索引递增，超出数组长度自动重置，循环遍历调色板；点击调色板色块通过当前索引值，切换到对应颜色。
