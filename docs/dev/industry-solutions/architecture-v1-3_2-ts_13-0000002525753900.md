...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/architecture-v1-3_2-ts_13-0000002525753900
---
...
* 拍照、录像等场景的镜像设置接口说明：
  ...
  | 场景 | 接口 | 说明 |
  | --- | --- | --- |
  | 拍照 | [isMirrorSupported（PhotoOutput）](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-camera-photooutput#ismirrorsupported) | 查询是否支持镜像拍照。 |
  | 拍照 | [PhotoCaptureSetting](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-camera-i#photocapturesetting) | 拍摄照片的设置。其中mirror属性即为镜像使能开关（默认关）。 |
  | 拍照 | [enableMirror（PhotoOutput）](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-camera-photooutput#enablemirror13) | 是否启用动态照片镜像拍照。需先调用isMirrorSupported（PhotoOutput）检查设备支持情况。 |
  | 录像 | [isMirrorSupported（VideoOutput）](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-camera-videooutput#ismirrorsupported15) | 查询是否支持镜像录像。后置镜头返回为false。 |
  | 录像 | [enableMirror（VideoOutput）](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-camera-videooutput#enablemirror15) | 启用/关闭镜像录像。需先调用isMirrorSupported（VideoOutput）检查当前设备支持情况。 |
