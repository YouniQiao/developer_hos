...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/health-v1_2-ts_9-0000002395860142
---
...
* 移除module.json5中label和icon字段后，调用showAssetsCreationDialog接口报错：showAssetsCreationDialog failed,errCode is 401,errMsg is Invalid input parameter.
  ...
  原因是showAssetsCreationDialog接口保存图片到图库时，会先创建一个对应APP的相册，相册的图标和名称会使用到module.json5的label和icon字段。

## 修改建议

不能移除module.json5中label和icon字段，showAssetsCreationDialog接口保存图片到图库时，会先创建相册，其底层代码逻辑使用到module.json5的label和icon字段。
