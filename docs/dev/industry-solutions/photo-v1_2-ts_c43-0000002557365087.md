...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/photo-v1_2-ts_c43-0000002557365087
---
...
struct SwiperArcPage {
  ...
  build() {
    Column() {
      Swiper() {
        ForEach(this.swiperItems, (item: number, index: number) => {
          Text(item.toString())
            .width('100%')
            .height('100%')
            .fontSize(50)
