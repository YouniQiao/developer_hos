...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_c131-0000002552436693
---
...
3. 在需要更新数据的地方调用MyUtils类中的更新方法即可。
   * 在UI页面中调用：
     ...
     ```
   * 在应用业务逻辑中调用：在应用业务逻辑中使用PersistenceV2，需要注意持久化操作需在UI实例初始化完成后调用（即loadContent回调触发后）。应用的生命周期可以参考官网[UIAbility组件生命周期](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-lifecycle)。基于此，在应用的onWindowStageDestroy和onDestroy中可以正常读取PersistenceV2中的内容，但是不能进行数据的修改，持久化过程会失败。
