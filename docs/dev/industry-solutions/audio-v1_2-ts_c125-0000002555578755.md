...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/audio-v1_2-ts_c125-0000002555578755
---
...
* 获取原始视频数据：以Buffer模式启动屏幕录制时，可以直接获取当前屏幕的原始视频数据（RGBA格式）。开发者可以根据应用场景将屏幕原始视频数据流转到其他模块处理。可通过如下方式实时获取原始视频数据：
  1. 在启动录屏前，通过[OH\_AVScreenCapture\_SetDataCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-h#oh_avscreencapture_setdatacallback)接口设置原始视频数据处理回调方法。

     ```
     OH_AVScreenCapture_SetDataCallback(g_avCapture, OnBufferAvailable, nullptr);
     ```
  2. 通过[OH\_AVScreenCapture\_StartScreenCapture](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-h#oh_avscreencapture_startscreencapture)接口以Buffer模式启动录屏。
     ...
     ```
  3. 启动录屏后，通过设置的[OH\_AVScreenCapture\_OnBufferAvailable](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-base-h#oh_avscreencapture_onbufferavailable)回调函数来监听是否有视频数据产生并获取屏幕录制的视频数据（RGBA格式）。
...
* 获取编码视频码流：以Surface模式启动屏幕录制时，需要通过OHNativeWindow与视频编码器对接。此时，无法直接获取屏幕的原始视频数据，视频数据会实时流转到视频编码器模块完成视频编码，最后通过视频编码器输出编码后的视频码流（H264或H265格式）。可通过如下方法获取屏幕编码视频码流：
  1. 创建视频编码器，并通过[OH\_VideoEncoder\_GetSurface](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-videoencoder-h#oh_videoencoder_getsurface)接口获取编码器Surface模式的输入[OHNativeWindow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativewindow-nativewindow)。
     ...
     ```
  2. 通过[OH\_AVScreenCapture\_StartScreenCaptureWithSurface](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-h#oh_avscreencapture_startscreencapturewithsurface)接口以Surface模式启动录屏，传入视频编码器的输入OHNativeWindow对象。
     ...
     ```
  3. 屏幕录制的视频数据自动输入视频编码器进行视频编码，可以通过监听视频编码器的输出回调[OH\_AVCodecOnNewOutputBuffer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-base-h#oh_avcodeconnewoutputbuffer)获取编码视频码流数据（H264或H265格式）。
