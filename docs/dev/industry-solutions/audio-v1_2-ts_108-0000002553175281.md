...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/audio-v1_2-ts_108-0000002553175281
---
...
* Dialog1初始页面，仅用于页面跳转。
  ...
  ```
* DialogTwo页面。@Builder函数customDialogComponent()为弹窗自定义内容，通过使用UIContext中的getPromptAction方法获取当前UI上下文关联的PromptAction对象，拉起弹窗，调用onWillDismiss()交互式关闭回调函数，执行dismissDialogAction.dismiss()并关闭Popup回调函数。
  ...
  ```
* router\_map.json路由表：配置详见[系统路由表](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-navigation-navigation#系统路由表)。
