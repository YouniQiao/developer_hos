...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/educate-v1_1-ts_16-0000002326967601
---
...
![](./img/22557b77.png "点击放大")

上下左右存在留白，如何实现可以去除内边距的导航指示器效果？

## 效果预览

![](./img/44d0528b.gif "点击放大")
...
* **方案一：采用API19的bottom接口。**

  通过bottom接口，可以实现指示器与底部间距为0。实现方式详见：[示例9](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-swiper#示例9演示导航点space与bottom)。
* **方案二：自定义Indicator。**

  当前DotIndicator的规格固定存在内边距，无法满足问题需求。可以通过以下步骤实现无内边距的导航指示器效果：
...
![](./img/966e05b2.png "点击放大")

* **方案三：采用API15中Indicator组件。**

  Indicator组件采用的依旧是Swiper中的默认指示器样式，一样有32vp的交互高度限制，不过该组件将Swiper与指示器单独分割，可以通过Column组件单独封装指示器，再通过clip属性裁剪，控制Indicator组件高度，其它的与方案二类似采用Stack组件封装。
