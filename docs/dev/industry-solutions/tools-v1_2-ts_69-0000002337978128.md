...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/tools-v1_2-ts_69-0000002337978128
---
...
E/AI_INFRA (16075): model_util.cp BuildOrigin2IRGraph(233）::"ret == SUCCESS" "false,return ret
E/AI_FMK (16075): omg.cpp GenerateIRModel(113)::"Failed to generator IR graph!."
E/0MG_TOOL (16075):command_util.cpp ProcessCommand(1226)::"OMG Generate execute failed!!"
...
当前目录下check\_result.json文件的部分信息如下：
...
* [离线模型转换](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-offline-model-conversion)：使用CANN Kit SDK时，可以预先使用OMG工具将Caffe、TensorFlow、ONNX、MindSpore模型转换为OM离线模型，移动端AI程序直接读取离线模型进行推理。模型转换成功后会在当前目录下生成对应的OM文件，转换失败则会在当前目录下生成check\_result.json文件。

## 问题定位

1. 根据以下执行日志，可得知OMG命令转换离线模型失败，更多的信息需要查看预检查报告，即check\_result.json文件信息。

   ```
   E/OMG_TOOL (16075): main.cpp main(21)::"OMG generate offline model failed. Please see the log or pre-checking report for more details."
   ```
2. 根据当前转换目录下生成的check\_result.json文件内的以下报错信息可知，该ONNX模型使用了QuantizeLinear和DequantizeLinear这2个ONNX的量化算子，当前HarmonyOS 5.0的CANN Kit仅支持官网提供的量化方案：[模型轻量化](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-lightweight-tool-instructions)。
