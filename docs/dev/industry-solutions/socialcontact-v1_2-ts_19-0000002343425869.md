---
title: "图片像素错误导致显示异常"
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/socialcontact-v1_2-ts_19-0000002343425869
---
...
* **场景一**：图片颜色显示不正确：
  ...
  ![](./img/8bf8f857.png "点击放大")

  图二：

  ![](./img/4ab80337.png "点击放大")
...
* **场景二**：图片颜色错乱，像素点明亮可见：
  ...
  ![](./img/7ad1c324.png "点击放大")

  图四：

  ![](./img/fa087276.png "点击放大")
  ...
  解决方案：确定好相关像素格式后，可由正确的像素格式渲染图片，如创建PixelMap时初始化缓冲区中的像素格式需要由[InitializationOptions.srcPixelFormat](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-i#initializationoptions8)指定（默认值为BGRA\_8888）。
...
A：根据不同的硬件限制，编解码器自动对齐的字节不同，可以使用[OH\_VideoEncoder\_GetInputDescription](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-videoencoder-h#oh_videoencoder_getinputdescription)查询得到AVformat，再用[OH\_AVFormat\_GetIntValue](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avformat-h#oh_avformat_getintvalue)获取[OH\_MD\_KEY\_VIDEO\_STRIDE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-codecbase#视频专有的键值对)的值进行处理。详细处理方式参考视频编码文档中的[调用OH\_VideoEncoder\_PushInputBuffer写入编码图像](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding#buffer模式)步骤。
