---
title: "DNS域名解析错误"
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/common-v1_26-ts_471-0000002545168243
---
...
* 当存在连接复用\自定义DNS规则\本地DNS缓存时，请求按如下顺序依次执行：
  ...
  ![](./img/8a8de896.png "点击放大")
* [wireshark](https://github.com/wireshark/wireshark)：一款强大的网络协议分析工具，用于捕获和分析网络数据包，帮助用户深入理解和调试网络通信问题。
* [WebNetErrorList](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-neterrorlist#webneterrorlist)：ArkWeb的网络协议栈错误列表，其中ERR\_NAME\_NOT\_RESOLVED表示域名无法解析。
* [1007900006 域名解析失败](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/remote-communication-error-code#section1059580840)：服务器的域名无法解析。
...
1. **检查网络连通性**，通过浏览器访问域名或在终端执行hdc shell ping [域名]检查网络连接情况。在通畅的网络下，使用默认DNS服务器，终端返回域名解析结果，丢包率0%：
   ...
   ```
2. **查看hilog日志**，分析报错信息有1007900006、Couldn't resolve host name、ERR\_NAME\_NOT\_RESOLVED报错。
   1. RCP请求分析示例如下：
      * dnsStatus：DNS结果的状态，1表示有IP地址，2表示只拿到了CNAME，3代表非法报文。
      * dnsFromNetsys：DNS查询的结果是否来自网络系统缓存，0代表从DNS服务器获取，1代表从缓存中获取。
      * tryDns：尝试连接的DNS服务器。
      * recvDns：收到响应的DNS服务器。
