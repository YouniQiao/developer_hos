...
original_url: https://developer.huawei.com/consumer/cn/doc/architecture-guides/convenient-life-v1_2-ts_c27-0000002358958960
---
...
1. 应用中尝试搜索[createAVPlayer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-f#mediacreateavplayer9)，查看应用创建AVPlayer实例的时机，是否在aboutToAppear函数中创建的AVPlayer实例，而aboutToAppear函数只会在组件创建时调用一次。
2. 应用中尝试搜索AVPlayer实例的[release](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avplayer#release9)销毁接口，查看应用什么时机销毁的AVPlayer播放器的实例，是否在aboutToDisappear函数中销毁的AVPlayer实例，该函数会在组件销毁时调用一次。
3. 应用中尝试排查音频页面流程，查看其是否有创建AVPlayer实例的代码。

## 分析结论

1. 应用中AVPlayer实例是在首页的aboutToAppear函数中创建的，应用首页在组件创建时调用了该函数，AVPlayer实例成功创建，所以第一次进入音频作品页面时，音频正常播放。
2. 应用返回到首页，子页面销毁时在aboutToDisappear函数中调用release，销毁了AVPlayer实例。
3. 应用再次进入音频作品页面，流程中并没有重新创建AVPlayer实例，导致音频播放失败。
...
1. 工程配置文件module.json5中配置{"routerMap": "$profile:route\_map"}。
   ...
   ```
2. 在工程resources/base/profile中创建route\_map.json文件，添加如下配置信息。
...
3. 应用首页，点击子页面音频会跳转到音频播放子页面。
   ...
   ```
4. 音频播放页面在组件初始化aboutToAppear中创建AVPlayer实例，播放音频，返回首页时在aboutToDisappear函数中调用release，销毁了AVPlayer实例，后续再次进入会重新创建AVPlayer实例并播放音频。
