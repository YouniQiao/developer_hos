...
format: md
original_url: /docs/dev/release-notes/harmonyos-610
upstream_id: dev/release-notes/harmonyos-610
...
| compileSdkVersion | 无需显性配置，编译时默认使用配套的SDK版本，即默认为：  “compileSdkVersion”: “6.1.0(23)” | NA | 推荐使用[6.0.0(20)](/docs/dev/release-notes/overview-600)进行新应用的开发。 |
| compatibleSdkVersion | 建议与工程升级前的compatibleSdkVersion保持一致。 | 和升级前保持一致，如：  “compatibleSdkVersion”: “6.0.0(20)” | 推荐使用[6.0.0(20)](/docs/dev/release-notes/overview-600)进行新应用的开发。 |
| targetSdkVersion | 推荐您适配新版本的最新变更，然后配置为：“targetSdkVersion”: “6.1.0(23)”。  如果您期望延迟适配变更，可配置targetSdkVersion与工程升级前的targetSdkVersion一致。 | 1、应用适配变更，变更适配完成后配置为：  “targetSdkVersion”: “6.1.0(23)”  2、应用暂不适配变更，需配置为工程升级前的值，如：  “targetSdkVersion”: “6.0.0(20)” | 推荐使用[6.0.0(20)](/docs/dev/release-notes/overview-600)进行新应用的开发。 |
...
游戏渲染加速服务新增支持TV设备（[指南](/docs/dev/app-dev/graphics/graphics-accelerate-kit-guide/graphics-accelerate-introduction#支持的设备)）。
...
* 应用程序包管理新增支持对Native软件包（hnp）进行独立签名的能力。可通过module.json5配置文件的hnpPackages标签进行标识。（[指南](/docs/dev/app-dev/getting-started/dev-fundamentals/module-configuration-file#hnppackages标签)）
...
应用归因服务新增支持PC/2in1、TV设备。（[指南](/docs/dev/app-dev/application-services/store-kit-guide/store-attribution/store-attribution-introduction#约束与限制)）

### App Linking Kit

* 新增支持配置应用链接跳转的优先级。（[指南](/docs/dev/app-dev/application-services/app-linking-kit-guide/app-linking-startupapp#建立域名与应用关联关系)）
* 延迟链接新增支持PC/2in1设备。（[指南](/docs/dev/app-dev/application-services/app-linking-kit-guide/applinking-deferredlink#约束与限制)）

### AR Engine

* 新增支持人脸识别与跟踪能力。（[指南-ArkTS](/docs/dev/app-dev/graphics/ar-engine-kit-guide/get-face/arengine-face)、[指南-C/C++](/docs/dev/app-dev/graphics/ar-engine-kit-guide/get-face/arengine-c-face)）
* 新增支持人体骨骼关键点识别与跟踪能力。（[指南-ArkTS](/docs/dev/app-dev/graphics/ar-engine-kit-guide/get-body/arengine-body)、[指南-C/C++](/docs/dev/app-dev/graphics/ar-engine-kit-guide/get-body/arengine-c-body)）
...
* PersistenceV2的globalConnect新增支持集合类型（Array、Map、Set、Date、collections.Array、collections.Map、collections.Set）的持久化。（[指南](/docs/dev/app-dev/application-framework/arkui/arkts-ui-development/arkts-state-management/arkts-state-management-v2/arkts-v2-manage-application-state/arkts-new-persistencev2)）
...
* 新增支持Web应用模拟点击检测。（[指南](/docs/dev/app-dev/application-framework/arkweb/web-manage-page-interaction/web-detect-simulated-click-risk-enhanced)）
...
* 新增系统音效管理和播放的API，提供管理系统声音的基础能力，包括对系统音效类型的定义、获取系统音效播放器等。（[指南](/docs/dev/app-dev/media/audio-kit/audio-playback/using-soundplayer-for-playback)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-systemsoundmanager)）
...
AVCodec新增支持AV1/VP9/VP8/RV30/RV40/WVC1/DVVIDEO/RAWVIDEO/MPEG1格式的视频软解码能力。（[指南](/docs/dev/app-dev/media/avcodec-kit/avcodec-support-formats#媒体编解码)）
...
* 新增支持跳转陌生号码和信息识别设置页面能力。（[指南](/docs/dev/app-dev/application-services/call-kit-guide/callservice-enterprise-contact-display#应用跳转陌生号码和信息识别页面)）
...
* 新增支持HDR动态照片拍摄能力，即组成动态照片的静态图片与动态短视频均为高动态范围（HDR）内容。（[指南](/docs/dev/app-dev/media/camera-kit/camera-dev-arkts/camera-moving-photo#hdr动态照片)）
...
* 新增支持PC/2in1设备。（[指南](/docs/dev/app-dev/application-services/cloud-foundation-kit-guide/cloudfoundation-introduction#支持的设备)）
* 预加载模块新增支持跳链安装预加载类型。（[指南](/docs/dev/app-dev/application-services/cloudfoundation-prefetch-overview)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/cloudfoundation-cloudresprefetch#prefetchmode)）
...
空间管理场景下，新增支持企业账号认证、获取企业应用访问令牌、设置工作空间状态栏图标、设置工作空间本地名称功能。（[指南](/docs/dev/app-dev/application-services/enterprise-space-kit-guide/enterprisespace-spacemanager-guide)）

### Form Kit

ArkTS卡片开发支持V2装饰器语法。（[指南](/docs/dev/app-dev/application-framework/form-kit/arkts-ui/arkts-ui-widget-adapt-faq)）

### Game Service Kit

游戏近场快传新增支持安装包传输。（[指南](/docs/dev/app-dev/application-services/game-service-kit-guide/gameservice-nearbytransfer-dev/gameservice-nearbytransfer-access-procedure/gameservice-nearbytransfer-installation-package)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransferonremoteinstallationinfonotify)）
...
* 游戏启动加速服务新增支持PC/2in1设备。（[指南](/docs/dev/app-dev/graphics/graphics-accelerate-kit-guide/graphics-accelerate-introduction#支持的设备)）

### IAP Kit

* 新增开票接口，实现在用户在购买数字商品场景下，申请开发票。（[指南](/docs/dev/app-dev/application-services/iap-kit-guide/iap-after-sales)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/iap-iap#iapshowmanagedinvoices)）
...
* 新增3D地图城市灯光效果。（[指南](/docs/dev/app-dev/application-services/map-kit-guide/map-creation/map-presenting#开启3d地球)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-map-mapcomponentcontroller#setsphereenabled-2)）
* 新增支持在地图左下角设置审图号。（[指南](/docs/dev/app-dev/application-services/map-kit-guide/map-interaction/map-controls-and-interaction#审图号)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-map-mapcomponentcontroller#setapprovenumberenabled)）
...
新增查询当前设备是否支持NearLink Kit的能力。（[指南](/docs/dev/app-dev/system/system-network/nearlink-kit-guide/nearlink-issupported)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/nearlink-manager#isnearlinksupported)）
...
* 新增支持批量提取视频缩略图的能力，通过传入一个时间戳数组，可获取时间戳对应视频帧的缩略图。（[指南](/docs/dev/app-dev/media/media-kit/media-kit-dev-arkts/media-info-arkts/avmetadataextractor)-详见步骤7、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avmetadataextractor#fetchframesbytimes23)）
* 支持将HDR VIVID/HLG/HDR10视频转换为SDR视频，以及SDR视频的转码。（[指南](/docs/dev/app-dev/media/media-kit/media-kit-intro#支持的格式-4)）
...
* DNS解析能力支持转码，可将Unicode编码形式的主机名转换为ASCII编码形式（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#connectiongetdnsascii23)），也可将ASCII编码形式的主机名转换为Unicode编码形式（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#connectiongetdnsunicode23)）。（[指南](/docs/dev/app-dev/system/system-network/network-kit/network-kit-data-transmission/net-dns)）
...
新增营销组件，开发者可在应用内实现领券、选券能力。（[指南](/docs/dev/app-dev/application-services/payment-kit-guide/payment-promotion-service)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-promotionservice)）
...
新增禁用画布缩放能力、设置滚动位置ScrollTo及监听长画布滚动位置能力、自定义长画布最大高度能力。（[指南](/docs/dev/app-dev/system/system-hardware/pen-kit-guide/pen-features/pen-suite)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pen-handwritecomponent#handwritecomponent)）

### Performance Analysis Kit

HiappEvent新增ArkWeb抛滑触控操作丢帧检测能力。（[指南](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/hiappevent/event-subscription/system-events/scroll-arkweb-fling-jank-events/hiappevent-watcher-web-fling-jank-events)）

HiDebug新增提供添加维测信息的接口，开发者可根据业务需要将维测信息添加到崩溃日志中，若程序发生崩溃，可在崩溃日志中找到该维测信息。（[指南](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/hidebug/hidebug-guidelines#添加维测信息到崩溃日志中)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hidebug-h#oh_hidebug_setcrashobj)）

AppFreeze日志中调用栈的堆栈信息增加线程状态信息。（[指南](/docs/dev/app-dev/system/system-debug-optimize/performance-analysis-kit/fault-analysis/appfreeze-guidelines#堆栈信息)）

### Preview Kit

新增C API，支持通用文件缓存加速功能，即通过缓存解码后的图片、视频等数据至磁盘，实现后续文件打开或浏览时的直接读取，减少重复解码耗时，提升用户操作流畅度。（[指南](/docs/dev/app-dev/application-services/preview-kit-guide/preview-filecacheboost)）
...
* 新增支持应用通过network\_config.json文件，配置HTTP明文请求禁用策略。（[指南](/docs/dev/app-dev/system/system-network/remote-communication-kit-guide/remote-communication-preparations#http明文设置)）
...
默认界面扫码能力、图像识码能力和自定义界面扫码能力支持带后置相机的Wearable。（[指南](/docs/dev/app-dev/media/scan-kit-guide/scan-introduction#支持的设备)）
...
* 新增底部页签悬浮样式以及迷你栏，支持迷你栏动态折叠展开。（[指南](/docs/dev/app-dev/application-framework/ui-design-kit-guide/ui-design-hds-tabs/ui-design-hds-tabs-bar-floating)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ui-design-hdstabs#hdstabsfloatingstyle)）
...
HUKS新增支持群组密钥功能，针对同一开发者开发的多个HAP应用提供跨应用密钥共享能力。（[指南](/docs/dev/app-dev/system/system-security/huks-kit/huks-local-key-management/huks-other-operations/huks-group-key/huks-group-key-overview)）
...
* 新增支持TV设备。（[指南](/docs/dev/app-dev/application-services/weather-service-kit-guide/weather-service-introduction#约束与限制)）

### 调测调优

打包工具打包hap/hsp时，支持传入已存在的hap/hsp（--exist-src-path）和指定要保留的目录（--lib-path-retain），要保留的目录会从已存在的hap/hsp中直接拷贝到目标包，不会从源文件中重新压缩打包。当前保留目录只支持--lib-path。（[指南](/docs/dev/app-dev/system/packing-tool)）
