---
title: "运行调试元服务"
original_url: /docs/dev/atomic-dev/ascf/ascf-development-process/debug-ascf-code
format: md
upstream_id: dev/atomic-dev/ascf/ascf-development-process/debug-ascf-code
...
具体操作可参考[DevEco Studio操作指导-使用本地真机运行应用/元服务](/docs/tools/coding-debug/ide-run-device)。
...
* 与真机相比，模拟器当前仅支持部分能力，具体可查阅[模拟器与真机的差异](/docs/tools/coding-debug/ide-emulator-specification)。
* 如果运行报错，开发者可参考[模拟器运行问题处理](/docs/dev/atomic-dev/ascf/faqs-ascf-runtime/faqs-runtime-emulator)。
...
开发者也可以通过ASCF命令行工具直接启用热重载功能，详细信息请参阅[开启热重载](/docs/dev/atomic-dev/ascf/ascf-development-process/run-ascf-cli#开启热重载)。
...
执行命令成功后，根据日志提示，[在chrome浏览器上打开调试工具页面](/docs/dev/app-dev/application-framework/arkweb/web-debugging/web-debugging-with-devtools#在chrome浏览器上打开调试工具页面)。
...
* Webview调试：开启方法请参考[使用Devtools工具调试前端页面](/docs/dev/app-dev/application-framework/arkweb/web-debugging/web-debugging-with-devtools)。
...
使用ASCF助手在VSCode中开发，查看开发者打印日志，请参考[打开日志](/docs/dev/atomic-dev/ascf/ascf-development-process/ascf-assistant#打开日志)。
